...
enum { WEATHER_UPDATED, SET_CURRENT, LAST_SIGNAL };
...
ClockLocation *clock_location_find_and_ref(GSList *locations, const gchar *name,
                                           const gchar *city,
  ...
  for (l = locations; l != NULL; l = l->next) {
    ClockLocationPrivate *priv = clock_location_get_instance_private(l->data);

    if (priv->latitude == latitude && priv->longitude == longitude &&
        g_strcmp0(priv->weather_code, code) == 0 &&
        g_strcmp0(priv->timezone, timezone) == 0 &&
        g_strcmp0(priv->city, city) == 0 && g_strcmp0(priv->name, name) == 0)
      break;
  }
...
ClockLocation *clock_location_new(const gchar *name, const gchar *city,
                                  const gchar *timezone, gfloat latitude,
                                  gfloat longitude, const gchar *code,
                                  WeatherPrefs *prefs) {
  ClockLocation *this;
...
static void clock_location_class_init(ClockLocationClass *this_class) {
  GObjectClass *g_obj_class = G_OBJECT_CLASS(this_class);

  g_obj_class->finalize = clock_location_finalize;

  location_signals[WEATHER_UPDATED] = g_signal_new(
      "weather-updated", G_OBJECT_CLASS_TYPE(g_obj_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(ClockLocationClass, weather_updated), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  location_signals[SET_CURRENT] = g_signal_new(
      "set-current", G_OBJECT_CLASS_TYPE(g_obj_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(ClockLocationClass, set_current), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void network_changed(GNetworkMonitor *monitor, gboolean available,
                            ClockLocation *loc) {
...
static void clock_location_init(ClockLocation *this) {
  ...
  g_signal_connect(monitor, "network-changed", G_CALLBACK(network_changed),
                   this);
...
static void clock_location_finalize(GObject *g_obj) {
  ClockLocationPrivate *priv =
      clock_location_get_instance_private(CLOCK_LOCATION(g_obj));
  GNetworkMonitor *monitor;

  monitor = g_network_monitor_get_default();
  g_signal_handlers_disconnect_by_func(monitor, G_CALLBACK(network_changed),
...
void clock_location_get_coords(ClockLocation *loc, gfloat *latitude,
                               gfloat *longitude) {
  ClockLocationPrivate *priv = clock_location_get_instance_private(loc);
...
void clock_location_set_coords(ClockLocation *loc, gfloat latitude,
                               gfloat longitude) {
  ClockLocationPrivate *priv = clock_location_get_instance_private(loc);
...
static void clock_location_set_tzname(ClockLocation *this, const char *tzname) {
  ClockLocationPrivate *priv =
      clock_location_get_instance_private(CLOCK_LOCATION(this));

  if (priv->tzname && strcmp(priv->tzname, tzname) == 0) return;
...
gboolean clock_location_is_current(ClockLocation *loc) {
  ...
  if (clock_location_is_current_timezone(loc)) {
    ...
    g_signal_emit(current_location, location_signals[SET_CURRENT], 0, NULL);
...
static void make_current_cb(gpointer data, GError *error) {
  ...
  if (mcdata->callback) mcdata->callback(mcdata->data, error);
}

static void free_make_current_data(gpointer data) {
  MakeCurrentData *mcdata = data;

  if (mcdata->destroy) mcdata->destroy(mcdata->data);

  g_object_unref(mcdata->location);
  g_free(mcdata);
}

void clock_location_make_current(ClockLocation *loc, GFunc callback,
  ...
  set_system_timezone_async(filename, (GFunc)make_current_cb, mcdata,
...
static void set_weather_update_timeout(ClockLocation *loc) {
  ClockLocationPrivate *priv = clock_location_get_instance_private(loc);
  guint timeout;
  ...
  if (priv->weather_timeout) g_source_remove(priv->weather_timeout);
  priv->weather_timeout =
      g_timeout_add_seconds(timeout, update_weather_info, loc);
}

static void weather_info_updated(WeatherInfo *info, gpointer data) {
  ClockLocation *loc = data;
  ClockLocationPrivate *priv = clock_location_get_instance_private(loc);

  set_weather_update_timeout(loc);
  g_signal_emit(loc, location_signals[WEATHER_UPDATED], 0, priv->weather_info);
}
...
static gboolean update_weather_info(gpointer data) {
  ...
  weather_info_update(priv->weather_info, &prefs, weather_info_updated, loc);

  return TRUE;
}

static gchar *rad2dms(gfloat lat, gfloat lon) {
  gchar h, h2;
  ...
  return g_strdup_printf("%02d-%02d%c %02d-%02d%c", (int)deg, (int)min, h,
...
static void setup_weather_updates(ClockLocation *loc) {
  ...
  wl = weather_location_new(priv->city, priv->weather_code, NULL, NULL, dms,
                            NULL, NULL);

  priv->weather_info = weather_info_new(wl, &prefs, weather_info_updated, loc);
...
void clock_location_set_weather_prefs(ClockLocation *loc, WeatherPrefs *prefs) {
  ClockLocationPrivate *priv = clock_location_get_instance_private(loc);
