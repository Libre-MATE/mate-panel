...
static char *files_to_check[CHECK_NB] = {ETC_TIMEZONE, ETC_TIMEZONE_MAJ,
                                         ETC_SYSCONFIG_CLOCK, ETC_CONF_D_CLOCK,
                                         ETC_LOCALTIME};
...
static GObject *system_timezone_constructor(
...
static void system_timezone_class_init(SystemTimezoneClass *class) {
  ...
  system_timezone_signals[CHANGED] = g_signal_new(
      "changed", G_OBJECT_CLASS_TYPE(g_obj_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(SystemTimezoneClass, changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
}

static void system_timezone_init(SystemTimezone *systz) {
  int i;
  SystemTimezonePrivate *priv;
  priv = system_timezone_get_instance_private(systz);
  ...
  priv->tz = NULL;
  priv->env_tz = NULL;
  for (i = 0; i < CHECK_NB; i++) priv->monitors[i] = NULL;
}
...
static GObject *system_timezone_constructor(
  ...
  if (systz_singleton) return g_object_ref(systz_singleton);

  obj = G_OBJECT_CLASS(system_timezone_parent_class)
...
static void system_timezone_monitor_changed(GFileMonitor *handle, GFile *file,
  ...
  if (strcmp(priv->tz, new_tz) != 0) {
    ...
    g_signal_emit(G_OBJECT(user_data), system_timezone_signals[CHANGED], 0,
                  priv->tz);
  }
  g_free(new_tz);
}
...
static char *system_timezone_read_etc_timezone(void) {
  FILE *etc_timezone;
  GString *reading;
  int c;

  etc_timezone = g_fopen(ETC_TIMEZONE, "r");
  if (!etc_timezone) return NULL;
...
static gboolean system_timezone_write_etc_timezone(const char *tz,
                                                   GError **error) {
  char *content;
  GError *our_error;
  gboolean retval;

  if (!g_file_test(ETC_TIMEZONE, G_FILE_TEST_IS_REGULAR)) return TRUE;
  ...
  if (!retval) {
    g_set_error(error, SYSTEM_TIMEZONE_ERROR, SYSTEM_TIMEZONE_ERROR_GENERAL,
                ETC_TIMEZONE " cannot be overwritten: %s", our_error->message);
...
static char *system_timezone_read_key_file(const char *filename,
                                           const char *key) {
  GIOChannel *channel;
  char *key_eq;
  char *line;
  ...
  while (g_io_channel_read_line(channel, &line, NULL, NULL, NULL) ==
    ...
    if (g_str_has_prefix(line, key_eq)) {
      ...
      if (value[0] == '\"') {
        if (value[len - 1] == '\"') {
          g_free(retval);
          retval = g_strndup(value + 1, len - 2);
        }
      } else {
        g_free(retval);
...
static gboolean system_timezone_write_key_file(const char *filename,
                                               const char *key,
                                               const char *value,
                                               GError **error) {
  GError *our_error;
  ...
  if (!g_file_test(filename, G_FILE_TEST_IS_REGULAR)) return TRUE;

  our_error = NULL;

  if (!g_file_get_contents(filename, &content, &len, &our_error)) {
    g_set_error(error, SYSTEM_TIMEZONE_ERROR, SYSTEM_TIMEZONE_ERROR_GENERAL,
                "%s cannot be read: %s", filename, our_error->message);
    g_error_free(our_error);
    return FALSE;
  }
  ...
  for (n = 0; lines[n] != NULL; n++) {
    if (g_str_has_prefix(lines[n], key_eq)) {
      ...
      if (use_quotes)
        lines[n] = g_strdup_printf("%s\"%s\"", key_eq, value);
      else
        lines[n] = g_strdup_printf("%s%s", key_eq, value);
  ...
  if (!retval) {
    g_set_error(error, SYSTEM_TIMEZONE_ERROR, SYSTEM_TIMEZONE_ERROR_GENERAL,
                "%s cannot be overwritten: %s", filename, our_error->message);
    g_error_free(our_error);
  }
...
static char *system_timezone_read_etc_TIMEZONE(void) {
  return system_timezone_read_key_file(ETC_TIMEZONE_MAJ, "TZ");
}

static gboolean system_timezone_write_etc_TIMEZONE(const char *tz,
                                                   GError **error) {
  return system_timezone_write_key_file(ETC_TIMEZONE_MAJ, "TZ", tz, error);
}

/* This works for Fedora and Mandriva */
static char *system_timezone_read_etc_sysconfig_clock(void) {
  return system_timezone_read_key_file(ETC_SYSCONFIG_CLOCK, "ZONE");
}

static gboolean system_timezone_write_etc_sysconfig_clock(const char *tz,
                                                          GError **error) {
  return system_timezone_write_key_file(ETC_SYSCONFIG_CLOCK, "ZONE", tz, error);
}

/* This works for openSUSE */
static char *system_timezone_read_etc_sysconfig_clock_alt(void) {
  return system_timezone_read_key_file(ETC_SYSCONFIG_CLOCK, "TIMEZONE");
}

static gboolean system_timezone_write_etc_sysconfig_clock_alt(const char *tz,
                                                              GError **error) {
  return system_timezone_write_key_file(ETC_SYSCONFIG_CLOCK, "TIMEZONE", tz,
                                        error);
}

/* This works for old Gentoo */
static char *system_timezone_read_etc_conf_d_clock(void) {
  return system_timezone_read_key_file(ETC_CONF_D_CLOCK, "TIMEZONE");
}

static gboolean system_timezone_write_etc_conf_d_clock(const char *tz,
                                                       GError **error) {
  return system_timezone_write_key_file(ETC_CONF_D_CLOCK, "TIMEZONE", tz,
                                        error);
}

/* This works for Arch Linux */
static char *system_timezone_read_etc_rc_conf(void) {
  return system_timezone_read_key_file(ETC_RC_CONF, "TIMEZONE");
}

static gboolean system_timezone_write_etc_rc_conf(const char *tz,
                                                  GError **error) {
  return system_timezone_write_key_file(ETC_RC_CONF, "TIMEZONE", tz, error);
}
...
static char *system_timezone_strip_path_if_valid(const char *filename) {
  int skip;
...
static char *system_timezone_read_etc_localtime_softlink(void) {
  char *file;
  char *tz;

  if (!g_file_test(ETC_LOCALTIME, G_FILE_TEST_IS_SYMLINK)) return NULL;
...
typedef gboolean (*CompareFiles)(struct stat *a_stat, struct stat *b_stat,
                                 const char *a_content, gsize a_content_len,
                                 const char *b_filename);

static char *recursive_compare(struct stat *localtime_stat,
                               const char *localtime_content,
                               gsize localtime_content_len, char *file,
                               CompareFiles compare_func) {
  struct stat file_stat;

  if (g_stat(file, &file_stat) != 0) return NULL;

  if (S_ISREG(file_stat.st_mode)) {
    if (compare_func(localtime_stat, &file_stat, localtime_content,
                     localtime_content_len, file))
    ...
    while ((subfile = g_dir_read_name(dir)) != NULL) {
      ...
      ret = recursive_compare(localtime_stat, localtime_content,
                              localtime_content_len, subpath, compare_func);
...
static gboolean files_are_identical_inode(struct stat *a_stat,
                                          struct stat *b_stat,
 ...
 * the inodes */
static char *system_timezone_read_etc_localtime_hardlink(void) {
  struct stat stat_localtime;

  if (g_stat(ETC_LOCALTIME, &stat_localtime) != 0) return NULL;

  if (!S_ISREG(stat_localtime.st_mode)) return NULL;

  return recursive_compare(&stat_localtime, NULL, 0, SYSTEM_ZONEINFODIR,
                           files_are_identical_inode);
}

static gboolean files_are_identical_content(struct stat *a_stat,
                                            struct stat *b_stat,
                                            const char *a_content,
                                            gsize a_content_len,
                                            const char *b_filename) {
  ...
  if (a_stat->st_size != b_stat->st_size) return FALSE;

  if (!g_file_get_contents(b_filename, &b_content, &b_content_len, NULL))
    return FALSE;
...
static char *system_timezone_read_etc_localtime_content(void) {
  struct stat stat_localtime;
  ...
  if (!g_file_get_contents(ETC_LOCALTIME, &localtime_content,
                           &localtime_content_len, NULL))
    return NULL;

  retval = recursive_compare(&stat_localtime, localtime_content,
                             localtime_content_len, SYSTEM_ZONEINFODIR,
...
static GetSystemTimezone get_system_timezone_methods[] = {
    ...
    system_timezone_read_etc_timezone, system_timezone_read_etc_sysconfig_clock,
    system_timezone_read_etc_sysconfig_clock_alt,
    system_timezone_read_etc_TIMEZONE, system_timezone_read_etc_rc_conf,
    ...
    system_timezone_read_etc_localtime_content, NULL};

static gboolean system_timezone_is_valid(const char *tz) {
  const char *c;
...
static gboolean system_timezone_is_zone_file_valid(const char *zone_file,
                                                   GError **error) {
  GError *our_error;
  GIOChannel *channel;
  ...
  if (!our_error)
    g_io_channel_read_chars(channel, buffer, strlen(TZ_MAGIC), &read,
                            &our_error);
  ...
  if (our_error) {
    g_set_error(error, SYSTEM_TIMEZONE_ERROR,
                SYSTEM_TIMEZONE_ERROR_INVALID_TIMEZONE_FILE,
                "Timezone file %s cannot be read: %s", zone_file,
                our_error->message);
    g_error_free(our_error);
    return FALSE;
  }

  if (read != strlen(TZ_MAGIC) ||
      strncmp(buffer, TZ_MAGIC, strlen(TZ_MAGIC)) != 0) {
    g_set_error(error, SYSTEM_TIMEZONE_ERROR,
                SYSTEM_TIMEZONE_ERROR_INVALID_TIMEZONE_FILE,
                "%s is not a timezone file", zone_file);
    return FALSE;
  }
...
static gboolean system_timezone_set_etc_timezone(const char *zone_file,
                                                 GError **error) {
  GError *our_error;
  char *content;
  gsize len;

  if (!system_timezone_is_zone_file_valid(zone_file, error)) return FALSE;

  /* If /etc/localtime is a symlink, write a symlink */
  if (g_file_test(ETC_LOCALTIME, G_FILE_TEST_IS_SYMLINK)) {
    if (g_unlink(ETC_LOCALTIME) == 0 && symlink(zone_file, ETC_LOCALTIME) == 0)
      return TRUE;
  ...
  if (!g_file_set_contents(ETC_LOCALTIME, content, len, &our_error)) {
    g_set_error(error, SYSTEM_TIMEZONE_ERROR, SYSTEM_TIMEZONE_ERROR_GENERAL,
                ETC_LOCALTIME " cannot be overwritten: %s", our_error->message);
    g_error_free(our_error);
    g_free(content);
    return FALSE;
  }
...
typedef gboolean (*SetSystemTimezone)(const char *tz, GError **error);
...
static SetSystemTimezone set_system_timezone_methods[] = {
    ...
    system_timezone_write_etc_TIMEZONE, system_timezone_write_etc_rc_conf,
    /* writing deprecated config files if they exist and have the
     * setting already present */
    system_timezone_write_etc_conf_d_clock, NULL};

static gboolean system_timezone_update_config(const char *tz, GError **error) {
  int i;

  for (i = 0; set_system_timezone_methods[i] != NULL; i++) {
    if (!set_system_timezone_methods[i](tz, error)) return FALSE;
