...
static GDBusProxy *get_bus_proxy(void) {
  GError *error = NULL;
  static GDBusProxy *proxy = NULL;
  if (proxy == NULL) {
    proxy = g_dbus_proxy_new_for_bus_sync(
        G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL, DATETIME_DBUS_NAME,
        DATETIME_DBUS_PATH, DATETIME_DBUS_NAME, NULL, &error);
    if (proxy == NULL) {
...
static void notify_can_do(GObject *source_object, GAsyncResult *res,
                          gpointer user_data) {
  GDBusProxy *proxy;
  GVariant *variant;
  GError *error = NULL;
...
static void refresh_can_do(const gchar *action, CanDoFunc callback) {
  GDBusProxy *proxy;

  proxy = get_bus_proxy();
  if (proxy == NULL) return;

  g_dbus_proxy_call(proxy, action, g_variant_new("()"), G_DBUS_CALL_FLAGS_NONE,
                    G_MAXINT, NULL, notify_can_do, callback);
}

static gint settimezone_cache = 0;
static time_t settimezone_stamp = 0;

static void update_can_settimezone(gint res) {
...
static void update_can_settime(gint res) {
  settime_cache = res;
  time(&settime_stamp);
}
...
static void free_data(gpointer d) {
  ...
  data->ref_count--;
  if (data->ref_count == 0) {
    if (data->notify) data->notify(data->data);
    g_free(data->filename);
    g_free(data);
  }
}

static void set_time_notify(GObject *source_object, GAsyncResult *res,
                            gpointer user_data) {
  ...
  if (variant == NULL) {
    if (error != NULL) {
      if (data->callback) data->callback(data->data, error);
      g_error_free(error);
    } else {
      if (data->callback) data->callback(data->data, NULL);
    }
  } else {
    g_variant_unref(variant);
    if (data->callback) data->callback(data->data, NULL);
...
static void set_time_async(SetTimeCallbackData *data) {
  ...
  if (strcmp(data->call, "SetTime") == 0)
    g_dbus_proxy_call(proxy, "SetTime", g_variant_new("(x)", data->time),
                      G_DBUS_CALL_FLAGS_NONE, G_MAXINT, NULL, set_time_notify,
                      data);
  else
    g_dbus_proxy_call(
        proxy, "SetTimezone", g_variant_new("(s)", data->filename),
        G_DBUS_CALL_FLAGS_NONE, G_MAXINT, NULL, set_time_notify, data);
}

void set_system_time_async(gint64 time, GFunc callback, gpointer d,
                           GDestroyNotify notify) {
...
void set_system_timezone_async(const gchar *filename, GFunc callback,
                               gpointer d, GDestroyNotify notify) {
  SetTimeCallbackData *data;
