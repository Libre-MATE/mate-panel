...
#define _clock_get_widget(x, y) \
  (GTK_WIDGET(gtk_builder_get_object((x)->builder, (y))))

enum { COL_CITY_NAME = 0, COL_CITY_TZ, COL_CITY_LOC, COL_CITY_LAST };
...
static void set_atk_name_description(GtkWidget *widget, const char *name,
                                     const char *desc);
static void verb_display_properties_dialog(GtkAction *action, ClockData *cd);

static void display_properties_dialog(ClockData *cd,
                                      gboolean start_in_locations_page);
static void display_help_dialog(GtkAction *action, ClockData *cd);
...
static void applet_change_orient(MatePanelApplet *applet,
                                 MatePanelAppletOrient orient, ClockData *cd);
...
static void clock_box_init(ClockBox *box) {}

static void clock_box_class_init(ClockBoxClass *klass) {}

/* Clock */

static void unfix_size(ClockData *cd) {
  cd->fixed_width = -1;
  cd->fixed_height = -1;
  gtk_widget_queue_resize(cd->panel_button);
}

static int calculate_minimum_width(GtkWidget *widget, const gchar *text) {
...
static void clock_set_timeout(ClockData *cd, time_t now) {
  int timeouttime;
  ...
  if (cd->format == CLOCK_FORMAT_INTERNET) {
    ...
    if (cd->format != CLOCK_FORMAT_UNIX && !cd->showseconds &&
        (!cd->set_time_window || !gtk_widget_get_visible(cd->set_time_window)))
      timeouttime += 1000 * (59 - now % 60);
  }

  cd->timeout = g_timeout_add(timeouttime, clock_timeout_callback, cd);
}
...
static gboolean use_two_line_format(ClockData *cd) {
...
static char *get_updated_timeformat(ClockData *cd) {
  ...
  use_lctime = (env_language != NULL) && (env_lc_time != NULL) &&
               (g_strcmp0(env_language, env_lc_time) != 0);
  ...
  if (!cd->showdate)
    ...
    if (use_two_line_format(cd))
       ...
       */
      clock_format = g_strdup_printf(_("%1$s\n%2$s"), date_format, time_format);
    else
      /* translators: reverse the order of these arguments
       *              if the time should come before the
       *              date on a clock in your locale.
       */
      clock_format = g_strdup_printf(_("%1$s, %2$s"), date_format, time_format);
  }
...
static void update_timeformat(ClockData *cd) {
...
static void set_atk_name_description(GtkWidget *widget, const char *name,
  ...
  if (desc != NULL) atk_object_set_description(obj, desc);
  if (name != NULL) atk_object_set_name(obj, name);
}

static void update_location_tiles(ClockData *cd) {
  GSList *l;
...
static char *format_time(ClockData *cd) {
  struct tm *tm;
  char hour[256];
  char *utf8;
  ...
  if (cd->format == CLOCK_FORMAT_UNIX) {
    if (use_two_line_format(cd)) {
      ...
      utf8 = g_strdup_printf("%lu", (unsigned long)cd->current_time);
    }
    ...
    char *timeformat =
        g_locale_from_utf8(cd->custom_format, -1, NULL, NULL, NULL);
    if (!timeformat)
      strcpy(hour, "???");
  ...
  if (!utf8) utf8 = g_strdup(hour);

  return utf8;
}

static gchar *format_time_24(ClockData *cd) {
  struct tm *tm;
  gchar buf[128];
...
static void update_clock(ClockData *cd) {
  ...
  if (cd->map_widget && cd->calendar_popup &&
      gtk_widget_get_visible(cd->calendar_popup))
...
static void update_tooltip(ClockData *cd) {
  ...
  g_free(old_tip);
  if (!cd->showdate) g_free(tip);
}

static void refresh_clock(ClockData *cd) {
  unfix_size(cd);
  update_clock(cd);
}

static void refresh_clock_timeout(ClockData *cd) {
  unfix_size(cd);

  update_timeformat(cd);

  if (cd->timeout) g_source_remove(cd->timeout);
 ...
 */
static void refresh_click_timeout_time_only(ClockData *cd) {
  if (cd->timeout) g_source_remove(cd->timeout);
  clock_timeout_callback(cd);
}

static void free_locations(ClockData *cd) {
  if (cd->locations != NULL) {
    GSList *l;

    for (l = cd->locations; l; l = l->next) g_object_unref(l->data);
...
static void destroy_clock(GtkWidget *widget, ClockData *cd) {
  if (cd->settings) g_signal_handlers_disconnect_by_data(cd->settings, cd);

  if (cd->systz) g_signal_handlers_disconnect_by_data(cd->systz, cd);

  if (cd->settings) g_object_unref(cd->settings);
  cd->settings = NULL;

  if (cd->timeout) g_source_remove(cd->timeout);
  cd->timeout = 0;

  if (cd->props) gtk_widget_destroy(cd->props);
  cd->props = NULL;

  if (cd->calendar_popup) gtk_widget_destroy(cd->calendar_popup);
  ...
  if (cd->location_tiles) g_slist_free(cd->location_tiles);
...
static gboolean close_on_escape(GtkWidget *widget, GdkEventKey *event,
...
static void edit_locations_cb(CalendarWindow *calwin, gpointer data) {
  ClockData *cd;
...
static GtkWidget *create_calendar(ClockData *cd) {
  GtkWidget *window;
  char *prefs_path;

  prefs_path =
      mate_panel_applet_get_preferences_path(MATE_PANEL_APPLET(cd->applet));
  window = calendar_window_new(&cd->current_time, prefs_path,
                               cd->orient == MATE_PANEL_APPLET_ORIENT_UP);
  g_free(prefs_path);

  calendar_window_set_show_weeks(CALENDAR_WINDOW(window), cd->showweek);

  gtk_window_set_screen(GTK_WINDOW(window), gtk_widget_get_screen(cd->applet));

  g_signal_connect(window, "edit-locations", G_CALLBACK(edit_locations_cb), cd);

  g_signal_connect(window, "delete-event", G_CALLBACK(delete_event),
                   cd->panel_button);
  g_signal_connect(window, "key-press-event", G_CALLBACK(close_on_escape),
                   cd->panel_button);
...
static void position_calendar_popup(ClockData *cd) {
  ...
  gdk_window_get_origin(gtk_widget_get_window(cd->panel_button), &x, &y);
...
static void add_to_group(GtkWidget *child, gpointer data) {
  GtkSizeGroup *group = data;

  gtk_size_group_add_widget(group, child);
}

static void create_clock_window(ClockData *cd) {
  GtkWidget *locations_box;

  locations_box =
      calendar_window_get_locations_box(CALENDAR_WINDOW(cd->calendar_popup));
  ...
  gtk_container_foreach(GTK_CONTAINER(locations_box), (GtkCallback)add_to_group,
                        cd->clock_group);
}

static gint sort_locations_by_name(gconstpointer a, gconstpointer b) {
...
static void create_cities_store(ClockData *cd) {
  GtkTreeIter iter;
  ...
  cd->cities_store =
      gtk_list_store_new(COL_CITY_LAST, G_TYPE_STRING, /* COL_CITY_NAME */
  ...
  for (l = list; l; l = l->next) {
    ...
    gtk_list_store_set(cd->cities_store, &iter, COL_CITY_NAME,
                       clock_location_get_display_name(loc),
...
static gint sort_locations_by_time(gconstpointer a, gconstpointer b) {
  ClockLocation *loc_a = (ClockLocation *)a;
  ClockLocation *loc_b = (ClockLocation *)b;
...
static gint sort_locations_by_time_reverse(gconstpointer a, gconstpointer b) {
  return sort_locations_by_time(b, a);
}

static gint sort_locations_by_time_reverse_and_name(gconstpointer a,
                                                    gconstpointer b) {
  gint ret;
...
static ClockFormat location_tile_need_clock_format_cb(ClockLocationTile *tile,
                                                      gpointer data) {
  ClockData *cd = data;
...
static void create_cities_section(ClockData *cd) {
  GSList *node;
  GSList *cities;
  GSList *l;
  ...
  for (l = node; l; l = g_slist_next(l)) {
    ...
    g_signal_connect(city, "tile-pressed", G_CALLBACK(location_tile_pressed_cb),
                     cd);
    g_signal_connect(city, "need-clock-format",
                     G_CALLBACK(location_tile_need_clock_format_cb), cd);

    gtk_box_pack_start(GTK_BOX(cd->cities_section), GTK_WIDGET(city), FALSE,
                       FALSE, 0);
  ...
  gtk_box_pack_end(GTK_BOX(cd->clock_vbox), cd->cities_section, FALSE, FALSE,
                   0);

  gtk_widget_show_all(cd->cities_section);
}

static GSList *map_need_locations_cb(ClockMap *map, gpointer data) {
  ClockData *cd = data;

  return cd->locations;
}

static void create_map_section(ClockData *cd) {
  ClockMap *map;
  ...
  map = clock_map_new();
  g_signal_connect(map, "need-locations", G_CALLBACK(map_need_locations_cb),
                   cd);
...
static void update_calendar_popup(ClockData *cd) {
...
static void toggle_calendar(GtkWidget *button, ClockData *cd) {
...
static gboolean do_not_eat_button_press(GtkWidget *widget,
...
*/
static void clock_update_text_gravity(GtkWidget *label) {
  PangoLayout *layout;
...
static inline void force_no_button_vertical_padding(GtkWidget *widget) {
...
static GtkWidget *create_main_clock_button(void) {
  GtkWidget *button;
...
static GtkWidget *create_main_clock_label(ClockData *cd) {
  GtkWidget *label;

  label = gtk_label_new(NULL);
  /*Fixme-this is invalid for labels with any recent GTK3 version, maybe all of
   * them*/
  /*
          g_signal_connect (label, "size-request",
                            ...
                            cd);
  */
  g_signal_connect_swapped(label, "style-set", G_CALLBACK(unfix_size), cd);
  gtk_label_set_justify(GTK_LABEL(label), GTK_JUSTIFY_CENTER);
  clock_update_text_gravity(label);
  g_signal_connect(label, "screen-changed",
                   G_CALLBACK(clock_update_text_gravity), NULL);

  return label;
}

static gboolean weather_tooltip(GtkWidget *widget, gint x, gint y,
  ...
  for (l = locations; l; l = l->next) {
    ...
    if (clock_location_is_current(location)) {
      ...
      if (!info || !weather_info_is_valid(info)) continue;
...
static void create_clock_widget(ClockData *cd) {
  /* Main toggle button */
  cd->panel_button = create_main_clock_button();
  g_signal_connect(cd->panel_button, "button-press-event",
                   G_CALLBACK(do_not_eat_button_press), NULL);
  g_signal_connect(cd->panel_button, "toggled", G_CALLBACK(toggle_calendar),
                   cd);
  g_signal_connect(cd->panel_button, "destroy", G_CALLBACK(destroy_clock), cd);
  gtk_widget_show(cd->panel_button);

  /* Main orientable box */
  cd->main_obox = g_object_new(clock_box_get_type(), NULL);
  gtk_box_set_spacing(GTK_BOX(cd->main_obox),
                      12); /* spacing between weather and time */
  ...
  gtk_box_set_spacing(GTK_BOX(cd->weather_obox),
                      2); /* spacing between weather icon and temperature */
  ...
  gtk_box_pack_start(GTK_BOX(cd->weather_obox), cd->panel_weather_icon, FALSE,
                     FALSE, 0);

  cd->panel_temperature_label = gtk_label_new(NULL);
  gtk_box_pack_start(GTK_BOX(cd->weather_obox), cd->panel_temperature_label,
                     FALSE, FALSE, 0);
  ...
  set_atk_name_description(GTK_WIDGET(cd->applet), NULL, _("Computer Clock"));
  ...
  applet_change_orient(
      MATE_PANEL_APPLET(cd->applet),
      mate_panel_applet_get_orient(MATE_PANEL_APPLET(cd->applet)), cd);
}
...
static void panel_button_change_pixel_size(GtkWidget *widget,
                                           GtkAllocation *allocation,
                                           ClockData *cd) {
  int new_size;
...
static void copy_time(GtkAction *action, ClockData *cd) {
  char string[256];
  char *utf8;

  if (cd->format == CLOCK_FORMAT_UNIX) {
    g_snprintf(string, sizeof(string), "%lu", (unsigned long)cd->current_time);
  } else if (cd->format == CLOCK_FORMAT_INTERNET) {
    float itime = get_itime(cd->current_time);
    if (cd->showseconds)
      g_snprintf(string, sizeof(string), "@%3.2f", itime);
    else
      g_snprintf(string, sizeof(string), "@%3d", (unsigned int)itime);
  } else {
    struct tm *tm;
    char *format;

    if (cd->format == CLOCK_FORMAT_CUSTOM) {
      format = g_locale_from_utf8(cd->custom_format, -1, NULL, NULL, NULL);
    } else if (cd->format == CLOCK_FORMAT_12) {
      if (cd->showseconds)
  ...
  gtk_clipboard_set_text(gtk_clipboard_get(GDK_SELECTION_PRIMARY), utf8, -1);
  gtk_clipboard_set_text(gtk_clipboard_get(GDK_SELECTION_CLIPBOARD), utf8, -1);
  g_free(utf8);
}

static void copy_date(GtkAction *action, ClockData *cd) {
  struct tm *tm;
  char string[256];
  char *utf8, *loc;
...
static void update_set_time_button(ClockData *cd) {
  gint can_set;
  ...
  if (cd->set_time_button) {
    ...
    gtk_button_set_label(
        GTK_BUTTON(cd->set_time_button),
        can_set == 1 ? _("Set System Time...") : _("Set System Time"));
  }
}

static void set_time_callback(ClockData *cd, GError *error) {
  GtkWidget *window;
...
static void set_time(GtkWidget *widget, ClockData *cd) {
  struct tm t;
  time_t tim;
  ...
  t.tm_sec =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(cd->seconds_spin));
  t.tm_min =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(cd->minutes_spin));
...
static void cancel_time_settings(GtkWidget *button, ClockData *cd) {
  gtk_widget_hide(cd->set_time_window);

  refresh_click_timeout_time_only(cd);
}

static gboolean delete_time_settings(GtkWidget *widget, GdkEvent *event,
                                     gpointer data) {
...
static void fill_time_settings_window(ClockData *cd) {
  time_t now_t;
  struct tm now;
...
static void wrap_cb(GtkSpinButton *spin, ClockData *cd) {
  gdouble value;
  gdouble min, max;
...
static gboolean output_cb(GtkSpinButton *spin, gpointer data) {
  GtkAdjustment *adj;
  gchar *text;
  gdouble value;
...
static void ensure_time_settings_window_is_created(ClockData *cd) {
  GtkWidget *cancel_button;

  if (cd->set_time_window) return;
  ...
  g_signal_connect(cancel_button, "clicked", G_CALLBACK(cancel_time_settings),
                   cd);

  cd->current_time_label = _clock_get_widget(cd, "current_time_label");
}

static void run_time_settings(GtkWidget *unused, ClockData *cd) {
...
static void config_date(GtkAction *action, ClockData *cd) {
  run_time_settings(NULL, cd);
}

/* current timestamp */
static const GtkActionEntry clock_menu_actions[] = {
    {"ClockPreferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(verb_display_properties_dialog)},
    {"ClockHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(display_help_dialog)},
    {"ClockAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(display_about_dialog)},
    {"ClockCopyTime", "edit-copy", N_("Copy _Time"), NULL, NULL,
     G_CALLBACK(copy_time)},
    {"ClockCopyDate", "edit-copy", N_("Copy _Date"), NULL, NULL,
     G_CALLBACK(copy_date)},
    {"ClockConfig", "preferences-system", N_("Ad_just Date & Time"), NULL, NULL,
     G_CALLBACK(config_date)}};

static void format_changed(GSettings *settings, gchar *key, ClockData *clock) {
  ...
  if (new_format == clock->format) return;
...
static void show_seconds_changed(GSettings *settings, gchar *key,
                                 ClockData *clock) {
  clock->showseconds = g_settings_get_boolean(settings, key);
  refresh_clock_timeout(clock);
}

static void show_date_changed(GSettings *settings, gchar *key,
                              ClockData *clock) {
...
static void update_weather_bool_value_and_toggle_from_gsettings(
    ClockData *cd, gchar *key, gboolean *value_loc, const char *widget_name) {
  GtkWidget *widget;
  gboolean value;
  ...
  *value_loc = (value != 0);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), *value_loc);

  update_panel_weather(cd);
}

static void show_weather_changed(GSettings *settings, gchar *key,
                                 ClockData *cd) {
  update_weather_bool_value_and_toggle_from_gsettings(
      cd, key, &cd->show_weather, "weather_check");
}

static void show_temperature_changed(GSettings *settings, gchar *key,
                                     ClockData *cd) {
  update_weather_bool_value_and_toggle_from_gsettings(
      cd, key, &cd->show_temperature, "temperature_check");
}

static void weather_icon_updated_cb(MatePanelApplet *applet, gint icon_size,
                                    gpointer data) {
  ClockData *cd = data;
  GtkIconTheme *theme;
  ...
  if (cd->weather_icon_name == NULL) return;

  theme = gtk_icon_theme_get_for_screen(
      gtk_widget_get_screen(GTK_WIDGET(cd->applet)));
  ...
  surface = gtk_icon_theme_load_surface(
      theme, cd->weather_icon_name, icon_size, icon_scale, NULL,
...
static void location_weather_updated_cb(ClockLocation *location,
                                        WeatherInfo *info, gpointer data) {
  ClockData *cd = data;
  const gchar *temp;
  GtkIconTheme *theme;
  cairo_surface_t *surface;
  ...
  if (!info || !weather_info_is_valid(info)) return;

  if (!clock_location_is_current(location)) return;
...
static void location_set_current_cb(ClockLocation *loc, gpointer data) {
  ClockData *cd = data;
  WeatherInfo *info;
  ...
  if (cd->map_widget) clock_map_refresh(CLOCK_MAP(cd->map_widget));
  update_location_tiles(cd);
  save_cities_store(cd);
}

static void locations_changed(ClockData *cd) {
  GSList *l;
  glong id;

  if (!cd->locations) {
    if (cd->weather_obox) gtk_widget_hide(cd->weather_obox);
    if (cd->panel_weather_icon)
      gtk_image_set_from_pixbuf(GTK_IMAGE(cd->panel_weather_icon), NULL);
    if (cd->panel_temperature_label)
      gtk_label_set_text(GTK_LABEL(cd->panel_temperature_label), "");
  } else {
    if (cd->weather_obox) gtk_widget_show(cd->weather_obox);
  }
  ...
  for (l = cd->locations; l; l = l->next) {
    ...
    if (id == 0) {
      ...
      g_signal_connect(loc, "set-current", G_CALLBACK(location_set_current_cb),
                       cd);
    }
  }
  ...
  if (cd->clock_vbox) create_cities_section(cd);
}

static void set_locations(ClockData *cd, GSList *locations) {
  free_locations(cd);
...
static void location_start_element(GMarkupParseContext *context,
  ...
  loc = clock_location_find_and_ref(cd->locations, name, city, timezone,
                                    latitude, longitude, code);
  if (!loc)
    loc = clock_location_new(name, city, timezone, latitude, longitude, code,
                             &prefs);
...
static GMarkupParser location_parser = {location_start_element, NULL, NULL,
                                        NULL, NULL};

static void cities_changed(GSettings *settings, gchar *key, ClockData *cd) {
...
static void update_weather_locations(ClockData *cd) {
  GSList *locations, *l;
  WeatherPrefs prefs = {FORECAST_STATE,       FALSE,         NULL,
                        TEMP_UNIT_CENTIGRADE, SPEED_UNIT_MS, PRESSURE_UNIT_MB,
                        DISTANCE_UNIT_KM};
...
static void clock_timezone_changed(SystemTimezone *systz, const char *new_tz,
                                   ClockData *cd) {
...
#define unit_combo_box_set_active(x, y) \
  (gtk_combo_box_set_active(            \
      GTK_COMBO_BOX(gtk_builder_get_object(cd->builder, (x))), (y)))

static void temperature_unit_changed(GSettings *settings, gchar *key,
                                     ClockData *cd) {
  TempUnit temperature_unit = g_settings_get_enum(settings, key);

  if (cd->temperature_unit == temperature_unit) return;

  /* TEMP_UNIT_INVALID = 0 */
  if (temperature_unit == TEMP_UNIT_INVALID) return;

  unit_combo_box_set_active("temperature_combo", ((int)temperature_unit) - 2);
  cd->temperature_unit = temperature_unit;
  update_weather_locations(cd);
}

static void speed_unit_changed(GSettings *settings, gchar *key, ClockData *cd) {
  SpeedUnit speed_unit = g_settings_get_enum(settings, key);

  if (cd->speed_unit == speed_unit) return;

  /* SPEED_UNIT_INVALID = 0 */
  if (speed_unit == SPEED_UNIT_INVALID) return;

  unit_combo_box_set_active("wind_speed_combo", ((int)speed_unit) - 2);
  cd->speed_unit = speed_unit;
  update_weather_locations(cd);
}

static void custom_format_changed(GSettings *settings, gchar *key,
                                  ClockData *clock) {
  gchar *value;
  ...
  if (clock->format == CLOCK_FORMAT_CUSTOM) refresh_clock(clock);
  g_free(value);
}

static void show_week_changed(GSettings *settings, gchar *key,
                              ClockData *clock) {
  gboolean value;

  value = g_settings_get_boolean(settings, key);

  if (clock->showweek == (value != 0)) return;
...
static void setup_gsettings(ClockData *cd) {
  gint format;
  gchar *custom_format;

  cd->settings = mate_panel_applet_settings_new(MATE_PANEL_APPLET(cd->applet),
                                                CLOCK_SCHEMA);
  ...
  g_signal_connect(cd->settings, "changed::" KEY_FORMAT,
                   G_CALLBACK(format_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_SHOW_SECONDS,
                   G_CALLBACK(show_seconds_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_SHOW_DATE,
                   G_CALLBACK(show_date_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_SHOW_WEATHER,
                   G_CALLBACK(show_weather_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_SHOW_TEMPERATURE,
                   G_CALLBACK(show_temperature_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_CUSTOM_FORMAT,
                   G_CALLBACK(custom_format_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_SHOW_WEEK,
                   G_CALLBACK(show_week_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_CITIES,
                   G_CALLBACK(cities_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_TEMPERATURE_UNIT,
                   G_CALLBACK(temperature_unit_changed), cd);
  g_signal_connect(cd->settings, "changed::" KEY_SPEED_UNIT,
                   G_CALLBACK(speed_unit_changed), cd);
}

static GSList *parse_gsettings_cities(ClockData *cd, gchar **values) {
  gint i;
  LocationParserData data;
  GMarkupParseContext *context;
...
static void load_gsettings(ClockData *cd) {
  gchar **values;
  GSList *cities = NULL;

  cd->format = g_settings_get_enum(cd->settings, KEY_FORMAT);

  if (cd->format == CLOCK_FORMAT_INVALID) cd->format = clock_locale_format();
  ...
  cd->show_temperature =
      g_settings_get_boolean(cd->settings, KEY_SHOW_TEMPERATURE);
  ...
  cd->temperature_unit =
      g_settings_get_enum(cd->settings, KEY_TEMPERATURE_UNIT);
...
static gboolean fill_clock_applet(MatePanelApplet *applet) {
  ClockData *cd;
  ...
  gtk_builder_add_from_resource(cd->builder, CLOCK_RESOURCE_PATH "clock.ui",
                                NULL);
  ...
  g_signal_connect(cd->applet, "change-orient",
                   G_CALLBACK(applet_change_orient), cd);

  g_signal_connect(cd->panel_button, "size-allocate",
                   G_CALLBACK(panel_button_change_pixel_size), cd);

  action_group = gtk_action_group_new("ClockApplet Menu Actions");
  gtk_action_group_set_translation_domain(action_group, GETTEXT_PACKAGE);
  gtk_action_group_add_actions(action_group, clock_menu_actions,
                               G_N_ELEMENTS(clock_menu_actions), cd);
  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(cd->applet), CLOCK_RESOURCE_PATH "clock-menu.xml",
      action_group);
  ...
  g_signal_connect(cd->systz, "changed", G_CALLBACK(clock_timezone_changed),
                   cd);
  ...
  g_signal_connect(cd->applet, "change-size",
                   G_CALLBACK(weather_icon_updated_cb), cd);

  return TRUE;
}

static void prefs_locations_changed(GtkTreeSelection *selection,
                                    ClockData *cd) {
  gint n;
...
static gchar *loc_to_string(ClockLocation *loc) {
  ...
  ret = g_markup_printf_escaped(
      "<location name=\"%s\" city=\"%s\" timezone=\"%s\" latitude=\"%f\" "
      "longitude=\"%f\" code=\"%s\" current=\"%s\"/>",
      name ? name : "", city ? city : "", clock_location_get_timezone(loc),
      latitude, longitude, clock_location_get_weather_code(loc),
...
static void run_prefs_edit_save(GtkButton *button, ClockData *cd) {
  GtkWidget *edit_window = _clock_get_widget(cd, "edit-location-window");

  ClockLocation *loc =
      g_object_get_data(G_OBJECT(edit_window), "clock-location");
...
static void update_coords_helper(gfloat value, GtkWidget *entry,
                                 GtkWidget *combo) {
  gchar *tmp;
...
static void update_coords(ClockData *cd, gboolean valid, gfloat lat,
                          gfloat lon) {
  GtkWidget *lat_entry = _clock_get_widget(cd, "edit-location-latitude-entry");
  GtkWidget *lon_entry = _clock_get_widget(cd, "edit-location-longitude-entry");
  GtkWidget *lat_combo = _clock_get_widget(cd, "edit-location-latitude-combo");
...
static void fill_timezone_combo_from_location(ClockData *cd,
                                              ClockLocation *loc) {
  if (loc != NULL) {
...
static void location_update_ok_sensitivity(ClockData *cd) {
  GtkWidget *ok_button;
...
static void location_changed(GObject *object, GParamSpec *param,
                             ClockData *cd) {
  ...
  if (zone)
    mateweather_timezone_menu_set_tzid(cd->zone_combo,
                                       mateweather_timezone_get_tzid(zone));
  else
    mateweather_timezone_menu_set_tzid(cd->zone_combo, NULL);

  if (gloc) mateweather_location_unref(gloc);
}

static void location_name_changed(GObject *object, ClockData *cd) {
  location_update_ok_sensitivity(cd);
}

static void location_timezone_changed(GObject *object, GParamSpec *param,
                                      ClockData *cd) {
  location_update_ok_sensitivity(cd);
}

static void edit_clear(ClockData *cd) {
  GtkWidget *lat_entry = _clock_get_widget(cd, "edit-location-latitude-entry");
  GtkWidget *lon_entry = _clock_get_widget(cd, "edit-location-longitude-entry");
  GtkWidget *lat_combo = _clock_get_widget(cd, "edit-location-latitude-combo");
  GtkWidget *lon_combo = _clock_get_widget(cd, "edit-location-longitude-combo");
...
static gboolean edit_hide_event(GtkWidget *widget, GdkEvent *event,
                                ClockData *cd) {
  edit_hide(widget, cd);
...
static void prefs_hide(GtkWidget *widget, ClockData *cd) {
  GtkWidget *tree;
...
static gboolean prefs_hide_event(GtkWidget *widget, GdkEvent *event,
                                 ClockData *cd) {
...
static void prefs_help(GtkWidget *widget, ClockData *cd) {
  clock_utils_display_help(cd->prefs_window, "mate-clock", "clock-settings");
}

static void remove_tree_row(GtkTreeModel *model, GtkTreePath *path,
...
static void run_prefs_locations_remove(GtkButton *button, ClockData *cd) {
  GtkTreeSelection *sel =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(cd->prefs_locations));

  gtk_tree_selection_selected_foreach(sel, remove_tree_row, cd);
}

static void run_prefs_locations_add(GtkButton *button, ClockData *cd) {
  GtkWidget *edit_window = _clock_get_widget(cd, "edit-location-window");
  ...
  if (g_object_get_data(G_OBJECT(edit_window), "delete-handler") == NULL) {
    g_object_set_data(
        G_OBJECT(edit_window), "delete-handler",
        GINT_TO_POINTER(g_signal_connect(edit_window, "delete-event",
                                         G_CALLBACK(edit_delete), cd)));
  }
  ...
  gtk_window_present_with_time(GTK_WINDOW(edit_window),
                               gtk_get_current_event_time());
}

static void edit_tree_row(GtkTreeModel *model, GtkTreePath *path,
                          GtkTreeIter *iter, gpointer data) {
  ClockData *cd = data;
  ClockLocation *loc;
  const char *name;
...
static void run_prefs_locations_edit(GtkButton *unused, ClockData *cd) {
  GtkTreeSelection *sel =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(cd->prefs_locations));
...
static void set_12hr_format_radio_cb(GtkWidget *widget, ClockData *cd) {
  ClockFormat format;
...
static void temperature_combo_changed(GtkComboBox *combo, ClockData *cd) {
  int value;
  int old_value;
...
static void speed_combo_changed(GtkComboBox *combo, ClockData *cd) {
  int value;
  int old_value;
...
static void fill_prefs_window(ClockData *cd) {
  static const int temperatures[] = {TEMP_UNIT_KELVIN, TEMP_UNIT_CENTIGRADE,
                                     TEMP_UNIT_FAHRENHEIT, -1};

  static const int speeds[] = {SPEED_UNIT_MS,    SPEED_UNIT_KPH, SPEED_UNIT_MPH,
                               SPEED_UNIT_KNOTS, SPEED_UNIT_BFT, -1};
  ...
  g_signal_connect(radio_12hr, "toggled", G_CALLBACK(set_12hr_format_radio_cb),
                   cd);
  ...
  col = gtk_tree_view_column_new_with_attributes(_("City Name"), renderer,
                                                 "text", COL_CITY_NAME, NULL);
  gtk_tree_view_insert_column(GTK_TREE_VIEW(widget), col, -1);

  renderer = gtk_cell_renderer_text_new();
  col = gtk_tree_view_column_new_with_attributes(_("City Time Zone"), renderer,
                                                 "text", COL_CITY_TZ, NULL);
  gtk_tree_view_insert_column(GTK_TREE_VIEW(widget), col, -1);

  if (cd->cities_store == NULL) create_cities_store(cd);
  ...
  for (i = 0; temperatures[i] != -1; i++)
    gtk_list_store_insert_with_values(
        store, &iter, -1, 0,
        mateweather_prefs_get_temp_display_name(temperatures[i]), -1);

  if (cd->temperature_unit > 0)
    gtk_combo_box_set_active(GTK_COMBO_BOX(widget), cd->temperature_unit - 2);
  g_signal_connect(widget, "changed", G_CALLBACK(temperature_combo_changed),
                   cd);
  ...
  for (i = 0; speeds[i] != -1; i++)
    gtk_list_store_insert_with_values(
        store, &iter, -1, 0,
        mateweather_prefs_get_speed_display_name(speeds[i]), -1);

  if (cd->speed_unit > 0)
    gtk_combo_box_set_active(GTK_COMBO_BOX(widget), cd->speed_unit - 2);
  g_signal_connect(widget, "changed", G_CALLBACK(speed_combo_changed), cd);
}
...
static void ensure_prefs_window_is_created(ClockData *cd) {
  ...
  if (!clock_locale_supports_am_pm()) gtk_widget_hide(clock_options);

  selection = gtk_tree_view_get_selection(cd->prefs_locations);
  g_signal_connect(selection, "changed", G_CALLBACK(prefs_locations_changed),
                   cd);

  g_signal_connect(cd->prefs_window, "delete-event",
                   G_CALLBACK(prefs_hide_event), cd);

  g_signal_connect(prefs_close_button, "clicked", G_CALLBACK(prefs_hide), cd);

  g_signal_connect(prefs_help_button, "clicked", G_CALLBACK(prefs_help), cd);

  cd->prefs_location_remove_button =
      _clock_get_widget(cd, "prefs-locations-remove-button");

  g_signal_connect(cd->prefs_location_remove_button, "clicked",
                   G_CALLBACK(run_prefs_locations_remove), cd);

  cd->prefs_location_add_button =
      _clock_get_widget(cd, "prefs-locations-add-button");

  g_signal_connect(cd->prefs_location_add_button, "clicked",
                   G_CALLBACK(run_prefs_locations_add), cd);

  cd->prefs_location_edit_button =
      _clock_get_widget(cd, "prefs-locations-edit-button");
  ...
  g_signal_connect(edit_window, "delete-event", G_CALLBACK(edit_hide_event),
                   cd);
  ...
  cd->location_entry =
      MATEWEATHER_LOCATION_ENTRY(mateweather_location_entry_new(world));
  gtk_widget_show(GTK_WIDGET(cd->location_entry));
  gtk_container_add(GTK_CONTAINER(location_box),
  ...
  cd->zone_combo =
      MATEWEATHER_TIMEZONE_MENU(mateweather_timezone_menu_new(world));
  ...
  g_signal_connect(edit_cancel_button, "clicked", G_CALLBACK(edit_hide), cd);

  g_signal_connect(edit_ok_button, "clicked", G_CALLBACK(run_prefs_edit_save),
                   cd);
...
static void display_about_dialog(GtkAction *action, ClockData *cd) {
  static const gchar *authors[] = {"George Lebl <jirka@5z.com>",
                                   "Gediminas Paulauskas <menesis@delfi.lt>",
                                   NULL};

  static const char *documenters[] = {"Dan Mueth <d-mueth@uchicago.edu>", NULL};

  gtk_show_about_dialog(
      NULL, "program-name", _("Clock"), "title", _("About Clock"), "authors",
      authors, "comments", _("The Clock displays the current time and date"),
      "copyright",
      _("Copyright \xc2\xa9 1998-2004 Free Software Foundation, Inc.\n"
        "Copyright \xc2\xa9 2012-2021 MATE developers"),
      "documenters", documenters, "logo-icon-name", CLOCK_ICON,
      "translator-credits", _("translator-credits"), "version", VERSION,
      "website", PACKAGE_URL, NULL);
}

static gboolean clock_factory(MatePanelApplet *applet, const char *iid,
                              gpointer data) {
  gboolean retval = FALSE;

  if (!strcmp(iid, "ClockApplet")) retval = fill_clock_applet(applet);

  return retval;
}

#ifdef CLOCK_INPROCESS
MATE_PANEL_APPLET_IN_PROCESS_FACTORY("ClockAppletFactory", PANEL_TYPE_APPLET,
                                     "ClockApplet", clock_factory, NULL)
#else
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("ClockAppletFactory", PANEL_TYPE_APPLET,
                                      "ClockApplet", clock_factory, NULL)
#endif
