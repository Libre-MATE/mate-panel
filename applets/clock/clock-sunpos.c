...
#define EPSILON_G \
  279.403303 /* degrees */             /* ecliptic longitude at epoch 1990.0 */
...
#define MEAN_OBLIQUITY \
  23.440592 /* degrees */ /* mean obliquity of earth's axis at epoch 1990.0 */
...
#define RADS_TO_DEG(x) (x * 180.0 / G_PI)

/* Calculate number of days since 4713BC.
 */
static gdouble unix_time_to_julian_date(gint unix_time) {
...
static gdouble solve_keplers_equation(gdouble e, gdouble M) {
  gdouble d, E;
 ...
 * Section 27.  */
static void ecliptic_to_equatorial(gdouble lambda, gdouble beta, gdouble *ra,
                                   gdouble *dec) {
  gdouble cos_mo;
  gdouble sin_mo;
...
static gdouble greenwich_sidereal_time(gdouble unix_time) {
