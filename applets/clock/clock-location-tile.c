...
enum { TILE_PRESSED, NEED_CLOCK_FORMAT, LAST_SIGNAL };
...
static void update_weather_icon(ClockLocation *loc, WeatherInfo *info,
                                gpointer data);
...
ClockLocationTile *clock_location_tile_new(ClockLocation *loc,
  ...
  priv->location_weather_updated_id = g_signal_connect(
      loc, "weather-updated", G_CALLBACK(update_weather_icon), this);

  return this;
}

static void clock_location_tile_class_init(ClockLocationTileClass *this_class) {
  GObjectClass *g_obj_class = G_OBJECT_CLASS(this_class);

  g_obj_class->finalize = clock_location_tile_finalize;

  signals[TILE_PRESSED] = g_signal_new(
      "tile-pressed", G_TYPE_FROM_CLASS(g_obj_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(ClockLocationTileClass, tile_pressed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[NEED_CLOCK_FORMAT] = g_signal_new(
      "need-clock-format", G_TYPE_FROM_CLASS(g_obj_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(ClockLocationTileClass, need_clock_format), NULL, NULL,
      _clock_marshal_INT__VOID, G_TYPE_INT, 0);
}

static void clock_location_tile_init(ClockLocationTile *this) {
  ClockLocationTilePrivate *priv =
      clock_location_tile_get_instance_private(this);
...
static void clock_location_tile_finalize(GObject *g_obj) {
  ClockLocationTile *this;
  ClockLocationTilePrivate *priv;
  ...
  if (priv->location) {
    ...
    g_clear_signal_handler(&priv->location_weather_updated_id, priv->location);
#else
...
static gboolean press_on_tile(GtkWidget *widget, GdkEventButton *event,
...
static void make_current_cb(gpointer data, GError *error) {
  if (error) {
    GtkWidget *dialog =
...
static void make_current(GtkWidget *widget, ClockLocationTile *tile) {
  ClockLocationTilePrivate *priv =
      clock_location_tile_get_instance_private(tile);

  clock_location_make_current(priv->location, (GFunc)make_current_cb, tile,
                              NULL);
}

static gboolean enter_or_leave_tile(GtkWidget *widget, GdkEventCrossing *event,
                                    ClockLocationTile *tile) {
  ClockLocationTilePrivate *priv =
      clock_location_tile_get_instance_private(tile);
  ...
  if (event->type == GDK_ENTER_NOTIFY) {
    ...
    if (can_set != 0) {
      gtk_label_set_markup(
          GTK_LABEL(priv->current_label),
          can_set == 1 ? _("<small>Set...</small>") : _("<small>Set</small>"));
      gtk_widget_hide(priv->current_spacer);
...
static void clock_location_tile_fill(ClockLocationTile *this) {
  ...
  gtk_widget_add_events(priv->box, GDK_BUTTON_PRESS_MASK |
                                       GDK_ENTER_NOTIFY_MASK |
                                       GDK_LEAVE_NOTIFY_MASK);
  g_signal_connect(priv->box, "button-press-event", G_CALLBACK(press_on_tile),
                   this);
  ...
  gtk_widget_set_tooltip_text(priv->current_button,
                              _("Set location as current location and use its "
  ...
  g_signal_connect(priv->current_button, "clicked", G_CALLBACK(make_current),
                   this);

  priv->clock_face =
      clock_face_new_with_location(priv->size, priv->location, head_section);
...
static gboolean clock_needs_face_refresh(ClockLocationTile *this) {
  ClockLocationTilePrivate *priv =
      clock_location_tile_get_instance_private(this);
  struct tm now;
...
static gboolean clock_needs_label_refresh(ClockLocationTile *this) {
  ClockLocationTilePrivate *priv =
      clock_location_tile_get_instance_private(this);
  struct tm now;
  long offset;
  ...
  if (now.tm_year > priv->last_refresh.tm_year ||
      ...
      now.tm_mday > priv->last_refresh.tm_mday ||
      now.tm_hour > priv->last_refresh.tm_hour ||
      now.tm_min > priv->last_refresh.tm_min || offset != priv->last_offset) {
    return TRUE;
  }
...
static void copy_tm(struct tm *from, struct tm *to) {
...
static char *format_time(struct tm *now, char *tzname, ClockFormat clock_format,
                         long offset) {
  char buf[256];
  char *format;
  time_t local_t;
  ...
  if (minutes != 0) {
    tmp = g_strdup_printf("%s <small>%s %+ld:%ld</small>", buf, tzname, hours,
                          minutes);
  } else if (hours != 0) {
...
static char *convert_time_to_str(time_t now, ClockFormat clock_format) {
  const gchar *format;
  struct tm *tm;
  gchar buf[128];
...
void weather_info_setup_tooltip(WeatherInfo *info, ClockLocation *location,
                                GtkTooltip *tooltip, ClockFormat clock_format) {
  ...
  pixbuf = gtk_icon_theme_load_icon_for_scale(
      theme, icon_name, 48, icon_scale, GTK_ICON_LOOKUP_GENERIC_FALLBACK, NULL);
  if (pixbuf) gtk_tooltip_set_icon(tooltip, pixbuf);

  conditions = weather_info_get_conditions(info);
  if (strcmp(conditions, "-") != 0)
    line1 =
        g_strdup_printf(_("%s, %s"), conditions, weather_info_get_sky(info));
  else
  ...
  if (strcmp(apparent, temp) != 0 &&
      /* FMQ: it's broken to read from another module's translations; add some
         API to libmateweather. */
  ...
  if (weather_info_get_value_sunset(info, &sunset_time))
    ...
    sunset_str = g_strdup("???");
  line4 =
      g_strdup_printf(_("Sunrise: %s / Sunset: %s"), sunrise_str, sunset_str);
  g_free(sunrise_str);
  g_free(sunset_str);
...
static gboolean weather_tooltip(GtkWidget *widget, gint x, gint y,
  ...
  if (!info || !weather_info_is_valid(info)) return FALSE;
...
static void update_weather_icon(ClockLocation *loc, WeatherInfo *info,
  ...
  theme = gtk_icon_theme_get_for_screen(
      gtk_widget_get_screen(GTK_WIDGET(priv->weather_icon)));
  icon_name = weather_info_get_icon_name(info);
  icon_scale = gtk_widget_get_scale_factor(GTK_WIDGET(priv->weather_icon));

  surface = gtk_icon_theme_load_surface(theme, icon_name, 16, icon_scale, NULL,
