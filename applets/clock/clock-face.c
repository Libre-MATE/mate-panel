...
static void clock_face_get_preferred_width(GtkWidget *this, gint *minimal_width,
                                           gint *natural_width);
...
static void update_time_and_face(ClockFace *this, gboolean force_face_loading);
static void clock_face_load_face(ClockFace *this, gint width, gint height);
...
static void clock_face_init(ClockFace *this) {
...
static gboolean clock_face_draw(GtkWidget *this, cairo_t *cr) {
  ...
  cairo_line_to(
      cr,
      x + radius * hour_length * sin(M_PI / 6 * hours + M_PI / 360 * minutes),
      y + radius * hour_length * -cos(M_PI / 6 * hours + M_PI / 360 * minutes));
  cairo_stroke(cr);
  cairo_restore(cr);
  /* minute hand:
...
static void clock_face_redraw_canvas(ClockFace *this) {
...
static void clock_face_get_preferred_width(GtkWidget *this, gint *minimal_width,
  ...
  if (priv->size_widget != NULL) {
    ...
    *minimal_width = *natural_width =
        gdk_pixbuf_get_width(GDK_PIXBUF(priv->face_pixbuf));
  } else {
...
static void clock_face_get_preferred_height(GtkWidget *this,
  ...
  if (priv->size_widget != NULL) {
    ...
    *minimal_height = *natural_height =
        gdk_pixbuf_get_height(GDK_PIXBUF(priv->face_pixbuf));
  } else {
    /* we don't know anything, so use known dimensions for the svg
     * files */
    if (priv->size == CLOCK_FACE_LARGE)
...
static void clock_face_size_allocate(GtkWidget *this,
                                     GtkAllocation *allocation) {
...
static void update_time_and_face(ClockFace *this, gboolean force_face_loading) {
  ...
  if (priv->timeofday != timeofday || force_face_loading) {
    ...
    if (width > 1 && height > 1) clock_face_load_face(this, width, height);
  }
}
...
GtkWidget *clock_face_new_with_location(ClockFaceSize size, ClockLocation *loc,
                                        GtkWidget *size_widget) {
  GObject *obj = g_object_new(INTL_TYPE_CLOCK_FACE, NULL);
  ClockFacePrivate *priv = clock_face_get_instance_private(CLOCK_FACE(obj));
...
static void clock_face_finalize(GObject *obj) {
  ClockFacePrivate *priv = clock_face_get_instance_private(CLOCK_FACE(obj));
...
static void remove_pixbuf_from_cache(const char *key, GObject *pixbuf) {
...
static void clock_face_load_face(ClockFace *this, gint width, gint height) {
  ...
  if (!pixbuf_cache)
    pixbuf_cache = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  ...
  cache_name = g_strdup_printf("%d-%d-%d-%d", priv->size, priv->timeofday,
                               width, height);
  ...
  name = g_strconcat(CLOCK_RESOURCE_PATH "icons/", "clock-face-",
                     size_string[priv->size], "-",
                     daytime_string[priv->timeofday], ".svg", NULL);
  priv->face_pixbuf =
      gdk_pixbuf_new_from_resource_at_scale(name, width, height, TRUE, NULL);
  g_free(name);
  ...
  if (priv->face_pixbuf) {
    g_hash_table_replace(pixbuf_cache, cache_name, priv->face_pixbuf);
    ...
    g_object_weak_ref(G_OBJECT(priv->face_pixbuf),
                      (GWeakNotify)remove_pixbuf_from_cache, cache_name);
  } else
    g_free(cache_name);
}
