...
enum { NEED_LOCATIONS, LAST_SIGNAL };

enum { MARKER_NORMAL = 0, MARKER_HILIGHT, MARKER_CURRENT, MARKER_NB };

static char *marker_files[MARKER_NB] = {"clock-map-location-marker.png",
...
static void clock_map_size_allocate(GtkWidget *this, GtkAllocation *allocation);
static gboolean clock_map_draw(GtkWidget *this, cairo_t *cr);
...
static void clock_map_class_init(ClockMapClass *this_class) {
   ...
   * it alive.
   */
  signals[NEED_LOCATIONS] = g_signal_new(
      "need-locations", G_TYPE_FROM_CLASS(g_obj_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(ClockMapClass, need_locations), NULL, NULL,
      _clock_marshal_POINTER__VOID, G_TYPE_POINTER, 0);
}

static void clock_map_init(ClockMap *this) {
  int i;
  ...
  for (i = 0; i < MARKER_NB; i++) {
    ...
    priv->location_marker_pixbuf[i] =
        gdk_pixbuf_new_from_resource(resource, NULL);
    g_free(resource);
  }
}

static void clock_map_finalize(GObject *g_obj) {
...
void clock_map_refresh(ClockMap *this) {
  ...
  if (!priv->stock_map_pixbuf) {
    priv->stock_map_pixbuf = gdk_pixbuf_new_from_resource_at_scale(
        CLOCK_RESOURCE_PATH "icons/clock-map.png", priv->width, priv->height,
        FALSE, NULL);
  }
...
static void clock_map_get_preferred_width(GtkWidget *this, gint *minimum_width,
                                          gint *natural_width) {
...
static void clock_map_mark(ClockMap *this, gfloat latitude, gfloat longitude,
                           gint mark) {
  ClockMapPrivate *priv = clock_map_get_instance_private(this);
  ...
  if (dest_y < 0) {
    partial = gdk_pixbuf_new_subpixbuf(marker, 0, dest_y + marker_height,
                                       marker_width, -dest_y);
    ...
    partial =
        gdk_pixbuf_new_subpixbuf(marker, 0, 0, marker_width, height - dest_y);
    marker_height = gdk_pixbuf_get_height(partial);
  }
  ...
  if (dest_x < 0) {
    ...
    lhs = gdk_pixbuf_new_subpixbuf(marker, -dest_x, 0, marker_width + dest_x,
                                   marker_height);

    gdk_pixbuf_composite(lhs, priv->location_map_pixbuf, 0, dest_y,
                         gdk_pixbuf_get_width(lhs), gdk_pixbuf_get_height(lhs),
                         0, dest_y, 1.0, 1.0, GDK_INTERP_NEAREST, 0xFF);

    rhs = gdk_pixbuf_new_subpixbuf(marker, 0, 0, -dest_x, marker_height);

    gdk_pixbuf_composite(rhs, priv->location_map_pixbuf,
                         width - gdk_pixbuf_get_width(rhs) - 1, dest_y,
                         gdk_pixbuf_get_width(rhs), gdk_pixbuf_get_height(rhs),
                         width - gdk_pixbuf_get_width(rhs) - 1, dest_y, 1.0,
                         1.0, GDK_INTERP_NEAREST, 0xFF);
    ...
    lhs =
        gdk_pixbuf_new_subpixbuf(marker, width - dest_x, 0,
                                 marker_width - width + dest_x, marker_height);

    gdk_pixbuf_composite(lhs, priv->location_map_pixbuf, 0, dest_y,
                         gdk_pixbuf_get_width(lhs), gdk_pixbuf_get_height(lhs),
                         0, dest_y, 1.0, 1.0, GDK_INTERP_NEAREST, 0xFF);

    rhs = gdk_pixbuf_new_subpixbuf(marker, 0, 0, width - dest_x, marker_height);
    ...
    gdk_pixbuf_composite(marker, priv->location_map_pixbuf, dest_x, dest_y,
                         gdk_pixbuf_get_width(marker),
                         gdk_pixbuf_get_height(marker), dest_x, dest_y, 1.0,
                         1.0, GDK_INTERP_NEAREST, 0xFF);
  }
 ...
 **/
static gboolean clock_map_place_location(ClockMap *this, ClockLocation *loc,
                                         gboolean hilight) {
...
static guchar clock_map_is_sunlit(gdouble pos_lat, gdouble pos_long,
...
static void clock_map_render_shadow_pixbuf(GdkPixbuf *pixbuf) {
  int x, y;
  int height, width;
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      guchar shade;
      gdouble lon = (x - width / 2.0) / (width / 2.0) * 180.0;
...
static void clock_map_render_shadow(ClockMap *this) {
  ...
  priv->shadow_pixbuf =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, priv->width, priv->height);
  ...
  gdk_pixbuf_composite(priv->shadow_pixbuf, priv->shadow_map_pixbuf, 0, 0,
                       priv->width, priv->height, 0, 0, 1, 1,
                       GDK_INTERP_NEAREST, 0x66);
}

static void clock_map_display(ClockMap *this) {
  ClockMapPrivate *priv = clock_map_get_instance_private(this);

  if (priv->width > 0 || priv->height > 0) clock_map_render_shadow(this);
...
static gboolean highlight(gpointer user_data) {
  BlinkData *data = user_data;

  if (data->count == 6) return FALSE;

  if (data->count % 2 == 0) {
    if (!clock_map_place_location(data->map, data->location, TRUE))
      return FALSE;
  } else
...
static void highlight_destroy(gpointer user_data) {
  BlinkData *data = user_data;
...
void clock_map_blink_location(ClockMap *this, ClockLocation *loc) {
  ...
  priv->highlight_timeout_id = g_timeout_add_full(
      G_PRIORITY_DEFAULT_IDLE, 300, highlight, data, highlight_destroy);
}

static gboolean clock_map_needs_refresh(ClockMap *this) {
  ClockMapPrivate *priv = clock_map_get_instance_private(this);
  time_t now_t;
