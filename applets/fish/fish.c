...
#define FISH_IS_APPLET(o) (G_TYPE_CHECK_INSTANCE_TYPE((o), FISH_TYPE_APPLET))
...
static void image_value_changed(GtkFileChooser *chooser, FishApplet *fish) {
  ...
  if (!strncmp(path, FISH_ICONDIR, strlen(FISH_ICONDIR))) {
    ...
    while (*image && *image == G_DIR_SEPARATOR) image++;
  } else
    image = path;
...
static void command_value_changed(GtkEntry *entry, FishApplet *fish) {
  ...
  if (!strncmp(text, "ps ", 3) || !strcmp(text, "ps") ||
      !strncmp(text, "who ", 4) || !strcmp(text, "who") ||
      !strcmp(text, "uptime") || !strncmp(text, "tail ", 5)) {
...
static void setup_sensitivity(FishApplet *fish, GtkBuilder *builder,
                              const char *wid, const char *label,
                              const char *label_post, const char *key) {
  GtkWidget *w;
...
static void chooser_preview_update(GtkFileChooser *file_chooser,
                                   gpointer data) {
  GtkWidget *preview;
  char *filename;
  GdkPixbuf *pixbuf;
...
static void display_preferences_dialog(GtkAction *action, FishApplet *fish) {
  ...
  fish->preferences_dialog =
      GTK_WIDGET(gtk_builder_get_object(builder, "fish_preferences_dialog"));

  g_object_add_weak_pointer(G_OBJECT(fish->preferences_dialog),
                            (void **)&fish->preferences_dialog);

  gtk_window_set_icon_name(GTK_WINDOW(fish->preferences_dialog), FISH_ICON);
  gtk_dialog_set_default_response(GTK_DIALOG(fish->preferences_dialog),
                                  GTK_RESPONSE_OK);
  ...
  g_signal_connect(fish->name_entry, "changed", G_CALLBACK(name_value_changed),
                   fish);

  setup_sensitivity(fish, builder, "name_entry" /* wid */,
                    "name_label" /* label */, NULL /* label_post */,
                    FISH_NAME_KEY /* key */);

  fish->preview_image =
      GTK_WIDGET(gtk_builder_get_object(builder, "preview_image"));
  if (fish->pixbuf)
    gtk_image_set_from_pixbuf(GTK_IMAGE(fish->preview_image), fish->pixbuf);

  fish->image_chooser =
      GTK_WIDGET(gtk_builder_get_object(builder, "image_chooser"));
  filter = gtk_file_filter_new();
  gtk_file_filter_set_name(filter, _("Images"));
  gtk_file_filter_add_pixbuf_formats(filter);
  gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(fish->image_chooser), filter);
  gtk_file_chooser_set_filter(GTK_FILE_CHOOSER(fish->image_chooser), filter);
  ...
  gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(fish->image_chooser), path);
  g_free(path);

  g_signal_connect(fish->image_chooser, "selection-changed",
                   G_CALLBACK(image_value_changed), fish);

  setup_sensitivity(fish, builder, "image_chooser" /* wid */,
                    "image_label" /* label */, NULL /* label_post */,
                    FISH_IMAGE_KEY /* key */);

  fish->command_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "command_label"));
  fish->command_entry =
      GTK_WIDGET(gtk_builder_get_object(builder, "command_entry"));
  ...
  setup_sensitivity(fish, builder, "command_entry" /* wid */,
                    "command_label" /* label */, NULL /* label_post */,
                    FISH_COMMAND_KEY /* key */);

  if (g_settings_get_boolean(fish->lockdown_settings,
  ...
  fish->frames_spin =
      GTK_WIDGET(gtk_builder_get_object(builder, "frames_spin"));
  ...
  setup_sensitivity(
      fish, builder, "frames_spin" /* wid */, "frames_label" /* label */,
      "frames_post_label" /* label_post */, FISH_FRAMES_KEY /* key */);
  ...
  setup_sensitivity(
      fish, builder, "speed_spin" /* wid */, "speed_label" /* label */,
      "speed_post_label" /* label_post */, FISH_SPEED_KEY /* key */);

  fish->rotate_toggle =
      GTK_WIDGET(gtk_builder_get_object(builder, "rotate_toggle"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fish->rotate_toggle),
                               fish->rotate);

  g_signal_connect(fish->rotate_toggle, "toggled",
                   G_CALLBACK(rotate_value_changed), fish);

  setup_sensitivity(fish, builder, "rotate_toggle" /* wid */, NULL /* label */,
                    NULL /* label_post */, FISH_ROTATE_KEY /* key */);
...
static void display_about_dialog(GtkAction *action, FishApplet *fish) {
  ...
  const char *about_format =
      _("%s has no use what-so-ever. "
  ...
  const char *documenters[] = {
      ...
      "Sun GNOME Documentation Team <gdocteam@sun.com>", NULL};
  ...
  gtk_show_about_dialog(
      NULL, "program-name", _("Fish"), "title", _("About Fish"), "authors",
      authors, "comments", descr, "copyright",
      ...
      "documenters", documenters, "logo-icon-name", FISH_ICON,
      "translator-credits", _("translator-credits"), "version",
      VERSION, /* "3.4.7.4ac19" */
...
static gboolean locate_fortune_command(FishApplet *fish, int *argcp,
                                       char ***argvp) {
  char *prog = NULL;

  if (fish->command && g_shell_parse_argv(fish->command, argcp, argvp, NULL)) {
  ...
  if (prog) {
    g_free(prog);
    if (g_shell_parse_argv("fortune", argcp, argvp, NULL)) return FALSE;
  }
...
static void handle_fortune_response(GtkWidget *widget, int id,
                                    FishApplet *fish) {
  if (id == FISH_RESPONSE_SPEAK)
    ...
    if (fish->source_id) g_source_remove(fish->source_id);
    fish->source_id = 0;
...
static void update_fortune_dialog(FishApplet *fish) {
  ...
  if (!fish->fortune_dialog || !fish->name) return;
...
static void insert_fortune_text(FishApplet *fish, const char *text) {
  ...
  gtk_text_buffer_insert_with_tags_by_name(fish->fortune_buffer, &iter, text,
                                           -1, "monospace_tag", NULL);
...
static void clear_fortune_text(FishApplet *fish) {
  ...
  gtk_text_buffer_remove_tag_by_name(fish->fortune_buffer, "monospace_tag",
                                     &begin, &end);
...
static gboolean fish_read_output(GIOChannel *source, GIOCondition condition,
                                 gpointer data) {
  char output[4096];
  gsize bytes_read;
  GError *error = NULL;
  ...
  status = g_io_channel_read_chars(source, output, 4096, &bytes_read, &error);

  if (error) {
    char *message;

    message = g_strdup_printf(
        _("Unable to read output from command\n\nDetails: %s"), error->message);
    something_fishy_going_on(fish, message);
    g_free(message);
    g_error_free(error);
    fish->source_id = 0;
  ...
  if (bytes_read > 0) {
    ...
    if (!g_utf8_validate(output, -1, NULL))
      utf8_output = g_locale_to_utf8(output, bytes_read, NULL, NULL, NULL);
    else
      utf8_output = g_strndup(output, bytes_read);

    if (utf8_output) insert_fortune_text(fish, utf8_output);
...
static GtkWidget *panel_dialog_add_button(GtkDialog *dialog,
...
static void display_fortune_dialog(FishApplet *fish) {
  ...
  if (!fish->fortune_dialog) {
    ...
    gtk_dialog_add_button(GTK_DIALOG(fish->fortune_dialog), _("_Speak again"),
                          FISH_RESPONSE_SPEAK);

    panel_dialog_add_button(GTK_DIALOG(fish->fortune_dialog), _("_Close"),
                            "window-close", GTK_RESPONSE_CLOSE);

    gtk_window_set_icon_name(GTK_WINDOW(fish->fortune_dialog), FISH_ICON);

    gtk_dialog_set_default_response(GTK_DIALOG(fish->fortune_dialog),
                                    GTK_RESPONSE_CLOSE);
    ...
    monitor = gdk_display_get_monitor_at_window(
        gtk_widget_get_display(GTK_WIDGET(fish)),
    ...
    gtk_box_pack_start(GTK_BOX(vbox), fish->fortune_label, FALSE, FALSE, 6);

    gtk_box_pack_start(GTK_BOX(vbox), scrolled, TRUE, TRUE, 6);

    gtk_box_pack_start(GTK_BOX(vbox), fish->fortune_cmd_label, FALSE, FALSE, 6);
  ...
  if (!user_command) {
    ...
    text = g_strdup_printf(_("The configured command is not "
                             ...
                             "%s"),
                           command);
    gtk_label_set_markup(GTK_LABEL(fish->fortune_cmd_label), text);
    g_free(command);
    g_free(text);
  ...
  g_spawn_async_with_pipes(NULL,       /* working directory */
                           ...
                           set_environment, &display_name, NULL, /* child pid */
                           NULL,                                 /* stdin */
                           &output, NULL,                        /* stderr */
                           &error);
  g_free(display_name);
...
static void name_changed_notify(GSettings *settings, gchar *key,
                                FishApplet *fish) {
  char *value;
...
static void image_changed_notify(GSettings *settings, gchar *key,
                                 FishApplet *fish) {
  char *value;
  ...
  if (!value || *value == '\0' ||
      (fish->image && !strcmp(fish->image, value))) {
    g_free(value);
    return;
  }
  ...
  if (fish->image_chooser) {
    ...
    path_gsettings = get_image_path(fish);
    path_chooser =
        gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(fish->image_chooser));
...
static void command_changed_notify(GSettings *settings, gchar *key,
                                   FishApplet *fish) {
  char *value;
  ...
  if (!value || *value == '\0' ||
      (fish->command && !strcmp(fish->command, value))) {
    g_free(value);
    return;
  }
...
static void n_frames_changed_notify(GSettings *settings, gchar *key,
                                    FishApplet *fish) {
  int value;

  value = g_settings_get_int(settings, key);

  if (fish->n_frames == value) return;

  fish->n_frames = value;

  if (fish->n_frames <= 0) fish->n_frames = 1;

  update_pixmap(fish);

  if (fish->frames_spin && gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(
                               fish->frames_spin)) != fish->n_frames)
...
static void init_fools_day(void) {
  const char *spanish_timezones[] = {"Europe/Madrid",   "Africa/Ceuta",
                                     "Atlantic/Canary", "America/Mexico_City",
                                     "Mexico/BajaSur",  "Mexico/BajaNorte",
                                     "Mexico/General",  NULL};
  char *location;
  int i;
...
static void check_april_fools(FishApplet *fish) {
  ...
  if (fish->april_fools &&
      (tm->tm_mon != fools_month || tm->tm_mday != fools_day ||
       tm->tm_hour >= fools_hour_end)) {
    fish->april_fools = FALSE;
    update_pixmap(fish);
  } else if (tm->tm_mon == fools_month && tm->tm_mday == fools_day &&
             tm->tm_hour >= fools_hour_start && tm->tm_hour <= fools_hour_end) {
...
static gboolean timeout_handler(gpointer data) {
  ...
  if (fish->april_fools) return TRUE;

  fish->current_frame++;
  if (fish->current_frame >= fish->n_frames) fish->current_frame = 0;
...
static void setup_timeout(FishApplet *fish) {
  if (fish->timeout) g_source_remove(fish->timeout);

  fish->timeout = g_timeout_add(fish->speed * 1000, timeout_handler, fish);
}

static void speed_changed_notify(GSettings *settings, gchar *key,
                                 FishApplet *fish) {
  gdouble value;

  value = g_settings_get_double(settings, key);

  if (fish->speed == value) return;
  fish->speed = value;

  setup_timeout(fish);

  if (fish->speed_spin && gtk_spin_button_get_value(GTK_SPIN_BUTTON(
                              fish->frames_spin)) != fish->speed)
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(fish->speed_spin), fish->speed);
}

static void rotate_changed_notify(GSettings *settings, gchar *key,
                                  FishApplet *fish) {
  gboolean value;

  value = g_settings_get_boolean(settings, key);

  if (fish->rotate == value) return;
  fish->rotate = value;
  ...
  if (fish->rotate_toggle && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                                 fish->rotate_toggle)) != fish->rotate)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fish->rotate_toggle),
                                 fish->rotate);
}

static void fish_disable_commande_line_notify(GSettings *settings, gchar *key,
                                              FishApplet *fish) {
...
static void setup_gsettings(FishApplet *fish) {
  ...
  g_signal_connect(fish->settings, "changed::" FISH_NAME_KEY,
                   G_CALLBACK(name_changed_notify), fish);
  g_signal_connect(fish->settings, "changed::" FISH_IMAGE_KEY,
                   G_CALLBACK(image_changed_notify), fish);
  g_signal_connect(fish->settings, "changed::" FISH_COMMAND_KEY,
                   G_CALLBACK(command_changed_notify), fish);
  g_signal_connect(fish->settings, "changed::" FISH_FRAMES_KEY,
                   G_CALLBACK(n_frames_changed_notify), fish);
  g_signal_connect(fish->settings, "changed::" FISH_SPEED_KEY,
                   G_CALLBACK(speed_changed_notify), fish);
  g_signal_connect(fish->settings, "changed::" FISH_ROTATE_KEY,
                   G_CALLBACK(rotate_changed_notify), fish);
  g_signal_connect(fish->lockdown_settings,
                   "changed::" LOCKDOWN_DISABLE_COMMAND_LINE_KEY,
                   G_CALLBACK(fish_disable_commande_line_notify), fish);
}
...
static gboolean load_fish_image(FishApplet *fish) {
  ...
  if (fish->pixbuf) g_object_unref(fish->pixbuf);
  fish->pixbuf = pixbuf;
...
static gboolean update_pixmap_in_idle(gpointer data) {
...
static void update_pixmap(FishApplet *fish) {
  ...
  if (!gtk_widget_get_realized(widget) || allocation.width <= 0 ||
      allocation.height <= 0)
    return;

  if (!fish->pixbuf && !load_fish_image(fish)) return;

  if (fish->rotate && (fish->orientation == MATE_PANEL_APPLET_ORIENT_LEFT ||
                       fish->orientation == MATE_PANEL_APPLET_ORIENT_RIGHT))
    rotate = TRUE;
  ...
  if (prev_requisition.width != fish->requisition.width ||
    ...
    gtk_widget_set_size_request(widget, fish->requisition.width,
  ...
  if (fish->surface) cairo_surface_destroy(fish->surface);
  ...
  if (fish->april_fools) {
    cairo_matrix_translate(&matrix, pixbuf_width - 1, pixbuf_height - 1);
  ...
  if (rotate) {
    ...
    cairo_matrix_scale(&matrix, (double)(pixbuf_height - 1) / width,
                       (double)(pixbuf_width - 1) / height);
  } else {
    cairo_matrix_scale(&matrix, (double)(pixbuf_width - 1) / width,
...
static gboolean fish_applet_draw(GtkWidget *widget, cairo_t *cr,
                                 FishApplet *fish) {
  int width, height;
  int src_x, src_y;
  ...
  if (fish->rotate) {
    if (fish->orientation == MATE_PANEL_APPLET_ORIENT_RIGHT)
      src_y += ((height * (fish->n_frames - 1 - fish->current_frame)) /
                fish->n_frames);
...
static void fish_applet_size_allocate(GtkWidget *widget,
                                      GtkAllocation *allocation,
                                      FishApplet *fish) {
...
static void fish_applet_realize(GtkWidget *widget, FishApplet *fish) {
  if (!fish->surface) update_pixmap(fish);
}
...
static void fish_applet_change_orient(MatePanelApplet *applet,
                                      MatePanelAppletOrient orientation) {
  FishApplet *fish = (FishApplet *)applet;

  if (fish->orientation == orientation) return;
...
static gboolean handle_button_release(FishApplet *fish, GdkEventButton *event) {
  if (!fish->in_applet || event->button != 1) return FALSE;
...
static void setup_fish_widget(FishApplet *fish) {
  ...
  g_signal_connect(fish->drawing_area, "draw", G_CALLBACK(fish_applet_draw),
                   fish);
  ...
  g_signal_connect(fish, "key-press-event", G_CALLBACK(handle_keypress), fish);

  gtk_widget_show_all(widget);
}

static const GtkActionEntry fish_menu_verbs[] = {
    {"FishPreferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(display_preferences_dialog)},
    {"FishHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(display_help_dialog)},
    {"FishAbout", "help-about", N_("_About"), NULL, NULL,
...
static gboolean fish_applet_fill(FishApplet *fish) {
  ...
  if (!fish->image) fish->image = g_strdup("fishanim.png"); /* Fallback */
  ...
  gtk_action_group_add_actions(action_group, fish_menu_verbs,
                               G_N_ELEMENTS(fish_menu_verbs), fish);
  mate_panel_applet_setup_menu_from_resource(
      applet, FISH_RESOURCE_PATH "fish-menu.xml", action_group);
...
static gboolean fishy_factory(MatePanelApplet *applet, const char *iid,
                              gpointer data) {
...
static void fish_applet_dispose(GObject *object) {
  ...
  if (fish->settings != NULL)
    g_signal_handlers_disconnect_by_data(fish->settings, fish);
  ...
  if (fish->preferences_dialog) gtk_widget_destroy(fish->preferences_dialog);
  fish->preferences_dialog = NULL;

  if (fish->fortune_dialog) gtk_widget_destroy(fish->fortune_dialog);
...
static GType fish_applet_get_type(void) {
  ...
  if (!type) {
    static const GTypeInfo info = {sizeof(MatePanelAppletClass),
                                   NULL,
                                   NULL,
...
MATE_PANEL_APPLET_IN_PROCESS_FACTORY("FishAppletFactory",
                                     fish_applet_get_type(), "That-stupid-fish",
                                     fishy_factory, NULL)
#else
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("FishAppletFactory",
                                      fish_applet_get_type(),
                                      "That-stupid-fish", fishy_factory, NULL)
#endif
