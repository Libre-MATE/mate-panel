...
static void pager_update_wnck(PagerData* pager, WnckPager* wnck_pager) {
  ...
  if (pager->display_names &&
      (pager->wm == PAGER_WM_MARCO || pager->wm == PAGER_WM_METACITY ||
       pager->wm == PAGER_WM_I3 || pager->wm == PAGER_WM_XMONAD)) {
...
static void update_properties_for_wm(PagerData* pager) {
  switch (pager->wm) {
    case PAGER_WM_MARCO:
      if (pager->workspaces_frame) gtk_widget_show(pager->workspaces_frame);
      ...
      if (pager->cell) g_object_set(pager->cell, "editable", TRUE, NULL);
      break;
    ...
    case PAGER_WM_I3:
      ...
      if (pager->display_workspaces_toggle)
        gtk_widget_show(pager->display_workspaces_toggle);
      if (pager->cell) g_object_set(pager->cell, "editable", FALSE, NULL);
      break;
    ...
    case PAGER_WM_UNKNOWN:
      if (pager->workspaces_frame) gtk_widget_hide(pager->workspaces_frame);
      break;
    default:
...
static void applet_realized(MatePanelApplet* applet, PagerData* pager) {
  ...
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    ...
    wncklet_connect_while_alive(pager->screen, "window_manager_changed",
                                G_CALLBACK(window_manager_changed), pager,
                                pager->applet);
  }
#endif /* HAVE_X11 */
...
static void applet_change_orient(MatePanelApplet* applet,
  ...
  if (new_orient == pager->orientation) return;
...
static void applet_change_background(MatePanelApplet* applet,
  ...
  gtk_style_context_set_path(new_context,
                             gtk_widget_get_path(GTK_WIDGET(pager->pager)));
  g_object_unref(new_context);

#ifdef HAVE_X11
  if (WNCK_IS_PAGER(pager->pager)) {
    wnck_pager_set_shadow_type(
        WNCK_PAGER(pager->pager),
        type == PANEL_NO_BACKGROUND ? GTK_SHADOW_NONE : GTK_SHADOW_IN);
  }
#endif /* HAVE_X11 */
}

static void applet_style_updated(MatePanelApplet* applet,
  ...
  /* Provide a fallback color for the highlighted workspace based on the current
   * theme */
...
/* Replacement for the default scroll handler that also cares about the wrapping
 * property. Alternative: Add behaviour to libwnck (to the WnckPager widget).
 */
static gboolean applet_scroll(MatePanelApplet* applet, GdkEventScroll* event,
                              PagerData* pager) {
#ifdef HAVE_X11
...
#ifdef HAVE_X11
  if (pager->screen) {
    index = wnck_workspace_get_number(
  ...
  if (n_workspaces % pager->n_rows != 0) n_columns++;
  ...
  switch (absolute_direction) {
    ...
    case GDK_SCROLL_UP:
      ...
      if (index >= n_workspaces) index -= n_columns;
      break;
  ...
  if (pager->screen) {
    wnck_workspace_activate(wnck_screen_get_workspace(pager->screen, index),
                            event->time);
  }
#endif /* HAVE_X11 */
...
static const GtkActionEntry pager_menu_actions[] = {
    {"PagerPreferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(display_properties_dialog)},
    {"PagerHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(display_help_dialog)},
    {"PagerAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(display_about_dialog)}};

static void num_rows_changed(GSettings* settings, gchar* key,
                             PagerData* pager) {
  int n_rows;

  n_rows = CLAMP(
      g_settings_get_int(settings, key), 1,
      MIN(wnck_screen_get_workspace_count(pager->screen), MAX_REASONABLE_ROWS));

  pager->n_rows = n_rows;
  pager_update(pager);

  if (pager->num_rows_spin && gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(
                                  pager->num_rows_spin)) != n_rows)
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(pager->num_rows_spin),
                              pager->n_rows);
}

static void display_workspace_names_changed(GSettings* settings, gchar* key,
                                            PagerData* pager) {
  ...
  if (pager->display_workspaces_toggle &&
      gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(pager->display_workspaces_toggle)) != value) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(pager->display_workspaces_toggle), value);
  }
}

static void all_workspaces_changed(GSettings* settings, gchar* key,
                                   PagerData* pager) {
  ...
  if (pager->all_workspaces_radio) {
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(pager->all_workspaces_radio)) != value) {
      if (value) {
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(pager->all_workspaces_radio), TRUE);
      } else {
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(pager->current_only_radio), TRUE);
      }
    }
...
static void wrap_workspaces_changed(GSettings* settings, gchar* key,
                                    PagerData* pager) {
  gboolean value = FALSE; /* Default value */
  ...
  if (pager->wrap_workspaces_toggle &&
      gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(pager->wrap_workspaces_toggle)) != value) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(pager->wrap_workspaces_toggle), value);
  }
}
...
static void setup_gsettings(PagerData* pager) {
  pager->settings = mate_panel_applet_settings_new(
      MATE_PANEL_APPLET(pager->applet), WORKSPACE_SWITCHER_SCHEMA);
  ...
  g_signal_connect(pager->settings, "changed::wrap-workspaces",
                   G_CALLBACK(wrap_workspaces_changed), pager);
}
...
gboolean workspace_switcher_applet_fill(MatePanelApplet* applet) {
  ...
  mate_panel_applet_set_flags(MATE_PANEL_APPLET(pager->applet),
  ...
  pager->display_names =
      g_settings_get_boolean(pager->settings, "display-workspace-names");

  pager->wrap_workspaces =
      g_settings_get_boolean(pager->settings, "wrap-workspaces");

  pager->display_all =
      g_settings_get_boolean(pager->settings, "display-all-workspaces");
  ...
  g_signal_connect(pager->pager, "destroy", G_CALLBACK(destroy_pager), pager);

  /* overwrite default WnckPager widget scroll-event */
  g_signal_connect(pager->pager, "scroll-event", G_CALLBACK(applet_scroll),
                   pager);

  gtk_container_add(GTK_CONTAINER(pager->applet), pager->pager);

  g_signal_connect(pager->applet, "realize", G_CALLBACK(applet_realized),
                   pager);
  g_signal_connect(pager->applet, "unrealize", G_CALLBACK(applet_unrealized),
                   pager);
  g_signal_connect(pager->applet, "change-orient",
                   G_CALLBACK(applet_change_orient), pager);
  g_signal_connect(pager->applet, "change-background",
                   G_CALLBACK(applet_change_background), pager);
  g_signal_connect(pager->applet, "style-updated",
                   G_CALLBACK(applet_style_updated), context);
  ...
  gtk_action_group_add_actions(action_group, pager_menu_actions,
                               G_N_ELEMENTS(pager_menu_actions), pager);
  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(pager->applet),
      WNCKLET_RESOURCE_PATH "workspace-switcher-menu.xml", action_group);
...
static void display_help_dialog(GtkAction* action, PagerData* pager) {
  wncklet_display_help(pager->applet, "mate-user-guide", "overview-workspaces",
...
static void display_about_dialog(GtkAction* action, PagerData* pager) {
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(pager->applet), "program-name", _("Workspace Switcher"),
      "title", _("About Workspace Switcher"), "authors", authors, "comments",
      _("The Workspace Switcher shows you a small version of your workspaces "
        "that lets you manage your windows."),
      "copyright",
      _("Copyright \xc2\xa9 2002 Red Hat, Inc.\n"
        "Copyright \xc2\xa9 2011 Perberos\n"
        "Copyright \xc2\xa9 2012-2021 MATE developers"),
      "documenters", documenters, "icon-name", WORKSPACE_SWITCHER_ICON,
      "logo-icon-name", WORKSPACE_SWITCHER_ICON, "translator-credits",
...
static void wrap_workspaces_toggled(GtkToggleButton* button, PagerData* pager) {
  g_settings_set_boolean(pager->settings, "wrap-workspaces",
                         gtk_toggle_button_get_active(button));
}

static void display_workspace_names_toggled(GtkToggleButton* button,
                                            PagerData* pager) {
  g_settings_set_boolean(pager->settings, "display-workspace-names",
                         gtk_toggle_button_get_active(button));
}

static void all_workspaces_toggled(GtkToggleButton* button, PagerData* pager) {
  g_settings_set_boolean(pager->settings, "display-all-workspaces",
                         gtk_toggle_button_get_active(button));
}

static void num_rows_value_changed(GtkSpinButton* button, PagerData* pager) {
  g_settings_set_int(pager->settings, "num-rows",
...
static const char* get_workspace_name(PagerData* pager, int workspace_index) {
  ...
  if (pager->screen) {
    WnckWorkspace* workspace =
        wnck_screen_get_workspace(pager->screen, workspace_index);
...
static void update_workspaces_model(PagerData* pager) {
...
#ifdef HAVE_X11
  if (pager->screen) nr_ws = wnck_screen_get_workspace_count(pager->screen);
#endif /* HAVE_X11 */
  ...
  if (pager->properties_dialog) {
    ...
    if (nr_ws != gtk_spin_button_get_value_as_int(
                     GTK_SPIN_BUTTON(pager->num_workspaces_spin)))
...
static void workspace_renamed(WnckWorkspace* space, PagerData* pager) {
  ...
  if (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(pager->workspaces_store),
    ...
    gtk_list_store_set(pager->workspaces_store, &iter, 0,
                       wnck_workspace_get_name(space), -1);
}

static void workspace_created(WnckScreen* screen, WnckWorkspace* space,
                              PagerData* pager) {
  ...
  wncklet_connect_while_alive(space, "name_changed",
                              G_CALLBACK(workspace_renamed), pager,
                              pager->properties_dialog);
}

static void workspace_destroyed(WnckScreen* screen, WnckWorkspace* space,
                                PagerData* pager) {
  g_return_if_fail(WNCK_IS_SCREEN(screen));
  update_workspaces_model(pager);
}
...
static gboolean workspaces_tree_focused_out(GtkTreeView* treeview,
                                            GdkEventFocus* event,
                                            PagerData* pager) {
...
static void workspace_name_edited(GtkCellRendererText* cell_renderer_text,
...
static void response_cb(GtkWidget* widget, int id, PagerData* pager) {
  if (id == GTK_RESPONSE_HELP)
    wncklet_display_help(widget, "mate-user-guide", "overview-workspaces",
                         WORKSPACE_SWITCHER_ICON);
  else
    gtk_widget_destroy(widget);
}
...
static void close_dialog(GtkWidget* button, gpointer data) {
  ...
  */
  ...
  if (edit_widget) gtk_cell_editable_editing_done(edit_widget);
...
static void setup_sensitivity(PagerData* pager, GtkBuilder* builder,
                              const char* wid1, const char* wid2,
                              const char* wid3, GSettings* settings,
                              const char* key) {
  GtkWidget* w;
...
static void setup_dialog(GtkBuilder* builder, PagerData* pager) {
  ...
  setup_sensitivity(pager, builder, "workspace_name_toggle", NULL, NULL,
                    pager->settings, "display-workspace-names" /* key */);

  pager->wrap_workspaces_toggle = WID("workspace_wrap_toggle");
  setup_sensitivity(pager, builder, "workspace_wrap_toggle", NULL, NULL,
                    pager->settings, "wrap-workspaces" /* key */);

  pager->all_workspaces_radio = WID("all_workspaces_radio");
  pager->current_only_radio = WID("current_only_radio");
  setup_sensitivity(pager, builder, "all_workspaces_radio",
                    "current_only_radio", "label_row_col", pager->settings,
                    "display-all-workspaces" /* key */);

  pager->num_rows_spin = WID("num_rows_spin");
  pager->label_row_col = WID("label_row_col");
  setup_sensitivity(pager, builder, "num_rows_spin", NULL, NULL,
                    pager->settings, "num-rows" /* key */);

  pager->num_workspaces_spin = WID("num_workspaces_spin");
  setup_sensitivity(pager, builder, "num_workspaces_spin", NULL, NULL,
                    marco_general_settings, NUM_WORKSPACES /* key */);

  pager->workspaces_tree = WID("workspaces_tree_view");
  setup_sensitivity(pager, builder, "workspaces_tree_view", NULL, NULL,
                    marco_workspaces_settings, WORKSPACE_NAME /* key */);

  if (marco_general_settings != NULL) g_object_unref(marco_general_settings);
  ...
  if (pager->wrap_workspaces_toggle) {
    ...
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(pager->wrap_workspaces_toggle),
        pager->wrap_workspaces);
  }

  g_signal_connect(pager->wrap_workspaces_toggle, "toggled",
                   (GCallback)wrap_workspaces_toggled, pager);
  ...
  /* Num rows: */
  g_signal_connect(pager->num_rows_spin, "value-changed",
                   G_CALLBACK(num_rows_value_changed), pager);
  ...
  g_signal_connect(pager->properties_dialog, "destroy",
                   G_CALLBACK(properties_dialog_destroyed), pager);
  g_signal_connect(pager->properties_dialog, "delete-event",
                   G_CALLBACK(delete_event), pager);
  g_signal_connect(pager->properties_dialog, "response",
                   G_CALLBACK(response_cb), pager);

  g_signal_connect(WID("done_button"), "clicked", (GCallback)close_dialog,
                   pager);
  ...
  if (pager->screen) {
    ...
    wncklet_connect_while_alive(pager->screen, "workspace_created",
                                G_CALLBACK(workspace_created), pager,
                                pager->properties_dialog);
    wncklet_connect_while_alive(pager->screen, "workspace_destroyed",
                                G_CALLBACK(workspace_destroyed), pager,
                                pager->properties_dialog);
    ...
    for (i = 0; i < nr_ws; i++) {
      wncklet_connect_while_alive(
          G_OBJECT(wnck_screen_get_workspace(pager->screen, i)), "name_changed",
          G_CALLBACK(workspace_renamed), pager, pager->properties_dialog);
    }
  }
#endif /* HAVE_X11 */

  g_signal_connect(pager->num_workspaces_spin, "value-changed",
                   G_CALLBACK(on_num_workspaces_value_changed), pager);
  ...
  pager->cell = cell;
  column = gtk_tree_view_column_new_with_attributes("workspace", cell, "text",
                                                    0, NULL);
...
static void display_properties_dialog(GtkAction* action, PagerData* pager) {
  if (pager->properties_dialog == NULL) {
    ...
    gtk_builder_add_from_resource(
        builder, WNCKLET_RESOURCE_PATH "workspace-switcher.ui", NULL);
...
static void destroy_pager(GtkWidget* widget, PagerData* pager) {
  ...
  if (pager->properties_dialog) gtk_widget_destroy(pager->properties_dialog);
  g_free(pager);
}
