...
static ToplevelTask *toplevel_task_new(
    TasklistManager *tasklist, struct zwlr_foreign_toplevel_handle_v1 *handle);

static void wl_registry_handle_global(void *_data, struct wl_registry *registry,
  ...
  if (strcmp(interface, zwlr_foreign_toplevel_manager_v1_interface.name) == 0) {
    ...
    foreign_toplevel_manager_global_version = MIN(
        (uint32_t)zwlr_foreign_toplevel_manager_v1_interface.version, version);
  }
}

static void wl_registry_handle_global_remove(void *_data,
...
static void wayland_tasklist_init_if_needed(void) {
...
static void foreign_toplevel_manager_handle_toplevel(
...
static void foreign_toplevel_manager_handle_finished(
    void *data, struct zwlr_foreign_toplevel_manager_v1 *manager) {
  TasklistManager *tasklist = data;
  ...
  if (tasklist->outer_box)
    g_object_set_data(G_OBJECT(tasklist->outer_box), tasklist_manager_key,
                      NULL);
...
static void tasklist_manager_disconnected_from_widget(
  ...
  if (tasklist->outer_box) tasklist->outer_box = NULL;

  if (tasklist->manager)
    zwlr_foreign_toplevel_manager_v1_stop(tasklist->manager);
}

static TasklistManager *tasklist_manager_new(void) {
  if (!foreign_toplevel_manager_global_id) return NULL;
  ...
  gtk_box_pack_start(GTK_BOX(tasklist->outer_box), tasklist->list, FALSE, FALSE,
                     0);
  gtk_widget_show(tasklist->list);
  tasklist->manager =
      wl_registry_bind(wl_registry_global, foreign_toplevel_manager_global_id,
                       &zwlr_foreign_toplevel_manager_v1_interface,
                       foreign_toplevel_manager_global_version);
  zwlr_foreign_toplevel_manager_v1_add_listener(
      tasklist->manager, &foreign_toplevel_manager_listener, tasklist);
  g_object_set_data_full(
      G_OBJECT(tasklist->outer_box), tasklist_manager_key, tasklist,
      (GDestroyNotify)tasklist_manager_disconnected_from_widget);
  return tasklist;
}

static void foreign_toplevel_handle_title(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *toplevel,
    const char *title) {
...
static void foreign_toplevel_handle_closed(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *toplevel) {
  ToplevelTask *task = data;
  if (task->button) gtk_widget_destroy(task->button);
}
...
static void toplevel_task_disconnected_from_widget(ToplevelTask *task) {
  ...
  if (toplevel) zwlr_foreign_toplevel_handle_v1_destroy(toplevel);

  g_free(task);
}

static void toplevel_task_handle_clicked(GtkButton *button,
                                         ToplevelTask *task) {
  if (task->toplevel) {
    if (task->active) {
...
static gboolean on_toplevel_button_press(GtkWidget *widget, GdkEvent *event,
                                         gpointer user_data) {
  /* Assume event is a button press */
  if (((GdkEventButton *)event)->button == GDK_BUTTON_SECONDARY) {
    /* Returning true for secondary clicks suppresses the applet's default
     * context menu, which we do not want to show up for task buttons */
    return TRUE;
  } else {
    return FALSE;
  }
...
static ToplevelTask *toplevel_task_new(
  ...
  g_signal_connect(task->button, "clicked",
                   G_CALLBACK(toplevel_task_handle_clicked), task);
  ...
  zwlr_foreign_toplevel_handle_v1_add_listener(
      toplevel, &foreign_toplevel_handle_listener, task);
  g_object_set_data_full(
      G_OBJECT(task->button), toplevel_task_key, task,
      (GDestroyNotify)toplevel_task_disconnected_from_widget);

  g_signal_connect(task->button, "button-press-event",
                   G_CALLBACK(on_toplevel_button_press), task);

  return task;
}

GtkWidget *wayland_tasklist_new() {
...
static TasklistManager *tasklist_widget_get_tasklist(
