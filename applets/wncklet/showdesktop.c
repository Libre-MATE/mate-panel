...
static void applet_change_orient(MatePanelApplet* applet,
  ...
  if (new_orient == sdd->orient) return;
...
static void button_size_allocated(GtkWidget* button, GtkAllocation* allocation,
                                  ShowDesktopData* sdd) {
  /*
  if ((sdd->orient == GTK_ORIENTATION_HORIZONTAL) && (sdd->size ==
  allocation->height))
  {
          return;
  }
  else if ((sdd->orient == GTK_ORIENTATION_VERTICAL) && (sdd->size ==
  allocation->width))
  {
          return;
  }
...
static void update_icon(ShowDesktopData* sdd) {
  ...
  error = NULL;
  icon = gtk_icon_theme_load_surface(sdd->icon_theme, SHOW_DESKTOP_ICON,
                                     icon_size, icon_scale, NULL, 0, &error);
  ...
  scaled = cairo_surface_create_similar(icon, cairo_surface_get_content(icon),
                                        width, height);
...
static const GtkActionEntry show_desktop_menu_actions[] = {
    {"ShowDesktopHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(display_help_dialog)},
    {"ShowDesktopAbout", "help-about", N_("_About"), NULL, NULL,
...
static void button_drag_leave(GtkWidget* widget, GdkDragContext* context,
                              guint time, ShowDesktopData* sdd) {
  if (sdd->button_activate != 0) {
    g_source_remove(sdd->button_activate);
    sdd->button_activate = 0;
  }
...
static gboolean button_drag_motion(GtkWidget* widget, GdkDragContext* context,
                                   gint x, gint y, guint time,
                                   ShowDesktopData* sdd) {
  if (sdd->button_activate == 0)
    sdd->button_activate = g_timeout_add_seconds(TIMEOUT_ACTIVATE_SECONDS,
                                                 button_motion_timeout, sdd);
...
static void show_desktop_applet_realized(MatePanelApplet* applet,
                                         gpointer data) {
  ShowDesktopData* sdd;
  GdkScreen* screen;
  ...
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    sdd->wnck_screen =
        wnck_screen_get(gdk_x11_screen_get_screen_number(screen));
    if (sdd->wnck_screen != NULL)
  ...
  wncklet_connect_while_alive(sdd->icon_theme, "changed",
                              G_CALLBACK(theme_changed_callback), sdd,
                              sdd->applet);
...
gboolean show_desktop_applet_fill(MatePanelApplet* applet) {
  ...
  g_signal_connect(sdd->button, "toggled", G_CALLBACK(button_toggled_callback),
                   sdd);
  ...
  g_signal_connect(sdd->button, "size-allocate",
                   G_CALLBACK(button_size_allocated), sdd);
  ...
  g_signal_connect(sdd->applet, "change-orient",
                   G_CALLBACK(applet_change_orient), sdd);

  action_group = gtk_action_group_new("ShowDesktop Applet Actions");
  gtk_action_group_set_translation_domain(action_group, GETTEXT_PACKAGE);
  gtk_action_group_add_actions(action_group, show_desktop_menu_actions,
                               G_N_ELEMENTS(show_desktop_menu_actions), sdd);
  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(sdd->applet),
      WNCKLET_RESOURCE_PATH "showdesktop-menu.xml", action_group);
  g_object_unref(action_group);

  g_signal_connect(sdd->applet, "destroy", G_CALLBACK(applet_destroyed), sdd);

  gtk_drag_dest_set(GTK_WIDGET(sdd->button), 0, NULL, 0, 0);

  g_signal_connect(sdd->button, "drag-motion", G_CALLBACK(button_drag_motion),
                   sdd);
  g_signal_connect(sdd->button, "drag-leave", G_CALLBACK(button_drag_leave),
                   sdd);
...
static void display_help_dialog(GtkAction* action, ShowDesktopData* sdd) {
  wncklet_display_help(sdd->applet, "mate-user-guide", "gospanel-564",
                       SHOW_DESKTOP_ICON);
}
...
static void display_about_dialog(GtkAction* action, ShowDesktopData* sdd) {
  static const gchar* authors[] = {
      ...
      "Havoc Pennington <hp@redhat.com>", NULL};
  static const char* documenters[] = {
      "Sun GNOME Documentation Team <gdocteam@sun.com>", NULL};

  gtk_show_about_dialog(
      GTK_WINDOW(sdd->applet), "program-name", _("Show Desktop Button"),
      "title", _("About Show Desktop Button"), "authors", authors, "comments",
...
static void button_toggled_callback(GtkWidget* button, ShowDesktopData* sdd) {
...
#ifdef HAVE_X11
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    can_show_desktop = gdk_x11_screen_supports_net_wm_hint(
  ...
  if (!can_show_desktop) {
    ...
    dialog = gtk_message_dialog_new(
        ...
        _("Your window manager does not support the show desktop button, or "
          "you are not running a window manager."));
...
static void show_desktop_changed_callback(WnckScreen* screen,
  ...
  if (sdd->wnck_screen != NULL)
    sdd->showing_desktop =
        (wnck_screen_get_showing_desktop(sdd->wnck_screen) != FALSE);
#endif /* HAVE_X11 */
