...
static void window_menu_help(GtkAction* action, WindowMenu* window_menu) {
  wncklet_display_help(window_menu->applet, "mate-user-guide",
                       "panel-windowselector", WINDOW_MENU_ICON);
}

static void window_menu_about(GtkAction* action, WindowMenu* window_menu) {
  static const char* authors[] = {"Perberos <perberos@gmail.com>",
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(window_menu->applet), "program-name", _("Window Selector"),
      "title", _("About Window Selector"), "authors", authors, "comments",
      _("The Window Selector shows a list of all windows in a menu and lets "
        "you browse them."),
      "copyright",
      _("Copyright \xc2\xa9 2000 Helix Code, Inc.\n"
        "Copyright \xc2\xa9 2001 Free Software Foundation, Inc.\n"
        "Copyright \xc2\xa9 2003 Sun Microsystems, Inc.\n"
        "Copyright \xc2\xa9 2011 Perberos\n"
        "Copyright \xc2\xa9 2012-2021 MATE developers"),
      "documenters", documenters, "icon-name", WINDOW_MENU_ICON,
      "logo-icon-name", WINDOW_MENU_ICON, "translator-credits",
      _("translator-credits"), "version", VERSION, "website", PACKAGE_URL,
      NULL);
}

static const GtkActionEntry window_menu_actions[] = {
    {"WindowMenuHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(window_menu_help)},
    {"WindowMenuAbout", "help-about", N_("_About"), NULL, NULL,
...
static gboolean window_menu_on_draw(GtkWidget* widget, cairo_t* cr,
                                    gpointer data) {
  GtkStyleContext* context;
  GtkStateFlags state;
  WindowMenu* window_menu = data;

  if (!gtk_widget_has_focus(window_menu->applet)) return FALSE;
...
static void window_menu_size_allocate(MatePanelApplet* applet,
  ...
  if (!GTK_IS_CONTAINER(window_menu->selector)) return;
...
static gboolean window_menu_key_press_event(GtkWidget* widget,
                                            GdkEventKey* event,
                                            WindowMenu* window_menu) {
...
static gboolean filter_button_press(GtkWidget* widget, GdkEventButton* event,
                                    gpointer data) {
  if (event->button != 1)
...
gboolean window_menu_applet_fill(MatePanelApplet* applet) {
  ...
  g_signal_connect(window_menu->applet, "key-press-event",
  ...
  gtk_action_group_add_actions(action_group, window_menu_actions,
                               G_N_ELEMENTS(window_menu_actions), window_menu);
  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(window_menu->applet),
      WNCKLET_RESOURCE_PATH "window-menu-menu.xml", action_group);
  ...
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    ...
    window_menu->selector =
        gtk_label_new("[Window menu not supported on Wayland]");
  } else
#endif /* HAVE_WAYLAND */

  {
    window_menu->selector =
        gtk_label_new("[Window menu not supported on this platform]");
  }
