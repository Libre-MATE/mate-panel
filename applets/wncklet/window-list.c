...
static void tasklist_update(TasklistData* tasklist) {
  if (tasklist->orientation == GTK_ORIENTATION_HORIZONTAL) {
    gtk_widget_set_size_request(GTK_WIDGET(tasklist->tasklist), -1,
                                tasklist->size);
  } else {
    gtk_widget_set_size_request(GTK_WIDGET(tasklist->tasklist), tasklist->size,
                                -1);
  }
...
#ifdef HAVE_X11
  if (WNCK_IS_TASKLIST(tasklist->tasklist)) {
    wnck_tasklist_set_orientation(WNCK_TASKLIST(tasklist->tasklist),
...
static void tasklist_set_button_relief(TasklistData* tasklist,
...
static const int* tasklist_get_size_hint_list(TasklistData* tasklist,
                                              int* n_elements) {
#ifdef HAVE_X11
  if (WNCK_IS_TASKLIST(tasklist->tasklist)) {
    return wnck_tasklist_get_size_hint_list(WNCK_TASKLIST(tasklist->tasklist),
                                            n_elements);
  } else
#endif /* HAVE_X11 */
...
static void response_cb(GtkWidget* widget, int id, TasklistData* tasklist) {
  if (id == GTK_RESPONSE_HELP) {
    wncklet_display_help(widget, "mate-user-guide", "windowlist-prefs",
                         WINDOW_LIST_ICON);
  } else {
...
static void applet_realized(MatePanelApplet* applet, TasklistData* tasklist) {
  tasklist->icon_theme =
      gtk_icon_theme_get_for_screen(gtk_widget_get_screen(tasklist->applet));
}
...
static void applet_change_orient(MatePanelApplet* applet,
  ...
  if (new_orient == tasklist->orientation) return;
...
static void applet_change_background(MatePanelApplet* applet,
                                     MatePanelAppletBackgroundType type,
                                     GdkColor* color, cairo_pattern_t* pattern,
                                     TasklistData* tasklist) {
  switch (type) {
...
#ifdef HAVE_X11
#ifdef HAVE_WINDOW_PREVIEWS
static cairo_surface_t* preview_window_thumbnail(WnckWindow* wnck_window,
                                                 TasklistData* tasklist,
                                                 int* thumbnail_width,
  ...
  if ((window = gdk_x11_window_foreign_new_for_display(
           gdk_display_get_default(), win)) == NULL) {
    return NULL;
  }
  ...
  *thumbnail_scale = scale = gdk_window_get_scale_factor(window);
  ...
  thumbnail = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, *thumbnail_width,
                                         *thumbnail_height);
...
static void preview_window_reposition(TasklistData* tasklist,
                                      cairo_surface_t* thumbnail, int width,
                                      int height, int scale) {
  GdkMonitor* monitor;
  GdkRectangle monitor_geom;
  int x_pos, y_pos;

  /* Set position at pointer, then re-adjust from there to just outside of the
   * pointer */
  ...
  /* Get geometry of monitor where tasklist is located to calculate correct
   * position of preview */
  monitor =
      gdk_display_get_monitor_at_point(gdk_display_get_default(), x_pos, y_pos);
  gdk_monitor_get_geometry(monitor, &monitor_geom);

  /* Add padding to clear the panel */
  switch (mate_panel_applet_get_orient(MATE_PANEL_APPLET(tasklist->applet))) {
    case MATE_PANEL_APPLET_ORIENT_LEFT:
      x_pos = monitor_geom.width + monitor_geom.x -
              (width / scale + tasklist->size) - PREVIEW_PADDING;
      break;
    case MATE_PANEL_APPLET_ORIENT_RIGHT:
      x_pos = tasklist->size + PREVIEW_PADDING;
      break;
    case MATE_PANEL_APPLET_ORIENT_UP:
      y_pos = monitor_geom.height + monitor_geom.y -
              (height / scale + tasklist->size) - PREVIEW_PADDING;
      break;
    case MATE_PANEL_APPLET_ORIENT_DOWN:
    default:
...
static gboolean preview_window_draw(GtkWidget* widget, cairo_t* cr,
                                    cairo_surface_t* thumbnail) {
...
static gboolean applet_enter_notify_event(WnckTasklist* tl, GList* wnck_windows,
                                          TasklistData* tasklist) {
  cairo_surface_t* thumbnail;
  ...
  if (!tasklist->show_window_thumbnails || wnck_windows == NULL) return FALSE;

  n_windows = g_list_length(wnck_windows);
  /* TODO: Display a list of stacked thumbnails for grouped windows. */
  if (n_windows == 1) {
    GList* l = wnck_windows;
    if (l != NULL) wnck_window = (WnckWindow*)l->data;
  }

  if (wnck_window == NULL) return FALSE;
  ...
  thumbnail = preview_window_thumbnail(wnck_window, tasklist, &thumbnail_width,
                                       &thumbnail_height, &thumbnail_scale);
  ...
  gtk_window_set_default_size(GTK_WINDOW(tasklist->preview),
                              thumbnail_width / thumbnail_scale,
                              thumbnail_height / thumbnail_scale);
  gtk_window_set_resizable(GTK_WINDOW(tasklist->preview), TRUE);

  preview_window_reposition(tasklist, thumbnail, thumbnail_width,
                            thumbnail_height, thumbnail_scale);
  ...
  g_signal_connect_data(tasklist->preview, "draw",
                        ...
                        (GClosureNotify)G_CALLBACK(cairo_surface_destroy), 0);

  return FALSE;
}

static gboolean applet_leave_notify_event(WnckTasklist* tl, GList* wnck_windows,
                                          TasklistData* tasklist) {
  if (tasklist->preview != NULL) {
    gtk_widget_destroy(tasklist->preview);
    tasklist->preview = NULL;
...
static void applet_change_pixel_size(MatePanelApplet* applet, gint size,
                                     TasklistData* tasklist) {
  if (tasklist->size == size) return;
...
static const GtkActionEntry tasklist_menu_actions[] = {
    {"TasklistSystemMonitor", "utilities-system-monitor", N_("_System Monitor"),
     NULL, NULL, G_CALLBACK(call_system_monitor)},
    {"TasklistPreferences", "document-properties", N_("_Preferences"), NULL,
     NULL, G_CALLBACK(display_properties_dialog)},
    {"TasklistHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(display_help_dialog)},
    {"TasklistAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(display_about_dialog)}};

static void tasklist_properties_update_content_radio(TasklistData* tasklist) {
  GtkWidget* button;

  if (tasklist->show_current_radio == NULL) return;
...
static void display_all_workspaces_changed(GSettings* settings, gchar* key,
                                           TasklistData* tasklist) {
  gboolean value;
...
static void tasklist_update_thumbnail_size_spin(TasklistData* tasklist) {
  ...
  if (!tasklist->thumbnail_size) return;
...
static void show_thumbnails_changed(GSettings* settings, gchar* key,
                                    TasklistData* tasklist) {
  tasklist->show_window_thumbnails = g_settings_get_boolean(settings, key);
}

static void thumbnail_size_changed(GSettings* settings, gchar* key,
                                   TasklistData* tasklist) {
...
static GtkWidget* get_grouping_button(TasklistData* tasklist,
...
static void group_windows_changed(GSettings* settings, gchar* key,
                                  TasklistData* tasklist) {
  TasklistGroupingType type;
  GtkWidget* button;
...
static void tasklist_update_unminimization_radio(TasklistData* tasklist) {
  ...
  if (tasklist->move_minimized_radio == NULL) return;
...
static void move_unminimized_windows_changed(GSettings* settings, gchar* key,
                                             TasklistData* tasklist) {
  gboolean value;
...
static void scroll_enabled_changed(GSettings* settings, gchar* key,
                                   TasklistData* tasklist) {
...
static void setup_gsettings(TasklistData* tasklist) {
  tasklist->settings = mate_panel_applet_settings_new(
      MATE_PANEL_APPLET(tasklist->applet), WINDOW_LIST_SCHEMA);

  g_signal_connect(tasklist->settings, "changed::display-all-workspaces",
                   G_CALLBACK(display_all_workspaces_changed), tasklist);

#ifdef HAVE_WINDOW_PREVIEWS
  tasklist->preview_settings = mate_panel_applet_settings_new(
      MATE_PANEL_APPLET(tasklist->applet), WINDOW_LIST_PREVIEW_SCHEMA);

  g_signal_connect(tasklist->preview_settings,
                   "changed::show-window-thumbnails",
                   G_CALLBACK(show_thumbnails_changed), tasklist);

  g_signal_connect(tasklist->preview_settings, "changed::thumbnail-window-size",
                   G_CALLBACK(thumbnail_size_changed), tasklist);
#endif
  ...
  g_signal_connect(tasklist->settings, "changed::scroll-enabled",
                   G_CALLBACK(scroll_enabled_changed), tasklist);
}

static void applet_size_allocate(GtkWidget* widget, GtkAllocation* allocation,
                                 TasklistData* tasklist) {
  int len;
...
#if !defined(WNCKLET_INPROCESS) && !GTK_CHECK_VERSION(3, 23, 0)
  /* HACK: When loading the WnckTasklist initially, it reports size hints as
   * though there were no elements in the Tasklist. This causes a rendering
   * issue when built out-of-process in HiDPI displays. We keep a flag to skip
   * size hinting until WnckTasklist has something to show. */
  ...
  if (tasklist->needs_hints)
#endif
    mate_panel_applet_set_size_hints(MATE_PANEL_APPLET(tasklist->applet),
                                     size_hints, len, 0);
}

#ifdef HAVE_X11
/* Currently only used on X11, but should work on Wayland as well when needed */
static GdkPixbuf* icon_loader_func(const char* icon, int size,
  ...
  if (icon == NULL || strcmp(icon, "") == 0) return NULL;
  ...
  retval = gtk_icon_theme_load_icon(tasklist->icon_theme, icon_no_extension,
                                    size, 0, NULL);
...
gboolean window_list_applet_fill(MatePanelApplet* applet) {
  ...
  mate_panel_applet_set_flags(MATE_PANEL_APPLET(tasklist->applet),
  ...
  tasklist->include_all_workspaces =
      g_settings_get_boolean(tasklist->settings, "display-all-workspaces");

#ifdef HAVE_WINDOW_PREVIEWS
  tasklist->show_window_thumbnails = g_settings_get_boolean(
      tasklist->preview_settings, "show-window-thumbnails");

  tasklist->thumbnail_size =
      g_settings_get_int(tasklist->preview_settings, "thumbnail-window-size");
#endif

  tasklist->grouping = g_settings_get_enum(tasklist->settings, "group-windows");

  tasklist->move_unminimized_windows =
      g_settings_get_boolean(tasklist->settings, "move-unminimized-windows");

  tasklist->scroll_enable =
      g_settings_get_boolean(tasklist->settings, "scroll-enabled");
  ...
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    ...
    wnck_tasklist_set_middle_click_close(WNCK_TASKLIST(tasklist->tasklist),
                                         TRUE);
    ...
    g_signal_connect(tasklist->tasklist, "task-enter-notify",
                     G_CALLBACK(applet_enter_notify_event), tasklist);
    g_signal_connect(tasklist->tasklist, "task-leave-notify",
                     G_CALLBACK(applet_leave_notify_event), tasklist);
#endif /* HAVE_WINDOW_PREVIEWS */
  } else
#endif /* HAVE_X11 */
  ...
  {
    tasklist->tasklist =
        gtk_label_new("[Tasklist not supported on this platform]");
  }

  tasklist_apply_orientation(tasklist);

  g_signal_connect(tasklist->tasklist, "destroy", G_CALLBACK(destroy_tasklist),
                   tasklist);
  g_signal_connect(tasklist->applet, "size-allocate",
                   G_CALLBACK(applet_size_allocate), tasklist);

  gtk_container_add(GTK_CONTAINER(tasklist->applet), tasklist->tasklist);

  g_signal_connect(tasklist->applet, "realize", G_CALLBACK(applet_realized),
                   tasklist);
  ...
  gtk_action_group_add_actions(action_group, tasklist_menu_actions,
                               G_N_ELEMENTS(tasklist_menu_actions), tasklist);
  ...
  gtk_action_set_visible(
      gtk_action_group_get_action(action_group, "TasklistSystemMonitor"),
      FALSE);
...
_system_monitor_found:;
  ...
  mate_panel_applet_setup_menu_from_resource(
      MATE_PANEL_APPLET(tasklist->applet),
      WNCKLET_RESOURCE_PATH "window-list-menu.xml", action_group);
  ...
  for (i = 0; i < G_N_ELEMENTS(system_monitors); i += 1) {
    ...
    if (programpath != NULL) {
      ...
      mate_gdk_spawn_command_line_on_screen(
          gtk_widget_get_screen(tasklist->applet), system_monitors[i], NULL);
      return;
    }
  }
}
...
static void display_help_dialog(GtkAction* action, TasklistData* tasklist) {
  wncklet_display_help(tasklist->applet, "mate-user-guide", "windowlist",
                       WINDOW_LIST_ICON);
}

static void display_about_dialog(GtkAction* action, TasklistData* tasklist) {
  static const gchar* authors[] = {
      "Perberos <perberos@gmail.com>", "Steve Zesch <stevezesch2@gmail.com>",
      "Stefano Karapetsas <stefano@karapetsas.com>",
      "Alexander Larsson <alla@lysator.liu.se>", NULL};

  const char* documenters[] = {
      "Sun GNOME Documentation Team <gdocteam@sun.com>", NULL};

  gtk_show_about_dialog(GTK_WINDOW(tasklist->applet), "program-name",
                        _("Window List"), "title", _("About Window List"),
                        "authors", authors, "comments",
                        _("The Window List shows a list of all windows in a "
                          "set of buttons and lets you browse them."),
                        "copyright",
                        ...
                        "version", VERSION, "website", PACKAGE_URL, NULL);
}

static void group_windows_toggled(GtkToggleButton* button,
                                  TasklistData* tasklist) {
...
#ifdef HAVE_WINDOW_PREVIEWS
static void thumbnail_size_spin_changed(GtkSpinButton* button,
                                        TasklistData* tasklist) {
  g_settings_set_int(tasklist->preview_settings, "thumbnail-window-size",
                     gtk_spin_button_get_value_as_int(button));
}
#endif

static void move_minimized_toggled(GtkToggleButton* button,
                                   TasklistData* tasklist) {
  g_settings_set_boolean(tasklist->settings, "move-unminimized-windows",
                         gtk_toggle_button_get_active(button));
}

static void display_all_workspaces_toggled(GtkToggleButton* button,
                                           TasklistData* tasklist) {
...
static void setup_sensitivity(TasklistData* tasklist, GtkBuilder* builder,
                              const char* wid1, const char* wid2,
                              const char* wid3, const char* key) {
  GtkWidget* w;
...
static void setup_dialog(GtkBuilder* builder, TasklistData* tasklist) {
  ...
  setup_sensitivity(tasklist, builder, "show_current_radio", "show_all_radio",
                    NULL, "display-all-workspaces" /* key */);
  ...
  setup_sensitivity(tasklist, builder, "never_group_radio", "auto_group_radio",
                    "always_group_radio", "group-windows" /* key */);
  ...
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(tasklist->show_thumbnails_check))) {
  ...
  g_object_bind_property(tasklist->show_thumbnails_check, "active",
                         tasklist->thumbnail_size_label, "sensitive",
                         G_BINDING_DEFAULT);
  g_object_bind_property(tasklist->show_thumbnails_check, "active",
                         tasklist->thumbnail_size_spin, "sensitive",
                         G_BINDING_DEFAULT);
  ...
  setup_sensitivity(tasklist, builder, "move_minimized_radio",
                    "change_workspace_radio", NULL,
                    "move-unminimized-windows" /* key */);

  /* Window grouping: */
  button = get_grouping_button(tasklist, tasklist->grouping);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), TRUE);
  g_object_set_data(G_OBJECT(tasklist->never_group_radio), "group_value",
                    "never");
  g_object_set_data(G_OBJECT(tasklist->auto_group_radio), "group_value",
                    "auto");
  g_object_set_data(G_OBJECT(tasklist->always_group_radio), "group_value",
                    "always");
  ...
  g_signal_connect(tasklist->thumbnail_size_spin, "value-changed",
                   (GCallback)thumbnail_size_spin_changed, tasklist);
#endif
...
static void display_properties_dialog(GtkAction* action,
  ...
  if (tasklist->properties_dialog == NULL) {
    ...
    gtk_builder_add_from_resource(builder,
                                  WNCKLET_RESOURCE_PATH "window-list.ui", NULL);
...
#ifdef HAVE_WINDOW_PREVIEWS
  if (tasklist->preview) gtk_widget_destroy(tasklist->preview);
#endif
