...
static void na_host_default_init(NaHostInterface *iface) {
  signals[SIGNAL_ITEM_ADDED] = g_signal_new(
      "item-added", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, NULL, G_TYPE_NONE, 1, NA_TYPE_ITEM);

  signals[SIGNAL_ITEM_REMOVED] = g_signal_new(
      "item-removed", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, NULL, G_TYPE_NONE, 1, NA_TYPE_ITEM);

  g_object_interface_install_property(
      iface, g_param_spec_int(
                 "icon-padding", "Padding around icons",
  ...
  g_object_interface_install_property(
      iface, g_param_spec_int(
                 "icon-size", "Icon size",
                 "If non-zero, hardcodes the size of the icons in pixels", 0,
                 G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
void na_host_force_redraw(NaHost *host) {
  ...
  if (iface->force_redraw != NULL) iface->force_redraw(host);
}
...
void na_host_style_updated(NaHost *host, GtkStyleContext *context) {
  ...
  if (iface->style_updated != NULL) iface->style_updated(host, context);
}
