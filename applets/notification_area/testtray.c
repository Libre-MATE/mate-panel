...
static void do_add(GtkWidget *child, guint *n_children) { *n_children += 1; }

static void update_child_count(TrayData *data) {
  guint n_children = 0;
  char text[64];

  if (!gtk_widget_get_realized(data->window)) return;

  gtk_container_foreach(GTK_CONTAINER(data->traybox), (GtkCallback)do_add,
                        &n_children);
...
static void tray_added_cb(GtkContainer *box, GtkWidget *icon, TrayData *data) {
  g_print("[Screen %u tray %p] Child %p added to tray: \"%s\"\n",
          data->screen_num, data->traybox, icon,
          "XXX"); /* na_tray_child_get_title (icon)); */

  update_child_count(data);
}

static void tray_removed_cb(GtkContainer *box, GtkWidget *icon,
                            TrayData *data) {
...
static void orientation_changed_cb(GtkComboBox *combo, TrayData *data) {
  ...
  if ((active = gtk_combo_box_get_active(combo)) != -1) {
    ...
    g_print(
        "[Screen %u tray %p] Setting orientation to \"%s\"\n", data->screen_num,
        data->traybox,
...
static void maybe_quit(gpointer data, GObject *zombie) {
  if (--n_windows == 0) {
    gtk_main_quit();
  }
}
...
static void warning_dialog_response_cb(GtkWidget *dialog, gint response,
                                       GdkScreen *screen) {
...
static void add_tray_cb(GtkWidget *button, TrayData *data) {
...
static TrayData *create_tray_on_screen(GdkScreen *screen, gboolean force) {
  ...
  if (!force && na_tray_manager_check_running(screen)) {
    ...
    dialog =
        gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_WARNING, GTK_BUTTONS_YES_NO,
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(warning_dialog_response_cb),
                     screen);
  ...
  g_signal_connect(combo, "changed", G_CALLBACK(orientation_changed_cb), data);
  ...
  g_signal_connect_after(data->traybox, "remove", G_CALLBACK(tray_removed_cb),
                         data);
...
#ifdef PROVIDE_WATCHER_SERVICE
static GfStatusNotifierWatcher *status_notifier_watcher_maybe_new(void) {
  GDBusProxy *proxy;
  GError *error = NULL;
  ...
  if (proxy)
    ...
    g_warning(
        "Failed to connect to org.kde.StatusNotifierWatcher (%s), starting our "
        "own.",
        error->message);
    g_clear_error(&error);
