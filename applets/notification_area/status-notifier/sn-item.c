...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE(
    SnItem, sn_item, SN_TYPE_FLAT_BUTTON,
    G_ADD_PRIVATE(SnItem) G_IMPLEMENT_INTERFACE(GTK_TYPE_ORIENTABLE, NULL)
        G_IMPLEMENT_INTERFACE(NA_TYPE_ITEM, na_item_init))

static void sn_item_dispose(GObject *object) {
  SnItem *item;
  SnItemPrivate *priv;
...
static void sn_item_get_property(GObject *object, guint property_id,
...
static void sn_item_set_property(GObject *object, guint property_id,
...
static void sn_item_get_action_coordinates(SnItem *item, gint *x, gint *y) {
  GtkWidget *widget;
  SnItemPrivate *priv;
  GdkWindow *window;
  GtkWidget *toplevel;
...
static gboolean sn_item_button_press_event(GtkWidget *widget,
  ...
  if (event->button < 2 || event->button > 3)
    return GTK_WIDGET_CLASS(sn_item_parent_class)
  ...
  if (event->button == 2) {
    ...
    if (priv->menu != NULL) {
      gtk_menu_popup_at_widget(priv->menu, widget, GDK_GRAVITY_SOUTH_WEST,
...
static gboolean sn_item_popup_menu(GtkWidget *widget) {
  SnItem *item;
  SnItemPrivate *priv;
...
static void sn_item_clicked(GtkButton *button) {
  SnItem *item;
  gint x;
  gint y;
...
static gboolean sn_item_scroll_event(GtkWidget *widget, GdkEventScroll *event) {
  SnItem *item;
...
static void sn_item_ready(SnItem *item) {
  const gchar *menu;
  SnItemPrivate *priv;

  menu = SN_ITEM_GET_CLASS(item)->get_menu(item);

  if (menu == NULL || *menu == '\0' || g_strcmp0(menu, "/") == 0) return;
...
static const gchar *sn_item_get_id(NaItem *item) {
  return SN_ITEM_GET_CLASS(item)->get_id(SN_ITEM(item));
}

static NaItemCategory sn_item_get_category(NaItem *item) {
  const gchar *string;
...
static void install_signals(SnItemClass *item_class) {
  signals[SIGNAL_READY] = g_signal_new(
      "ready", G_TYPE_FROM_CLASS(item_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(SnItemClass, ready), NULL, NULL, NULL, G_TYPE_NONE, 0);
}

static void sn_item_class_init(SnItemClass *item_class) {
...
static void sn_item_init(SnItem *item) {
