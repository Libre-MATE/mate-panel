...
static GdkPixbuf *pxibuf_new(GVariant *variant) {
  gsize length;
  const guchar *data;
  GInputStream *stream;
  GdkPixbuf *pixbuf;
...
static SnShortcut *sn_shortcut_new(guint key, GdkModifierType mask) {
  SnShortcut *shortcut;
...
static SnShortcut **sn_shortcuts_new(GVariant *variant) {
  GPtrArray *array;
...
static void sn_shortcuts_free(SnShortcut **shortcuts) {
  guint i;

  if (shortcuts == NULL) return;

  for (i = 0; shortcuts[i] != NULL; i++) g_free(shortcuts[i]);
...
SnDBusMenuItem *sn_dbus_menu_item_new(GVariant *props) {
  ...
  if (g_strcmp0(item->type, "separator") == 0) {
    ...
    if (g_strcmp0(item->toggle_type, "checkmark") == 0) {
      ...
      item->item = gtk_check_menu_item_new();
      gtk_check_menu_item_set_draw_as_radio(GTK_CHECK_MENU_ITEM(item->item),
                                            TRUE);
      AtkObject *atk_obj;
      ...
      if (item->icon_name) {
        image =
            gtk_image_new_from_icon_name(item->icon_name, GTK_ICON_SIZE_MENU);
      } else if (item->icon_data) {
        cairo_surface_t *surface;
        surface =
            gdk_cairo_surface_create_from_pixbuf(item->icon_data, 0, NULL);
  ...
  while (g_variant_iter_next(&iter, "&s", &prop)) {
    if (g_strcmp0(prop, "accessible-desc") == 0) {
      ...
      if (GTK_IS_IMAGE_MENU_ITEM(item->item)) {
        gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item->item), NULL);
      }
    } else if (g_strcmp0(prop, "icon-data") == 0) {
      g_clear_object(&item->icon_data);
      if (GTK_IS_IMAGE_MENU_ITEM(item->item)) {
