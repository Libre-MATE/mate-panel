...
static cairo_surface_t *scale_surface(SnIconPixmap *pixmap,
                                      GtkOrientation orientation, gint size) {
  gdouble ratio;
  gdouble new_width;
  gdouble new_height;
  gdouble scale_x;
  ...
  scaled =
      cairo_surface_create_similar(pixmap->surface, content, width, height);
...
static gint compare_size(gconstpointer a, gconstpointer b, gpointer user_data) {
  SnIconPixmap *p1;
  SnIconPixmap *p2;
...
static cairo_surface_t *get_surface(SnItemV0 *v0, GtkOrientation orientation,
                                    gint size) {
  gint i;
  GList *pixmaps = NULL;
...
static cairo_surface_t *get_icon_by_name(const gchar *icon_name,
                                         gint requested_size, gint scale) {
  ...
  for (i = 0; sizes[i] != 0; i++) {
    if (sizes[i] == requested_size || sizes[i] == -1) /* scalable */
    {
  ...
  if (chosen_size == 0) chosen_size = requested_size;

  return gtk_icon_theme_load_surface(icon_theme, icon_name, chosen_size, scale,
                                     NULL, GTK_ICON_LOOKUP_FORCE_SIZE, NULL);
}

static void update(SnItemV0 *v0) {
  ...
  if (v0->icon_name != NULL && v0->icon_name[0] != '\0') {
    ...
    if (!surface) {
      ...
      if (pixbuf && icon_size > 1) {
        /*An icon specified by path and filename may be the wrong size for the
         * tray */
        pixbuf = gdk_pixbuf_scale_simple(pixbuf, icon_size - 2, icon_size - 2,
                                         GDK_INTERP_BILINEAR);
    ...
    surface = get_surface(
        v0, gtk_orientable_get_orientation(GTK_ORIENTABLE(v0)), icon_size);
    if (surface != NULL) {
      gtk_image_set_from_surface(image, surface);
      cairo_surface_destroy(surface);
    }
...
static void queue_update(SnItemV0 *v0) {
  if (v0->update_id != 0) return;

  v0->update_id = g_timeout_add(10, update_cb, v0);
  g_source_set_name_by_id(v0->update_id, "[status-notifier] update_cb");
}

static cairo_surface_t *surface_from_variant(GVariant *variant, gint width,
                                             gint height) {
...
#if G_BYTE_ORDER == G_LITTLE_ENDIAN
  {
    ...
    for (i = 0; i < width * height; i++) data[i] = GUINT32_FROM_BE(data[i]);
  }
#endif
  ...
  return cairo_image_surface_create_for_data((guchar *)data, format, width,
                                             height, stride);
}

static cairo_surface_t *icon_surface_new(GVariant *variant, gint width,
                                         gint height) {
  cairo_surface_t *surface;
  cairo_surface_t *tmp;
  cairo_t *cr;
...
static SnIconPixmap **icon_pixmap_new(GVariant *variant) {
  GPtrArray *array;
  GVariantIter iter;
  gint width;
  gint height;
  ...
  if (variant == NULL || g_variant_iter_init(&iter, variant) == 0) return NULL;
...
static void icon_pixmap_free(SnIconPixmap **data) {
  gint i;
...
static SnTooltip *sn_tooltip_new(GVariant *variant) {
  ...
  if (!g_variant_is_of_type(variant, G_VARIANT_TYPE("(sa(iiay)ss)"))) {
    g_warning(
        "Type for 'ToolTip' property should be '(sa(iiay)ss)' "
        "but got '%s'",
  ...
  g_variant_get(variant, "(&s@a(iiay)&s&s)", &icon_name, &icon_pixmap, &title,
                &text);
...
static void sn_tooltip_free(SnTooltip *tooltip) {
...
static GVariant *get_property(GObject *source_object, GAsyncResult *res,
  ...
  error = NULL;
  variant = g_dbus_connection_call_finish(G_DBUS_CONNECTION(source_object), res,
                                          &error);
...
static void update_property(SnItemV0 *v0, const gchar *property,
  ...
  g_dbus_connection_call(
      g_dbus_proxy_get_connection(proxy), sn_item_get_bus_name(item),
      sn_item_get_object_path(item), "org.freedesktop.DBus.Properties", "Get",
      g_variant_new("(ss)", SN_ITEM_INTERFACE, property), G_VARIANT_TYPE("(v)"),
      G_DBUS_CALL_FLAGS_NONE, -1, v0->cancellable, callback, v0);
}

static void update_title(GObject *source_object, GAsyncResult *res,
                         gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
  gboolean cancelled;
...
static void new_title_cb(SnItemV0 *v0) {
  update_property(v0, "Title", update_title);
}

static void update_icon_name(GObject *source_object, GAsyncResult *res,
                             gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
  gboolean cancelled;
...
static void update_icon_pixmap(GObject *source_object, GAsyncResult *res,
                               gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
  gboolean cancelled;
...
static void new_icon_cb(SnItemV0 *v0) {
  update_property(v0, "IconName", update_icon_name);
  update_property(v0, "IconPixmap", update_icon_pixmap);
}

static void update_overlay_icon_name(GObject *source_object, GAsyncResult *res,
                                     gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
  gboolean cancelled;
...
static void update_overlay_icon_pixmap(GObject *source_object,
...
static void new_overlay_icon_cb(SnItemV0 *v0) {
...
static void update_attention_icon_name(GObject *source_object,
                                       GAsyncResult *res, gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
  gboolean cancelled;
...
static void update_attention_icon_pixmap(GObject *source_object,
                                         GAsyncResult *res,
                                         gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
...
static void new_attention_icon_cb(SnItemV0 *v0) {
...
static void update_tooltip(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  SnItemV0 *v0;
  GVariant *variant;
  gboolean cancelled;
...
static void new_tooltip_cb(SnItemV0 *v0) {
  update_property(v0, "ToolTip", update_tooltip);
}

static void new_status_cb(SnItemV0 *v0, GVariant *parameters) {
  GVariant *variant;
...
static void new_label_cb(SnItemV0 *v0, GVariant *parameters) {
  GVariant *variant;
...
static void new_icon_theme_path_cb(SnItemV0 *v0, GVariant *parameters) {
  GVariant *variant;
...
static void g_properties_changed_cb(GDBusProxy *proxy,
...
static void g_signal_cb(GDBusProxy *proxy, gchar *sender_name,
...
static void get_all_cb(GObject *source_object, GAsyncResult *res,
                       gpointer user_data) {
  SnItemV0 *v0;
  GVariant *properties;
  GError *error;
  ...
  if (v0->id == NULL || v0->category == NULL || v0->status == NULL) {
    ...
    g_warning("Invalid Status Notifier Item (%s, %s)", bus_name, object_path);
  ...
  g_signal_connect(v0->proxy, "g-signal", G_CALLBACK(g_signal_cb), v0);

  update(v0);
  sn_item_emit_ready(SN_ITEM(v0));
}

static void proxy_ready_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  SnItemV0 *v0;
  SnItemV0Gen *proxy;
  GError *error;
  ...
  g_dbus_connection_call(
      g_dbus_proxy_get_connection(G_DBUS_PROXY(proxy)),
      sn_item_get_bus_name(SN_ITEM(v0)), sn_item_get_object_path(SN_ITEM(v0)),
      "org.freedesktop.DBus.Properties", "GetAll",
      g_variant_new("(s)", SN_ITEM_INTERFACE), G_VARIANT_TYPE("(a{sv})"),
      G_DBUS_CALL_FLAGS_NONE, -1, v0->cancellable, get_all_cb, v0);
}

static void sn_item_v0_constructed(GObject *object) {
  SnItemV0 *v0;
  SnItem *item;
  ...
  sn_item_v0_gen_proxy_new_for_bus(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, sn_item_get_bus_name(item),
      sn_item_get_object_path(item), v0->cancellable, proxy_ready_cb, v0);
}

static void sn_item_v0_dispose(GObject *object) {
  SnItemV0 *v0;
...
static void sn_item_v0_finalize(GObject *object) {
  SnItemV0 *v0;
...
static const gchar *sn_item_v0_get_id(SnItem *item) {
  SnItemV0 *v0;
...
static const gchar *sn_item_v0_get_category(SnItem *item) {
  SnItemV0 *v0;
...
static const gchar *sn_item_v0_get_menu(SnItem *item) {
  SnItemV0 *v0;
...
static void context_menu_cb(GObject *source_object, GAsyncResult *res,
                            gpointer user_data) {
  SnItemV0 *v0;
...
static void sn_item_v0_context_menu(SnItem *item, gint x, gint y) {
  SnItemV0 *v0;

  v0 = SN_ITEM_V0(item);

  sn_item_v0_gen_call_context_menu(v0->proxy, x, y, NULL, context_menu_cb, v0);
}

static void activate_cb(GObject *source_object, GAsyncResult *res,
                        gpointer user_data) {
  SnItemV0 *v0;
...
static void sn_item_v0_activate(SnItem *item, gint x, gint y) {
  SnItemV0 *v0;

  v0 = SN_ITEM_V0(item);

  sn_item_v0_gen_call_activate(v0->proxy, x, y, NULL, activate_cb, v0);
}

static void secondary_activate_cb(GObject *source_object, GAsyncResult *res,
                                  gpointer user_data) {
  SnItemV0 *v0;
...
static void sn_item_v0_secondary_activate(SnItem *item, gint x, gint y) {
  SnItemV0 *v0;
...
static void scroll_cb(GObject *source_object, GAsyncResult *res,
                      gpointer user_data) {
  SnItemV0 *v0;
...
static void sn_item_v0_scroll(SnItem *item, gint delta,
...
static void sn_item_v0_size_allocate(GtkWidget *widget,
  ...
  if (v0->icon_size <= 0) {
    ...
    if (gtk_orientable_get_orientation(GTK_ORIENTABLE(v0)) ==
...
static void sn_item_v0_get_property(GObject *object, guint property_id,
...
static void sn_item_v0_set_property(GObject *object, guint property_id,
...
static void sn_item_v0_class_init(SnItemV0Class *v0_class) {
...
static void sn_item_v0_init(SnItemV0 *v0) {
  v0->icon_size = 16;
...
SnItem *sn_item_v0_new(const gchar *bus_name, const gchar *object_path) {
  return g_object_new(SN_TYPE_ITEM_V0, "bus-name", bus_name, "object-path",
                      object_path, NULL);
}
...
void sn_item_v0_set_icon_padding(SnItemV0 *v0, gint padding) {
  GtkOrientation orientation;
  gint padding_x = 0;
  gint padding_y = 0;
...
gint sn_item_v0_get_icon_size(SnItemV0 *v0) { return v0->icon_size; }

void sn_item_v0_set_icon_size(SnItemV0 *v0, gint size) {
  if (v0->icon_size != size) {
    v0->icon_size = size;
    g_object_notify_by_pspec(G_OBJECT(v0), obj_properties[PROP_ICON_SIZE]);

    if (v0->id != NULL) queue_update(v0);
  }
}
