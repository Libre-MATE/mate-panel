...
G_DEFINE_TYPE_WITH_CODE(SnHostV0, sn_host_v0, SN_TYPE_HOST_V0_GEN_SKELETON,
                        G_IMPLEMENT_INTERFACE(SN_TYPE_HOST_V0_GEN,
                                              sn_host_v0_gen_init)
...
static void get_bus_name_and_object_path(const gchar *service, gchar **bus_name,
                                         gchar **object_path) {
  gchar *tmp;
...
static void ready_cb(SnItem *item, SnHostV0 *v0) {
  na_host_emit_item_added(NA_HOST(v0), NA_ITEM(item));
}

static void add_registered_item(SnHostV0 *v0, const gchar *service) {
  gchar *bus_name;
  gchar *object_path;
  SnItem *item;
...
static void item_registered_cb(SnWatcherV0Gen *watcher, const gchar *service,
                               SnHostV0 *v0) {
  add_registered_item(v0, service);
}

static void item_unregistered_cb(SnWatcherV0Gen *watcher, const gchar *service,
                                 SnHostV0 *v0) {
  GSList *l;
...
static void register_host_cb(GObject *source_object, GAsyncResult *res,
                             gpointer user_data) {
  GError *error;
  SnHostV0 *v0;
  gchar **items;
  ...
  if (items) {
    for (i = 0; items[i] != NULL; i++) add_registered_item(v0, items[i]);
  }
...
static void emit_item_removed_signal(gpointer data, gpointer user_data) {
...
static void bus_acquired_cb(GDBusConnection *connection, const gchar *name,
  ...
  error = NULL;
  g_dbus_interface_skeleton_export(skeleton, connection, v0->object_path,
                                   &error);
  ...
  v0->watcher_id = g_bus_watch_name(
      G_BUS_TYPE_SESSION, SN_WATCHER_BUS_NAME, G_BUS_NAME_WATCHER_FLAGS_NONE,
      name_appeared_cb, name_vanished_cb, v0, NULL);
}

static void sn_host_v0_get_property(GObject *object, guint property_id,
...
static void sn_host_v0_set_property(GObject *object, guint property_id,
...
static void install_properties(GObjectClass *object_class) {
  /* NaHost properties */
  g_object_class_override_property(object_class, PROP_ICON_PADDING,
                                   "icon-padding");
  g_object_class_override_property(object_class, PROP_ICON_SIZE, "icon-size");
}

static void sn_host_v0_dispose(GObject *object) {
  SnHostV0 *v0;
...
static void sn_host_v0_finalize(GObject *object) {
  SnHostV0 *v0;
...
static void sn_host_v0_class_init(SnHostV0Class *v0_class) {
...
static void sn_host_v0_init(SnHostV0 *v0) {
...
NaHost *sn_host_v0_new(void) { return g_object_new(SN_TYPE_HOST_V0, NULL); }
