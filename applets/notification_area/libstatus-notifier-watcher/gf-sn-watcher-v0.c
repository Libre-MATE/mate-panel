...
G_DEFINE_TYPE_WITH_CODE(GfSnWatcherV0, gf_sn_watcher_v0,
                        GF_TYPE_SN_WATCHER_V0_GEN_SKELETON,
                        G_IMPLEMENT_INTERFACE(GF_TYPE_SN_WATCHER_V0_GEN,
                                              gf_sn_watcher_v0_gen_init))

static void update_registered_items(GfSnWatcherV0 *v0) {
...
static void gf_watch_free(gpointer data) {
  GfWatch *watch;
...
static void name_vanished_cb(GDBusConnection *connection, const char *name,
                             gpointer user_data) {
  GfWatch *watch;
  GfSnWatcherV0 *v0;
...
static GfWatch *gf_watch_new(GfSnWatcherV0 *v0, GfWatchType type,
                             const gchar *service, const gchar *bus_name,
...
static GfWatch *gf_watch_find(GSList *list, const gchar *bus_name,
                              const gchar *object_path) {
  GSList *l;
...
static gboolean gf_sn_watcher_v0_handle_register_host(
    GfSnWatcherV0Gen *object, GDBusMethodInvocation *invocation,
    const gchar *service) {
  GfSnWatcherV0 *v0;
  ...
  if (watch != NULL) {
    g_dbus_method_invocation_return_error(
        invocation, G_DBUS_ERROR, G_DBUS_ERROR_INVALID_ARGS,
        "Status Notifier Host with bus name '%s' and object path '%s' is "
        "already registered",
...
static gboolean gf_sn_watcher_v0_handle_register_item(
    GfSnWatcherV0Gen *object, GDBusMethodInvocation *invocation,
    const gchar *service) {
  GfSnWatcherV0 *v0;
  const gchar *bus_name;
...
static void gf_sn_watcher_v0_dispose(GObject *object) {
  GfSnWatcherV0 *v0;
...
static void gf_sn_watcher_v0_class_init(GfSnWatcherV0Class *v0_class) {
...
static void gf_sn_watcher_v0_init(GfSnWatcherV0 *v0) {
  ...
  v0->bus_name_id =
      g_bus_own_name(G_BUS_TYPE_SESSION, "org.kde.StatusNotifierWatcher", flags,
