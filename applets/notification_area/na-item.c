...
G_DEFINE_INTERFACE_WITH_CODE(
    ...
    g_type_interface_add_prerequisite(g_define_type_id, GTK_TYPE_ORIENTABLE);)

static gboolean na_item_draw_on_parent_default(NaItem *item, GtkWidget *parent,
                                               cairo_t *parent_cr) {
  return FALSE;
}

static void na_item_default_init(NaItemInterface *iface) {
...
NaItemCategory na_item_get_category(NaItem *item) {
  ...
  g_return_val_if_fail(NA_IS_ITEM(item), NA_ITEM_CATEGORY_APPLICATION_STATUS);
 ...
 */
gboolean na_item_draw_on_parent(NaItem *item, GtkWidget *parent,
                                cairo_t *parent_cr) {
  NaItemInterface *iface;
