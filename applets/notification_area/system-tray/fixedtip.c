...
static gboolean button_press_handler(GtkWidget *fixedtip, GdkEventButton *event,
                                     gpointer data) {
...
static gboolean na_fixed_tip_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  cairo_save(cr);
  gtk_render_background(context, cr, 0., 0., (gdouble)width, (gdouble)height);
  cairo_restore(cr);
...
static void na_fixed_tip_class_init(NaFixedTipClass *class) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(class);
  widget_class->draw = na_fixed_tip_draw;

  fixedtip_signals[CLICKED] =
      g_signal_new("clicked", G_OBJECT_CLASS_TYPE(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(NaFixedTipClass, clicked), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

/* Did you already see this code? Yes, it's gtk_tooltips_ force_window() ;-) */
static void na_fixed_tip_init(NaFixedTip *fixedtip) {
  GtkWidget *label;

  fixedtip->priv = na_fixed_tip_get_instance_private(fixedtip);

  gtk_window_set_type_hint(GTK_WINDOW(fixedtip), GDK_WINDOW_TYPE_HINT_TOOLTIP);
...
static void na_fixed_tip_position(NaFixedTip *fixedtip) {
  GdkScreen *screen;
...
#define PAD 5
  ...
  /* Push onscreen */
  if ((root_x + req.width) > screen_width) root_x = screen_width - req.width;
...
static void na_fixed_tip_parent_size_allocated(GtkWidget *parent,
                                               GtkAllocation *allocation,
                                               NaFixedTip *fixedtip) {
  na_fixed_tip_position(fixedtip);
}

static void na_fixed_tip_parent_screen_changed(GtkWidget *parent,
...
void na_fixed_tip_set_markup(GtkWidget *widget, const char *markup_text) {
  ...
  gtk_label_set_markup(GTK_LABEL(fixedtip->priv->label), markup_text);
...
void na_fixed_tip_set_orientation(GtkWidget *widget,
  ...
  if (orientation == fixedtip->priv->orientation) return;
