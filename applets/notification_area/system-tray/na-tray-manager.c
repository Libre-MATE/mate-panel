...
static void na_tray_manager_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void na_tray_manager_get_property(GObject *object, guint prop_id,
...
static void na_tray_manager_class_init(NaTrayManagerClass *klass) {
  ...
  manager_signals[TRAY_ICON_ADDED] = g_signal_new(
      "tray-icon-added", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NaTrayManagerClass, tray_icon_added), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_SOCKET);

  manager_signals[TRAY_ICON_REMOVED] = g_signal_new(
      "tray-icon-removed", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NaTrayManagerClass, tray_icon_removed), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_SOCKET);
  manager_signals[MESSAGE_SENT] = g_signal_new(
      "message-sent", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NaTrayManagerClass, message_sent), NULL, NULL,
      _na_marshal_VOID__OBJECT_STRING_LONG_LONG, G_TYPE_NONE, 4,
      GTK_TYPE_SOCKET, G_TYPE_STRING, G_TYPE_LONG, G_TYPE_LONG);
  manager_signals[MESSAGE_CANCELLED] = g_signal_new(
      "message-cancelled", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NaTrayManagerClass, message_cancelled), NULL, NULL,
      _na_marshal_VOID__OBJECT_LONG, G_TYPE_NONE, 2, GTK_TYPE_SOCKET,
      G_TYPE_LONG);
  manager_signals[LOST_SELECTION] = g_signal_new(
      "lost_selection", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NaTrayManagerClass, lost_selection), NULL, NULL,
...
static gboolean na_tray_manager_plug_removed(GtkSocket *socket,
                                             NaTrayManager *manager) {
...
static void na_tray_manager_handle_dock_request(NaTrayManager *manager,
...
static void na_tray_manager_handle_message_data(NaTrayManager *manager,
                                                XClientMessageEvent *xevent) {
  GList *p;
  int len;
  ...
  for (p = manager->messages; p; p = p->next) {
    ...
    if (xevent->window == msg->window) {
      ...
      if (msg->remaining_len == 0) {
        ...
        if (socket)
          g_signal_emit(manager, manager_signals[MESSAGE_SENT], 0, socket,
                        msg->str, msg->id, msg->timeout);
...
static void na_tray_manager_handle_begin_message(NaTrayManager *manager,
                                                 XClientMessageEvent *xevent) {
  GtkSocket *socket;
  GList *p;
  PendingMessage *msg;
  ...
  for (p = manager->messages; p; p = p->next) {
    ...
    if (xevent->window == pmsg->window && id == pmsg->id) {
  ...
  if (len == 0) {
    g_signal_emit(manager, manager_signals[MESSAGE_SENT], 0, socket, "", id,
                  timeout);
  } else {
...
static void na_tray_manager_handle_cancel_message(NaTrayManager *manager,
                                                  XClientMessageEvent *xevent) {
  GList *p;
  GtkSocket *socket;
  long id;
  ...
  for (p = manager->messages; p; p = p->next) {
    ...
    if (xevent->window == msg->window && id == msg->id) {
      pending_message_free(msg);
      manager->messages = g_list_remove_link(manager->messages, p);
      g_list_free_1(p);
      break;
  ...
  if (socket) {
    g_signal_emit(manager, manager_signals[MESSAGE_CANCELLED], 0, socket,
                  xevent->data.l[2]);
  }
}

static GdkFilterReturn na_tray_manager_window_filter(GdkXEvent *xev,
                                                     GdkEvent *event,
                                                     gpointer data) {
...
static void na_tray_manager_unmanage(NaTrayManager *manager) {
  ...
  if (manager->invisible == NULL) return;
  ...
  if (gdk_selection_owner_get_for_display(display, manager->selection_atom) ==
    ...
    gdk_selection_owner_set_for_display(display, NULL, manager->selection_atom,
                                        timestamp, TRUE);
  }

  gdk_window_remove_filter(window, na_tray_manager_window_filter, manager);
...
static void na_tray_manager_set_orientation_property(NaTrayManager *manager) {
#ifdef GDK_WINDOWING_X11
  GdkWindow *window;
  GdkDisplay *display;
...
static void na_tray_manager_set_visual_property(NaTrayManager *manager) {
#ifdef GDK_WINDOWING_X11
  GdkWindow *window;
  GdkDisplay *display;
  Visual *xvisual;
  ...
  display = gtk_widget_get_display(manager->invisible);
  visual_atom =
      gdk_x11_get_xatom_by_name_for_display(display, "_NET_SYSTEM_TRAY_VISUAL");
...
static void na_tray_manager_set_padding_property(NaTrayManager *manager) {
#ifdef GDK_WINDOWING_X11
  GdkWindow *window;
  GdkDisplay *display;
  Atom atom;
  ...
  XChangeProperty(GDK_DISPLAY_XDISPLAY(display), GDK_WINDOW_XID(window), atom,
                  XA_CARDINAL, 32, PropModeReplace, (guchar *)&data, 1);
#endif
}

static void na_tray_manager_set_icon_size_property(NaTrayManager *manager) {
#ifdef GDK_WINDOWING_X11
  GdkWindow *window;
  GdkDisplay *display;
  Atom atom;
...
static void na_tray_manager_set_colors_property(NaTrayManager *manager) {
#ifdef GDK_WINDOWING_X11
  GdkWindow *window;
  GdkDisplay *display;
  Atom atom;
  ...
  display = gtk_widget_get_display(manager->invisible);
  atom =
      gdk_x11_get_xatom_by_name_for_display(display, "_NET_SYSTEM_TRAY_COLORS");
  ...
  XChangeProperty(GDK_DISPLAY_XDISPLAY(display), GDK_WINDOW_XID(window), atom,
                  XA_CARDINAL, 32, PropModeReplace, (guchar *)&data, 12);
#endif
}

#ifdef GDK_WINDOWING_X11

static gboolean na_tray_manager_manage_screen_x11(NaTrayManager *manager,
                                                  GdkScreen *screen) {
  GdkDisplay *display;
  Screen *xscreen;
  GtkWidget *invisible;
  ...
  selection_atom_name = g_strdup_printf(
      "_NET_SYSTEM_TRAY_S%d", gdk_x11_screen_get_screen_number(screen));
  ...
  if (gdk_selection_owner_set_for_display(
          display, window, manager->selection_atom, timestamp, TRUE)) {
...
void na_tray_manager_set_padding(NaTrayManager *manager, gint padding) {
  g_return_if_fail(NA_IS_TRAY_MANAGER(manager));
...
void na_tray_manager_set_icon_size(NaTrayManager *manager, gint icon_size) {
  g_return_if_fail(NA_IS_TRAY_MANAGER(manager));
...
void na_tray_manager_set_colors(NaTrayManager *manager, GdkRGBA *fg,
                                GdkRGBA *error, GdkRGBA *warning,
                                GdkRGBA *success) {
  g_return_if_fail(NA_IS_TRAY_MANAGER(manager));
