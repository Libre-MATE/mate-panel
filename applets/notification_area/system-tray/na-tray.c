...
static void na_tray_style_updated(NaHost *host, GtkStyleContext *context);
...
G_DEFINE_TYPE_WITH_CODE(NaTray, na_tray, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(NaTray)
                            G_IMPLEMENT_INTERFACE(GTK_TYPE_ORIENTABLE, NULL)
                                G_IMPLEMENT_INTERFACE(NA_TYPE_HOST,
                                                      na_host_init))
...
static NaTray *get_tray(TraysScreen *trays_screen) {
  if (trays_screen->all_trays == NULL) return NULL;

  return trays_screen->all_trays->data;
}

static void tray_added(NaTrayManager *manager, NaTrayChild *icon,
                       TraysScreen *trays_screen) {
  NaTray *tray;
  NaTrayPrivate *priv;
...
static void tray_removed(NaTrayManager *manager, NaTrayChild *icon,
                         TraysScreen *trays_screen) {
  NaTray *tray;
...
static void icon_tip_buffer_free(gpointer data, gpointer userdata) {
...
static void icon_tip_free(gpointer data) {
  IconTip *icontip;
  ...
  if (icontip->source_id != 0) g_source_remove(icontip->source_id);
  icontip->source_id = 0;
...
static int icon_tip_buffer_compare(gconstpointer a, gconstpointer b) {
  const IconTipBuffer *buffer_a = a;
  const IconTipBuffer *buffer_b = b;

  if (buffer_a == NULL || buffer_b == NULL) return !(buffer_a == buffer_b);

  return buffer_a->id - buffer_b->id;
}

static void icon_tip_show_next_clicked(GtkWidget *widget, gpointer data) {
  icon_tip_show_next((IconTip *)data);
}

static gboolean icon_tip_show_next_timeout(gpointer data) {
...
static void icon_tip_show_next(IconTip *icontip) {
  ...
  if (icontip->fixedtip == NULL) {
    icontip->fixedtip = na_fixed_tip_new(
        icontip->icon,
  ...
  if (buffer->timeout > 0)
    icontip->source_id = g_timeout_add_seconds(
        buffer->timeout, icon_tip_show_next_timeout, icontip);

  icon_tip_buffer_free(buffer, NULL);
}

static void message_sent(NaTrayManager *manager, GtkWidget *icon,
                         const char *text, glong id, glong timeout,
                         TraysScreen *trays_screen) {
  IconTip *icontip;
  ...
  if (icontip == NULL) {
    ...
    if (tray == NULL) {
      ...
      g_critical(
          "Ignoring a message sent by a tray icon "
          "we don't know: \"%s\".\n",
          text);
      return;
    }
  ...
  if (show_now) icon_tip_show_next(icontip);
}

static void message_cancelled(NaTrayManager *manager, GtkWidget *icon, glong id,
                              TraysScreen *trays_screen) {
  IconTip *icontip;
  IconTipBuffer find_buffer;
  ...
  if (cancel_buffer_l == NULL) return;
...
static void update_orientation_for_messages(gpointer key, gpointer value,
                                            gpointer data) {
  NaTray *tray;
  IconTip *icontip;
...
static void na_tray_init(NaTray *tray) {
  NaTrayPrivate *priv;
...
static GObject *na_tray_constructor(GType type, guint n_construct_properties,
  ...
  object = G_OBJECT_CLASS(na_tray_parent_class)
  ...
  if (trays_screens[screen_number].tray_manager == NULL) {
    ...
    if (na_tray_manager_manage_screen(tray_manager, priv->screen)) {
      ...
      g_signal_connect(tray_manager, "tray-icon-added", G_CALLBACK(tray_added),
                       &trays_screens[screen_number]);
      g_signal_connect(tray_manager, "tray-icon-removed",
                       G_CALLBACK(tray_removed), &trays_screens[screen_number]);
      g_signal_connect(tray_manager, "message-sent", G_CALLBACK(message_sent),
                       &trays_screens[screen_number]);
      g_signal_connect(tray_manager, "message-cancelled",
                       G_CALLBACK(message_cancelled),
                       &trays_screens[screen_number]);

      trays_screens[screen_number].icon_table = g_hash_table_new(NULL, NULL);
      trays_screens[screen_number].tip_table =
          g_hash_table_new_full(NULL, NULL, NULL, icon_tip_free);
    } else {
      g_printerr(
          "System tray didn't get the system tray manager selection for screen "
          "%d\n",
          screen_number);
  ...
  trays_screens[screen_number].all_trays =
      g_slist_append(trays_screens[screen_number].all_trays, tray);
...
static void na_tray_dispose(GObject *object) {
...
static void na_tray_set_orientation(NaTray *tray, GtkOrientation orientation) {
  NaTrayPrivate *priv = tray->priv;

  if (orientation == priv->orientation) return;
...
static void na_tray_set_property(GObject *object, guint prop_id,
...
static void na_tray_get_property(GObject *object, guint prop_id, GValue *value,
                                 GParamSpec *pspec) {
  NaTray *tray = NA_TRAY(object);
  NaTrayPrivate *priv = tray->priv;
...
static void na_tray_class_init(NaTrayClass *klass) {
  ...
  g_object_class_override_property(gobject_class, PROP_ICON_PADDING,
                                   "icon-padding");
  g_object_class_override_property(gobject_class, PROP_ICON_SIZE, "icon-size");

  g_object_class_install_property(
      gobject_class, PROP_SCREEN,
      g_param_spec_object("screen", "screen", "screen", GDK_TYPE_SCREEN,
...
NaHost *na_tray_new_for_screen(GdkScreen *screen, GtkOrientation orientation) {
  return g_object_new(NA_TYPE_TRAY, "screen", screen, "orientation",
                      orientation, NULL);
}
...
void na_tray_set_icon_size(NaTray *tray, gint size) {
  NaTrayPrivate *priv = tray->priv;
...
static void na_tray_set_colors(NaTray *tray, GdkRGBA *fg, GdkRGBA *error,
                               GdkRGBA *warning, GdkRGBA *success) {
  NaTrayPrivate *priv = tray->priv;

  if (get_tray(priv->trays_screen) == tray)
    na_tray_manager_set_colors(priv->trays_screen->tray_manager, fg, error,
                               warning, success);
}
...
static gboolean idle_redraw_cb(NaTray *tray) {
  NaTrayPrivate *priv = tray->priv;
