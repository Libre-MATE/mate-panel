...
static void na_tray_child_finalize(GObject *object) {
...
static void na_tray_child_realize(GtkWidget *widget) {
...
static void na_tray_child_style_set(GtkWidget *widget,
...
static void na_tray_child_get_preferred_width(GtkWidget *widget,
                                              gint *minimal_width,
                                              gint *natural_width) {
  gint scale;
  ...
  if (*minimal_width < 16) *minimal_width = 16;

  if (*natural_width < 16) *natural_width = 16;

  *minimal_width = *minimal_width / scale;
  *natural_width = *natural_width / scale;
}

static void na_tray_child_get_preferred_height(GtkWidget *widget,
                                               gint *minimal_height,
                                               gint *natural_height) {
  gint scale;
  scale = gtk_widget_get_scale_factor(widget);
  ...
  if (*minimal_height < 16) *minimal_height = 16;

  if (*natural_height < 16) *natural_height = 16;
 ...
 */
static gboolean na_tray_child_draw(GtkWidget *widget, cairo_t *cr) {
  NaTrayChild *child = NA_TRAY_CHILD(widget);
  ...
  if (na_tray_child_has_alpha(child)) {
    ...
    XClearArea(GDK_WINDOW_XDISPLAY(window), GDK_WINDOW_XID(window), clip_rect.x,
               clip_rect.y, clip_rect.width, clip_rect.height, False);
    cairo_surface_mark_dirty_rectangle(target, clip_rect.x, clip_rect.y,
 ...
 */
static gboolean na_tray_child_draw_on_parent(NaItem *item, GtkWidget *parent,
                                             cairo_t *parent_cr) {
  if (na_tray_child_has_alpha(NA_TRAY_CHILD(item))) {
    ...
    gdk_cairo_set_source_window(parent_cr, gtk_widget_get_window(widget),
                                allocation.x, allocation.y);
    cairo_rectangle(parent_cr, allocation.x, allocation.y, allocation.width,
                    allocation.height);
...
static void na_tray_child_get_property(GObject *object, guint property_id,
...
static void na_tray_child_set_property(GObject *object, guint property_id,
...
static const gchar *na_tray_child_get_id(NaItem *item) {
...
static NaItemCategory na_tray_child_get_category(NaItem *item) {
  guint i;
...
static void na_tray_child_init(NaTrayChild *child) { child->id = NULL; }

static void na_tray_child_class_init(NaTrayChildClass *klass) {
...
GtkWidget *na_tray_child_new(GdkScreen *screen, Window icon_window) {
  ...
  visual =
      gdk_x11_screen_lookup_visual(screen, window_attributes.visual->visualid);
  ...
  child->has_alpha = (visual_has_alpha && gdk_display_supports_composite(
...
char *na_tray_child_get_title(NaTrayChild *child) {
  ...
  result = XGetWindowProperty(GDK_DISPLAY_XDISPLAY(display), child->icon_window,
                              atom, 0, G_MAXLONG, False, utf8_string, &type,
                              &format, &nitems, &bytes_after, (guchar **)&val);

  if (gdk_x11_display_error_trap_pop(display) || result != Success) return NULL;
...
void na_tray_child_set_composited(NaTrayChild *child, gboolean composited) {
  ...
  if (child->composited == composited) return;
...
static void _get_wmclass(Display *xdisplay, Window xwindow, char **res_class,
                         char **res_name) {
  GdkDisplay *display;
  XClassHint ch;
  ...
  if (res_class) *res_class = NULL;

  if (res_name) *res_name = NULL;

  if (ch.res_name) {
    if (res_name) *res_name = latin1_to_utf8(ch.res_name);

    XFree(ch.res_name);
  }

  if (ch.res_class) {
    if (res_class) *res_class = latin1_to_utf8(ch.res_class);
...
void na_tray_child_get_wm_class(NaTrayChild *child, char **res_name,
  ...
  _get_wmclass(GDK_DISPLAY_XDISPLAY(display), child->icon_window, res_class,
               res_name);
}
