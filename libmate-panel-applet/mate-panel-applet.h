...
G_DECLARE_DERIVABLE_TYPE(MatePanelApplet, mate_panel_applet, MATE_PANEL, APPLET,
                         GtkEventBox)
...
#define MATE_PANEL_APPLET_FLAGS_ALL                                  \
  (MATE_PANEL_APPLET_EXPAND_MAJOR | MATE_PANEL_APPLET_EXPAND_MINOR | \
...
struct _MatePanelAppletClass {
  ...
  void (*change_background)(MatePanelApplet *applet,
                            MatePanelAppletBackgroundType type, GdkRGBA *color,
...
MatePanelAppletBackgroundType mate_panel_applet_get_background(
    ...
    /* return values */ GdkRGBA *color, cairo_pattern_t **pattern);
...
int mate_panel_applet_factory_main(const gchar *factory_id,
                                   gboolean out_process, GType applet_type,
...
int mate_panel_applet_factory_setup_in_process(
    ...
    MatePanelAppletFactoryCallback callback, gpointer data);
...
#define _MATE_PANEL_APPLET_SETUP_GETTEXT(call_textdomain) \
  do {                                                    \
    ...
    if (call_textdomain) textdomain(GETTEXT_PACKAGE);     \
  } while (0)
#endif /* !defined(ENABLE_NLS) */

#define MATE_PANEL_APPLET_OUT_PROCESS_FACTORY(factory_id, type, name,         \
                                              callback, data)                 \
  int main(int argc, char *argv[]) {                                          \
    ...
    gtk_init(&argc, &argv);                                                   \
                                                                              \
    retval = mate_panel_applet_factory_main(factory_id, TRUE, type, callback, \
                                            data);                            \
    g_option_context_free(context);                                           \
                                                                              \
    return retval;                                                            \
...
#define MATE_PANEL_APPLET_IN_PROCESS_FACTORY(factory_id, type, descr,   \
                                             callback, data)            \
