...
static void mate_panel_applet_factory_finalize(GObject *object) {
...
static void mate_panel_applet_factory_init(MatePanelAppletFactory *factory) {
  factory->applets = g_hash_table_new(NULL, NULL);
  factory->next_uid = 1;
}

static void mate_panel_applet_factory_class_init(
    MatePanelAppletFactoryClass *klass) {
...
static void mate_panel_applet_factory_applet_removed(
    MatePanelAppletFactory *factory, GObject *applet) {
  guint uid;
  ...
  factory->n_applets--;
  if (factory->n_applets == 0) g_object_unref(factory);
}

MatePanelAppletFactory *mate_panel_applet_factory_new(const gchar *factory_id,
                                                      gboolean out_of_process,
                                                      GType applet_type,
                                                      GClosure *closure) {
  ...
  factory =
      MATE_PANEL_APPLET_FACTORY(g_object_new(PANEL_TYPE_APPLET_FACTORY, NULL));
  ...
  if (factories == NULL) factories = g_hash_table_new(g_str_hash, g_str_equal);
...
static void set_applet_constructor_properties(GObject *applet,
                                              GVariant *props) {
  GVariantIter iter;
  GVariant *value;
  gchar *key;
...
static void mate_panel_applet_factory_get_applet(
    MatePanelAppletFactory *factory, GDBusConnection *connection,
  ...
  applet = g_object_new(factory->applet_type, "out-of-process",
                        factory->out_of_process, "id", applet_id, "connection",
                        connection, "closure", factory->closure, NULL);
  factory->n_applets++;
  g_object_weak_ref(
      applet, (GWeakNotify)mate_panel_applet_factory_applet_removed, factory);
  ...
  return_value =
      g_variant_new("(obuu)", object_path, factory->out_of_process, xid, uid);
...
static void method_call_cb(GDBusConnection *connection, const gchar *sender,
  ...
  if (g_strcmp0(method_name, "GetApplet") == 0)
    mate_panel_applet_factory_get_applet(factory, connection, parameters,
                                         invocation);
}
...
static void on_bus_acquired(GDBusConnection *connection, const gchar *name,
  ...
  factory->registration_id = g_dbus_connection_register_object(
      connection, object_path, introspection_data->interfaces[0],
      &interface_vtable, factory, NULL, &error);
  if (error) {
...
gboolean mate_panel_applet_factory_register_service(
  ...
  factory->owner_id = g_bus_own_name(
      ...
      (GBusNameLostCallback)on_name_lost, factory, NULL);
  g_free(service_name);
...
GtkWidget *mate_panel_applet_factory_get_applet_widget(const gchar *id,
  ...
  if (!object || !GTK_IS_WIDGET(object)) return NULL;
