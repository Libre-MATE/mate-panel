...
#ifdef HAVE_X11
#ifdef HAVE_RANDR
static gboolean _panel_multimonitor_output_should_be_first(Display *xdisplay,
                                                           RROutput output,
                                                           XRROutputInfo *info,
                                                           RROutput primary) {
  if (primary) return output == primary;
  ...
  if (XRRGetOutputProperty(xdisplay, output, connector_type_atom, 0, 100, False,
                           False, None, &actual_type, &actual_format, &nitems,
...
static gboolean panel_multimonitor_get_randr_monitors(
    int *monitors_ret, GdkRectangle **geometries_ret) {
  GdkDisplay *display;
  GdkScreen *screen;
  GdkMonitor *monitor;
  Display *xdisplay;
  ...
  /* Use scale factor to bring geometries down to device pixels to support HiDPI
   * displays */
  scale = gdk_monitor_get_scale_factor(monitor);

  geometries_array =
      g_array_sized_new(FALSE, FALSE, sizeof(GdkRectangle), resources->noutput);
  ...
  for (i = 0; i < resources->noutput; i++) {
    ...
    if (output->connection != RR_Disconnected && output->crtc != 0) {
      XRRCrtcInfo *crtc;
      GdkRectangle rect;
      ...
      if (_panel_multimonitor_output_should_be_first(
              xdisplay, resources->outputs[i], output, primary))
...
static void panel_multimonitor_get_gdk_monitors(int *monitors_ret,
...
static void panel_multimonitor_get_raw_monitors(int *monitors_ret,
                                                GdkRectangle **geometries_ret) {
  *monitors_ret = 0;
  *geometries_ret = NULL;
...
static inline gboolean rectangle_overlaps(GdkRectangle *a, GdkRectangle *b) {
  return gdk_rectangle_intersect(a, b, NULL);
}

static long pixels_in_rectangle(GdkRectangle *r) {
  return (long)(r->width * r->height);
}

static void panel_multimonitor_compress_overlapping_monitors(
    int *num_monitors_inout, GdkRectangle **geometries_inout) {
  int num_monitors;
  GdkRectangle *geometries_array;
  int i;
  ...
  for (i = 0; i < num_monitors; i++) {
    ...
    while (j < num_monitors) {
      if (rectangle_overlaps(&geometries_array[i], &geometries_array[j])) {
        long pixels;
        ...
        if (num_monitors - j - 1 > 0)
          memmove(&geometries_array[j], &geometries_array[j + 1],
...
static gboolean panel_multimonitor_reinit_idle(gpointer data) {
...
static void panel_multimonitor_handle_screen_changed(GdkScreen *screen,
                                                     gpointer user_data) {
  if (reinit_id) return;
...
static void panel_multimonitor_handle_monitor_changed(GdkDisplay *display,
                                                      GdkMonitor *monitor,
                                                      gpointer user_data) {
  if (reinit_id) return;
...
static void panel_multimonitor_handle_monitor_invalidate(GdkMonitor *monitor,
                                                         gpointer user_data) {
  if (reinit_id) return;
...
#ifdef HAVE_X11
#ifdef HAVE_RANDR
static void panel_multimonitor_init_randr(GdkDisplay *display) {
  Display *xdisplay;
  ...
  if (XRRQueryExtension(xdisplay, &event_base, &error_base)) {
    ...
    if ((major == 1 && minor >= 3) || major > 1) have_randr = TRUE;
  }
}
#endif /* HAVE_RANDR */
#endif /* HAVE_X11 */
...
void panel_multimonitor_init(void) {
...
#ifdef HAVE_X11
#ifdef HAVE_RANDR
  if (GDK_IS_X11_DISPLAY(display)) panel_multimonitor_init_randr(display);
#endif /* HAVE_RANDR */
#endif /* HAVE_X11 */

  /*
   * The screen signals probably shouldn't be needed, but sometimes on X11 they
   * are the only ones that get fired
   */

  g_signal_handlers_disconnect_by_func(
      screen, panel_multimonitor_handle_screen_changed, NULL);
  ...
  g_signal_handlers_disconnect_by_func(
      display, panel_multimonitor_handle_monitor_changed, NULL);
  ...
  for (i = 0; i < gdk_display_get_n_monitors(display); i++) {
    ...
    g_signal_handlers_disconnect_by_func(
        display, panel_multimonitor_handle_monitor_invalidate, NULL);
    g_signal_connect(monitor, "invalidate",
                     G_CALLBACK(panel_multimonitor_handle_monitor_invalidate),
                     NULL);
  }
...
void panel_multimonitor_reinit(void) {
  ...
  if (geometries) g_free(geometries);
  ...
  for (l = toplevels; l; l = l->next) gtk_widget_queue_resize(l->data);

  g_list_free(toplevels);
}

int panel_multimonitor_monitors() { return monitor_count; }
...
static int axis_distance(int p, int axis_start, int axis_size) {
...
static inline void get_monitor_bounds(int n_monitor, MonitorBounds *bounds) {
 ...
 */
void panel_multimonitor_is_at_visible_extreme(int n_monitor, gboolean *leftmost,
                                              gboolean *rightmost,
                                              gboolean *topmost,
                                              gboolean *bottommost) {
  ...
  *bottommost = TRUE;
  ...
  for (i = 0; i < monitor_count; i++) {
    ...
    if ((iter.y0 >= monitor.y0 && iter.y0 < monitor.y1) ||
      ...
      if (iter.x0 < monitor.x0) *leftmost = FALSE;
      if (iter.x1 > monitor.x1) *rightmost = FALSE;
    }

    if ((iter.x0 >= monitor.x0 && iter.x0 < monitor.x1) ||
        (iter.x1 > monitor.x0 && iter.x1 <= monitor.x1)) {
      if (iter.y0 < monitor.y0) *topmost = FALSE;
      if (iter.y1 > monitor.y1) *bottommost = FALSE;
    }
  }
}
