...
enum { PROP_0, PROP_ACTION_TYPE, PROP_DND_ENABLED };
...
G_DEFINE_TYPE_WITH_PRIVATE(PanelActionButton, panel_action_button,
                           BUTTON_TYPE_WIDGET)

static void panel_action_button_type_changed(GSettings *settings, gchar *key,
...
static const gchar *get_action_type_name(gint id) {
  if ((id >= 0) && (id < PANEL_ACTION_LAST)) return panel_action_type[id];

  return NULL;
}

static gboolean get_action_type_id(const gchar *name, gint *id) {
  gint i;
...
static gboolean screensaver_properties_enabled(void) {
...
static gboolean screensaver_enabled(void) {
  if (panel_lockdown_get_disable_lock_screen()) return FALSE;

  return panel_lock_screen_action_available("lock");
}

static gboolean panel_action_lock_is_disabled(void) {
  return !screensaver_enabled();
}

static void panel_action_lock_setup_menu(PanelActionButton *button) {
  mate_panel_applet_add_callback(button->priv->info, "activate", NULL,
                                 _("_Activate Screensaver"),
                                 screensaver_enabled);

  mate_panel_applet_add_callback(button->priv->info, "lock", NULL,
                                 _("_Lock Screen"), screensaver_enabled);

  mate_panel_applet_add_callback(button->priv->info, "prefs",
                                 "document-properties", _("_Properties"),
                                 screensaver_properties_enabled);
}

static void panel_action_lock_invoke_menu(PanelActionButton *button,
...
static void panel_action_logout(GtkWidget *widget) {
  ...
  if (mate_gsettings_schema_exists(MATE_SESSION_SCHEMA)) {
    ...
    prompt =
        g_settings_get_boolean(msm_settings, MATE_SESSION_LOGOUT_PROMPT_KEY);
  ...
  if (!prompt)
    panel_session_manager_request_logout(
...
static gboolean panel_action_shutdown_reboot_is_disabled(void) {
  PanelSessionManager *manager;

  if (panel_lockdown_get_disable_log_out()) return TRUE;
...
/* Run Application
 */
static void panel_action_run_program(GtkWidget *widget) {
...
/* Search For Files
 */
static void panel_action_search(GtkWidget *widget) {
  GdkScreen *screen;

  screen = gtk_widget_get_screen(widget);
  panel_launch_desktop_file_with_fallback("mate-search-tool.desktop",
                                          "mate-search-tool", screen, NULL);
}

/* Force Quit
 */
static void panel_action_force_quit(GtkWidget *widget) {
  GtkWidget *dialog;
  ...
  dialog = gtk_message_dialog_new(GTK_WINDOW(widget), flags, GTK_MESSAGE_ERROR,
                                  GTK_BUTTONS_CLOSE,
...
/* Connect Server
 */
static void panel_action_connect_server(GtkWidget *widget) {
  GdkScreen *screen;
  char *command;
  GError *error;
  ...
  if (error) {
    panel_error_dialog(NULL, screen, "cannot_connect_server", TRUE,
                       _("Could not connect to server"), error->message);
    g_clear_error(&error);
  }
}
...
typedef struct {
  ...
  void (*invoke_menu)(PanelActionButton *button, const char *callback_name);
...
static PanelAction actions[PANEL_ACTION_LAST] = {
    [PANEL_ACTION_NONE] = {PANEL_ACTION_NONE, NULL, NULL, NULL, NULL, NULL,
                           NULL, NULL, NULL, NULL},
    [PANEL_ACTION_LOCK] = {PANEL_ACTION_LOCK, PANEL_ICON_LOCKSCREEN,
                           N_("Lock Screen"),
                           N_("Protect your computer from unauthorized use"),
                           "gospanel-21", "ACTION:lock:NEW",
                           panel_action_lock_screen,
                           panel_action_lock_setup_menu,
                           panel_action_lock_invoke_menu,
                           panel_action_lock_is_disabled},
    [PANEL_ACTION_LOGOUT] =
        {PANEL_ACTION_LOGOUT, PANEL_ICON_LOGOUT,
         /* when changing one of those two strings, don't forget to
          * update the ones in panel-menu-items.c (look for
          * "1" (msgctxt: "panel:showusername")) */
         N_("Log Out..."),
         N_("Log out of this session to log in as a different user"),
         "gospanel-20", "ACTION:logout:NEW", panel_action_logout, NULL, NULL,
         panel_lockdown_get_disable_log_out},
    [PANEL_ACTION_RUN] =
        {PANEL_ACTION_RUN, PANEL_ICON_RUN, N_("Run Application..."),
         N_("Run an application by typing a command or choosing from a list"),
         "gospanel-555", "ACTION:run:NEW", panel_action_run_program, NULL, NULL,
         panel_lockdown_get_disable_command_line},
    [PANEL_ACTION_SEARCH] =
        {PANEL_ACTION_SEARCH, PANEL_ICON_SEARCHTOOL, N_("Search for Files..."),
         N_("Locate documents and folders on this computer by name or content"),
         "gospanel-554", "ACTION:search:NEW", panel_action_search, NULL, NULL,
         NULL},
    [PANEL_ACTION_FORCE_QUIT] = {PANEL_ACTION_FORCE_QUIT, PANEL_ICON_FORCE_QUIT,
                                 N_("Force Quit"),
                                 N_("Force a misbehaving application to quit"),
                                 "gospanel-563", "ACTION:force-quit:NEW",
                                 panel_action_force_quit, NULL, NULL,
                                 panel_lockdown_get_disable_force_quit},
    [PANEL_ACTION_CONNECT_SERVER] =
        {PANEL_ACTION_CONNECT_SERVER, PANEL_ICON_REMOTE, /* FIXME icon */
         N_("Connect to Server..."),
         N_("Connect to a remote computer or shared disk"),
         "caja-server-connect", "ACTION:connect-server:NEW",
         panel_action_connect_server, NULL, NULL, NULL},
    [PANEL_ACTION_SHUTDOWN] = {PANEL_ACTION_SHUTDOWN, PANEL_ICON_SHUTDOWN,
                               N_("Shut Down..."), N_("Shut down the computer"),
                               "gospanel-20", "ACTION:shutdown:NEW",
...
gboolean panel_action_get_is_disabled(PanelActionButtonType type) {
  g_return_val_if_fail(type > PANEL_ACTION_NONE && type < PANEL_ACTION_LAST,
                       FALSE);

  if (actions[type].is_disabled) return actions[type].is_disabled();
...
static void panel_action_button_update_sensitivity(PanelActionButton *button) {
...
static void panel_action_button_finalize(GObject *object) {
  PanelActionButton *button = PANEL_ACTION_BUTTON(object);

  if (button->priv->settings) {
    g_signal_handlers_disconnect_by_func(
        button->priv->settings, G_CALLBACK(panel_action_button_type_changed),
        button);
  ...
  panel_lockdown_notify_remove(
      G_CALLBACK(panel_action_button_update_sensitivity), button);

  G_OBJECT_CLASS(panel_action_button_parent_class)->finalize(object);
}

static void panel_action_button_get_property(GObject *object, guint prop_id,
...
static void panel_action_button_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
  PanelActionButton *button;
  ...
  switch (prop_id) {
    case PROP_ACTION_TYPE:
      panel_action_button_set_type(button, g_value_get_enum(value));
      break;
    case PROP_DND_ENABLED:
      panel_action_button_set_dnd_enabled(button, g_value_get_boolean(value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void panel_action_button_drag_data_get(GtkWidget *widget,
  ...
  drag_data =
      g_strdup_printf("ACTION:%s:%d", get_action_type_name(button->priv->type),
...
static void panel_action_button_clicked(GtkButton *gtk_button) {
  PanelActionButton *button;
  ...
  if (panel_global_config_get_drawer_auto_close()) {
    ...
    toplevel =
        PANEL_WIDGET(gtk_widget_get_parent(GTK_WIDGET(button)))->toplevel;
...
static void panel_action_button_class_init(PanelActionButtonClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_ACTION_TYPE,
      g_param_spec_enum("action-type", "Action Type",
                        "The type of action this button implements",
                        PANEL_TYPE_ACTION_BUTTON_TYPE, PANEL_ORIENTATION_TOP,
                        G_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean(
          ...
          "Whether or not drag and drop is enabled on the widget", TRUE,
          G_PARAM_READWRITE));
}

static void panel_action_button_init(PanelActionButton *button) {
...
void panel_action_button_set_type(PanelActionButton *button,
  ...
  if (type == button->priv->type) return;
  ...
  panel_util_set_tooltip_text(GTK_WIDGET(button), _(actions[type].tooltip));
  panel_a11y_set_atk_name_desc(GTK_WIDGET(button), _(actions[type].tooltip),
                               NULL);
...
static void panel_action_button_connect_to_gsettings(
    PanelActionButton *button) {
  gchar *settings_path;
  gchar *signal_name;

  settings_path =
      g_strdup_printf(PANEL_OBJECT_PATH "%s/", button->priv->info->id);
  button->priv->settings =
      g_settings_new_with_path(PANEL_OBJECT_SCHEMA, settings_path);

  signal_name = g_strdup_printf("changed::%s", PANEL_OBJECT_ACTION_TYPE_KEY);
  g_signal_connect(button->priv->settings, signal_name,
                   G_CALLBACK(panel_action_button_type_changed), button);
...
static void panel_action_button_style_updated(PanelActionButton *button) {
  if (actions[button->priv->type].icon_name != NULL)
    button_widget_set_icon_name(BUTTON_WIDGET(button),
                                actions[button->priv->type].icon_name);
}

static void panel_action_button_load(PanelActionButtonType type,
  ...
  button->priv->info =
      mate_panel_applet_register(GTK_WIDGET(button), NULL, NULL, panel, locked,
                                 position, exactpos, PANEL_OBJECT_ACTION, id);
  if (!button->priv->info) {
    gtk_widget_destroy(GTK_WIDGET(button));
    return;
  }

  mate_panel_applet_add_callback(button->priv->info, "help", "help-browser",
                                 _("_Help"), NULL);
...
void panel_action_button_create(PanelToplevel *toplevel, int position,
  ...
  g_settings_set_enum(settings, PANEL_OBJECT_ACTION_TYPE_KEY, type);
...
void panel_action_button_load_from_gsettings(PanelWidget *panel,
  ...
  panel_action_button_load(type, panel, locked, position, exactpos, id);
}
...
void panel_action_button_invoke_menu(PanelActionButton *button,
  ...
  if (!strcmp(callback_name, "help")) {
    ...
    if (!actions[button->priv->type].help_index) return;
...
gboolean panel_action_button_load_from_drag(PanelToplevel *toplevel,
  ...
  if (strncmp(drag_string, "ACTION:", strlen("ACTION:"))) return retval;
...
void panel_action_button_set_dnd_enabled(PanelActionButton *button,
  ...
  if (!button->priv->type) return; /* wait until we know what type it is */

  enabled = enabled != FALSE;

  if (button->priv->dnd_enabled == enabled) return;
