...
static GdkFilterReturn popup_filter(GdkXEvent *gdk_xevent, GdkEvent *event,
                                    GtkWidget *popup);

static Atom wm_state_atom = None;

static GtkWidget *display_popup_window(GdkScreen *screen) {
  GtkWidget *retval;
  GtkWidget *vbox;
  GtkWidget *image;
  GtkWidget *frame;
  ...
  image =
      gtk_image_new_from_icon_name(PANEL_ICON_FORCE_QUIT, GTK_ICON_SIZE_DIALOG);
  ...
  label =
      gtk_label_new(_("Click on a window to force the application to quit. "
                      "To cancel press <ESC>."));
  ...
  gtk_window_move(GTK_WINDOW(retval), (screen_width - allocation.width) / 2,
...
static void remove_popup(GtkWidget *popup) {
  GdkWindow *root;
  GdkDisplay *display;
  GdkSeat *seat;

  root = gdk_screen_get_root_window(gtk_window_get_screen(GTK_WINDOW(popup)));
...
static gboolean wm_state_set(Display *xdisplay, Window window) {
  GdkDisplay *display;
  gulong nitems;
  gulong bytes_after;
  gulong *prop;
  ...
  result = XGetWindowProperty(xdisplay, window, wm_state_atom, 0, G_MAXLONG,
                              False, wm_state_atom, &ret_type, &ret_format,
                              &nitems, &bytes_after, (gpointer)&prop);
  ...
  if (ret_type != wm_state_atom) return FALSE;

  return TRUE;
}

static Window find_managed_window(Display *xdisplay, Window window) {
  GdkDisplay *display;
  Window root;
  Window parent;
  Window *kids = NULL;
  ...
  if (wm_state_set(xdisplay, window)) return window;

  display = gdk_display_get_default();
  gdk_x11_display_error_trap_push(display);
  result = XQueryTree(xdisplay, window, &root, &parent, &kids, &nkids);
  if (gdk_x11_display_error_trap_pop(display) || !result) return None;
...
/* From marco */
static void kill_window_question(gpointer window) {
  GtkWidget *dialog;

  dialog =
      gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE,
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), PANEL_STOCK_FORCE_QUIT,
                        GTK_RESPONSE_ACCEPT);
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(kill_window_response),
                   window);

  gtk_widget_show(dialog);
}

static void handle_button_press_event(GtkWidget *popup, Display *display,
                                      Window subwindow) {
  Window window;
  ...
  if (window != None) {
    if (!gdk_x11_window_lookup_for_display(gdk_x11_lookup_xdisplay(display),
                                           window))
  ...
  switch (xevent->type) {
    ...
    case KeyPress:
      if (xevent->xkey.keycode ==
          XKeysymToKeycode(xevent->xany.display, XK_Escape)) {
        remove_popup(popup);
        return GDK_FILTER_REMOVE;
      }
      break;
...
static void prepare_root_window(GdkSeat *seat, GdkWindow *window,
                                gpointer user_data) {
...
void panel_force_quit(GdkScreen *screen, guint time) {
  ...
  cross = gdk_cursor_new_for_display(gdk_display_get_default(), GDK_CROSS);
  caps = GDK_SEAT_CAPABILITY_POINTER | GDK_SEAT_CAPABILITY_KEYBOARD;
  display = gdk_window_get_display(root);
  seat = gdk_display_get_default_seat(display);

  status = gdk_seat_grab(seat, root, caps, FALSE, cross, NULL,
                         prepare_root_window, NULL);
