...
static void mate_panel_applet_frame_activating_free(
...
static void mate_panel_applet_frame_load(const gchar *iid, PanelWidget *panel,
...
G_DEFINE_TYPE_WITH_PRIVATE(MatePanelAppletFrame, mate_panel_applet_frame,
                           GTK_TYPE_EVENT_BOX)

static gboolean mate_panel_applet_frame_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (!frame->priv->has_handle) return FALSE;
  ...
  if (background->type == PANEL_BACK_IMAGE ||
    ...
    gtk_style_context_get(context, state, "background-image", &bg_pattern,
                          NULL);
    if (bg_pattern) {
      cairo_matrix_t ptm;

      cairo_matrix_init_translate(&ptm, frame->priv->handle_rect.x,
                                  frame->priv->handle_rect.y);
      cairo_matrix_scale(&ptm, frame->priv->handle_rect.width,
  ...
  cairo_rectangle(cr, frame->priv->handle_rect.x, frame->priv->handle_rect.y,
                  frame->priv->handle_rect.width,
                  frame->priv->handle_rect.height);
  cairo_clip(cr);
  gtk_render_handle(context, cr, 0, 0, gtk_widget_get_allocated_width(widget),
...
static void mate_panel_applet_frame_update_background_size(
    MatePanelAppletFrame *frame, GtkAllocation *old_allocation,
...
static void mate_panel_applet_frame_get_preferred_width(GtkWidget *widget,
                                                        gint *minimal_width,
                                                        gint *natural_width) {
  ...
  if (!frame->priv->has_handle) {
    GTK_WIDGET_CLASS(mate_panel_applet_frame_parent_class)
...
static void mate_panel_applet_frame_get_preferred_height(GtkWidget *widget,
                                                         gint *minimal_height,
                                                         gint *natural_height) {
  MatePanelAppletFrame *frame;
  GtkBin *bin;
...
static void mate_panel_applet_frame_size_allocate(GtkWidget *widget,
  ...
  if (!frame->priv->has_handle) {
    ...
    mate_panel_applet_frame_update_background_size(frame, &old_allocation,
                                                   allocation);
    return;
  }
  ...
  if (gtk_widget_get_realized(widget)) {
    ...
    border_width = gtk_container_get_border_width(GTK_CONTAINER(widget));
    gdk_window_move_resize(window, allocation->x + border_width,
...
static inline gboolean button_event_in_rect(GdkEventButton *event,
                                            GdkRectangle *rect) {
...
static gboolean mate_panel_applet_frame_button_changed(GtkWidget *widget,
  ...
  if (!frame->priv->has_handle) return handled;
  ...
  switch (event->button) {
    ...
    case 2:
      if (button_event_in_rect(event, &frame->priv->handle_rect)) {
        if (event->type == GDK_BUTTON_PRESS ||
          ...
          panel_widget_applet_drag_start(frame->priv->panel, GTK_WIDGET(frame),
    ...
    case 3:
      if (event->type == GDK_BUTTON_PRESS || event->type == GDK_2BUTTON_PRESS) {
        ...
        MATE_PANEL_APPLET_FRAME_GET_CLASS(frame)->popup_menu(
            frame, event->button, event->time);
...
static void mate_panel_applet_frame_finalize(GObject *object) {
  ...
  panel_lockdown_notify_remove(
      G_CALLBACK(mate_panel_applet_frame_sync_menu_state), frame);
...
static void mate_panel_applet_frame_class_init(
    MatePanelAppletFrameClass *klass) {
  ...
  widget_class->get_preferred_width =
      mate_panel_applet_frame_get_preferred_width;
  widget_class->get_preferred_height =
      mate_panel_applet_frame_get_preferred_height;
...
static void mate_panel_applet_frame_init(MatePanelAppletFrame *frame) {
...
static void mate_panel_applet_frame_init_properties(
    MatePanelAppletFrame *frame) {
...
void mate_panel_applet_frame_sync_menu_state(MatePanelAppletFrame *frame) {
  ...
  MATE_PANEL_APPLET_FRAME_GET_CLASS(frame)->sync_menu_state(
      frame, movable, removable, lockable, locked, locked_down);
}

void mate_panel_applet_frame_change_orientation(MatePanelAppletFrame *frame,
                                                PanelOrientation orientation) {
  if (orientation == frame->priv->orientation) return;

  frame->priv->orientation = orientation;
  MATE_PANEL_APPLET_FRAME_GET_CLASS(frame)->change_orientation(frame,
                                                               orientation);
}
...
void _mate_panel_applet_frame_activated(
    MatePanelAppletFrame *frame, MatePanelAppletFrameActivating *frame_act,
    GError *error) {
  AppletInfo *info;
  ...
  if (error != NULL) {
    ...
    mate_panel_applet_frame_loading_failed(frame->priv->iid, frame_act->panel,
                                           frame_act->id);
  ...
  info = mate_panel_applet_register(GTK_WIDGET(frame), GTK_WIDGET(frame), NULL,
                                    frame->priv->panel, frame_act->locked,
                                    frame_act->position, frame_act->exactpos,
...
void _mate_panel_applet_frame_update_flags(MatePanelAppletFrame *frame,
  ...
  panel_widget_set_applet_expandable(frame->priv->panel, GTK_WIDGET(frame),
                                     major, minor);
...
void _mate_panel_applet_frame_update_size_hints(MatePanelAppletFrame *frame,
  ...
  if (frame->priv->has_handle) {
    ...
    for (i = 0; i < n_elements; i++) size_hints[i] += extra_size;
  }

  /* It takes the ownership of size-hints array */
  panel_widget_set_applet_size_hints(frame->priv->panel, GTK_WIDGET(frame),
                                     size_hints, n_elements);
}

char *_mate_panel_applet_frame_get_background_string(
    MatePanelAppletFrame *frame, PanelWidget *panel, PanelBackgroundType type) {
  GtkAllocation allocation;
  int x;
  int y;
...
static void mate_panel_applet_frame_reload_response(
    GtkWidget *dialog, int response, MatePanelAppletFrame *frame) {
  AppletInfo *info;
...
void _mate_panel_applet_frame_applet_broken(MatePanelAppletFrame *frame) {
...
#ifdef HAVE_X11
  if (is_using_x11() && xstuff_is_display_dead()) return;
#endif

  if (frame->priv->iid) {
    MatePanelAppletInfo *info;

    info = (MatePanelAppletInfo *)mate_panel_applets_manager_get_applet_info(
        frame->priv->iid);
  ...
  if (applet_name)
    dialog_txt =
        g_strdup_printf(_("\"%s\" has quit unexpectedly"), applet_name);
  else
  ...
  if (panel_profile_id_lists_are_writable()) {
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("D_elete"),
                           PANEL_RESPONSE_DELETE, _("_Don't Reload"),
                           PANEL_RESPONSE_DONT_RELOAD, _("_Reload"),
                           PANEL_RESPONSE_RELOAD, NULL);
  } else {
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("_Don't Reload"),
                           PANEL_RESPONSE_DONT_RELOAD, _("_Reload"),
                           PANEL_RESPONSE_RELOAD, NULL);
  }

  gtk_dialog_set_default_response(GTK_DIALOG(dialog), PANEL_RESPONSE_RELOAD);
...
void _mate_panel_applet_frame_applet_remove(MatePanelAppletFrame *frame) {
  ...
  if (!frame->priv->applet_info) return;
...
void _mate_panel_applet_frame_applet_move(MatePanelAppletFrame *frame) {
  ...
  if (!PANEL_IS_WIDGET(parent)) return;

  panel_widget_applet_drag_start(PANEL_WIDGET(parent), widget,
                                 PW_DRAG_OFF_CENTER, GDK_CURRENT_TIME);
}

void _mate_panel_applet_frame_applet_lock(MatePanelAppletFrame *frame,
                                          gboolean locked) {
  PanelWidget *panel_widget =
      PANEL_WIDGET(gtk_widget_get_parent(GTK_WIDGET(frame)));
...
enum { LOADING_FAILED_RESPONSE_DONT_DELETE, LOADING_FAILED_RESPONSE_DELETE };
...
GdkScreen *panel_applet_frame_activating_get_screen(
    MatePanelAppletFrameActivating *frame_act) {
  return gtk_widget_get_screen(GTK_WIDGET(frame_act->panel));
}

PanelOrientation mate_panel_applet_frame_activating_get_orientation(
    MatePanelAppletFrameActivating *frame_act) {
  return panel_widget_get_applet_orientation(frame_act->panel);
}

guint32 mate_panel_applet_frame_activating_get_size(
    MatePanelAppletFrameActivating *frame_act) {
  return frame_act->panel->sz;
}

gboolean mate_panel_applet_frame_activating_get_locked(
    MatePanelAppletFrameActivating *frame_act) {
  return frame_act->locked;
}

gboolean mate_panel_applet_frame_activating_get_locked_down(
    MatePanelAppletFrameActivating *frame_act) {
...
static void mate_panel_applet_frame_loading_failed_response(GtkWidget *dialog,
                                                            guint response,
                                                            char *id) {
  gtk_widget_destroy(dialog);
...
static void mate_panel_applet_frame_loading_failed(const char *iid,
  ...
  no_reload_applets = g_slist_prepend(no_reload_applets, g_strdup(id));
  ...
  dialog = gtk_message_dialog_new(
      NULL, 0, locked_down ? GTK_MESSAGE_INFO : GTK_MESSAGE_WARNING,
      GTK_BUTTONS_NONE, "%s", problem_txt);
  g_free(problem_txt);

  if (locked_down) {
    panel_dialog_add_button(GTK_DIALOG(dialog), _("_OK"), "gtk-ok",
                            LOADING_FAILED_RESPONSE_DONT_DELETE);
  } else {
    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(dialog), _("Do you want to delete the applet "
                                      "from your configuration?"));

    gtk_dialog_add_button(GTK_DIALOG(dialog), PANEL_STOCK_DONT_DELETE,
                          LOADING_FAILED_RESPONSE_DONT_DELETE);
...
static void mate_panel_applet_frame_load(const gchar *iid, PanelWidget *panel,
  ...
  if (g_slist_find_custom(no_reload_applets, id, (GCompareFunc)strcmp)) {
    mate_panel_applet_stop_loading(id);
    return;
  }
...
void mate_panel_applet_frame_load_from_gsettings(PanelWidget *panel_widget,
  ...
  mate_panel_applet_frame_load(applet_iid, panel_widget, locked, position, TRUE,
                               id);

  g_free(applet_iid);
}

void mate_panel_applet_frame_create(PanelToplevel *toplevel, int position,
                                    const char *iid) {
  GSettings *settings;
  gchar *path;
  char *id;
