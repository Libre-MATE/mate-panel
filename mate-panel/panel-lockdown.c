...
static PanelLockdown panel_lockdown = {
    0,
};

static inline void panel_lockdown_invoke_closures(PanelLockdown *lockdown) {
  GSList *l;
...
static void locked_down_notify(GSettings *settings, gchar *key,
                               PanelLockdown *lockdown) {
  lockdown->locked_down = (g_settings_get_boolean(settings, key) != FALSE);
  panel_lockdown_invoke_closures(lockdown);
}

static void disable_command_line_notify(GSettings *settings, gchar *key,
                                        PanelLockdown *lockdown) {
...
static void disable_lock_screen_notify(GSettings *settings, gchar *key,
                                       PanelLockdown *lockdown) {
...
static void disable_log_out_notify(GSettings *settings, gchar *key,
                                   PanelLockdown *lockdown) {
  lockdown->disable_log_out = (g_settings_get_boolean(settings, key) != FALSE);
  panel_lockdown_invoke_closures(lockdown);
}

static void disable_force_quit_notify(GSettings *settings, gchar *key,
                                      PanelLockdown *lockdown) {
  lockdown->disable_force_quit =
      (g_settings_get_boolean(settings, key) != FALSE);
  panel_lockdown_invoke_closures(lockdown);
}

static void disabled_applets_notify(GSettings *settings, gchar *key,
                                    PanelLockdown *lockdown) {
...
static gboolean panel_lockdown_load_bool(PanelLockdown *lockdown,
  ...
  g_signal_connect(settings, signal_name, G_CALLBACK(notify_func), lockdown);
...
static gchar **panel_lockdown_load_disabled_applets(PanelLockdown *lockdown,
                                                    GSettings *settings) {
  gchar **retval;

  retval = g_settings_get_strv(settings, PANEL_DISABLED_APPLETS_KEY);

  g_signal_connect(settings, "changed::" PANEL_DISABLED_APPLETS_KEY,
                   G_CALLBACK(disabled_applets_notify), lockdown);
...
void panel_lockdown_init(void) {
  ...
  panel_lockdown.locked_down =
      (panel_lockdown_load_bool(&panel_lockdown, panel_lockdown.panel_settings,
                                PANEL_LOCKED_DOWN_KEY,
                                G_CALLBACK(locked_down_notify)) != FALSE);

  panel_lockdown.disable_command_line =
      (panel_lockdown_load_bool(
           &panel_lockdown, panel_lockdown.lockdown_settings,
  ...
  panel_lockdown.disabled_applets = panel_lockdown_load_disabled_applets(
      &panel_lockdown, panel_lockdown.panel_settings);
...
void panel_lockdown_finalize(void) {
  ...
  for (l = panel_lockdown.closures; l; l = l->next) g_closure_unref(l->data);
...
gboolean panel_lockdown_is_applet_disabled(const char *iid) {
  ...
  if (panel_lockdown.disabled_applets)
    for (i = 0; panel_lockdown.disabled_applets[i]; i++)
      if (!strcmp(panel_lockdown.disabled_applets[i], iid)) return TRUE;

  return FALSE;
}

static GClosure *panel_lockdown_notify_find(GSList *closures,
  ...
  for (l = closures; l; l = l->next) {
    ...
    if (closure->data == user_data && cclosure->callback == callback_func)
      return closure;
  }
...
static void marshal_user_data(GClosure *closure, GValue *return_value,
...
void panel_lockdown_notify_add(GCallback callback_func, gpointer user_data) {
  ...
  g_assert(panel_lockdown_notify_find(panel_lockdown.closures, callback_func,
                                      user_data) == NULL);

  closure = g_cclosure_new(callback_func, user_data, NULL);
  g_closure_set_marshal(closure, marshal_user_data);

  panel_lockdown.closures = g_slist_append(panel_lockdown.closures, closure);
}

void panel_lockdown_notify_remove(GCallback callback_func, gpointer user_data) {
  GClosure *closure;

  closure = panel_lockdown_notify_find(panel_lockdown.closures, callback_func,
                                       user_data);

  g_assert(closure != NULL);

  panel_lockdown.closures = g_slist_remove(panel_lockdown.closures, closure);
