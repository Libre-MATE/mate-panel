...
static PanelAddtoItemInfo special_addto_items[] = {
    ...
    {PANEL_ADDTO_LAUNCHER_NEW, N_("Custom Application Launcher"),
     N_("Create a new launcher"), PANEL_ICON_LAUNCHER, PANEL_ACTION_NONE, NULL,
     NULL, NULL, "LAUNCHER:ASK", TRUE, TRUE},

    {PANEL_ADDTO_LAUNCHER_MENU, N_("Application Launcher..."),
     N_("Copy a launcher from the applications menu"), PANEL_ICON_LAUNCHER,
     PANEL_ACTION_NONE, NULL, NULL, NULL, "LAUNCHER:MENU", TRUE, TRUE}

};

static PanelAddtoItemInfo internal_addto_items[] = {

    {PANEL_ADDTO_MENU, N_("Compact Menu"), N_("A compact menu"),
     PANEL_ICON_MAIN_MENU, PANEL_ACTION_NONE, NULL, NULL, NULL, "MENU:MAIN",
     TRUE, TRUE},

    {PANEL_ADDTO_MENUBAR, N_("Classic Menu"),
     N_("The classic Applications, Places and System menu bar"),
     PANEL_ICON_MAIN_MENU, PANEL_ACTION_NONE, NULL, NULL, NULL, "MENUBAR:NEW",
     TRUE, TRUE},

    {PANEL_ADDTO_SEPARATOR, N_("Separator"),
     N_("A separator to organize the panel items"), PANEL_ICON_SEPARATOR,
     PANEL_ACTION_NONE, NULL, NULL, NULL, "SEPARATOR:NEW", TRUE, TRUE},

    {PANEL_ADDTO_DRAWER, N_("Drawer"),
     N_("A pop out drawer to store other items in"), PANEL_ICON_DRAWER,
     PANEL_ACTION_NONE, NULL, NULL, NULL, "DRAWER:NEW", TRUE, TRUE}};
...
enum { PANEL_ADDTO_RESPONSE_BACK, PANEL_ADDTO_RESPONSE_ADD };
...
static gboolean panel_addto_filter_func(GtkTreeModel *model, GtkTreeIter *iter,
                                        gpointer data);

static int panel_addto_applet_info_sort_func(PanelAddtoItemInfo *a,
                                             PanelAddtoItemInfo *b) {
  return g_utf8_collate(a->name, b->name);
}

static GSList *panel_addto_prepend_internal_applets(GSList *list) {
  ...
  for (i = 0; i < G_N_ELEMENTS(internal_addto_items); i++) {
    if (!translated) {
      ...
      internal_addto_items[i].description =
          _(internal_addto_items[i].description);
    }
  ...
  for (p = PANEL_ACTION_LOCK; p < PANEL_ACTION_LAST; p++) {
    ...
    if (panel_action_get_is_disabled(p)) continue;
...
static char *panel_addto_make_text(const char *name, const char *desc) {
  ...
  if (!PANEL_GLIB_STR_EMPTY(desc)) {
    ...
    result =
        g_markup_printf_escaped("<span weight=\"bold\">%s</span>", real_name);
  }

  return result;
}

static void panel_addto_drag_data_get_cb(GtkWidget *widget,
...
static void panel_addto_drag_begin_cb(GtkWidget *widget,
  ...
  gtk_tree_model_get(child_model, &iter, COLUMN_ICON_NAME, &icon_name, -1);

  gtk_drag_set_icon_name(context, icon_name, 0, 0);
  g_free(icon_name);
}

static void panel_addto_setup_drag(GtkTreeView *tree_view,
                                   const GtkTargetEntry *target,
                                   const char *text) {
  if (!text || panel_lockdown_get_locked_down()) return;
  ...
  g_signal_connect_data(tree_view, "drag-data-get",
                        ...
                        g_strdup(text), (GClosureNotify)G_CALLBACK(g_free),
                        0 /* connect_flags */);
  g_signal_connect_after(tree_view, "drag-begin",
                         G_CALLBACK(panel_addto_drag_begin_cb), NULL);
}

static void panel_addto_setup_launcher_drag(GtkTreeView *tree_view,
                                            const char *path) {
...
static void panel_addto_setup_applet_drag(GtkTreeView *tree_view,
                                          const char *iid) {
...
static void panel_addto_setup_internal_applet_drag(GtkTreeView *tree_view,
...
static GSList *panel_addto_query_applets(GSList *list) {
  GList *applet_list, *l;
...
static void panel_addto_append_item(PanelAddtoDialog *dialog,
                                    GtkListStore *model,
                                    PanelAddtoItemInfo *applet) {
  GtkTreeIter iter;
  ...
  if (applet == NULL) {
    ...
    gtk_list_store_set(model, &iter, COLUMN_ICON_NAME, NULL, COLUMN_TEXT, NULL,
                       COLUMN_DATA, NULL, COLUMN_SEARCH, NULL, COLUMN_ENABLED,
                       TRUE, -1);
  } else {
    gtk_list_store_append(model, &iter);
    ...
    gtk_list_store_set(model, &iter, COLUMN_ICON_NAME, applet->icon,
                       COLUMN_TEXT, text, COLUMN_DATA, applet, COLUMN_SEARCH,
                       applet->name, COLUMN_ENABLED, applet->enabled, -1);

    g_free(text);
  }
}

static void panel_addto_append_special_applets(PanelAddtoDialog *dialog,
                                               GtkListStore *model) {
  static gboolean translated = FALSE;
  gsize i;
  ...
  for (i = 0; i < G_N_ELEMENTS(special_addto_items); i++) {
    if (!translated) {
      ...
      special_addto_items[i].description =
          _(special_addto_items[i].description);
    }
...
static void panel_addto_make_applet_model(PanelAddtoDialog *dialog) {
  GtkListStore *model;
  GSList *l;

  if (dialog->filter_applet_model != NULL) return;
  ...
  dialog->applet_list = g_slist_sort(
      dialog->applet_list, (GCompareFunc)panel_addto_applet_info_sort_func);

  model = gtk_list_store_new(NUMBER_COLUMNS, G_TYPE_STRING, G_TYPE_STRING,
                             G_TYPE_POINTER, G_TYPE_STRING, G_TYPE_BOOLEAN);

  if (panel_profile_id_lists_are_writable()) {
    panel_addto_append_special_applets(dialog, model);
    if (dialog->applet_list) panel_addto_append_item(dialog, model, NULL);
  }
  ...
  dialog->filter_applet_model =
      gtk_tree_model_filter_new(GTK_TREE_MODEL(dialog->applet_model), NULL);
  gtk_tree_model_filter_set_visible_func(
      GTK_TREE_MODEL_FILTER(dialog->filter_applet_model),
      panel_addto_filter_func, dialog, NULL);
}
...
static void panel_addto_prepend_directory(GSList **parent_list,
                                          MateMenuTreeDirectory *directory,
                                          const char *filename) {
  ...
  data->item_info.description =
      g_strdup(matemenu_tree_directory_get_comment(directory));
  data->item_info.icon =
      gicon ? g_icon_to_string(gicon) : g_strdup(PANEL_ICON_UNKNOWN);
  data->item_info.menu_filename = g_strdup(filename);
  data->item_info.menu_path =
      matemenu_tree_directory_make_path(directory, NULL);
...
static void panel_addto_prepend_entry(GSList **parent_list,
  ...
  data->item_info.name =
      g_strdup(g_app_info_get_display_name(G_APP_INFO(ginfo)));
  data->item_info.description =
      g_strdup(g_app_info_get_description(G_APP_INFO(ginfo)));
  data->item_info.icon =
      gicon ? g_icon_to_string(gicon) : g_strdup(PANEL_ICON_UNKNOWN);
  data->item_info.launcher_path =
      g_strdup(matemenu_tree_entry_get_desktop_file_path(entry));
  data->item_info.enabled = TRUE;
  data->item_info.static_data = FALSE;
...
static void panel_addto_prepend_alias(GSList **parent_list,
...
static void panel_addto_populate_application_model(GtkTreeStore *store,
                                                   GtkTreeIter *parent,
                                                   GSList *app_list) {
  GtkTreeIter iter;
  GSList *app;
  ...
  for (app = app_list; app != NULL; app = app->next) {
    ...
    char *text = panel_addto_make_text(data->item_info.name,
                                       data->item_info.description);
    gtk_tree_store_set(store, &iter, COLUMN_ICON_NAME, data->item_info.icon,
                       COLUMN_TEXT, text, COLUMN_DATA, &(data->item_info),
                       COLUMN_SEARCH, data->item_info.name, COLUMN_ENABLED,
                       data->item_info.enabled, -1);

    g_free(text);

    if (data->children != NULL)
      panel_addto_populate_application_model(store, &iter, data->children);
  }
}
...
static void panel_addto_make_application_model(PanelAddtoDialog *dialog) {
  ...
  if (dialog->filter_application_model != NULL) return;

  store = gtk_tree_store_new(NUMBER_COLUMNS, G_TYPE_STRING, G_TYPE_STRING,
                             G_TYPE_POINTER, G_TYPE_STRING, G_TYPE_BOOLEAN);
  ...
  tree = matemenu_tree_new("mate-settings.menu",
                           MATEMENU_TREE_FLAGS_SORT_DISPLAY_NAME);
  if (!matemenu_tree_load_sync(tree, &error)) {
  ...
  if ((root = matemenu_tree_get_root_directory(tree))) {
    ...
    gtk_tree_store_set(store, &iter, COLUMN_ICON_NAME, NULL, COLUMN_TEXT, NULL,
                       COLUMN_DATA, NULL, COLUMN_SEARCH, NULL, COLUMN_ENABLED,
                       TRUE, -1);

    panel_addto_make_application_list(&dialog->settings_list, root,
                                      "mate-settings.menu");
  ...
  dialog->filter_application_model = gtk_tree_model_filter_new(
      GTK_TREE_MODEL(dialog->application_model), NULL);
  gtk_tree_model_filter_set_visible_func(
      GTK_TREE_MODEL_FILTER(dialog->filter_application_model),
      panel_addto_filter_func, dialog, NULL);
}

static void panel_addto_add_item(PanelAddtoDialog *dialog,
                                 PanelAddtoItemInfo *item_info) {
  ...
  switch (item_info->type) {
    ...
    case PANEL_ADDTO_LAUNCHER_NEW:
      ask_about_launcher(NULL, dialog->panel_widget, dialog->insertion_position,
                         FALSE);
      break;
    case PANEL_ADDTO_MENU:
      panel_menu_button_create(dialog->panel_widget->toplevel,
                               dialog->insertion_position,
                               item_info->menu_filename, item_info->menu_path,
                               item_info->menu_path != NULL, item_info->name);
      break;
    ...
    case PANEL_ADDTO_DRAWER:
      panel_drawer_create(dialog->panel_widget->toplevel,
                          dialog->insertion_position, NULL, FALSE, NULL);
      break;
  }
}

static void panel_addto_dialog_response(GtkWidget *widget_dialog,
                                        guint response_id,
  ...
  switch (response_id) {
    ...
    case PANEL_ADDTO_RESPONSE_ADD:
      ...
      if (gtk_tree_selection_get_selected(selection, &filter_model,
        ...
        if (data != NULL) panel_addto_add_item(dialog, data);
      }
      break;
...
static void panel_addto_dialog_destroy(GtkWidget *widget_dialog,
                                       PanelAddtoDialog *dialog) {
  panel_toplevel_pop_autohide_disabler(
      PANEL_TOPLEVEL(dialog->panel_widget->toplevel));
  g_object_set_qdata(G_OBJECT(dialog->panel_widget->toplevel),
                     panel_addto_dialog_quark, NULL);
}
...
static void panel_addto_present_applications(PanelAddtoDialog *dialog) {
  ...
  gtk_window_set_focus(GTK_WINDOW(dialog->addto_dialog), dialog->search_entry);
...
static void panel_addto_dialog_free_item_info(PanelAddtoItemInfo *item_info) {
  if (item_info == NULL || item_info->static_data) return;
...
static void panel_addto_dialog_free_application_list(GSList *application_list) {
  PanelAddtoAppList *data;
  GSList *app;

  if (application_list == NULL) return;
...
static void panel_addto_name_notify(GSettings *settings, gchar *key,
                                    PanelAddtoDialog *dialog);

static void panel_addto_dialog_free(PanelAddtoDialog *dialog) {
  GSList *item;
  ...
  if (dialog->addto_dialog) gtk_widget_destroy(dialog->addto_dialog);
...
static void panel_addto_name_change(PanelAddtoDialog *dialog,
                                    const char *name) {
  char *title;
  char *label;
  ...
  if (!PANEL_GLIB_STR_EMPTY(name))
    label = g_strdup_printf(_("Find an _item to add to \"%s\":"), name);
...
static gboolean panel_addto_filter_func(GtkTreeModel *model, GtkTreeIter *iter,
  ...
  if (!dialog->search_text || !dialog->search_text[0]) return TRUE;
  ...
  return (panel_g_utf8_strstrcase(data->name, dialog->search_text) != NULL ||
          panel_g_utf8_strstrcase(data->description, dialog->search_text) !=
              NULL);
}

static void panel_addto_search_entry_changed(GtkWidget *entry,
                                             PanelAddtoDialog *dialog) {
  GtkTreeModel *model;
  char *new_text;
  GtkTreeIter iter;
  ...
  if (gtk_tree_model_get_iter(model, &iter, path)) {
    ...
    gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(dialog->tree_view), path, NULL,
                                 FALSE, 0, 0);
    selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(dialog->tree_view));
...
static void panel_addto_search_entry_activated(GtkWidget *entry,
                                               PanelAddtoDialog *dialog) {
  gtk_dialog_response(GTK_DIALOG(dialog->addto_dialog),
                      PANEL_ADDTO_RESPONSE_ADD);
}

static gboolean panel_addto_selection_func(GtkTreeSelection *selection,
                                           GtkTreeModel *model,
                                           GtkTreePath *path,
...
static void panel_addto_selection_changed(GtkTreeSelection *selection,
                                          PanelAddtoDialog *dialog) {
  GtkTreeModel *filter_model;
  GtkTreeModel *child_model;
  GtkTreeIter iter;
  ...
  if (panel_profile_id_lists_are_writable()) {
    switch (data->type) {
      ...
      case PANEL_ADDTO_MENU: {
        ...
        if (data->iid == NULL) {
          iid = g_strdup_printf("MENU:%s/%s", data->menu_filename,
                                data->menu_path);
        } else {
...
static void panel_addto_selection_activated(GtkTreeView *view,
                                            GtkTreePath *path,
...
static gboolean panel_addto_separator_func(GtkTreeModel *model,
...
static PanelAddtoDialog *panel_addto_dialog_new(PanelWidget *panel_widget) {
  PanelAddtoDialog *dialog;
  ...
  g_object_set_qdata_full(G_OBJECT(panel_widget->toplevel),
                          panel_addto_dialog_quark, dialog,
  ...
  g_signal_connect(dialog->panel_widget->toplevel->settings,
                   ...
                   G_CALLBACK(panel_addto_name_notify), dialog);

  dialog->addto_dialog = gtk_dialog_new();

  panel_dialog_add_button(GTK_DIALOG(dialog->addto_dialog), _("_Help"),
                          "help-browser", GTK_RESPONSE_HELP);

  dialog->back_button =
      panel_dialog_add_button(GTK_DIALOG(dialog->addto_dialog), _("_Back"),
                              "go-previous", PANEL_ADDTO_RESPONSE_BACK);

  dialog->add_button =
      panel_dialog_add_button(GTK_DIALOG(dialog->addto_dialog), _("_Add"),
                              "list-add", PANEL_ADDTO_RESPONSE_ADD);

  panel_dialog_add_button(GTK_DIALOG(dialog->addto_dialog), _("_Close"),
  ...
  gtk_box_pack_start(GTK_BOX(find_hbox), dialog->label, FALSE, FALSE, 0);
  ...
  gtk_box_pack_end(GTK_BOX(find_hbox), dialog->search_entry, TRUE, TRUE, 0);

  gtk_label_set_mnemonic_widget(GTK_LABEL(dialog->label), dialog->search_entry);
  ...
  renderer = g_object_new(GTK_TYPE_CELL_RENDERER_PIXBUF, "xpad", 4, "ypad", 4,
                          "stock-size", panel_add_to_icon_get_size(), NULL);

  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(dialog->tree_view), -1, NULL, renderer, "icon_name",
      COLUMN_ICON_NAME, "sensitive", COLUMN_ENABLED, NULL);
  renderer = gtk_cell_renderer_text_new();
  g_object_set(renderer, "ellipsize", PANGO_ELLIPSIZE_END, NULL);
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(dialog->tree_view), -1, NULL, renderer, "markup",
      COLUMN_TEXT, "sensitive", COLUMN_ENABLED, NULL);
  ...
  gtk_tree_view_set_row_separator_func(GTK_TREE_VIEW(dialog->tree_view),
                                       ...
                                       GINT_TO_POINTER(COLUMN_TEXT), NULL);

  selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(dialog->tree_view));
  gtk_tree_selection_set_mode(selection, GTK_SELECTION_SINGLE);

  column =
      gtk_tree_view_get_column(GTK_TREE_VIEW(dialog->tree_view), COLUMN_TEXT);
  gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_FIXED);

  gtk_tree_selection_set_select_function(selection, panel_addto_selection_func,
                                         NULL, NULL);

  g_signal_connect(selection, "changed",
                   G_CALLBACK(panel_addto_selection_changed), dialog);

  g_signal_connect(dialog->tree_view, "row-activated",
                   G_CALLBACK(panel_addto_selection_activated), dialog);
  ...
  panel_widget_register_open_dialog(panel_widget, dialog->addto_dialog);

  panel_addto_name_change(
      dialog, panel_toplevel_get_name(dialog->panel_widget->toplevel));
...
void panel_addto_present(GtkMenuItem *item, PanelWidget *panel_widget) {
  ...
  if (!panel_addto_dialog_quark)
    panel_addto_dialog_quark = g_quark_from_static_string("panel-addto-dialog");

  dialog = g_object_get_qdata(G_OBJECT(toplevel), panel_addto_dialog_quark);
  ...
  gtk_window_set_default_size(GTK_WINDOW(dialog->addto_dialog), height * 8 / 7,
                              height);
