...
G_DEFINE_TYPE_WITH_CODE(MatePanelAppletsManagerDBus,
                        ...
                        G_ADD_PRIVATE(MatePanelAppletsManagerDBus)
                            g_io_extension_point_implement(
                                ...
                                g_define_type_id, "dbus", 10))

typedef gint (*ActivateAppletFunc)(void);
typedef GtkWidget *(*GetAppletWidgetFunc)(const gchar *factory_id, guint uid);
...
static void mate_panel_applet_factory_info_free(
...
static MatePanelAppletInfo *_mate_panel_applets_manager_get_applet_info(
    GKeyFile *applet_file, const gchar *group, const gchar *factory_id) {
  ...
  name = g_key_file_get_locale_string(applet_file, group, "Name", NULL, NULL);
  comment = g_key_file_get_locale_string(applet_file, group, "Description",
                                         NULL, NULL);
  icon = g_key_file_get_string(applet_file, group, "Icon", NULL);
  /* MateComponent compatibility */
  old_ids = g_key_file_get_string_list(applet_file, group, "MateComponentId",
                                       NULL, NULL);

  supported_platforms =
      g_key_file_get_string_list(applet_file, group, "Platforms", NULL, NULL);
  if (supported_platforms == NULL) {
    ...
    for (i = 0; i < len; i++) {
      if (g_strcmp0(supported_platforms[i], "X11") == 0) {
        ...
        g_warning(
            "Unknown platform in %s applet: %s. "
  ...
  info = mate_panel_applet_info_new(iid, name, comment, icon,
                                    (const char **)old_ids, x11_supported,
                                    wayland_supported);
...
mate_panel_applets_manager_get_applet_factory_info_from_file(
  ...
  if (!g_key_file_load_from_file(applet_file, filename, G_KEY_FILE_NONE,
                                 &error)) {
    g_warning("Error opening panel applet file %s: %s", filename,
              error->message);
    g_error_free(error);
  ...
  info->id = g_key_file_get_string(applet_file, MATE_PANEL_APPLET_FACTORY_GROUP,
                                   "Id", NULL);
  if (!info->id) {
  ...
  info->in_process = g_key_file_get_boolean(
      applet_file, MATE_PANEL_APPLET_FACTORY_GROUP, "InProcess", NULL);
  if (info->in_process) {
    info->location = g_key_file_get_string(
        applet_file, MATE_PANEL_APPLET_FACTORY_GROUP, "Location", NULL);
    if (!info->location) {
      g_warning(
          "Bad panel applet file %s: In-process applet without 'Location'",
          filename);
      mate_panel_applet_factory_info_free(info);
      g_key_file_free(applet_file);
  ...
  for (i = 0; i < n_groups; i++) {
    ...
    if (g_strcmp0(groups[i], MATE_PANEL_APPLET_FACTORY_GROUP) == 0) continue;

    ainfo = _mate_panel_applets_manager_get_applet_info(applet_file, groups[i],
                                                        info->id);
...
static GSList *mate_panel_applets_manager_get_applets_dirs(void) {
...
static void applets_directory_changed(GFileMonitor *monitor, GFile *file,
                                      GFile *other_file,
                                      GFileMonitorEvent event_type,
                                      gpointer user_data) {
  MatePanelAppletsManagerDBus *manager =
      MATE_PANEL_APPLETS_MANAGER_DBUS(user_data);
  ...
  switch (event_type) {
    ...
    case G_FILE_MONITOR_EVENT_CREATED: {
      ...
      info = mate_panel_applets_manager_get_applet_factory_info_from_file(
          filename);
      g_free(filename);
      ...
      if (!old_info) {
        ...
        g_hash_table_insert(manager->priv->applet_factories, g_strdup(info->id),
                            info);
        return;
      }
...
static void mate_panel_applets_manager_dbus_load_applet_infos(
    MatePanelAppletsManagerDBus *manager) {
  GSList *dirs, *d;
  GDir *dir;
  const gchar *dirent;
  ...
  for (d = dirs; d; d = g_slist_next(d)) {
    ...
    if (monitor) {
      g_signal_connect(monitor, "changed",
                       G_CALLBACK(applets_directory_changed), manager);
      manager->priv->monitors =
          g_list_prepend(manager->priv->monitors, monitor);
    }
    ...
    while ((dirent = g_dir_read_name(dir))) {
      ...
      if (!g_str_has_suffix(dirent, MATE_PANEL_APPLETS_EXTENSION)) continue;
...
static GList *mate_panel_applets_manager_dbus_get_applets(
    MatePanelAppletsManager *manager) {
  MatePanelAppletsManagerDBus *dbus_manager =
      MATE_PANEL_APPLETS_MANAGER_DBUS(manager);
...
static MatePanelAppletFactoryInfo *get_applet_factory_info(
    MatePanelAppletsManager *manager, const gchar *iid) {
  MatePanelAppletsManagerDBus *dbus_manager =
      MATE_PANEL_APPLETS_MANAGER_DBUS(manager);
...
static gboolean mate_panel_applets_manager_dbus_factory_activate(
    MatePanelAppletsManager *manager, const gchar *iid) {
  MatePanelAppletFactoryInfo *info;
  ...
  applet_info = MATE_PANEL_APPLETS_MANAGER_GET_CLASS(manager)->get_applet_info(
      manager, iid);
  ...
  if (!info->in_process) return TRUE;
  ...
  if (!g_module_symbol(info->module, "_mate_panel_applet_shlib_factory",
                       (gpointer *)&activate_applet)) {
    /* FIXME: use a GError? */
    g_warning("Failed to load applet %s: %s\n", iid, g_module_error());
  ...
  if (!g_module_symbol(info->module, "mate_panel_applet_get_applet_widget",
                       (gpointer *)&get_applet_widget)) {
    /* FIXME: use a GError? */
...
static gboolean mate_panel_applets_manager_dbus_factory_deactivate(
    MatePanelAppletsManager *manager, const gchar *iid) {
  MatePanelAppletFactoryInfo *info;
...
static MatePanelAppletInfo *mate_panel_applets_manager_dbus_get_applet_info(
    MatePanelAppletsManager *manager, const gchar *iid) {
  MatePanelAppletFactoryInfo *info;
  GList *l;
...
mate_panel_applets_manager_dbus_get_applet_info_from_old_id(
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    for (l = info->applet_list; l; l = g_list_next(l)) {
      ...
      if (old_ids == NULL) continue;

      while (old_ids[i]) {
        if (g_strcmp0(old_ids[i], iid) == 0) return ainfo;
        i++;
      }
    }
  }
...
static gboolean mate_panel_applets_manager_dbus_load_applet(
    MatePanelAppletsManager *manager, const gchar *iid,
    MatePanelAppletFrameActivating *frame_act) {
  return mate_panel_applet_frame_dbus_load(iid, frame_act);
}

static GtkWidget *mate_panel_applets_manager_dbus_get_applet_widget(
    MatePanelAppletsManager *manager, const gchar *iid, guint uid) {
  MatePanelAppletFactoryInfo *info;
...
static void mate_panel_applets_manager_dbus_finalize(GObject *object) {
  MatePanelAppletsManagerDBus *manager =
      MATE_PANEL_APPLETS_MANAGER_DBUS(object);
  ...
  G_OBJECT_CLASS(mate_panel_applets_manager_dbus_parent_class)
      ->finalize(object);
}

static void mate_panel_applets_manager_dbus_init(
    MatePanelAppletsManagerDBus *manager) {
  manager->priv = mate_panel_applets_manager_dbus_get_instance_private(manager);

  manager->priv->applet_factories = g_hash_table_new_full(
...
static void mate_panel_applets_manager_dbus_class_init(
    MatePanelAppletsManagerDBusClass *class) {
  GObjectClass *gobject_class = G_OBJECT_CLASS(class);
  MatePanelAppletsManagerClass *manager_class =
      MATE_PANEL_APPLETS_MANAGER_CLASS(class);
  ...
  manager_class->factory_activate =
      mate_panel_applets_manager_dbus_factory_activate;
  manager_class->factory_deactivate =
      mate_panel_applets_manager_dbus_factory_deactivate;
  manager_class->get_applet_info =
      mate_panel_applets_manager_dbus_get_applet_info;
  manager_class->get_applet_info_from_old_id =
      mate_panel_applets_manager_dbus_get_applet_info_from_old_id;
  manager_class->load_applet = mate_panel_applets_manager_dbus_load_applet;
  manager_class->get_applet_widget =
      mate_panel_applets_manager_dbus_get_applet_widget;
}
