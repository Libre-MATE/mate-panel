...
static guint get_mate_panel_applet_orient(PanelOrientation orientation) {
...
static void mate_panel_applet_frame_dbus_update_flags(
    MatePanelAppletFrame *frame, GVariant *value) {
  guint32 flags;
  gboolean major;
  gboolean minor;
  gboolean has_handle;
...
static void mate_panel_applet_frame_dbus_get_flags_cb(
    MatePanelAppletContainer *container, GAsyncResult *res,
...
static void mate_panel_applet_frame_dbus_get_size_hints_cb(
    MatePanelAppletContainer *container, GAsyncResult *res,
    MatePanelAppletFrame *frame) {
  GVariant *value;
  const gint *sz;
...
static void mate_panel_applet_frame_dbus_init_properties(
    MatePanelAppletFrame *frame) {
  MatePanelAppletFrameDBus *dbus_frame = MATE_PANEL_APPLET_FRAME_DBUS(frame);

  mate_panel_applet_container_child_get(
      dbus_frame->priv->container, "flags", NULL,
      (GAsyncReadyCallback)mate_panel_applet_frame_dbus_get_flags_cb, frame);
  mate_panel_applet_container_child_get(
      dbus_frame->priv->container, "size-hints", NULL,
      (GAsyncReadyCallback)mate_panel_applet_frame_dbus_get_size_hints_cb,
      frame);
}

static void mate_panel_applet_frame_dbus_sync_menu_state(
    MatePanelAppletFrame *frame, gboolean movable, gboolean removable,
    gboolean lockable, gboolean locked, gboolean locked_down) {
  MatePanelAppletFrameDBus *dbus_frame = MATE_PANEL_APPLET_FRAME_DBUS(frame);

  mate_panel_applet_container_child_set(
      dbus_frame->priv->container, "locked",
      g_variant_new_boolean(lockable && locked), NULL, NULL, NULL);
  mate_panel_applet_container_child_set(
      dbus_frame->priv->container, "locked-down",
      g_variant_new_boolean(locked_down), NULL, NULL, NULL);
}

static void mate_panel_applet_frame_dbus_popup_menu(MatePanelAppletFrame *frame,
                                                    guint button,
                                                    guint32 timestamp) {
  MatePanelAppletFrameDBus *dbus_frame = MATE_PANEL_APPLET_FRAME_DBUS(frame);

  mate_panel_applet_container_child_popup_menu(
      dbus_frame->priv->container, button, timestamp, NULL, NULL, NULL);
}

static void change_orientation_cb(MatePanelAppletContainer *container,
                                  GAsyncResult *res,
                                  MatePanelAppletFrame *frame) {
  GError *error = NULL;
...
static void mate_panel_applet_frame_dbus_change_orientation(
    MatePanelAppletFrame *frame, PanelOrientation orientation) {
  MatePanelAppletFrameDBus *dbus_frame = MATE_PANEL_APPLET_FRAME_DBUS(frame);

  mate_panel_applet_container_child_set(
      dbus_frame->priv->container, "orient",
      g_variant_new_uint32(get_mate_panel_applet_orient(orientation)), NULL,
      (GAsyncReadyCallback)change_orientation_cb, frame);
}

static void mate_panel_applet_frame_dbus_change_size(
    MatePanelAppletFrame *frame, guint size) {
  MatePanelAppletFrameDBus *dbus_frame = MATE_PANEL_APPLET_FRAME_DBUS(frame);

  mate_panel_applet_container_child_set(dbus_frame->priv->container, "size",
                                        g_variant_new_uint32(size), NULL, NULL,
                                        NULL);
}

static void container_child_background_set(GObject *source_object,
                                           GAsyncResult *res,
                                           gpointer user_data) {
  MatePanelAppletContainer *container =
      MATE_PANEL_APPLET_CONTAINER(source_object);
...
static void mate_panel_applet_frame_dbus_change_background(
    MatePanelAppletFrame *frame, PanelBackgroundType type) {
  MatePanelAppletFrameDBus *dbus_frame = MATE_PANEL_APPLET_FRAME_DBUS(frame);
  ...
  if (bg_str != NULL) {
    if (priv->bg_operation)
      mate_panel_applet_container_cancel_operation(priv->container,
                                                   priv->bg_operation);

    priv->bg_operation = mate_panel_applet_container_child_set(
        priv->container, "background", g_variant_new_string(bg_str), NULL,
        container_child_background_set, dbus_frame);

    g_free(bg_str);
  }
}

static void mate_panel_applet_frame_dbus_flags_changed(
    MatePanelAppletContainer *container, const gchar *prop_name,
    GVariant *value, MatePanelAppletFrame *frame) {
  mate_panel_applet_frame_dbus_update_flags(frame, value);
}

static void mate_panel_applet_frame_dbus_size_hints_changed(
    MatePanelAppletContainer *container, const gchar *prop_name,
    GVariant *value, MatePanelAppletFrame *frame) {
  const gint *sz;
  gint *size_hints = NULL;
...
static void mate_panel_applet_frame_dbus_applet_broken(
    MatePanelAppletContainer *container, MatePanelAppletFrame *frame) {
  _mate_panel_applet_frame_applet_broken(frame);
}

static void mate_panel_applet_frame_dbus_applet_remove(
    MatePanelAppletContainer *container, MatePanelAppletFrame *frame) {
  _mate_panel_applet_frame_applet_remove(frame);
}

static void mate_panel_applet_frame_dbus_applet_move(
    MatePanelAppletContainer *container, MatePanelAppletFrame *frame) {
  _mate_panel_applet_frame_applet_move(frame);
}

static void mate_panel_applet_frame_dbus_applet_lock(
    MatePanelAppletContainer *container, gboolean locked,
    MatePanelAppletFrame *frame) {
  _mate_panel_applet_frame_applet_lock(frame, locked);
}

static void mate_panel_applet_frame_dbus_finalize(GObject *object) {
...
static void mate_panel_applet_frame_dbus_init(MatePanelAppletFrameDBus *frame) {
  GtkWidget *container;
  ...
  g_signal_connect(container, "applet-move",
                   G_CALLBACK(mate_panel_applet_frame_dbus_applet_move), frame);
  g_signal_connect(container, "applet-lock",
                   G_CALLBACK(mate_panel_applet_frame_dbus_applet_lock), frame);
}

static void mate_panel_applet_frame_dbus_class_init(
    MatePanelAppletFrameDBusClass *class) {
  ...
  frame_class->change_orientation =
      mate_panel_applet_frame_dbus_change_orientation;
  frame_class->change_size = mate_panel_applet_frame_dbus_change_size;
  frame_class->change_background =
      mate_panel_applet_frame_dbus_change_background;

  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(class);
  gtk_widget_class_set_css_name(widget_class, "MatePanelAppletFrameDBus");
}

static void mate_panel_applet_frame_dbus_activated(
    MatePanelAppletContainer *container, GAsyncResult *res,
    MatePanelAppletFrame *frame) {
  ...
  frame_act =
      g_object_get_data(G_OBJECT(frame), "mate-panel-applet-frame-activating");
  g_object_set_data(G_OBJECT(frame), "mate-panel-applet-frame-activating",
                    NULL);
...
gboolean mate_panel_applet_frame_dbus_load(
  ...
  if (!mate_panel_applets_manager_factory_activate(iid)) return FALSE;
  ...
  orient = get_mate_panel_applet_orient(
      mate_panel_applet_frame_activating_get_orientation(frame_act));
  ...
  g_variant_builder_add(&builder, "{sv}", "prefs-path",
                        g_variant_new_string(conf_path));
  g_variant_builder_add(&builder, "{sv}", "orient",
                        g_variant_new_uint32(orient));
  g_variant_builder_add(
      &builder, "{sv}", "size",
      g_variant_new_uint32(
          mate_panel_applet_frame_activating_get_size(frame_act)));
  g_variant_builder_add(
      &builder, "{sv}", "locked",
      g_variant_new_boolean(
          mate_panel_applet_frame_activating_get_locked(frame_act)));
  g_variant_builder_add(
      &builder, "{sv}", "locked-down",
      g_variant_new_boolean(
          mate_panel_applet_frame_activating_get_locked_down(frame_act)));
  ...
  */
  ...
  mate_panel_applet_container_add(
      dbus_frame->priv->container, screen, iid, NULL,
      (GAsyncReadyCallback)mate_panel_applet_frame_dbus_activated, frame,
