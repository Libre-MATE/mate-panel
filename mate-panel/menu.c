...
static GtkWidget *populate_menu_from_directory(
    GtkWidget *menu, MateMenuTreeDirectory *directory);
...
static inline gboolean desktop_is_home_dir(void) {
  ...
  if (mate_gsettings_schema_exists(CAJA_PREFS_SCHEMA)) {
    ...
    retval =
        g_settings_get_boolean(settings, CAJA_PREFS_DESKTOP_IS_HOME_DIR_KEY);
...
static void activate_app_def(GtkWidget *menuitem, MateMenuTreeEntry *entry) {
  const char *path;
...
PanelWidget *menu_get_panel(GtkWidget *menu) {
  ...
  if (GTK_IS_MENU_ITEM(menu)) menu = gtk_widget_get_parent(menu);
...
static void setup_menu_panel(GtkWidget *menu) {
  PanelWidget *panel;
...
GtkWidget *panel_create_menu(void) {
  ...
  g_signal_connect(retval, "key-press-event",
                   G_CALLBACK(panel_menu_key_press_handler), NULL);
...
static void add_app_to_panel(GtkWidget *item, MateMenuTreeEntry *entry) {
  PanelWidget *panel_widget;
  ...
  panel_launcher_create(toplevel, position,
                        matemenu_tree_entry_get_desktop_file_path(entry));
}

static void add_app_to_desktop(GtkWidget *item, MateMenuTreeEntry *entry) {
  char *source_uri;
  const char *source;
  char *target_dir;
  char *target_uri;
  ...
  if (error != NULL) {
    g_warning("Problem while copying launcher to desktop: %s", error->message);
    g_error_free(error);
  }
}

static void add_drawers_from_dir(MateMenuTreeDirectory *directory, int pos,
                                 const char *toplevel_id);

static void add_drawers_from_alias(MateMenuTreeAlias *alias,
                                   const char *toplevel_id) {
  gpointer item;
  ...
  switch (matemenu_tree_alias_get_aliased_item_type(alias)) {
    case MATEMENU_TREE_ITEM_DIRECTORY:
      ...
      add_drawers_from_dir(item, G_MAXINT / 2, toplevel_id);
...
static void add_drawers_from_dir(MateMenuTreeDirectory *directory, int pos,
  ...
  attached_toplevel_id =
      panel_drawer_create_with_id(toplevel_id, pos, icon, icon != NULL, name);
  if (!attached_toplevel_id) return;
  ...
  while ((type = matemenu_tree_iter_next(iter)) != MATEMENU_TREE_ITEM_INVALID) {
    ...
    switch (type) {
      case MATEMENU_TREE_ITEM_ENTRY:
        ...
        panel_launcher_create_with_id(
            attached_toplevel_id, G_MAXINT / 2,
            matemenu_tree_entry_get_desktop_file_path(item));
        matemenu_tree_item_unref(item);
        break;
      ...
      case MATEMENU_TREE_ITEM_DIRECTORY:
        ...
        add_drawers_from_dir(item, G_MAXINT / 2, attached_toplevel_id);
        matemenu_tree_item_unref(item);
        break;
...
static void add_menudrawer_to_panel(GtkWidget *menuitem,
...

{
  ...
  add_drawers_from_dir(directory, insertion_pos,
                       panel_profile_get_toplevel_id(panel->toplevel));

  matemenu_tree_item_unref(directory);
}

static void add_menu_to_panel(GtkWidget *menuitem, MateMenuTreeEntry *entry) {
  MateMenuTreeDirectory *directory;
  MateMenuTree *tree;
  PanelWidget *panel;
  PanelData *pd;
  ...
  panel_menu_button_create(panel->toplevel, insertion_pos, menu_filename,
                           menu_path, TRUE,
...
static void grab_widget(GtkWidget *widget) {
  ...
  gdk_seat_grab(seat, window, GDK_SEAT_CAPABILITY_ALL, TRUE, NULL, NULL, NULL,
                NULL);
}

static void restore_grabs(GtkWidget *menu, GtkWidget *item) {
...
static void menu_destroy_context_menu(GtkWidget *item, GtkWidget *menu) {
  g_signal_handlers_disconnect_by_func(menu, restore_grabs, item);
  gtk_widget_destroy(menu);
}

static GtkWidget *create_item_context_menu(GtkWidget *item,
                                           PanelWidget *panel_widget) {
  MateMenuTreeEntry *entry;
  MateMenuTreeDirectory *directory;
  MateMenuTree *tree;
  ...
  g_signal_connect(item, "destroy", G_CALLBACK(menu_destroy_context_menu),
                   menu);
  g_signal_connect(menu, "deactivate", G_CALLBACK(restore_grabs), item);

  menuitem = gtk_menu_item_new_with_mnemonic(_("Add this launcher to _panel"));
  g_signal_connect(menuitem, "activate", G_CALLBACK(add_app_to_panel), entry);
  gtk_widget_set_sensitive(menuitem, id_lists_writable);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
  gtk_widget_show(menuitem);

  menuitem =
      gtk_menu_item_new_with_mnemonic(_("Add this launcher to _desktop"));
  g_signal_connect(menuitem, "activate", G_CALLBACK(add_app_to_desktop), entry);
  gtk_widget_set_sensitive(menuitem, id_lists_writable);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
  gtk_widget_show(menuitem);
  ...
  g_signal_connect(menuitem, "activate", G_CALLBACK(add_menudrawer_to_panel),
                   entry);
  gtk_widget_set_sensitive(menuitem, id_lists_writable);
  ...
  g_signal_connect(menuitem, "activate", G_CALLBACK(add_menu_to_panel), entry);
  gtk_widget_set_sensitive(menuitem, id_lists_writable);
  gtk_menu_shell_append(GTK_MENU_SHELL(submenu), menuitem);
  gtk_widget_show(menuitem);
...
static gboolean show_item_menu(GtkWidget *item, GdkEvent *event) {
  PanelWidget *panel_widget;
  GtkWidget *menu;
  ...
  if (!menu) menu = create_item_context_menu(item, panel_widget);

  if (!menu) return FALSE;

  gtk_menu_set_screen(GTK_MENU(menu), gtk_window_get_screen(
                                          GTK_WINDOW(panel_widget->toplevel)));
...
static gboolean menuitem_button_press_event(GtkWidget *menuitem,
                                            GdkEventButton *event) {
  if (event->button == 3) return show_item_menu(menuitem, (GdkEvent *)event);

  return FALSE;
}

static void drag_begin_menu_cb(GtkWidget *widget, GdkDragContext *context) {
 ...
 */
static void drag_end_menu_cb(GtkWidget *widget, GdkDragContext *context) {
  ...
  while (parent) {
    ...
    if (viewable) xgrab_shell = parent;
...
static void drag_data_get_menu_cb(GtkWidget *widget, GdkDragContext *context,
                                  ...
                                  guint time, MateMenuTreeEntry *entry) {
  const char *path;
  char *uri;
  char *uri_list;
...
static char *menu_escape_underscores_and_prepend(const char *text) {
  ...
  while (*src) {
    ...
    if (c == (gunichar)-1) {
      ...
      g_string_insert_c(escaped_text, src - text + inserted, '_');
      inserted++;
    }
...
void setup_menuitem(GtkWidget *menuitem, GtkIconSize icon_size,
                    GtkWidget *image, const char *title)
...
static void drag_data_get_string_cb(GtkWidget *widget, GdkDragContext *context,
...
void setup_uri_drag(GtkWidget *menuitem, const char *uri, const char *icon,
  ...
  static GtkTargetEntry menu_item_targets[] = {{"text/uri-list", 0, 0}};

  if (panel_lockdown_get_locked_down()) return;

  gtk_drag_source_set(menuitem, GDK_BUTTON1_MASK | GDK_BUTTON2_MASK,
                      menu_item_targets, 1, action);

  if (icon != NULL) gtk_drag_source_set_icon_name(menuitem, icon);

  g_signal_connect(menuitem, "drag-begin", G_CALLBACK(drag_begin_menu_cb),
                   NULL);
  g_signal_connect_data(
      menuitem, "drag-data-get", G_CALLBACK(drag_data_get_string_cb),
      g_strdup(uri), (GClosureNotify)G_CALLBACK(g_free), 0 /* connect_flags */);
  g_signal_connect(menuitem, "drag-end", G_CALLBACK(drag_end_menu_cb), NULL);
}
...
void setup_internal_applet_drag(GtkWidget *menuitem,
  ...
  gtk_drag_source_set(menuitem, GDK_BUTTON1_MASK | GDK_BUTTON2_MASK,
                      menu_item_targets, 1, GDK_ACTION_COPY);

  if (panel_action_get_icon_name(type) != NULL)
    gtk_drag_source_set_icon_name(menuitem, panel_action_get_icon_name(type));
  ...
  g_signal_connect_data(
      ...
      (GClosureNotify)G_CALLBACK(g_free), 0 /* connect_flags */);
  g_signal_connect(menuitem, "drag-end", G_CALLBACK(drag_end_menu_cb), NULL);
}

static void submenu_to_display(GtkWidget *menu) {
  void (*append_callback)(GtkWidget *, gpointer);
  gpointer append_data;

  if (!g_object_get_data(G_OBJECT(menu), "panel-menu-needs-loading")) return;

  g_object_set_data(G_OBJECT(menu), "panel-menu-needs-loading", NULL);

  MateMenuTreeDirectory *directory =
      g_object_get_data(G_OBJECT(menu), "panel-menu-tree-directory");
  if (!directory) {
    const char *menu_path =
        g_object_get_data(G_OBJECT(menu), "panel-menu-tree-path");
    if (!menu_path) return;

    MateMenuTree *tree = g_object_get_data(G_OBJECT(menu), "panel-menu-tree");
    if (!tree) return;
    ...
    g_object_set_data_full(G_OBJECT(menu), "panel-menu-tree-directory",
                           directory, (GDestroyNotify)matemenu_tree_item_unref);
  }

  if (directory) populate_menu_from_directory(menu, directory);

  append_callback =
      g_object_get_data(G_OBJECT(menu), "panel-menu-append-callback");
  append_data =
      g_object_get_data(G_OBJECT(menu), "panel-menu-append-callback-data");
  if (append_callback) append_callback(menu, append_data);
}
...
static void remove_submenu_to_display_idle(gpointer data) {
  guint idle_id = GPOINTER_TO_UINT(data);

  g_source_remove(idle_id);
}

static GtkWidget *create_fake_menu(MateMenuTreeDirectory *directory) {
  GtkWidget *menu;
  guint idle_id;
  ...
  g_object_set_data(G_OBJECT(menu), "panel-menu-needs-loading",
                    GUINT_TO_POINTER(TRUE));

  g_signal_connect(menu, "show", G_CALLBACK(submenu_to_display), NULL);

  idle_id =
      g_idle_add_full(G_PRIORITY_LOW, submenu_to_display_in_idle, menu, NULL);
  g_object_set_data_full(G_OBJECT(menu), "panel-menu-idle-id",
...
static GtkWidget *create_submenu_entry(GtkWidget *menu,
  ...
  setup_menuitem_with_icon(menuitem, panel_menu_icon_get_size(),
                           matemenu_tree_directory_get_icon(directory), NULL,
                           matemenu_tree_directory_get_name(directory));
...
static void create_submenu(GtkWidget *menu, MateMenuTreeDirectory *directory,
  ...
  force_categories_icon =
      g_object_get_data(G_OBJECT(menu),
                        "panel-menu-force-icon-for-categories") != NULL;
  g_object_set_data(G_OBJECT(submenu), "panel-menu-force-icon-for-categories",
                    GINT_TO_POINTER(force_categories_icon));
}

static void create_header(GtkWidget *menu, MateMenuTreeHeader *header) {
  MateMenuTreeDirectory *directory;
  GtkWidget *menuitem;
  ...
  g_object_set_data_full(G_OBJECT(menuitem), "panel-matemenu-tree.header",
                         matemenu_tree_item_ref(header),
                         (GDestroyNotify)matemenu_tree_item_unref);

  g_signal_connect(menuitem, "activate", G_CALLBACK(gtk_false), NULL);
}

static void create_menuitem(GtkWidget *menu, MateMenuTreeEntry *entry,
                            MateMenuTreeDirectory *alias_directory) {
  GtkWidget *menuitem;

  menuitem = panel_image_menu_item_new();

  g_object_set_data_full(G_OBJECT(menuitem), "panel-menu-tree-entry",
  ...
  if (alias_directory) {
    ...
    if (matemenu_tree_directory_get_comment(alias_directory)) {
      panel_util_set_tooltip_text(
          menuitem, matemenu_tree_directory_get_comment(alias_directory));
    }
  } else {
    ...
    setup_menuitem_with_icon(menuitem, panel_menu_icon_get_size(),
                             g_app_info_get_icon(G_APP_INFO(ginfo)), NULL,
  ...
  g_signal_connect(menuitem, "activate", G_CALLBACK(activate_app_def), entry);
...
static void handle_matemenu_tree_changed(MateMenuTree *tree, GtkWidget *menu) {
  GError *error = NULL;
  guint idle_id;

  GList *list, *l;
  list = gtk_container_get_children(GTK_CONTAINER(menu));
  for (l = list; l; l = l->next) gtk_widget_destroy(l->data);
  g_list_free(list);
  ...
  g_object_set_data_full(G_OBJECT(menu), "panel-menu-tree-directory", NULL,
                         NULL);
...
static void remove_matemenu_tree_monitor(GtkWidget *menu, MateMenuTree *tree) {
  g_signal_handlers_disconnect_by_func(
      tree, G_CALLBACK(handle_matemenu_tree_changed), menu);
}
...
GtkWidget *create_applications_menu(const char *menu_file,
  ...
  if (always_show_image)
    g_object_set_data(G_OBJECT(menu), "panel-menu-force-icon-for-categories",
  ...
  g_object_set_data_full(G_OBJECT(menu), "panel-menu-tree", g_object_ref(tree),
                         (GDestroyNotify)g_object_unref);

  g_object_set_data_full(G_OBJECT(menu), "panel-menu-tree-path",
  ...
  g_signal_connect(tree, "changed", G_CALLBACK(handle_matemenu_tree_changed),
                   menu);
  g_signal_connect(menu, "destroy", G_CALLBACK(remove_matemenu_tree_monitor),
                   tree);

  g_object_unref(tree);

  /*HACK Fix any failures of compiz/other wm's to communicate with gtk for
   * transparency */
  GtkWidget *toplevel = gtk_widget_get_toplevel(menu);
  GdkScreen *screen = gtk_widget_get_screen(GTK_WIDGET(toplevel));
  GdkVisual *visual = gdk_screen_get_rgba_visual(screen);
...
static void main_menu_append(GtkWidget *main_menu, gpointer data) {
  PanelWidget *panel;
  GtkWidget *item;
  gboolean add_separator;
  GList *children;
  ...
  if (add_separator) add_menu_separator(main_menu);
...
GtkWidget *create_main_menu(PanelWidget *panel) {
  ...
  g_object_set_data(G_OBJECT(main_menu), "panel-menu-append-callback",
                    main_menu_append);
  g_object_set_data(G_OBJECT(main_menu), "panel-menu-append-callback-data",
                    panel);
...
static gboolean panel_menu_key_press_handler(GtkWidget *widget,
  ...
  if ((event->keyval == GDK_KEY_Menu) ||
    ...
    if (active_menu_item &&
        gtk_menu_item_get_submenu(GTK_MENU_ITEM(active_menu_item)) == NULL)
