...
enum { PROP_0, PROP_FALLBACK_ICON, PROP_ICON };

static guint panel_icon_chooser_signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(PanelIconChooser, panel_icon_chooser,
                           GTK_TYPE_BUTTON)
...
static GObject *panel_icon_chooser_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  GObject *obj;
  PanelIconChooser *chooser;

  obj = G_OBJECT_CLASS(panel_icon_chooser_parent_class)
...
static void panel_icon_chooser_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_FALLBACK_ICON:
      g_value_set_string(value,
                         panel_icon_chooser_get_fallback_icon_name(chooser));
      break;
    case PROP_ICON:
...
static void panel_icon_chooser_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  PanelIconChooser *chooser;
  ...
  switch (prop_id) {
    case PROP_FALLBACK_ICON:
      ...
      break;
    case PROP_ICON:
      panel_icon_chooser_set_icon(chooser, g_value_get_string(value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void panel_icon_chooser_dispose(GObject *object) {
  PanelIconChooser *chooser;
...
static void panel_icon_chooser_class_init(PanelIconChooserClass *class) {
  ...
  panel_icon_chooser_signals[CHANGED] = g_signal_new(
      "changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelIconChooserClass, changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  g_object_class_install_property(
      gobject_class, PROP_FALLBACK_ICON,
      g_param_spec_string("fallback-icon-name", "Fallback Icon Name",
                          "Icon name of the icon displayed (but not returned) "
                          "if the current icon does not exist",
                          NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_ICON,
      g_param_spec_string("icon", "Icon", "Icon name or path", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void panel_icon_chooser_init(PanelIconChooser *chooser) {
...
static void _panel_icon_chooser_update(PanelIconChooser *chooser) {
  if (!chooser->priv->icon) {
    ...
    if (g_file_test(chooser->priv->icon, G_FILE_TEST_EXISTS)) {
      ...
      gtk_icon_size_lookup(PANEL_ICON_CHOOSER_ICON_SIZE, &width, &height);
      pixbuf = gdk_pixbuf_new_from_file_at_size(chooser->priv->icon, width,
                                                height, NULL);
    ...
    gtk_image_set_from_icon_name(GTK_IMAGE(chooser->priv->image), icon,
                                 PANEL_ICON_CHOOSER_ICON_SIZE);
...
static char *_panel_icon_chooser_find_icon_from_path(PanelIconChooser *chooser,
                                                     const char *path) {
  GdkScreen *screen;
  char *icon;
  ...
  if (!icon) icon = g_strdup(path);

  return icon;
}

static void _panel_icon_chooser_file_chooser_response(
    GtkFileChooser *filechooser, gint response_id, PanelIconChooser *chooser) {
...
static void _panel_icon_chooser_clicked(GtkButton *button) {
  ...
  filechooser = panel_file_chooser_dialog_new(
      _("Choose an icon"), parent, GTK_FILE_CHOOSER_ACTION_OPEN, "process-stop",
  ...
  if (chooser->priv->icon) {
    ...
    if (g_path_is_absolute(chooser->priv->icon)) {
      ...
      icon_theme = gtk_icon_theme_get_for_screen(
          gtk_widget_get_screen(GTK_WIDGET(chooser)));
      no_ext = panel_xdg_icon_remove_extension(chooser->priv->icon);
      gtk_icon_size_lookup(PANEL_ICON_CHOOSER_ICON_SIZE, &size, NULL);

      info = gtk_icon_theme_lookup_icon(icon_theme, no_ext, size, 0);
      g_free(no_ext);
  ...
  g_signal_connect(filechooser, "destroy", G_CALLBACK(gtk_widget_destroyed),
...
static void _panel_icon_chooser_style_set(GtkWidget *widget,
  ...
  GTK_WIDGET_CLASS(panel_icon_chooser_parent_class)
      ->style_set(widget, prev_style);
...
GtkWidget *panel_icon_chooser_new(const char *icon) {
  ...
  chooser = g_object_new(PANEL_TYPE_ICON_CHOOSER, "icon", icon, NULL);
...
void panel_icon_chooser_set_icon(PanelIconChooser *chooser, const char *icon) {
  ...
  if (g_strcmp0(chooser->priv->icon, icon) == 0) return;
  ...
  g_signal_emit(G_OBJECT(chooser), panel_icon_chooser_signals[CHANGED], 0,
                icon);
}
