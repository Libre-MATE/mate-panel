...
static void _panel_show_error_dialog(const gchar *uri, GdkScreen *screen,
                                     const gchar *message) {
  char *primary;

  primary = g_markup_printf_escaped(_("Could not open location '%s'"), uri);
  panel_error_dialog(NULL, screen, "cannot_show_url", TRUE, primary, message);
  g_free(primary);
}
...
static void _panel_show_mount_async_callback(GObject *source_object,
                                             GAsyncResult *result,
                                             gpointer user_data) {
  GError *error = NULL;
  GFile *file;
  ...
  if (g_file_mount_enclosing_volume_finish(file, result, &error)) {
    ...
    panel_show_uri(handle->screen, uri, gtk_get_current_event_time(), NULL);
    g_free(uri);
  } else {
    if (!g_error_matches(error, G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED) &&
      ...
      _panel_show_error_dialog(uri, handle->screen, error->message);
      g_free(uri);
    }
    g_error_free(error);
  }

  if (handle->mount_op) g_object_unref(handle->mount_op);

  g_slice_free(PanelShowMountOperationHandle, handle);
}

static gboolean _panel_show_handle_error(const gchar *uri, GdkScreen *screen,
                                         GError *local_error, GError **error) {
  if (local_error == NULL) {
    return TRUE;
  }
  ...
  else if (g_error_matches(local_error, G_IO_ERROR, G_IO_ERROR_NOT_MOUNTED)) {
    GFile *file;
    ...
    g_file_mount_enclosing_volume(file, G_MOUNT_MOUNT_NONE, handle->mount_op,
                                  NULL, _panel_show_mount_async_callback,
                                  handle);
    g_object_unref(file);
...
static gboolean panel_show_caja_search_uri(GdkScreen *screen, const gchar *uri,
  ...
  desktopfile =
      panel_g_lookup_in_applications_dirs("caja-folder-handler.desktop");
...
gboolean panel_show_uri_force_mime_type(GdkScreen *screen, const gchar *uri,
  ...
  file = g_file_new_for_uri(uri);
  appinfo = g_app_info_get_default_for_type(mime_type, !g_file_is_native(file));
  g_object_unref(file);
  ...
  ret = g_app_info_launch_uris(appinfo, uris, G_APP_LAUNCH_CONTEXT(context),
                               error);
...
static void _panel_show_help_error_dialog(const gchar *doc, GdkScreen *screen,
                                          const gchar *message) {
  char *primary;

  primary =
      g_markup_printf_escaped(_("Could not display help document '%s'"), doc);
  panel_error_dialog(NULL, screen, "cannot_show_help", TRUE, primary, message);
  g_free(primary);
}

static gboolean _panel_show_help_handle_error(const gchar *doc,
                                              GdkScreen *screen,
                                              GError *local_error,
                                              GError **error) {
  if (local_error == NULL)
    ...
    _panel_show_help_error_dialog(doc, screen, local_error->message);
    g_error_free(local_error);
  }
