...
GKeyFile *panel_key_file_new_desktop(void) {
  ...
  /* FIXME? g_key_file_set_string (retval, G_KEY_FILE_DESKTOP_GROUP, "Name",
   * _("No Name")); */
...
static void _panel_key_file_make_executable(const gchar *path) {
  GFile *file;
  GFileInfo *info;
  ...
  info = g_file_query_info(
      file, G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_UNIX_MODE,
  ...
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_UNIX_MODE)) {
    ...
    if ((current_perms != new_perms) &&
        !g_file_set_attribute_uint32(
            file, G_FILE_ATTRIBUTE_UNIX_MODE, new_perms,
            G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, NULL))
      g_warning("Cannot mark %s executable", path);
  }
...
gboolean panel_key_file_load_from_uri(GKeyFile *keyfile, const gchar *uri,
  ...
  if (is_local) {
    ...
    result = g_key_file_load_from_file(keyfile, path, flags, error);
    g_free(path);
  } else {
    GFile *file;
    char *contents;
    ...
    ret = g_file_load_contents(file, NULL, &contents, &size, NULL, NULL);
    g_object_unref(file);

    if (!ret) return FALSE;

    result = g_key_file_load_from_data(keyfile, contents, size, flags, error);
...
void panel_key_file_remove_locale_key(GKeyFile *keyfile, const gchar *key) {
  ...
  for (i = 0; langs_pointer[i] != NULL; i++) {
    ...
    if (strchr(langs_pointer[i], '.') == NULL) {
      ...
      if (g_key_file_has_key(keyfile, G_KEY_FILE_DESKTOP_GROUP, locale_key,
                             NULL))
        break;
  ...
  if (locale_key) {
    g_key_file_remove_key(keyfile, G_KEY_FILE_DESKTOP_GROUP, locale_key, NULL);
    g_free(locale_key);
  } else
    g_key_file_remove_key(keyfile, G_KEY_FILE_DESKTOP_GROUP, key, NULL);
}
...
void panel_key_file_remove_all_locale_key(GKeyFile *keyfile, const gchar *key) {
  ...
  for (i = 0; keys[i] != NULL; i++) {
    ...
    if (strncmp(keys[i], key, key_len)) continue;

    len = strlen(keys[i]);
    if (len == key_len || (len > key_len && keys[i][key_len] == '['))
      g_key_file_remove_key(keyfile, G_KEY_FILE_DESKTOP_GROUP, keys[i], NULL);
  }
