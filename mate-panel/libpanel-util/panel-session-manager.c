...
static void panel_session_manager_finalize(GObject *object) {
...
static void panel_session_manager_init(PanelSessionManager *manager) {
  GError *error = NULL;
...
void panel_session_manager_request_logout(PanelSessionManager *manager,
...
void panel_session_manager_request_shutdown(PanelSessionManager *manager) {
  GError *error = NULL;
  GVariant *ret;
  ...
  ret = g_dbus_proxy_call_sync(manager->proxy, "Shutdown", g_variant_new("()"),
                               G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  if (ret == NULL) {
...
gboolean panel_session_manager_is_shutdown_available(
  ...
  if (manager->proxy == NULL) return FALSE;

  ret =
      g_dbus_proxy_call_sync(manager->proxy, "CanShutdown", g_variant_new("()"),
                             G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  if (ret == NULL) {
...
PanelSessionManager *panel_session_manager_get(void) {
  ...
  if (manager == NULL) {
    ...
    panel_cleanup_register(panel_cleanup_unref_and_nullify, &manager);
  }
