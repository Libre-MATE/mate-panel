...
static void _panel_launch_error_dialog(const gchar *name, GdkScreen *screen,
                                       const gchar *message) {
  char *primary;

  if (name)
    primary = g_markup_printf_escaped(_("Could not launch '%s'"), name);
  else
    primary = g_strdup(_("Could not launch application"));

  panel_error_dialog(NULL, screen, "cannot_launch", TRUE, primary, message);
  g_free(primary);
}

static gboolean _panel_launch_handle_error(const gchar *name, GdkScreen *screen,
                                           GError *local_error,
                                           GError **error) {
...
static void gather_pid_callback(GDesktopAppInfo *gapp, GPid pid,
                                gpointer data) {
...
gboolean panel_app_info_launch_uris(GDesktopAppInfo *appinfo, GList *uris,
  ...
  local_error = NULL;
  if (action == NULL) {
    retval = g_desktop_app_info_launch_uris_as_manager(
        appinfo, uris, G_APP_LAUNCH_CONTEXT(context),
        G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL,
        gather_pid_callback, appinfo, &local_error);
  } else {
    g_desktop_app_info_launch_action(appinfo, action,
  ...
  if ((local_error == NULL) && (retval == TRUE)) return TRUE;
...
gboolean panel_app_info_launch_uri(GDesktopAppInfo *appinfo, const gchar *uri,
  ...
  uris = NULL;
  if (uri) uris = g_list_prepend(uris, (gpointer)uri);

  retval =
      panel_app_info_launch_uris(appinfo, uris, screen, NULL, timestamp, error);
...
gboolean panel_launch_key_file(GKeyFile *keyfile, GList *uri_list,
  ...
  retval = panel_app_info_launch_uris(appinfo, uri_list, screen, action,
...
gboolean panel_launch_desktop_file_with_fallback(const char *desktop_file,
  ...
  retval = g_spawn_async(NULL,       /* working directory */
                         argv, NULL, /* envp */
                         G_SPAWN_SEARCH_PATH, set_environment, &display_name,
                         &pid, &local_error);
  g_free(display_name);
  ...
  return _panel_launch_handle_error(fallback_exec, screen, local_error, error);
}
