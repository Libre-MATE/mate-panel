...
GList *panel_g_list_swap_next(GList *list, GList *dl) {
  ...
  if (!dl->next) return list;

  if (dl->prev) dl->prev->next = dl->next;
  t = dl->prev;
  dl->prev = dl->next;
  dl->next->prev = t;
  if (dl->next->next) dl->next->next->prev = dl;
  t = dl->next->next;
  dl->next->next = dl;
  dl->next = t;

  if (list == dl) return dl->prev;
...
GList *panel_g_list_swap_prev(GList *list, GList *dl) {
  ...
  if (!dl->prev) return list;

  if (dl->next) dl->next->prev = dl->prev;
  t = dl->next;
  dl->next = dl->prev;
  dl->prev->next = t;
  if (dl->prev->prev) dl->prev->prev->next = dl;
  t = dl->prev->prev;
  dl->prev->prev = dl;
  dl->prev = t;

  if (list == dl->next) return dl;
...
GList *panel_g_list_resort_item(GList *list, gpointer data, GCompareFunc func) {
  ...
  if (dl == NULL) return list;

  while (dl->next && (*func)(dl->data, dl->next->data) > 0)
    list = panel_g_list_swap_next(list, dl);
  while (dl->prev && (*func)(dl->data, dl->prev->data) < 0)
