...
enum { PROP_0, PROP_EDGES };

static void panel_frame_get_preferred_width(GtkWidget *widget,
                                            gint *minimal_width,
                                            gint *natural_width) {
  ...
  gtk_style_context_get_padding(context, gtk_widget_get_state_flags(widget),
                                &padding);
...
static void panel_frame_get_preferred_height(GtkWidget *widget,
                                             gint *minimal_height,
                                             gint *natural_height) {
  PanelFrame *frame = (PanelFrame *)widget;
  GtkBin *bin = (GtkBin *)widget;
...
static void panel_frame_size_allocate(GtkWidget *widget,
  ...
  if (frame->edges & PANEL_EDGE_RIGHT) child_allocation.width -= padding.left;

  if (frame->edges & PANEL_EDGE_BOTTOM) child_allocation.height -= padding.top;
...
void panel_frame_draw(GtkWidget *widget, cairo_t *cr, PanelFrameEdge edges) {
  ...
  if (edges == PANEL_EDGE_NONE) return;
  ...
  gtk_style_context_get(context, state, "background-color", &bg, NULL);
...
static void panel_frame_set_property(GObject *object, guint prop_id,
...
static void panel_frame_get_property(GObject *object, guint prop_id,
...
static void panel_frame_class_init(PanelFrameClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_EDGES,
      g_param_spec_enum("edges", "Edges", "Which edges to draw",
                        PANEL_TYPE_FRAME_EDGE, PANEL_EDGE_NONE,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void panel_frame_init(PanelFrame *frame) {
...
void panel_frame_set_edges(PanelFrame *frame, PanelFrameEdge edges) {
  g_return_if_fail(PANEL_IS_FRAME(frame));

  if (frame->edges == edges) return;
...
PanelFrameEdge panel_frame_get_edges(PanelFrame *frame) {
