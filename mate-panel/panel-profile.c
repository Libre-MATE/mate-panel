...
static void panel_profile_set_toplevel_id(PanelToplevel *toplevel,
                                          const char *id) {
  if (!toplevel_id_quark)
    toplevel_id_quark = g_quark_from_static_string("panel-toplevel-id");

  g_object_set_qdata_full(G_OBJECT(toplevel), toplevel_id_quark, g_strdup(id),
                          g_free);
}

const char *panel_profile_get_toplevel_id(PanelToplevel *toplevel) {
  if (!toplevel_id_quark) return NULL;
...
PanelToplevel *panel_profile_get_toplevel_by_id(const char *toplevel_id) {
  ...
  if (!toplevel_id || !toplevel_id[0]) return NULL;
...
char *panel_profile_find_new_id(PanelGSettingsKeyType type) {
  ...
  if (existing_ids) g_strfreev(existing_ids);
...
static void panel_profile_remove_commit_timeout(guint timeout) {
  g_source_remove(timeout);
}

static void panel_profile_set_commit_timeout(PanelToplevel *toplevel,
                                             guint timeout) {
  ...
  g_object_set_qdata_full(G_OBJECT(toplevel), commit_timeout_quark,
                          GUINT_TO_POINTER(timeout), destroy_notify);
}

static guint panel_profile_get_commit_timeout(GObject *object) {
  if (!commit_timeout_quark) return 0;

  return GPOINTER_TO_UINT(g_object_get_qdata(object, commit_timeout_quark));
}

gboolean panel_profile_key_is_writable(PanelToplevel *toplevel, gchar *key) {
...
void panel_profile_set_background_type(PanelToplevel *toplevel,
  ...
  g_settings_set_enum(toplevel->background_settings, "type", background_type);
}

PanelBackgroundType panel_profile_get_background_type(PanelToplevel *toplevel) {
  PanelBackgroundType background_type;
  background_type = g_settings_get_enum(toplevel->background_settings, "type");
...
PanelOrientation panel_profile_get_toplevel_orientation(
...
#define TOPLEVEL_GET_SET_FUNCS(k, p, t, s, a)                      \
  void panel_profile_set_##p##_##s(PanelToplevel *toplevel, a s) { \
    g_settings_set_##t(toplevel->settings, k, s);                  \
  }                                                                \
  a panel_profile_get_##p##_##s(PanelToplevel *toplevel) {         \
    a retval;                                                      \
...
TOPLEVEL_GET_SET_FUNCS("enable-buttons", toplevel, boolean, enable_buttons,
                       gboolean)
TOPLEVEL_GET_SET_FUNCS("enable-arrows", toplevel, boolean, enable_arrows,
                       gboolean)
...
void panel_profile_set_attached_custom_icon(PanelToplevel *toplevel,
  ...
  g_settings_set_boolean(settings, PANEL_OBJECT_USE_CUSTOM_ICON_KEY,
                         custom_icon != NULL);
  g_settings_set_string(settings, PANEL_OBJECT_CUSTOM_ICON_KEY,
                        sure_string(custom_icon));
...
gboolean panel_profile_is_writable_attached_custom_icon(
    PanelToplevel *toplevel) {
  ...
  if (panel_toplevel_get_is_attached(toplevel)) {
    ...
    is_writable =
        g_settings_is_writable(settings, PANEL_OBJECT_USE_CUSTOM_ICON_KEY) &&
...
static char *get_background_image(PanelToplevel *toplevel, gboolean *fit,
                                  gboolean *stretch, gboolean *rotate) {
  char *image;
...
static void panel_profile_load_background(PanelToplevel *toplevel) {
  ...
  panel_background_set(background, background_type, &color, image, fit, stretch,
                       rotate);

  g_free(image);
}

static gboolean panel_profile_commit_toplevel_changes(PanelToplevel *toplevel) {
...
static void panel_profile_queue_toplevel_location_change(
    PanelToplevel *toplevel, ToplevelLocationChange *change) {
  guint commit_timeout;

  g_settings_delay(toplevel->queued_settings);

#ifdef HAVE_X11
  if (change->screen_changed && GDK_IS_X11_SCREEN(change->screen)) {
    g_settings_set_int(toplevel->queued_settings, "screen",
                       gdk_x11_screen_get_screen_number(change->screen));
  }
#endif

  if (change->monitor_changed)
    g_settings_set_int(toplevel->queued_settings, "monitor", change->monitor);

  if (change->size_changed)
    g_settings_set_int(toplevel->queued_settings, "size", change->size);

  if (change->orientation_changed)
    g_settings_set_enum(toplevel->queued_settings, "orientation",
                        change->orientation);

  if (change->x_changed)
    g_settings_set_int(toplevel->queued_settings, "x", change->x);

  if (change->x_right_changed)
    g_settings_set_int(toplevel->queued_settings, "x-right", change->x_right);

  if (change->x_centered_changed)
    g_settings_set_boolean(toplevel->queued_settings, "x-centered",
                           change->x_centered);

  if (change->y_changed)
    g_settings_set_int(toplevel->queued_settings, "y", change->y);

  if (change->y_bottom_changed)
    g_settings_set_int(toplevel->queued_settings, "y-bottom", change->y_bottom);

  if (change->y_centered_changed)
    g_settings_set_boolean(toplevel->queued_settings, "y-centered",
                           change->y_centered);

  commit_timeout = panel_profile_get_commit_timeout(G_OBJECT(toplevel));
  if (!commit_timeout) {
    commit_timeout = g_timeout_add(
        500, (GSourceFunc)panel_profile_commit_toplevel_changes, toplevel);
    panel_profile_set_commit_timeout(toplevel, commit_timeout);
  }
}

#define TOPLEVEL_LOCATION_CHANGED_HANDLER(c)                                  \
  static void panel_profile_toplevel_##c##_changed(PanelToplevel *toplevel) { \
...
static void panel_profile_toplevel_screen_changed(PanelToplevel *toplevel) {
  ToplevelLocationChange change = {NULL};
...
static void panel_profile_connect_to_toplevel(PanelToplevel *toplevel) {
  ...
  g_signal_connect(toplevel, "notify::orientation",
                   G_CALLBACK(panel_profile_toplevel_orientation_changed),
                   NULL);
}

static void panel_profile_toplevel_change_notify(GSettings *settings,
                                                 gchar *key,
                                                 PanelToplevel *toplevel) {
  if (toplevel == NULL || !PANEL_IS_TOPLEVEL(toplevel)) return;
...
#define UPDATE_ENUM(k, n)                                                 \
  if (!strcmp(key, k)) {                                                  \
    panel_toplevel_set_##n(toplevel, g_settings_get_enum(settings, key)); \
  }

#define UPDATE_INT(k, n)                                                 \
  if (!strcmp(key, k)) {                                                 \
    panel_toplevel_set_##n(toplevel, g_settings_get_int(settings, key)); \
  }

#define UPDATE_BOOL(k, n)                                                    \
  if (!strcmp(key, k)) {                                                     \
    panel_toplevel_set_##n(toplevel, g_settings_get_boolean(settings, key)); \
  }
...
#define UPDATE_POS(k, n, n2)                                                \
  if (!strcmp(key, k)) {                                                    \
    ...
    panel_toplevel_get_position(toplevel, &x, &x_right, &y, &y_bottom);     \
    panel_toplevel_set_##n(toplevel, g_settings_get_int(settings, key), n2, \
...
#define UPDATE_POS2(k, n, n2)                                              \
  if (!strcmp(key, k)) {                                                   \
    int x, x_right, y, y_bottom;                                           \
    panel_toplevel_get_position(toplevel, &x, &x_right, &y, &y_bottom);    \
    panel_toplevel_set_##n(toplevel, n, g_settings_get_int(settings, key), \
                           panel_toplevel_get_##n##_centered(toplevel));   \
  }
...
#define UPDATE_CENTERED(k, n, n2)                                       \
  if (!strcmp(key, k)) {                                                \
    int x, x_right, y, y_bottom;                                        \
    panel_toplevel_get_position(toplevel, &x, &x_right, &y, &y_bottom); \
    panel_toplevel_set_##n(toplevel, n, n2,                             \
...
static void panel_profile_background_change_notify(GSettings *settings,
                                                   gchar *key,
                                                   PanelToplevel *toplevel) {
  PanelWidget *panel_widget;
  PanelBackground *background;
  ...
  if (!strcmp(key, "type")) {
    ...
    panel_background_set_fit(background, g_settings_get_boolean(settings, key));
...
static const char *key_from_type(PanelGSettingsKeyType type) {
...
void panel_profile_add_to_list(PanelGSettingsKeyType type, const char *id) {
  char *new_id = id ? g_strdup(id) : panel_profile_find_new_id(type);

  if (new_id != NULL) {
    mate_gsettings_append_strv(profile_settings, key_from_type(type), new_id);
    g_free(new_id);
  }
}
...
void panel_profile_remove_from_list(PanelGSettingsKeyType type,
                                    const char *id) {
  mate_gsettings_remove_all_from_strv(profile_settings, key_from_type(type),
                                      id);
}

static gboolean panel_profile_id_list_is_writable(PanelGSettingsKeyType type) {
  return g_settings_is_writable(profile_settings, key_from_type(type));
}

gboolean panel_profile_id_lists_are_writable(void) {
  return panel_profile_id_list_is_writable(PANEL_GSETTINGS_TOPLEVELS) &&
         panel_profile_id_list_is_writable(PANEL_GSETTINGS_OBJECTS);
}

static gboolean panel_profile_find_empty_spot(GdkScreen *screen,
                                              PanelOrientation *orientation,
                                              int *monitor) {
  GSList *li;
  int i;
  ...
  *orientation = PANEL_ORIENTATION_TOP;
  ...
  for (li = panel_toplevel_list_toplevels(); li != NULL; li = li->next) {
    ...
    if (toplevel_screen != screen || panel_toplevel_get_is_attached(toplevel) ||
        toplevel_monitor < 0)
      continue;
...
static void panel_profile_delete_toplevel_objects(const char *toplevel_id) {
  gchar **list;
  GArray *newlist;
  int i;
  ...
  for (i = 0; list[i]; i++) {
    ...
    settings = g_settings_new_with_path(PANEL_OBJECT_SCHEMA, path);
    parent_toplevel_id =
        g_settings_get_string(settings, PANEL_OBJECT_TOPLEVEL_ID_KEY);
    g_free(path);
    g_object_unref(settings);
  ...
  g_settings_set_strv(profile_settings, PANEL_OBJECT_ID_LIST_KEY,
                      (const gchar **)newlist->data);
...
PanelToplevel *panel_profile_load_toplevel(const char *toplevel_id) {
  ...
  toplevel = g_object_new(PANEL_TYPE_TOPLEVEL, "screen", screen, NULL);

  panel_toplevel_set_settings_path(toplevel, toplevel_path);
  toplevel->settings =
      g_settings_new_with_path(PANEL_TOPLEVEL_SCHEMA, toplevel_path);
  toplevel->queued_settings =
      g_settings_new_with_path(PANEL_TOPLEVEL_SCHEMA, toplevel_path);

  toplevel_background_path = g_strdup_printf("%sbackground/", toplevel_path);
  toplevel->background_settings = g_settings_new_with_path(
      PANEL_TOPLEVEL_BACKGROUND_SCHEMA, toplevel_background_path);
...
#define GET_POSITION(a, b, c, fn)                                     \
  ...
  g_signal_connect(toplevel->settings, "changed",
                   G_CALLBACK(panel_profile_toplevel_change_notify), toplevel);
  g_signal_connect(toplevel->background_settings, "changed",
...
static void panel_profile_load_and_show_toplevel(char *toplevel_id) {
  PanelToplevel *toplevel;
  gchar **objects;
  ...
  if (!loading_queued_applets) mate_panel_applet_load_queued_applets(FALSE);

  g_strfreev(objects);
  g_object_unref(panel_settings);
}

static void panel_profile_load_and_show_toplevel_startup(
    const char *toplevel_id) {
  PanelToplevel *toplevel;
  toplevel = panel_profile_load_toplevel(toplevel_id);
  if (toplevel) gtk_widget_show(GTK_WIDGET(toplevel));
}

static void panel_profile_destroy_toplevel(const char *id) {
  PanelToplevel *toplevel;

  if (!(toplevel = panel_profile_get_toplevel_by_id(id))) return;
...
char *panel_profile_prepare_object(PanelObjectType object_type,
                                   PanelToplevel *toplevel, int position) {
  return panel_profile_prepare_object_with_id(
      object_type, panel_profile_get_toplevel_id(toplevel), position);
}
...
static void panel_profile_load_object(char *id) {
  ...
  if (g_settings_get_boolean(settings, PANEL_OBJECT_PANEL_RIGHT_STICK_KEY)) {
    ...
    g_settings_set_enum(settings, PANEL_OBJECT_RELATIVE_TO_EDGE_KEY,
                        edge_relativity);
  }

  mate_panel_applet_queue_applet_to_load(id, object_type, toplevel_id, position,
                                         edge_relativity, locked);
...
static void panel_profile_destroy_object(const char *id) {
  AppletInfo *info;
...
static void panel_profile_delete_dir(PanelGSettingsKeyType type,
                                     const char *id) {
  gchar *dir = NULL;
...
static gboolean panel_profile_object_exists(GSList *list, const char *id,
  ...
  for (l = list; l; l = l->next) {
    ...
    if (!strcmp(check_id, id)) return TRUE;
  }

  return FALSE;
}

static gboolean panel_profile_id_exists(GSList *id_list, const char *id) {
  GSList *l;

  if (!id_list || !id) return FALSE;

  for (l = id_list; l; l = l->next) {
    const char *check_id = l->data;

    if (!strcmp(id, check_id)) return TRUE;
  }

  return FALSE;
}

static void panel_profile_load_added_ids(
    GSList *list, GSList *id_list, PanelProfileGetIdFunc get_id_func,
    PanelProfileLoadFunc load_handler, PanelProfileOnLoadQueue on_load_queue) {
...
static void panel_profile_delete_removed_ids(
    PanelGSettingsKeyType type, GSList *list, GSList *id_list,
    PanelProfileGetIdFunc get_id_func,
...
static gboolean load_default_layout_idle(gpointer unused) {
...
static void panel_profile_toplevel_id_list_notify(GSettings *settings,
                                                  gchar *key,
                                                  gpointer user_data) {
  ...
  toplevel_ids =
      mate_gsettings_strv_to_gslist((const gchar **)toplevel_ids_strv);
  toplevel_ids =
      panel_g_slist_make_unique(toplevel_ids, (GCompareFunc)g_strcmp0, TRUE);
  ...
  panel_profile_delete_removed_ids(
      PANEL_GSETTINGS_TOPLEVELS, existing_toplevels, toplevel_ids,
      (PanelProfileGetIdFunc)panel_profile_get_toplevel_id,
...
static void panel_profile_object_id_list_update(gchar **objects) {
  ...
  object_ids =
      panel_g_slist_make_unique(object_ids, (GCompareFunc)g_strcmp0, TRUE);
  ...
  panel_profile_load_added_ids(
      sublist, object_ids, (PanelProfileGetIdFunc)mate_panel_applet_get_id,
      (PanelProfileLoadFunc)panel_profile_load_object,
      (PanelProfileOnLoadQueue)mate_panel_applet_on_load_queue);

  panel_profile_delete_removed_ids(
      PANEL_GSETTINGS_OBJECTS, sublist, object_ids,
...
static void panel_profile_object_id_list_notify(GSettings *settings, gchar *key,
                                                gpointer data) {
  gchar **objects;
...
static void panel_profile_load_list(GSettings *settings,
                                    PanelGSettingsKeyType type,
...
static void panel_profile_ensure_toplevel_per_screen() {
  GSList *toplevels;
  ...
  for (l = toplevels; l; l = l->next)
    if (gtk_window_get_screen(l->data) == screen) break;

  if (!l) empty_screens = g_slist_prepend(empty_screens, screen);
...
void panel_profile_load(void) {
  ...
  panel_profile_load_list(
      profile_settings, PANEL_GSETTINGS_TOPLEVELS,
      (PanelProfileLoadFunc)panel_profile_load_and_show_toplevel_startup,
      G_CALLBACK(panel_profile_toplevel_id_list_notify));
  panel_profile_load_list(profile_settings, PANEL_GSETTINGS_OBJECTS,
                          (PanelProfileLoadFunc)panel_profile_load_object,
...
static gboolean get_program_listing_setting(const char *key) {
  gboolean retval;
...
void panel_profile_set_show_program_list(gboolean show_program_list) {
  g_settings_set_boolean(profile_settings, "show-program-list",
                         show_program_list);
}
...
gboolean panel_profile_can_be_moved_freely(PanelToplevel *toplevel) {
  if (panel_lockdown_get_locked_down() ||
      !g_settings_is_writable(toplevel->settings,
  ...
  /* For expanded panels we don't really have to check
     x and y */
  if (panel_toplevel_get_expand(toplevel)) return TRUE;
  ...
  if (!g_settings_is_writable(toplevel->settings, PANEL_TOPLEVEL_X_RIGHT_KEY))
    return FALSE;
  if (!g_settings_is_writable(toplevel->settings,
