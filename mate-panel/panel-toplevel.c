...
struct _PanelToplevelPrivate {
  ...
  gint64 animation_start_time; /* monotonic start time in microseconds */
  GTimeSpan
      animation_duration_time; /* monotonic duration time in microseconds */
...
static void panel_toplevel_calculate_animation_end_geometry(
...
static void update_style_classes(PanelToplevel *toplevel) {
  GtkStyleContext *context;

  context = gtk_widget_get_style_context(GTK_WIDGET(toplevel));

  /*ensure the panel BG can always be themed*/
  /*Without this gtk3.19/20 cannot set the BG color and resetting the bg to
   * system is not immediately applied*/
...
GSList *panel_toplevel_list_toplevels(void) { return toplevel_list; }
...
static void panel_toplevel_get_monitor_geometry(PanelToplevel *toplevel,
                                                GdkRectangle *geom) {
...
static GdkCursorType panel_toplevel_grab_op_cursor(PanelToplevel *toplevel,
                                                   PanelGrabOpType grab_op) {
...
#ifdef HAVE_X11
static void panel_toplevel_init_resize_drag_offsets(PanelToplevel *toplevel,
                                                    PanelGrabOpType grab_op) {
...
static void panel_toplevel_begin_attached_move(PanelToplevel *toplevel,
                                               gboolean is_keyboard,
                                               guint32 time_) {
  PanelWidget *attached_panel_widget;

  attached_panel_widget =
      panel_toplevel_get_panel_widget(toplevel->priv->attach_toplevel);

  panel_widget_applet_drag_start(
      attached_panel_widget, toplevel->priv->attach_widget,
      is_keyboard ? PW_DRAG_OFF_CENTER : PW_DRAG_OFF_CURSOR, time_);
}

static void panel_toplevel_begin_grab_op(PanelToplevel *toplevel,
  ...
  if (panel_lockdown_get_locked_down()) return;
  ...
  if ((op_type == PANEL_GRAB_OP_RESIZE || op_type == PANEL_GRAB_OP_RESIZE_UP ||
  ...
  cursor_type =
      panel_toplevel_grab_op_cursor(toplevel, toplevel->priv->grab_op);

  cursor = gdk_cursor_new_for_display(gdk_display_get_default(), cursor_type);
  display = gdk_window_get_display(window);
  seat = gdk_display_get_default_seat(display);
  capabilities = GDK_SEAT_CAPABILITY_POINTER;
  if (grab_keyboard) capabilities |= GDK_SEAT_CAPABILITY_KEYBOARD;

  gdk_seat_grab(seat, window, capabilities, FALSE, cursor, NULL, NULL, NULL);
...
static void panel_toplevel_cancel_grab_op(PanelToplevel *toplevel,
  ...
  panel_toplevel_set_x(toplevel, toplevel->priv->orig_x,
                       toplevel->priv->orig_x_right,
                       toplevel->priv->orig_x_centered);
  panel_toplevel_set_y(toplevel, toplevel->priv->orig_y,
                       toplevel->priv->orig_y_bottom,
                       toplevel->priv->orig_y_centered);
}

static void panel_toplevel_resize_to_pointer(PanelToplevel *toplevel, int x,
                                             int y) {
  int new_size;
  int new_x, new_y;
  ...
  switch (toplevel->priv->grab_op) {
    case PANEL_GRAB_OP_RESIZE_UP:
      ...
      if (!toplevel->priv->y_centered &&
          (new_y + new_size / 2) > monitor_geom.height / 2)
    ...
    case PANEL_GRAB_OP_RESIZE_LEFT:
      ...
      if (!toplevel->priv->x_centered &&
          (new_x + new_size / 2) > monitor_geom.width / 2)
...
static void panel_toplevel_calc_new_orientation(PanelToplevel *toplevel,
...
static void panel_toplevel_move_to(PanelToplevel *toplevel, int new_x,
  ...
  if (toplevel->priv->orientation & PANEL_HORIZONTAL_MASK) {
    if (new_y <= display_min.y + snap_tolerance ||
      ...
      x_centered =
          abs(x - ((monitor_geom.width - width) / 2)) <= snap_tolerance;
    else
      x_centered = FALSE;
  } else {
    if (new_x <= display_min.x + snap_tolerance ||
      ...
      y_centered =
          abs(y - ((monitor_geom.height - height) / 2)) <= snap_tolerance;
    else
      y_centered = FALSE;
  }

  if (x_centered) x = (monitor_geom.width - width) / 2;
  if (y_centered) y = (monitor_geom.height - height) / 2;
...
static void panel_toplevel_move_to_pointer(PanelToplevel *toplevel,
                                           int pointer_x, int pointer_y) {
  int new_x, new_y;
...
static void panel_toplevel_rotate_to_pointer(PanelToplevel *toplevel,
                                             int pointer_x, int pointer_y) {
  int x_diff, y_diff;
  int x, y;
  int snap_tolerance;
...
#ifdef HAVE_X11
static gboolean panel_toplevel_warp_pointer_increment(PanelToplevel *toplevel,
                                                      int keyval,
                                                      int increment) {
  GdkScreen *screen;
  ...
  device = gdk_seat_get_pointer(gdk_display_get_default_seat(
      gtk_widget_get_display(GTK_WIDGET(root_window))));
  gdk_window_get_device_position(gtk_widget_get_window(GTK_WIDGET(root_window)),
                                 device, &new_x, &new_y, NULL);
...
static gboolean panel_toplevel_move_keyboard_floating(PanelToplevel *toplevel,
                                                      GdkEventKey *event) {
...
#define NORMAL_INCREMENT 10
  ...
  return panel_toplevel_warp_pointer_increment(toplevel, event->keyval,
                                               increment);
...
static gboolean panel_toplevel_move_keyboard_expanded(PanelToplevel *toplevel,
                                                      GdkEventKey *event) {
  PanelOrientation new_orientation;
...
static gboolean panel_toplevel_initial_resize_keypress(PanelToplevel *toplevel,
                                                       GdkEventKey *event) {
  PanelGrabOpType grab_op;
  ...
  switch (event->keyval) {
    case GDK_KEY_Up:
    case GDK_KEY_KP_Up:
      if (!(toplevel->priv->orientation & PANEL_HORIZONTAL_MASK)) return FALSE;
      grab_op = PANEL_GRAB_OP_RESIZE_UP;
      break;
    case GDK_KEY_Left:
    case GDK_KEY_KP_Left:
      if (!(toplevel->priv->orientation & PANEL_VERTICAL_MASK)) return FALSE;
...
static gboolean panel_toplevel_handle_grab_op_key_event(PanelToplevel *toplevel,
                                                        GdkEventKey *event) {
      ...
      switch (toplevel->priv->grab_op) {
        case PANEL_GRAB_OP_MOVE:
          if (toplevel->priv->expand) {
            retval = panel_toplevel_move_keyboard_expanded(toplevel, event);
          }
#ifdef HAVE_X11
          else if (GDK_IS_X11_DISPLAY(
                       gtk_widget_get_display(GTK_WIDGET(toplevel)))) {
            retval = panel_toplevel_move_keyboard_floating(toplevel, event);
          }
#endif /* HAVE_X11 */
          break;
        case PANEL_GRAB_OP_RESIZE:
        ...
        case PANEL_GRAB_OP_RESIZE_RIGHT:
          ...
          if (GDK_IS_X11_DISPLAY(gtk_widget_get_display(GTK_WIDGET(toplevel))))
            retval = panel_toplevel_warp_pointer_increment(toplevel,
                                                           event->keyval, 1);
#endif /* HAVE_X11 */
          break;
        default:
...
static gboolean panel_toplevel_handle_grab_op_motion_event(
    PanelToplevel *toplevel, GdkEventMotion *event) {
  switch (toplevel->priv->grab_op) {
    case PANEL_GRAB_OP_MOVE:
      if (toplevel->priv->expand)
        panel_toplevel_calc_new_orientation(toplevel, event->x_root,
                                            event->y_root);

      else if ((event->state & gtk_accelerator_get_default_mod_mask()) ==
               GDK_CONTROL_MASK)
        panel_toplevel_rotate_to_pointer(toplevel, event->x_root,
                                         event->y_root);

      else
        panel_toplevel_move_to_pointer(toplevel, event->x_root, event->y_root);
      return TRUE;
    case PANEL_GRAB_OP_RESIZE_UP:
    case PANEL_GRAB_OP_RESIZE_DOWN:
    case PANEL_GRAB_OP_RESIZE_LEFT:
...
static void panel_toplevel_calc_floating(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->x_right == -1)
    ...
    position_on_monitor.x =
        monitor_geom.width -
        (toplevel->priv->x_right + toplevel->priv->geometry.width);
  if (toplevel->priv->y_bottom == -1)
    position_on_monitor.y = toplevel->priv->y;
  else
    position_on_monitor.y =
        monitor_geom.height -
        (toplevel->priv->y_bottom + toplevel->priv->geometry.height);
  ...
  if (toplevel->priv->orientation & PANEL_HORIZONTAL_MASK)
    toplevel->priv->floating =
        ...
        (position_on_monitor.y <
         (monitor_geom.height - toplevel->priv->geometry.height -
          snap_tolerance));
  else
    toplevel->priv->floating =
        (position_on_monitor.x > snap_tolerance) &&
        (position_on_monitor.x <
         (monitor_geom.width - toplevel->priv->geometry.width -
          snap_tolerance));
}
...
static void panel_toplevel_hide_button_clicked(PanelToplevel *toplevel,
                                               GtkButton *button) {
  ...
  arrow_type =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "arrow-type"));
...
static void set_arrow_type(GtkImage *image, GtkArrowType arrow_type) {
  switch (arrow_type) {
    case GTK_ARROW_NONE:
    case GTK_ARROW_DOWN:
...
static GtkWidget *panel_toplevel_add_hide_button(PanelToplevel *toplevel,
                                                 GtkArrowType arrow_type,
                                                 int left, int top) {
  GtkWidget *button;
  AtkObject *obj;
  ...
  g_object_set_data(G_OBJECT(button), "arrow-type",
                    GINT_TO_POINTER(arrow_type));

  g_signal_connect_swapped(button, "clicked",
                           G_CALLBACK(panel_toplevel_hide_button_clicked),
                           toplevel);
  g_signal_connect_swapped(button, "button-press-event",
                           G_CALLBACK(panel_toplevel_hide_button_event),
                           toplevel);
...
static void panel_toplevel_update_hide_buttons_size(GtkWidget *button,
                                                    int panel_size) {
   ...
   * the previously loaded theme is NOT cleared from the css_provider. (reason
   * unknown) In other words, this works exactly, how we need it here. ALSO NOTE
   * that using gtk_css_provider_to_string () to convert the theme css data into
   * a string and then adding the custom css, then adding this updated css to a
   * css_provider with the gtk_css_provider_load_from_data () also works,
   * however some themes can't be easily converted to strings, beacuse of the
   * binary data they contain. This causes a delay of minutes in loading the
   * mate-panel, and so this approach is not viable. */
  if (panel_size < 30) {
    gtk_css_provider_load_from_data(
        css_provider,
        ".panel-button {min-height: 13px; min-width: 13px; padding: 0px;}", -1,
        NULL);
  }
...
static void panel_toplevel_update_hide_buttons(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->buttons_enabled) {
    ...
    panel_toplevel_update_hide_buttons_size(toplevel->priv->hide_button_top,
                                            panel_size);
    panel_toplevel_update_hide_buttons_size(toplevel->priv->hide_button_bottom,
                                            panel_size);
    panel_toplevel_update_hide_buttons_size(toplevel->priv->hide_button_left,
                                            panel_size);
    panel_toplevel_update_hide_buttons_size(toplevel->priv->hide_button_right,
                                            panel_size);

  } else {
    g_object_set(G_OBJECT(toplevel->priv->hide_button_top), "visible",
                 toplevel->priv->state == PANEL_STATE_HIDDEN_DOWN, NULL);
    g_object_set(G_OBJECT(toplevel->priv->hide_button_bottom), "visible",
                 toplevel->priv->state == PANEL_STATE_HIDDEN_UP, NULL);
    g_object_set(G_OBJECT(toplevel->priv->hide_button_left), "visible",
                 toplevel->priv->state == PANEL_STATE_HIDDEN_RIGHT, NULL);
    g_object_set(G_OBJECT(toplevel->priv->hide_button_right), "visible",
                 toplevel->priv->state == PANEL_STATE_HIDDEN_LEFT, NULL);
  }

  if (toplevel->priv->arrows_enabled) {
    gtk_widget_show(
        gtk_bin_get_child(GTK_BIN(toplevel->priv->hide_button_top)));
    gtk_widget_show(
        gtk_bin_get_child(GTK_BIN(toplevel->priv->hide_button_bottom)));
    gtk_widget_show(
        gtk_bin_get_child(GTK_BIN(toplevel->priv->hide_button_left)));
    gtk_widget_show(
        gtk_bin_get_child(GTK_BIN(toplevel->priv->hide_button_right)));
  ...
  if (toplevel->priv->buttons_enabled) {
    ...
    gtk_widget_set_size_request(toplevel->priv->hide_button_top, panel_size,
                                hb_size);
    gtk_widget_set_size_request(toplevel->priv->hide_button_bottom, panel_size,
                                hb_size);
    gtk_widget_set_size_request(toplevel->priv->hide_button_left, hb_size,
                                panel_size);
    gtk_widget_set_size_request(toplevel->priv->hide_button_right, hb_size,
                                panel_size);
  }
}
...
static gboolean panel_toplevel_contains_pointer(PanelToplevel *toplevel) {
  ...
  if (screen != gtk_window_get_screen(GTK_WINDOW(toplevel))) return FALSE;

  if (x == -1 || y == -1) return FALSE;

  if (x < toplevel->priv->geometry.x ||
      x >= (toplevel->priv->geometry.x + toplevel->priv->geometry.width) ||
      y < toplevel->priv->geometry.y ||
      y >= (toplevel->priv->geometry.y + toplevel->priv->geometry.height))
    return FALSE;

  return TRUE;
}

static inline int panel_toplevel_get_effective_auto_hide_size(
    PanelToplevel *toplevel) {
  int size;

  if (toplevel->priv->orientation & PANEL_HORIZONTAL_MASK)
    size = CLAMP(toplevel->priv->auto_hide_size, 1,
                 toplevel->priv->original_height / 2);
  else
    size = CLAMP(toplevel->priv->auto_hide_size, 1,
                 toplevel->priv->original_width / 2);

  /* paranoia */
  return (size <= 0) ? DEFAULT_AUTO_HIDE_SIZE : size;
}

static gboolean panel_toplevel_update_struts(PanelToplevel *toplevel,
  ...
  if (!toplevel->priv->updated_geometry_initial) return FALSE;
  ...
  if (GDK_IS_X11_DISPLAY(gtk_widget_get_display(GTK_WIDGET(toplevel)))) {
    if (strut > 0) {
      ...
      geometry_changed = panel_struts_register_strut(
          toplevel, screen, toplevel->priv->monitor, orientation, strut,
...
void panel_toplevel_update_edges(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->geometry.y > 0) edges |= PANEL_EDGE_TOP;

  if (toplevel->priv->geometry.x > 0) edges |= PANEL_EDGE_LEFT;
...
static const char *panel_toplevel_construct_description(
    PanelToplevel *toplevel) {
  if (toplevel->priv->attached) return _("Drawer");
...
static void panel_toplevel_update_description(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->description) g_free(toplevel->priv->description);
  toplevel->priv->description = g_strdup(description);

  if (!toplevel->priv->name)
    gtk_window_set_title(GTK_WINDOW(toplevel), toplevel->priv->description);

  panel_a11y_set_atk_name_desc(
      GTK_WIDGET(toplevel),
      toplevel->priv->name ? toplevel->priv->name : toplevel->priv->description,
      toplevel->priv->description);
}

static void panel_toplevel_update_attached_position(PanelToplevel *toplevel,
                                                    gboolean hidden, int *x,
                                                    int *y, int *w, int *h) {
  ...
  gtk_widget_get_allocation(GTK_WIDGET(toplevel->priv->attach_widget),
                            &attach_allocation);
  ...
  if (attach_box.x != -1) {
    gdk_window_get_origin(
        gtk_widget_get_window(GTK_WIDGET(toplevel->priv->attach_widget)),
        &x_origin, &y_origin);
...
static void panel_toplevel_update_normal_position(PanelToplevel *toplevel,
                                                  int *x, int *y, int *w,
                                                  int *h) {
  GdkRectangle monitor_geom;
  int width, height;
  ...
  *y = CLAMP(*y, 0, monitor_geom.height - height);
  ...
  if (toplevel->priv->x <= snap_tolerance && toplevel->priv->x_right == -1 &&
      !toplevel->priv->x_centered)
    *x = 0;
  ...
  if (toplevel->priv->y <= snap_tolerance && toplevel->priv->y_bottom == -1 &&
      !toplevel->priv->y_centered)
    *y = 0;
...
static void panel_toplevel_update_auto_hide_position(
    PanelToplevel *toplevel, int *x, int *y, int *w, int *h,
 ...
 */
static void panel_toplevel_update_hidden_position(PanelToplevel *toplevel,
                                                  int *x, int *y, int *w,
                                                  int *h) {
  int width, height;
  int min_hide_size;
 ...
 */
static int get_delta(int src, int dest, GTimeSpan elapsed_time,
                     GTimeSpan duration_time) {
  double x, percentage;

  if (abs(dest - src) <= 1 || elapsed_time >= duration_time) return dest - src;

  /* The cubic is: p(x) = (-2) x^2 (x-1.5) */
  /* running p(p(x)) to make it more "pronounced",
   * effectively making it a ninth-degree polynomial */

  x = (double)elapsed_time / duration_time;
  x = -2 * (x * x) * (x - 1.5);
  /* run it again */
...
static void panel_toplevel_update_animating_position(PanelToplevel *toplevel) {
  GTimeSpan animation_elapsed_time;
  int deltax, deltay, deltaw = 0, deltah = 0;
  int monitor_offset_x, monitor_offset_y;

  if ((toplevel->priv->animation_start_time <= 0) ||
      (toplevel->priv->animation_duration_time <= 0))
    return;

  animation_elapsed_time =
      g_get_monotonic_time() - toplevel->priv->animation_start_time;
  ...
  if (toplevel->priv->animation_end_width != -1)
    deltaw = get_delta(
        toplevel->priv->geometry.width, toplevel->priv->animation_end_width,
        animation_elapsed_time, toplevel->priv->animation_duration_time);

  if (toplevel->priv->animation_end_height != -1)
    deltah = get_delta(
        toplevel->priv->geometry.height, toplevel->priv->animation_end_height,
        animation_elapsed_time, toplevel->priv->animation_duration_time);

  deltax = get_delta(toplevel->priv->geometry.x - monitor_offset_x,
                     toplevel->priv->animation_end_x, animation_elapsed_time,
                     toplevel->priv->animation_duration_time);

  deltay = get_delta(toplevel->priv->geometry.y - monitor_offset_y,
                     toplevel->priv->animation_end_y, animation_elapsed_time,
                     toplevel->priv->animation_duration_time);

  if (deltaw != 0 && abs(deltaw) > abs(deltax)) deltax = deltaw;
  if (deltah != 0 && abs(deltah) > abs(deltay)) deltay = deltah;
  ...
  if (toplevel->priv->geometry.x - monitor_offset_x ==
          toplevel->priv->animation_end_x &&
      toplevel->priv->geometry.y - monitor_offset_y ==
...
static void panel_toplevel_update_expanded_position(PanelToplevel *toplevel) {
  GdkRectangle monitor_geom;
  int x, y;
  int x_right, y_bottom;
  int monitor;

  if (!toplevel->priv->expand) return;
  ...
  switch (toplevel->priv->orientation) {
    ...
    case PANEL_ORIENTATION_BOTTOM:
      x = monitor_geom.x;
      y = monitor_geom.y + monitor_geom.height -
...
static void panel_toplevel_update_position(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->position_centered) {
    ...
    if (!toplevel->priv->x_centered) {
      ...
      if ((toplevel->priv->x + toplevel->priv->geometry.width / 2) >
          monitor_geom.width / 2)
        x_right = monitor_geom.width -
                  (toplevel->priv->x + toplevel->priv->geometry.width);
      else
        x_right = -1;
      if (toplevel->priv->x_right != x_right) {
        toplevel->priv->x_right = x_right;
    ...
    if (!toplevel->priv->y_centered) {
      ...
      if ((toplevel->priv->y + toplevel->priv->geometry.height / 2) >
          monitor_geom.height / 2)
        y_bottom = monitor_geom.height -
                   (toplevel->priv->y + toplevel->priv->geometry.height);
      else
        y_bottom = -1;
      if (toplevel->priv->y_bottom != y_bottom) {
        toplevel->priv->y_bottom = y_bottom;
  ...
  panel_toplevel_calc_floating(
      toplevel); /* FIXME should probably be done after
                    panel_toplevel_update_normal_position() too */

  if (toplevel->priv->x_right == -1)
    x = toplevel->priv->x;
  else
    x = monitor_geom.width -
        (toplevel->priv->x_right + toplevel->priv->geometry.width);
  if (toplevel->priv->y_bottom == -1)
    y = toplevel->priv->y;
  else
    y = monitor_geom.height -
        (toplevel->priv->y_bottom + toplevel->priv->geometry.height);
  ...
  if (w != -1) toplevel->priv->geometry.width = w;
  if (h != -1) toplevel->priv->geometry.height = h;
  ...
  if (panel_background_effective_type(background) == PANEL_BACK_NONE) {
    ...
    context =
        gtk_widget_get_style_context(GTK_WIDGET(toplevel->priv->inner_frame));
    gtk_style_context_get_padding(context, state, &padding);
    geometry = &toplevel->priv->geometry;

    if (x <= padding.left && x > 0 && !toplevel->priv->x_centered) x = 0;

    if (y <= padding.top && y > 0 && !toplevel->priv->y_centered) y = 0;
...
static int calculate_minimum_height(GtkWidget *widget,
                                    PanelOrientation orientation) {
  GtkStateFlags state;
  ...
  gtk_style_context_get(style_context, state, GTK_STYLE_PROPERTY_FONT,
                        &font_desc, NULL);

  pango_context = gtk_widget_get_pango_context(widget);
  metrics = pango_context_get_metrics(
      pango_context, font_desc, pango_context_get_language(pango_context));
...
static int panel_toplevel_update_size_from_hints(PanelToplevel *toplevel,
  ...
  /* Scale down the size so that the panel only takes what it needs for the
   * applets it has. */
  total_size =
      non_panel_widget_size + (requisition_size / toplevel->priv->scale);

  nb_size_hints = toplevel->priv->panel_widget->nb_applets_size_hints;
  if (nb_size_hints <= 0) return total_size;
  ...
  if (total_size > monitor_size) return monitor_size;
  ...
  while (full_hints != nb_size_hints && total_size < monitor_size) {
    ...
    bonus = (monitor_size - total_size) / (nb_size_hints - full_hints);
    extra_bonus = (monitor_size - total_size) % (nb_size_hints - full_hints);
    full_hints = 0;
    ...
    for (i = 0; i < nb_size_hints; i++) {
      ...
      while (using_hint[i].index > 0 &&
             applets_hints[i].hints[using_hint[i].index - 1] <
                 using_hint[i].size + current_bonus) {
        new_size = applets_hints[i].hints[using_hint[i].index - 1];
        current_bonus = using_hint[i].size + current_bonus - new_size;
        total_size = total_size - using_hint[i].size + new_size;
      ...
      if (extra_bonus > 0) {
        ...
        if (new_size > using_hint[i].size) {
          total_size += (new_size - using_hint[i].size);
          extra_bonus -= (new_size - using_hint[i].size);
          using_hint[i].size = new_size;
        }
      }
...
static void panel_toplevel_update_size(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->animating) return;
  ...
  if (toplevel->priv->orientation & PANEL_HORIZONTAL_MASK) {
    ...
    if (toplevel->priv->expand)
      ...
      if (!toplevel->priv->attached)
        ...
        if (panel_toplevel_get_orientation(toplevel->priv->attach_toplevel) ==
            PANEL_ORIENTATION_LEFT)
          max_width = monitor_geom.width - toplevel->priv->geometry.x;
        else
          max_width =
              toplevel->priv->geometry.x + toplevel->priv->geometry.width;
      }
      ...
      width = panel_toplevel_update_size_from_hints(
          toplevel, requisition->width, max_width, non_panel_widget_size);
    }
    ...
    if (toplevel->priv->expand)
      ...
      if (!toplevel->priv->attached)
        ...
        if (panel_toplevel_get_orientation(toplevel->priv->attach_toplevel) ==
          ...
          max_height = monitor_geom.height - toplevel->priv->geometry.y;
        else
          max_height =
              toplevel->priv->geometry.y + toplevel->priv->geometry.height;
      }
      ...
      height = panel_toplevel_update_size_from_hints(
          toplevel, requisition->height, max_height, non_panel_widget_size);
    }

    height = MAX(MINIMUM_WIDTH, height);
  }

  if (toplevel->priv->edges & PANEL_EDGE_TOP) height += padding.top;
  if (toplevel->priv->edges & PANEL_EDGE_BOTTOM) height += padding.bottom;
  if (toplevel->priv->edges & PANEL_EDGE_LEFT) width += padding.left;
  if (toplevel->priv->edges & PANEL_EDGE_RIGHT) width += padding.right;
...
static void panel_toplevel_update_geometry(PanelToplevel *toplevel,
                                           GtkRequisition *requisition) {
  ...
  if (GDK_IS_X11_DISPLAY(gtk_widget_get_display(GTK_WIDGET(toplevel)))) {
    if (toplevel->priv->state == PANEL_STATE_NORMAL ||
        toplevel->priv->state != PANEL_STATE_AUTO_HIDDEN) {
      panel_struts_update_toplevel_geometry(
          toplevel, &toplevel->priv->geometry.x, &toplevel->priv->geometry.y,
          &toplevel->priv->geometry.width, &toplevel->priv->geometry.height);
    } else {
      panel_struts_update_toplevel_geometry(
          toplevel, &toplevel->priv->geometry.x, &toplevel->priv->geometry.y,
          NULL, NULL);
...
static void panel_toplevel_attach_widget_destroyed(PanelToplevel *toplevel) {
  panel_toplevel_detach(toplevel);
}

static gboolean panel_toplevel_attach_widget_configure(
    PanelToplevel *toplevel) {
  gtk_widget_queue_resize(GTK_WIDGET(toplevel));
...
static void panel_toplevel_update_attach_orientation(PanelToplevel *toplevel) {
  PanelOrientation attach_orientation;
  PanelOrientation orientation;
...
static void panel_toplevel_attach_widget_parent_set(PanelToplevel *toplevel,
                                                    GtkWidget *previous_parent,
                                                    GtkWidget *attach_widget) {
  GtkWidget *panel_widget;

  panel_widget = gtk_widget_get_parent(GTK_WIDGET(attach_widget));
  if (!panel_widget) return;
...
static void panel_toplevel_attach_toplevel_hiding(PanelToplevel *toplevel) {
...
static void panel_toplevel_attach_toplevel_unhiding(PanelToplevel *toplevel) {
  if (!toplevel->priv->attach_hidden) return;
...
static void panel_toplevel_reverse_arrow(PanelToplevel *toplevel,
                                         GtkWidget *button) {
  GtkArrowType arrow_type;
...
static void panel_toplevel_reverse_arrows(PanelToplevel *toplevel) {
...
static void panel_toplevel_connect_attached(PanelToplevel *toplevel) {
  ...
  signals[i++] =
      g_signal_connect_swapped(toplevel->priv->attach_widget, "show",
                               G_CALLBACK(gtk_widget_show), toplevel);
  signals[i++] =
      g_signal_connect_swapped(toplevel->priv->attach_widget, "hide",
...
void panel_toplevel_attach_to_widget(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->attached) panel_toplevel_disconnect_attached(toplevel);
...
void panel_toplevel_detach(PanelToplevel *toplevel) {
  ...
  if (!toplevel->priv->attached) return;
...
static gboolean panel_toplevel_popup_panel_menu(PanelToplevel *toplevel) {
  gboolean retval = FALSE;
...
static gboolean panel_toplevel_toggle_expand(PanelToplevel *toplevel) {
  panel_toplevel_set_expand(toplevel, !toplevel->priv->expand);

  return TRUE;
}

static gboolean panel_toplevel_expand(PanelToplevel *toplevel) {
  panel_toplevel_set_expand(toplevel, TRUE);

  return TRUE;
}

static gboolean panel_toplevel_unexpand(PanelToplevel *toplevel) {
  panel_toplevel_set_expand(toplevel, FALSE);

  return TRUE;
}

static gboolean panel_toplevel_toggle_hidden(PanelToplevel *toplevel) {
  if (toplevel->priv->state == PANEL_STATE_NORMAL)
...
static gboolean panel_toplevel_begin_move(PanelToplevel *toplevel) {
  if (toplevel->priv->grab_op != PANEL_GRAB_OP_NONE) return FALSE;

  panel_toplevel_begin_grab_op(toplevel, PANEL_GRAB_OP_MOVE, TRUE,
                               GDK_CURRENT_TIME);

  return TRUE;
}

static gboolean panel_toplevel_begin_resize(PanelToplevel *toplevel) {
  if (toplevel->priv->grab_op != PANEL_GRAB_OP_NONE) return FALSE;

  panel_toplevel_begin_grab_op(toplevel, PANEL_GRAB_OP_RESIZE, TRUE,
                               GDK_CURRENT_TIME);

  return TRUE;
}

static void panel_toplevel_move_resize_window(PanelToplevel *toplevel,
  ...
  if (move && resize)
    gdk_window_move_resize(
        gtk_widget_get_window(widget), toplevel->priv->geometry.x,
        toplevel->priv->geometry.y, toplevel->priv->geometry.width,
        toplevel->priv->geometry.height);
  else if (move)
    gdk_window_move(gtk_widget_get_window(widget), toplevel->priv->geometry.x,
...
static void panel_toplevel_initially_hide(PanelToplevel *toplevel) {
  if (!toplevel->priv->attached) {
...
static void set_background_default_style(GtkWidget *widget) {
  PanelToplevel *toplevel;
  GtkStyleContext *context;
  GtkStateFlags state;
  GdkRGBA *bg_color;
  ...
  gtk_style_context_get(context, state, "background-color", &bg_color,
                        "background-image", &bg_image, NULL);

  panel_background_set_default_style(&toplevel->background, bg_color, bg_image);

  if (bg_color) gdk_rgba_free(bg_color);

  if (bg_image) cairo_pattern_destroy(bg_image);
}

static void panel_toplevel_realize(GtkWidget *widget) {
  PanelToplevel *toplevel;
  GdkScreen *screen;
  GdkVisual *visual;
  GdkWindow *window;
...
static void panel_toplevel_disconnect_timeouts(PanelToplevel *toplevel) {
...
static void panel_toplevel_unrealize(GtkWidget *widget) {
  PanelToplevel *toplevel;
  toplevel = PANEL_TOPLEVEL(widget);
...
static void panel_toplevel_dispose(GObject *widget) {
  ...
  if (toplevel->background_settings) {
    g_signal_handlers_disconnect_by_data(toplevel->background_settings,
                                         toplevel);
  ...
  if (toplevel->priv->gtk_settings) {
    g_signal_handlers_disconnect_by_func(
        toplevel->priv->gtk_settings,
        G_CALLBACK(panel_toplevel_drag_threshold_changed), toplevel);
...
static void panel_toplevel_check_resize(GtkContainer *container) {
...
static void panel_toplevel_size_request(GtkWidget *widget,
                                        GtkRequisition *requisition) {
  PanelToplevel *toplevel;
  GtkBin *bin;
  GtkWidget *child;
...
static void panel_toplevel_get_preferred_width(GtkWidget *widget,
                                               gint *minimum_width,
                                               gint *natural_width) {
  GtkRequisition req;
...
static void panel_toplevel_get_preferred_height(GtkWidget *widget,
                                                gint *minimum_height,
                                                gint *natural_height) {
  GtkRequisition req;
  panel_toplevel_size_request(widget, &req);
...
static void set_background_region(PanelToplevel *toplevel) {
  GtkWidget *widget;
  GdkWindow *window;
  gint origin_x;
  gint origin_y;
  ...
  panel_background_change_region(&toplevel->background, orientation, origin_x,
                                 origin_y, allocation.width, allocation.height);
}

static void panel_toplevel_size_allocate(GtkWidget *widget,
                                         GtkAllocation *allocation) {
  PanelToplevel *toplevel = (PanelToplevel *)widget;
  ...
  if (toplevel->priv->edges & PANEL_EDGE_RIGHT) challoc.width -= padding.right;
  ...
  if (gtk_widget_get_mapped(widget) &&
      (challoc.x != child_allocation.x || challoc.y != child_allocation.y ||
...
static gboolean panel_toplevel_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (!gtk_widget_is_drawable(widget)) return retval;
  ...
  if (toplevel->priv->orientation & PANEL_HORIZONTAL_MASK) {
    ...
    if (edges & PANEL_EDGE_BOTTOM) height -= padding.bottom;
    if (edges & PANEL_EDGE_LEFT) x += padding.left;
    ...
    if (edges & PANEL_EDGE_RIGHT) x -= padding.right;
    ...
    if (edges & PANEL_EDGE_RIGHT) width -= padding.right;
    if (edges & PANEL_EDGE_TOP) y += padding.top;
    ...
    if (edges & PANEL_EDGE_BOTTOM) y -= padding.bottom;
...
static gboolean panel_toplevel_button_press_event(GtkWidget *widget,
                                                  GdkEventButton *event) {
  PanelToplevel *toplevel;
  ...
  if (toplevel->priv->animating) return FALSE;
  ...
  gtk_widget_translate_coordinates(event_widget, widget, event->x, event->y,
                                   &toplevel->priv->drag_offset_x,
                                   &toplevel->priv->drag_offset_y);

  panel_toplevel_begin_grab_op(toplevel, PANEL_GRAB_OP_MOVE, FALSE,
                               event->time);

  return TRUE;
}

static gboolean panel_toplevel_button_release_event(GtkWidget *widget,
                                                    GdkEventButton *event) {
  PanelToplevel *toplevel;
  ...
  if (toplevel->priv->grab_is_keyboard) return FALSE;
...
static gboolean panel_toplevel_configure_event(GtkWidget *widget,
...
static gboolean panel_toplevel_key_press_event(GtkWidget *widget,
                                               GdkEventKey *event) {
  PanelToplevel *toplevel = (PanelToplevel *)widget;
  ...
  if (GTK_WIDGET_CLASS(panel_toplevel_parent_class)->key_press_event)
    return GTK_WIDGET_CLASS(panel_toplevel_parent_class)
        ->key_press_event(widget, event);

  return FALSE;
}

static void panel_toplevel_state_flags_changed(GtkWidget *widget,
                                               GtkStateFlags previous_state) {
  GTK_WIDGET_CLASS(panel_toplevel_parent_class)
      ->state_flags_changed(widget, previous_state);

  set_background_default_style(widget);
}

static gboolean panel_toplevel_motion_notify_event(GtkWidget *widget,
                                                   GdkEventMotion *event) {
  if (gdk_event_get_screen((GdkEvent *)event) ==
      gtk_window_get_screen(GTK_WINDOW(widget)))
    return panel_toplevel_handle_grab_op_motion_event(PANEL_TOPLEVEL(widget),
                                                      event);
  else
    return FALSE;
}

static gboolean panel_toplevel_animation_timeout(PanelToplevel *toplevel) {
  gtk_widget_queue_resize(GTK_WIDGET(toplevel));
...
static GTimeSpan panel_toplevel_get_animation_time(PanelToplevel *toplevel) {
  /* The number of seconds to complete the animation.
   */
#define ANIMATION_TIME_FAST 400
#define ANIMATION_TIME_MEDIUM 1200
#define ANIMATION_TIME_SLOW 2000
...
static void panel_toplevel_calculate_animation_end_geometry(
  ...
  if (toplevel->priv->state == PANEL_STATE_NORMAL ||
      (!toplevel->priv->initial_animation_done && !toplevel->priv->auto_hide))
    panel_toplevel_update_normal_position(
        toplevel, &toplevel->priv->animation_end_x,
        &toplevel->priv->animation_end_y, &toplevel->priv->animation_end_width,
    ...
    panel_toplevel_update_auto_hide_position(
        toplevel, &toplevel->priv->animation_end_x,
        &toplevel->priv->animation_end_y, &toplevel->priv->animation_end_width,
        &toplevel->priv->animation_end_height, TRUE);
  else
    panel_toplevel_update_hidden_position(
        toplevel, &toplevel->priv->animation_end_x,
        &toplevel->priv->animation_end_y, &toplevel->priv->animation_end_width,
        &toplevel->priv->animation_end_height);
}

static void panel_toplevel_start_animation(PanelToplevel *toplevel) {
  GtkRequisition requisition;
  int deltax, deltay, deltaw = 0, deltah = 0;
  int cur_x = -1, cur_y = -1;
  ...
  gdk_window_get_origin(gtk_widget_get_window(GTK_WIDGET(toplevel)), &cur_x,
                        &cur_y);
  ...
  toplevel->priv->animation_start_time = g_get_monotonic_time();
  toplevel->priv->animation_duration_time =
      panel_toplevel_get_animation_time(toplevel);

  if (!toplevel->priv->animation_timeout)
    toplevel->priv->animation_timeout = g_timeout_add(
        20, (GSourceFunc)panel_toplevel_animation_timeout, toplevel);
}

void panel_toplevel_hide(PanelToplevel *toplevel, gboolean auto_hide,
                         gint direction) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  if (toplevel->priv->state != PANEL_STATE_NORMAL) return;
...
static gboolean panel_toplevel_auto_hide_timeout_handler(
    PanelToplevel *toplevel) {
  g_return_val_if_fail(PANEL_IS_TOPLEVEL(toplevel), FALSE);
   ...
   */
  if (toplevel->priv->animating) return TRUE;
...
static gboolean panel_toplevel_auto_unhide_timeout_handler(
    PanelToplevel *toplevel) {
  g_return_val_if_fail(PANEL_IS_TOPLEVEL(toplevel), FALSE);
  ...
  if (!toplevel->priv->animate) toplevel->priv->initial_animation_done = TRUE;

  /* initial animation for auto-hidden panels: we need to unhide and hide
   * again to get at the right size */
  if (!toplevel->priv->initial_animation_done && toplevel->priv->auto_hide) {
    toplevel->priv->unhide_timeout = 0;
    panel_toplevel_unhide(toplevel);
    panel_toplevel_hide(toplevel, TRUE, -1);
    return FALSE;
  ...
  if (!panel_toplevel_contains_pointer(toplevel) && toplevel->priv->auto_hide) {
    toplevel->priv->unhide_timeout = 0;
    return FALSE;
  }
...
void panel_toplevel_queue_auto_hide(PanelToplevel *toplevel) {
  ...
  if (!toplevel->priv->auto_hide || panel_toplevel_contains_pointer(toplevel) ||
  ...
  if (toplevel->priv->hide_delay > 0)
    toplevel->priv->hide_timeout = g_timeout_add(
        toplevel->priv->hide_delay,
        (GSourceFunc)panel_toplevel_auto_hide_timeout_handler, toplevel);
  else
    toplevel->priv->hide_timeout = g_idle_add(
        (GSourceFunc)panel_toplevel_auto_hide_timeout_handler, toplevel);
}
...
void panel_toplevel_queue_auto_unhide(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->unhide_timeout) return;

  if (toplevel->priv->hide_timeout)
    g_source_remove(toplevel->priv->hide_timeout);
  toplevel->priv->hide_timeout = 0;

  if (toplevel->priv->state != PANEL_STATE_AUTO_HIDDEN) return;

  if (toplevel->priv->unhide_delay > 0)
    toplevel->priv->unhide_timeout = g_timeout_add(
        toplevel->priv->unhide_delay,
        (GSourceFunc)panel_toplevel_auto_unhide_timeout_handler, toplevel);
  else
    toplevel->priv->unhide_timeout = g_idle_add(
        (GSourceFunc)panel_toplevel_auto_unhide_timeout_handler, toplevel);
}
...
void panel_toplevel_queue_initial_unhide(PanelToplevel *toplevel) {
  ...
  if (toplevel->priv->initial_animation_done) return;
...
static gboolean panel_toplevel_enter_notify_event(GtkWidget *widget,
...
static gboolean panel_toplevel_leave_notify_event(GtkWidget *widget,
                                                  GdkEventCrossing *event) {
  PanelToplevel *toplevel;
...
static gboolean panel_toplevel_focus_in_event(GtkWidget *widget,
                                              GdkEventFocus *event) {
...
static gboolean panel_toplevel_focus_out_event(GtkWidget *widget,
                                               GdkEventFocus *event) {
  PanelToplevel *toplevel = PANEL_TOPLEVEL(widget);
  ...
  if (toplevel->priv->auto_hide) panel_toplevel_queue_auto_hide(toplevel);
...
static void panel_toplevel_style_updated(GtkWidget *widget) {
...
static void panel_toplevel_drag_threshold_changed(PanelToplevel *toplevel) {
  ...
  threshold = 0;
  g_object_get(G_OBJECT(toplevel->priv->gtk_settings), "gtk-dnd-drag-threshold",
               &threshold, NULL);
...
static void panel_toplevel_update_gtk_settings(PanelToplevel *toplevel) {
  if (toplevel->priv->gtk_settings)
    g_signal_handlers_disconnect_by_func(
        toplevel->priv->gtk_settings,
        G_CALLBACK(panel_toplevel_drag_threshold_changed), toplevel);
  ...
  toplevel->priv->gtk_settings =
      gtk_widget_get_settings(GTK_WIDGET(toplevel->priv->panel_widget));

  g_signal_connect_swapped(
      toplevel->priv->gtk_settings, "notify::gtk-dnd-drag-threshold",
      G_CALLBACK(panel_toplevel_drag_threshold_changed), toplevel);

  panel_toplevel_drag_threshold_changed(toplevel);
}

static void panel_toplevel_screen_changed(GtkWidget *widget,
...
static void panel_toplevel_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  PanelToplevel *toplevel;
  ...
  switch (prop_id) {
    ...
    case PROP_SIZE:
      ...
      break;
    case PROP_X:
      panel_toplevel_set_x(toplevel, g_value_get_int(value),
                           toplevel->priv->x_right, toplevel->priv->x_centered);
      break;
    case PROP_X_RIGHT:
      panel_toplevel_set_x(toplevel, toplevel->priv->x, g_value_get_int(value),
                           toplevel->priv->x_centered);
      break;
    case PROP_X_CENTERED:
      panel_toplevel_set_x(toplevel, toplevel->priv->x, toplevel->priv->x_right,
                           g_value_get_boolean(value));
      break;
    case PROP_Y:
      panel_toplevel_set_y(toplevel, g_value_get_int(value),
                           toplevel->priv->y_bottom,
                           toplevel->priv->y_centered);
      break;
    case PROP_Y_BOTTOM:
      panel_toplevel_set_y(toplevel, toplevel->priv->y, g_value_get_int(value),
                           toplevel->priv->y_centered);
      break;
    case PROP_Y_CENTERED:
      panel_toplevel_set_y(toplevel, toplevel->priv->y,
                           toplevel->priv->y_bottom,
                           g_value_get_boolean(value));
      break;
    case PROP_MONITOR:
...
static void panel_toplevel_get_property(GObject *object, guint prop_id,
...
static GObject *panel_toplevel_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  GObject *object;

  object =
      G_OBJECT_CLASS(panel_toplevel_parent_class)
...
static void panel_toplevel_finalize(GObject *object) {
...
static void panel_toplevel_class_init(PanelToplevelClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("name", "Name", "The name of this panel", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_SETTINGS_PATH,
      g_param_spec_string("settings-path", "GSettings path",
                          "The GSettings path used for this panel", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_EXPAND,
      g_param_spec_boolean("expand", "Expand",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("orientation", "Orientation",
                        "The orientation of the panel", PANEL_TYPE_ORIENTATION,
                        PANEL_ORIENTATION_TOP,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int(
          "size", "Size", "The height (or width when vertical) of the panel", 0,
          G_MAXINT, DEFAULT_SIZE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_X,
      g_param_spec_int("x", "X position", "The X position of the panel", 0,
                       G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_X_RIGHT,
      g_param_spec_int(
          "x-right", "X position, from the right",
          "The X position of the panel, starting from the right of the screen",
          -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_X_CENTERED,
      g_param_spec_boolean("x-centered", "X centered",
                           "The x co-ordinate is relative to center screen",
                           FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_Y,
      g_param_spec_int("y", "Y position", "The Y position of the panel", 0,
                       G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_Y_BOTTOM,
      g_param_spec_int(
          "y_bottom", "Y position, from the bottom",
          "The Y position of the panel, starting from the bottom of the screen",
          -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_Y_CENTERED,
      g_param_spec_boolean("y-centered", "Y centered",
                           "The y co-ordinate is relative to center screen",
                           FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_MONITOR,
      g_param_spec_int(
          "monitor", "Xinerama monitor",
          "The monitor (in terms of Xinerama) which the panel is on", 0,
          G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_AUTOHIDE,
      g_param_spec_boolean(
          "auto-hide", "Auto hide",
          "Automatically hide the panel when the mouse leaves the panel", FALSE,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_HIDE_DELAY,
      g_param_spec_int(
          "hide-delay", "Hide delay",
          "The number of milliseconds to delay before automatically hiding", 0,
          G_MAXINT, DEFAULT_HIDE_DELAY, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_UNHIDE_DELAY,
      g_param_spec_int(
          "unhide-delay", "Un-hide delay",
          "The number of milliseconds to delay before automatically un-hiding",
          0, G_MAXINT, DEFAULT_UNHIDE_DELAY,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_AUTOHIDE_SIZE,
      g_param_spec_int("auto-hide-size", "Auto-hide size",
                       "The number of pixels visible when the panel has been "
                       "automatically hidden",
                       1, G_MAXINT, DEFAULT_AUTO_HIDE_SIZE,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_ANIMATE,
      g_param_spec_boolean("animate", "Animate",
                           "Enable hiding/showing animations", TRUE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_ANIMATION_SPEED,
      g_param_spec_enum("animation-speed", "Animation Speed",
                        "The speed at which to animate panel hiding/showing",
                        PANEL_TYPE_ANIMATION_SPEED, PANEL_ANIMATION_MEDIUM,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_BUTTONS_ENABLED,
      g_param_spec_boolean("buttons-enabled", "Buttons Enabled",
                           "Enable hide/show buttons", TRUE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_ARROWS_ENABLED,
      g_param_spec_boolean("arrows-enabled", "Arrows Enabled",
                           "Enable arrows on hide/show buttons", TRUE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_int("arrow-size", "Arrow Size",
                       "The size of the arrows on the hide/show buttons", 0,
                       G_MAXINT, DEFAULT_ARROW_SIZE, G_PARAM_READABLE));

  toplevel_signals[HIDE_SIGNAL] =
      g_signal_new("hiding", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, hiding), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  toplevel_signals[UNHIDE_SIGNAL] =
      g_signal_new("unhiding", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, unhiding), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  toplevel_signals[POPUP_PANEL_MENU_SIGNAL] =
      g_signal_new("popup-panel-menu", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, popup_panel_menu), NULL,
                   NULL, panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);

  toplevel_signals[TOGGLE_EXPAND_SIGNAL] =
      g_signal_new("toggle-expand", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, toggle_expand), NULL,
                   NULL, panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);

  toplevel_signals[EXPAND_SIGNAL] =
      g_signal_new("expand", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, expand), NULL, NULL,
                   panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);

  toplevel_signals[UNEXPAND_SIGNAL] =
      g_signal_new("unexpand", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, unexpand), NULL, NULL,
                   panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);

  toplevel_signals[TOGGLE_HIDDEN_SIGNAL] =
      g_signal_new("toggle-hidden", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, toggle_hidden), NULL,
                   NULL, panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);

  toplevel_signals[BEGIN_MOVE_SIGNAL] =
      g_signal_new("begin-move", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, begin_move), NULL, NULL,
                   panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);

  toplevel_signals[BEGIN_RESIZE_SIGNAL] =
      g_signal_new("begin-resize", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(PanelToplevelClass, begin_resize), NULL,
                   NULL, panel_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);
...
static void panel_toplevel_setup_widgets(PanelToplevel *toplevel) {
  GtkWidget *container;

  toplevel->priv->grid = gtk_grid_new();

  toplevel->priv->hide_button_top =
      panel_toplevel_add_hide_button(toplevel, GTK_ARROW_UP, 1, 0);
  toplevel->priv->hide_button_bottom =
      panel_toplevel_add_hide_button(toplevel, GTK_ARROW_DOWN, 1, 2);
  toplevel->priv->hide_button_left =
      panel_toplevel_add_hide_button(toplevel, GTK_ARROW_LEFT, 0, 1);
  toplevel->priv->hide_button_right =
      panel_toplevel_add_hide_button(toplevel, GTK_ARROW_RIGHT, 2, 1);
  ...
  gtk_grid_attach(GTK_GRID(toplevel->priv->grid),
                  GTK_WIDGET(toplevel->priv->inner_frame), 1, 1, 1, 1);
  gtk_widget_show(GTK_WIDGET(toplevel->priv->inner_frame));

  container =
      panel_widget_new(toplevel, !toplevel->priv->expand,
                       (toplevel->priv->orientation & PANEL_HORIZONTAL_MASK)
...
static void background_changed(PanelBackground *background,
                               PanelToplevel *toplevel) {
  panel_toplevel_update_edges(toplevel);
  panel_widget_emit_background_changed(toplevel->priv->panel_widget);
}

static void panel_toplevel_init(PanelToplevel *toplevel) {
  GtkWidget *widget;
  int i;
  ...
  toplevel->priv->snap_tolerance =
      DEFAULT_DND_THRESHOLD * SNAP_TOLERANCE_FACTOR;
  toplevel->priv->gtk_settings = NULL;
...
static void panel_toplevel_update_name(PanelToplevel *toplevel) {
  char *title;
  ...
  panel_a11y_set_atk_name_desc(GTK_WIDGET(toplevel), title,
                               toplevel->priv->description);
}

void panel_toplevel_set_name(PanelToplevel *toplevel, const char *name) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  if (!toplevel->priv->name && (!name || !name[0])) return;
...
void panel_toplevel_set_expand(PanelToplevel *toplevel, gboolean expand) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));
  ...
  if (toplevel->priv->expand == expand) return;
...
void panel_toplevel_set_orientation(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->orientation == orientation) return;
  ...
  if (toplevel->priv->x_centered && (orientation & PANEL_VERTICAL_MASK)) {
    toplevel->priv->x_centered = FALSE;
    toplevel->priv->x =
        (monitor_geom.width - toplevel->priv->geometry.width) / 2;
    g_object_notify(G_OBJECT(toplevel), "x");
  ...
  if (toplevel->priv->y_centered && (orientation & PANEL_HORIZONTAL_MASK)) {
    toplevel->priv->y_centered = FALSE;
    toplevel->priv->y =
        (monitor_geom.height - toplevel->priv->geometry.height) / 2;
    g_object_notify(G_OBJECT(toplevel), "y");
...
void panel_toplevel_set_size(PanelToplevel *toplevel, int size) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));
  g_return_if_fail(size >= 0);

  if (toplevel->priv->size == size) return;
...
void panel_toplevel_set_auto_hide_size(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->auto_hide_size == auto_hide_size) return;
...
void panel_toplevel_set_x(PanelToplevel *toplevel, int x, int x_right,
                          gboolean x_centered) {
  ...
  if (changed) gtk_widget_queue_resize(GTK_WIDGET(toplevel));

  g_object_thaw_notify(G_OBJECT(toplevel));
}

void panel_toplevel_set_y(PanelToplevel *toplevel, int y, int y_bottom,
                          gboolean y_centered) {
  gboolean changed = FALSE;
...
void panel_toplevel_get_position(PanelToplevel *toplevel, int *x, int *x_right,
                                 int *y, int *y_bottom) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  if (x) *x = toplevel->priv->x;

  if (y) *y = toplevel->priv->y;

  if (x_right) *x_right = toplevel->priv->x_right;

  if (y_bottom) *y_bottom = toplevel->priv->y_bottom;
}
...
static void panel_toplevel_set_monitor_internal(PanelToplevel *toplevel,
                                                int monitor,
                                                gboolean force_resize) {
  if (toplevel->priv->monitor == monitor) return;

  toplevel->priv->monitor = monitor;

  if (force_resize) gtk_widget_queue_resize(GTK_WIDGET(toplevel));
}
...
static void panel_toplevel_update_monitor(PanelToplevel *toplevel) {
  ...
  if ((toplevel->priv->configured_monitor != -1) &&
    ...
    panel_toplevel_set_monitor_internal(
        toplevel, toplevel->priv->configured_monitor, FALSE);

    /* else, can we still use the monitor we were using? */
  } else if (toplevel->priv->monitor >= panel_multimonitor_monitors()) {
    panel_toplevel_set_monitor_internal(toplevel, 0, FALSE);
  }
}

void panel_toplevel_set_monitor(PanelToplevel *toplevel, int monitor) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  if (toplevel->priv->configured_monitor == monitor) return;
...
void panel_toplevel_set_auto_hide(PanelToplevel *toplevel, gboolean auto_hide) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  auto_hide = auto_hide != FALSE;

  if (toplevel->priv->auto_hide == auto_hide) return;
...
void panel_toplevel_set_hide_delay(PanelToplevel *toplevel, int hide_delay) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  if (toplevel->priv->hide_delay == hide_delay) return;
...
void panel_toplevel_set_unhide_delay(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->unhide_delay == unhide_delay) return;
...
void panel_toplevel_set_animate(PanelToplevel *toplevel, gboolean animate) {
  g_return_if_fail(PANEL_IS_TOPLEVEL(toplevel));

  animate = animate != FALSE;

  if (toplevel->priv->animate == animate) return;
...
void panel_toplevel_set_animation_speed(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->animation_speed == animation_speed) return;
...
PanelAnimationSpeed panel_toplevel_get_animation_speed(
    PanelToplevel *toplevel) {
...
void panel_toplevel_set_enable_buttons(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->buttons_enabled == enable_buttons) return;
...
void panel_toplevel_set_enable_arrows(PanelToplevel *toplevel,
  ...
  if (toplevel->priv->arrows_enabled == enable_arrows) return;
...
void panel_toplevel_rotate(PanelToplevel *toplevel, gboolean clockwise) {
  PanelOrientation orientation;
