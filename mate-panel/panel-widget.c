...
static void panel_widget_cadd(GtkContainer *container, GtkWidget *widget);
static void panel_widget_cremove(GtkContainer *container, GtkWidget *widget);
...
static void panel_widget_free_move_applet(PanelWidget *panel,
                                          GtkDirectionType dir);
static void panel_widget_tab_move(PanelWidget *panel, gboolean next);
static void panel_widget_end_move(PanelWidget *panel);
static gboolean panel_widget_real_focus(GtkWidget *widget,
                                        GtkDirectionType direction);

static gboolean panel_widget_push_applet_right(PanelWidget *panel, GList *list,
                                               int push);
static gboolean panel_widget_push_applet_left(PanelWidget *panel, GList *list,
                                              int push);

/************************
 convenience functions
 ************************/
static int applet_data_compare(AppletData *ad1, AppletData *ad2) {
  return ad1->pos - ad2->pos;
}

static void emit_applet_moved(PanelWidget *panel_widget, AppletData *applet) {
  ...
  g_signal_emit(panel_widget, panel_widget_signals[APPLET_MOVE_SIGNAL], 0,
                applet->applet);
}
...
static void add_tab_bindings(GtkBindingSet *binding_set,
                             GdkModifierType modifiers, gboolean next) {
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Tab, modifiers, "tab_move",
                               1, G_TYPE_BOOLEAN, next);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Tab, modifiers,
                               "tab_move", 1, G_TYPE_BOOLEAN, next);
}

static void add_move_bindings(GtkBindingSet *binding_set,
                              GdkModifierType modifiers, const gchar *name) {
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Up, modifiers, name, 1,
                               GTK_TYPE_DIRECTION_TYPE, GTK_DIR_UP);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Down, modifiers, name, 1,
                               GTK_TYPE_DIRECTION_TYPE, GTK_DIR_DOWN);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Left, modifiers, name, 1,
                               GTK_TYPE_DIRECTION_TYPE, GTK_DIR_LEFT);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Right, modifiers, name, 1,
                               GTK_TYPE_DIRECTION_TYPE, GTK_DIR_RIGHT);
}

static void add_all_move_bindings(PanelWidget *panel) {
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Escape, 0, "end_move", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Enter, 0, "end_move", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Return, 0, "end_move", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Space, 0, "end_move", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_space, 0, "end_move", 0);
...
static void panel_widget_force_grab_focus(GtkWidget *widget) {
  gboolean can_focus = gtk_widget_get_can_focus(widget);
  /*
   * This follows what gtk_socket_claim_focus() does
   */
  if (!can_focus) gtk_widget_set_can_focus(widget, TRUE);
  gtk_widget_grab_focus(widget);
  if (!can_focus) gtk_widget_set_can_focus(widget, FALSE);
}

static void panel_widget_reset_saved_focus(PanelWidget *panel) {
...
static void remove_all_move_bindings(PanelWidget *panel) {
  GtkWidgetClass *class;
  GtkBindingSet *binding_set;
...
static void panel_widget_class_init(PanelWidgetClass *class) {
  GObjectClass *object_class = (GObjectClass *)class;
  GtkWidgetClass *widget_class = (GtkWidgetClass *)class;
  GtkContainerClass *container_class = (GtkContainerClass *)class;

  panel_widget_signals[SIZE_CHANGE_SIGNAL] = g_signal_new(
      "size-change", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelWidgetClass, size_change), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  panel_widget_signals[BACK_CHANGE_SIGNAL] = g_signal_new(
      "back-change", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelWidgetClass, back_change), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  panel_widget_signals[APPLET_MOVE_SIGNAL] = g_signal_new(
      "applet-move", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelWidgetClass, applet_move), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  panel_widget_signals[APPLET_ADDED_SIGNAL] = g_signal_new(
      "applet-added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelWidgetClass, applet_added), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  panel_widget_signals[APPLET_REMOVED_SIGNAL] = g_signal_new(
      "applet-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelWidgetClass, applet_removed), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  panel_widget_signals[PUSH_MOVE_SIGNAL] = g_signal_new(
      "push_move", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(PanelWidgetClass, push_move), NULL, NULL,
      g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1, GTK_TYPE_DIRECTION_TYPE);

  panel_widget_signals[SWITCH_MOVE_SIGNAL] = g_signal_new(
      "switch_move", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(PanelWidgetClass, switch_move), NULL, NULL,
      g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1, GTK_TYPE_DIRECTION_TYPE);

  panel_widget_signals[FREE_MOVE_SIGNAL] = g_signal_new(
      "free_move", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(PanelWidgetClass, free_move), NULL, NULL,
      g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1, GTK_TYPE_DIRECTION_TYPE);

  panel_widget_signals[TAB_MOVE_SIGNAL] = g_signal_new(
      "tab_move", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(PanelWidgetClass, tab_move), NULL, NULL,
      g_cclosure_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);

  panel_widget_signals[END_MOVE_SIGNAL] = g_signal_new(
      "end_move", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(PanelWidgetClass, end_move), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
static void remove_panel_from_forbidden(PanelWidget *panel, PanelWidget *r) {
  GSList *list;
  ...
  if (list) {
    ...
    g_object_set_data(G_OBJECT(panel->master_widget),
                      MATE_PANEL_APPLET_FORBIDDEN_PANELS, list);
  }
  parent_panel = gtk_widget_get_parent(panel->master_widget);
  if (parent_panel) remove_panel_from_forbidden(PANEL_WIDGET(parent_panel), r);
}

static void add_panel_to_forbidden(PanelWidget *panel, PanelWidget *r) {
  GSList *list;
  GtkWidget *parent_panel;
  ...
  parent_panel = gtk_widget_get_parent(panel->master_widget);
  if (parent_panel) add_panel_to_forbidden(PANEL_WIDGET(parent_panel), r);
}

static void run_up_forbidden(PanelWidget *panel,
  ...
  for (list = panel->applet_list; list != NULL; list = g_list_next(list)) {
    ...
    PanelWidget *p = g_object_get_data(G_OBJECT(ad->applet),
                                       MATE_PANEL_APPLET_ASSOC_PANEL_KEY);
    if (p) run_up_forbidden(p, runfunc);
  }
  (*runfunc)(panel, panel);
}

static void panel_widget_reset_focus(GtkContainer *container,
                                     GtkWidget *widget) {
  ...
  if (gtk_container_get_focus_child(container) == widget) {
    ...
    if (children && children->next) {
      ...
      if (l) {
        ...
        gtk_widget_child_focus(next_widget, GTK_DIR_TAB_FORWARD);
      }
    } else
...
static void panel_widget_cadd(GtkContainer *container, GtkWidget *widget) {
  ...
  p = g_object_get_data(G_OBJECT(widget), MATE_PANEL_APPLET_ASSOC_PANEL_KEY);
  if (p) {
    panel_toplevel_attach_to_widget(p->toplevel,
                                    PANEL_WIDGET(container)->toplevel, widget);
...
static void panel_widget_cremove(GtkContainer *container, GtkWidget *widget) {
  ...
  if (GTK_CONTAINER_CLASS(panel_widget_parent_class)->remove)
    (*GTK_CONTAINER_CLASS(panel_widget_parent_class)->remove)(container,
                                                              widget);
  if (ad) panel->applet_list = g_list_remove(panel->applet_list, ad);

  g_signal_emit(G_OBJECT(container),
                panel_widget_signals[APPLET_REMOVED_SIGNAL], 0, widget);
  g_object_unref(widget);
}

/*get the list item of the data on the position pos*/
static GList *get_applet_list_pos(PanelWidget *panel, int pos) {
  GList *l;
  ...
  for (l = panel->applet_list; l; l = l->next) {
    ...
    if (ad->pos <= pos) {
      if (ad->pos + ad->cells > pos) return l;
    } else
      return NULL;
  }
 ...
 */
PanelObjectEdgeRelativity panel_widget_determine_applet_edge_relativity(
    PanelWidget *panel_widget, GtkWidget *widget) {
  ...
  applet_in_question =
      g_object_get_data(G_OBJECT(widget), MATE_PANEL_APPLET_DATA);
  if (applet_in_question) {
    ...
    for (l = applet_list; l; l = l->prev) {
      ...
      if (applet->pos + applet->cells < end_pos) break;
    ...
    if (applet_is_centered) return PANEL_EDGE_CENTER;
...
static int get_size_from_hints(AppletData *ad, int cells) {
  int i;
  ...
  for (i = 0; i < ad->size_hints_len; i += 2) {
    ...
    if (cells <= ad->size_hints[i] && cells >= ad->size_hints[i + 1]) {
      /* Keep cell size */
      break;
    }
  }
 ...
 */
static void adjust_applet_position(PanelWidget *panel, AppletData *ad) {
  PanelObjectEdgeRelativity edge_relativity;

  edge_relativity =
      panel_widget_determine_applet_edge_relativity(panel, ad->applet);
...
static void panel_widget_jump_applet_right(PanelWidget *panel, GList *list,
                                           GList *next, int pos) {
  AppletData *ad;
  AppletData *nad = NULL;

  ad = list->data;
  if (next) nad = next->data;

  if (pos >= panel->size) return;

  if (!nad || nad->constrained >= pos + ad->min_cells) goto jump_right;

  if (!panel_widget_push_applet_right(panel, next,
                                      pos + ad->min_cells - nad->constrained)) {
    panel_widget_jump_applet_right(panel, list, next->next,
...
jump_right:
  ...
  panel->applet_list =
      panel_g_list_insert_before(panel->applet_list, next, list);
  gtk_widget_queue_resize(GTK_WIDGET(panel));
  emit_applet_moved(panel, ad);
}

static void panel_widget_switch_applet_right(PanelWidget *panel, GList *list) {
  AppletData *ad;
  AppletData *nad = NULL;
  ...
  ad = list->data;
  if (ad->constrained + ad->min_cells >= panel->size) return;

  if (list->next) nad = list->next->data;

  if (!nad ||
      nad->constrained >= ad->constrained + ad->min_cells + MOVE_INCREMENT) {
  ...
  if (nad->locked) {
    panel_widget_jump_applet_right(panel, list, list->next->next,
                                   nad->constrained + nad->min_cells);
    return;
  }
...
static void panel_widget_jump_applet_left(PanelWidget *panel, GList *list,
                                          GList *prev, int pos) {
  AppletData *ad;
  AppletData *pad = NULL;

  ad = list->data;
  if (prev) pad = prev->data;

  if (pos < 0) return;

  if (!pad || pad->constrained + pad->min_cells <= pos) goto jump_left;

  if (!panel_widget_push_applet_left(panel, prev,
                                     pad->constrained + pad->min_cells - pos)) {
    panel_widget_jump_applet_left(panel, list, prev->prev,
...
jump_left:
  ...
  panel->applet_list = g_list_remove_link(panel->applet_list, list);
  panel->applet_list =
      panel_g_list_insert_after(panel->applet_list, prev, list);
  gtk_widget_queue_resize(GTK_WIDGET(panel));
  emit_applet_moved(panel, ad);
}

static void panel_widget_switch_applet_left(PanelWidget *panel, GList *list) {
  AppletData *ad;
  AppletData *pad = NULL;

  ad = list->data;
  if (ad->constrained <= 0) return;

  if (list->prev) pad = list->prev->data;

  if (!pad ||
      pad->constrained + pad->min_cells <= ad->constrained - MOVE_INCREMENT) {
    ad->pos = ad->constrained -= MOVE_INCREMENT;
    adjust_applet_position(panel, ad);
    gtk_widget_queue_resize(GTK_WIDGET(panel));
    emit_applet_moved(panel, ad);
  ...
  if (pad->locked) {
    panel_widget_jump_applet_left(panel, list, list->prev->prev,
                                  pad->constrained - ad->min_cells);
    return;
  }
...
static gboolean panel_widget_try_push_right(PanelWidget *panel, GList *list,
                                            int push) {
  AppletData *ad;
  AppletData *nad = NULL;
  ...
  if (ad->locked) return FALSE;

  if (ad->constrained + ad->min_cells + push >= panel->size) return FALSE;
...
static int panel_widget_get_right_jump_pos(PanelWidget *panel, AppletData *ad,
                                           GList *next, int pos) {
  AppletData *nad = NULL;

  if (next) nad = next->data;

  if (!nad || nad->constrained >= pos + ad->min_cells) return pos;

  if (panel_widget_try_push_right(panel, next,
                                  pos + ad->min_cells - nad->constrained))
    return pos;

  return panel_widget_get_right_jump_pos(panel, ad, next->next,
                                         nad->constrained + nad->min_cells);
}

static int panel_widget_get_right_switch_pos(PanelWidget *panel, GList *list) {
  AppletData *ad;
  AppletData *nad = NULL;
  ...
  if (nad->locked)
    return panel_widget_get_right_jump_pos(panel, ad, list->next->next,
                                           nad->constrained + nad->min_cells);

  return nad->constrained + nad->min_cells - ad->cells;
}

static gboolean panel_widget_try_push_left(PanelWidget *panel, GList *list,
                                           int push) {
  AppletData *ad;
  AppletData *pad = NULL;
  ...
  if (ad->constrained - push < 0) return FALSE;
...
static int panel_widget_get_left_jump_pos(PanelWidget *panel, AppletData *ad,
                                          GList *prev, int pos) {
  AppletData *pad = NULL;

  if (prev) pad = prev->data;

  if (!pad || pad->constrained + pad->min_cells <= pos) return pos;

  if (panel_widget_try_push_left(panel, prev,
                                 pad->constrained + pad->min_cells - pos))
    return pos;

  return panel_widget_get_left_jump_pos(panel, ad, prev->prev,
                                        pad->constrained - ad->min_cells);
}

static int panel_widget_get_left_switch_pos(PanelWidget *panel, GList *list) {
  AppletData *ad;
  AppletData *pad = NULL;
  ...
  if (pad->locked)
    return panel_widget_get_left_jump_pos(panel, ad, list->prev->prev,
                                          pad->constrained - ad->min_cells);

  return pad->constrained;
}

static void panel_widget_switch_move(PanelWidget *panel, AppletData *ad,
                                     int moveby) {
  GList *list;
  int finalpos;
  int pos;
  ...
  if (ad->constrained < finalpos) {
    ...
    if (list->prev) {
      pad = list->prev->data;
      if (pad->expand_major) gtk_widget_queue_resize(GTK_WIDGET(panel));
    }
    ...
    if (list->next) {
      ...
      if (nad->expand_major) gtk_widget_queue_resize(GTK_WIDGET(panel));
    }
...
static int panel_widget_push_applet_right(PanelWidget *panel, GList *list,
                                          int push) {
  AppletData *ad;
  AppletData *nad = NULL;
  ...
  if (!panel_widget_push_applet_right(panel, list->next, push)) return FALSE;
...
static int panel_widget_push_applet_left(PanelWidget *panel, GList *list,
                                         int push) {
  AppletData *ad;
  AppletData *pad = NULL;
  ...
  if (!panel_widget_push_applet_left(panel, list->prev, push)) return FALSE;
...
static void panel_widget_push_move(PanelWidget *panel, AppletData *ad,
                                   int moveby) {
  int finalpos;
  GList *list;
  ...
  if (ad->constrained < finalpos) {
    while (ad->constrained < finalpos)
      if (!panel_widget_push_applet_right(panel, list, 1)) break;
    ...
    while (ad->constrained > finalpos)
      if (!panel_widget_push_applet_left(panel, list, 1)) break;
  }
}
...
static void panel_widget_get_preferred_size(GtkWidget *widget,
  ...
  for (list = panel->applet_list; list != NULL; list = g_list_next(list)) {
    ...
    gtk_widget_get_preferred_size(ad->applet, &child_min_size,
                                  &child_natural_size);

    if (panel->orient == GTK_ORIENTATION_HORIZONTAL) {
      if (minimum_size->height < child_min_size.height && !ad->size_constrained)
      ...
      if (minimum_size->width < child_min_size.width && !ad->size_constrained)
        minimum_size->width = child_min_size.width;
      if (natural_size->width < child_min_size.width && !ad->size_constrained)
  ...
  if (panel->packed) {
    ...
    if (panel->nb_applets_size_hints > 0) {
      int i;
      panel->applets_hints =
          g_new0(AppletSizeHints, panel->nb_applets_size_hints);

      i = 0;
      for (list = ad_with_hints; list != NULL; list = g_list_next(list)) {
        AppletData *ad = list->data;
      ...
      panel->applets_using_hint =
          g_new0(AppletSizeHintsAlloc, panel->nb_applets_size_hints);
    }
  }
  ...
  if (panel->orient == GTK_ORIENTATION_HORIZONTAL) {
    if (minimum_size->width < 12 && !dont_fill) minimum_size->width = 12;
    if (minimum_size->height < 12) minimum_size->height = 12;
    if (natural_size->width < 12 && !dont_fill) natural_size->width = 12;
    if (natural_size->height < 12) natural_size->height = 12;
  } else {
    if (minimum_size->width < 12) minimum_size->width = 12;
    if (minimum_size->height < 12 && !dont_fill) minimum_size->height = 12;
    if (natural_size->width < 12) natural_size->width = 12;
    if (natural_size->height < 12 && !dont_fill) natural_size->height = 12;
  }
}
...
static void queue_resize_on_all_applets(PanelWidget *panel) {
  GList *li;
  for (li = panel->applet_list; li != NULL; li = g_list_next(li)) {
...
static void panel_widget_size_allocate(GtkWidget *widget,
  ...
  if (panel->packed) {
    ...
    for (list = panel->applet_list; list != NULL; list = g_list_next(list)) {
      ...
      if (panel->orient == GTK_ORIENTATION_HORIZONTAL) {
        if (ad->expand_minor) challoc.height = allocation->height;
        ...
        challoc.x = ltr ? ad->constrained
                        : panel->size - ad->constrained - challoc.width;
        challoc.y = allocation->height / 2 - challoc.height / 2;
      } else {
        if (ad->expand_minor) challoc.width = allocation->width;
    ...
    for (list = panel->applet_list; list; list = list->next) {
      ...
      if (id) {
        ...
        position =
            g_settings_get_int(info->settings, PANEL_OBJECT_POSITION_KEY);
        edge_relativity = g_settings_get_enum(
            info->settings, PANEL_OBJECT_RELATIVE_TO_EDGE_KEY);
        right_stuck = g_settings_get_boolean(
            info->settings, PANEL_OBJECT_PANEL_RIGHT_STICK_KEY);
      }
      ...
      if (ad->constrained < i) {
        ...
        if (list->prev) {
          pad = list->prev->data;
          previous_edge_relativity =
              panel_widget_determine_applet_edge_relativity(panel, pad->applet);
        }

        if (edge_relativity == PANEL_EDGE_CENTER && pad &&
    ...
    i = panel->size;
    for (list = g_list_last(panel->applet_list); list != NULL;
      ...
      if (ad->expand_major) {
        ...
        if (ad->size_hints) cells = get_size_from_hints(ad, cells);
    ...
    if (i < 0) {
      ...
      for (list = panel->applet_list; list != NULL; list = g_list_next(list)) {
        ...
        if (ad->constrained < i) ad->constrained = i;
...
gboolean panel_widget_is_cursor(PanelWidget *panel, int overlap) {
  GtkWidget *widget;
  GtkAllocation allocation;
  GdkDevice *device;
  int x, y;
  ...
  if (!widget || !GTK_IS_WIDGET(widget) || !gtk_widget_get_visible(widget))
    return FALSE;

  device = gdk_seat_get_pointer(
      gdk_display_get_default_seat(gtk_widget_get_display(widget)));
  gdk_window_get_device_position(gtk_widget_get_window(widget), device, &x, &y,
                                 NULL);
  ...
  if ((x + overlap) >= 0 && (x - overlap) <= w && (y + overlap) >= 0 &&
      (y - overlap) <= h)
    return TRUE;
  return FALSE;
}
...
static void panel_widget_open_dialog_destroyed(PanelWidget *panel_widget,
                                               GtkWidget *dialog) {
  g_return_if_fail(panel_widget->open_dialogs != NULL);

  panel_widget->open_dialogs =
      g_slist_remove(panel_widget->open_dialogs, dialog);
}

static void panel_widget_destroy_open_dialogs(PanelWidget *panel_widget) {
  GSList *l, *list;
  ...
  for (l = list; l; l = l->next) {
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(l->data), G_CALLBACK(panel_widget_open_dialog_destroyed),
        panel_widget);
...
static void panel_widget_dispose(GObject *obj) {
  ...
  if (panel->master_widget != NULL) {
    g_object_set_data(G_OBJECT(panel->master_widget),
                      MATE_PANEL_APPLET_ASSOC_PANEL_KEY, NULL);
...
static void panel_widget_init(PanelWidget *panel) {
  GtkWidget *widget = (GtkWidget *)panel;

  gtk_widget_set_events(
      widget, gtk_widget_get_events(widget) | GDK_BUTTON_RELEASE_MASK);
...
GtkWidget *panel_widget_new(PanelToplevel *toplevel, gboolean packed,
                            GtkOrientation orient, int sz) {
  PanelWidget *panel;
...
static gboolean panel_widget_applet_drag_start_no_grab(PanelWidget *panel,
                                                       GtkWidget *applet,
                                                       int drag_off) {
  AppletData *ad;
  AppletInfo *info;
  ...
  if (info != NULL && !mate_panel_applet_can_freely_move(info)) return FALSE;
...
static void panel_widget_applet_drag_end_no_grab(PanelWidget *panel) {
  AppletInfo *info;
  ...
  info = g_object_get_data(G_OBJECT(panel->currently_dragged_applet->applet),
                           "applet_info");
  if (info) mate_panel_applet_save_position(info, info->id, TRUE);
...
void panel_widget_applet_drag_start(PanelWidget *panel, GtkWidget *applet,
                                    int drag_off, guint32 time_) {
  GdkWindow *window;
  ...
  if (!panel_widget_applet_drag_start_no_grab(panel, applet, drag_off)) return;
  ...
  if (window) {
    ...
    status = gdk_seat_grab(seat, window, GDK_SEAT_CAPABILITY_POINTER, FALSE,
                           fleur_cursor, NULL, NULL, NULL);

    g_object_unref(fleur_cursor);
    gdk_display_flush(display);

    if (status != GDK_GRAB_SUCCESS) {
      g_warning(G_STRLOC ": failed to grab pointer (errorcode: %d)", status);
      panel_widget_applet_drag_end(panel);
    }
  }
}
...
void panel_widget_applet_drag_end(PanelWidget *panel) {
  ...
  if (panel->currently_dragged_applet == NULL) return;
...
int panel_widget_get_cursorloc(PanelWidget *panel) {
  ...
  device = gdk_seat_get_pointer(
      gdk_display_get_default_seat(gtk_widget_get_display(GTK_WIDGET(panel))));
...
static int panel_widget_get_moveby(PanelWidget *panel, int pos, int offset) {
  g_return_val_if_fail(PANEL_IS_WIDGET(panel), -1);

  return panel_widget_get_cursorloc(panel) - offset - pos;
}

static GList *walk_up_to(int pos, GList *list) {
  AppletData *ad;
  ...
  if (ad->constrained <= pos && ad->constrained + ad->cells > pos) return list;
  while (list->next != NULL && ad->constrained + ad->cells <= pos) {
    list = list->next;
    ad = list->data;
  }
  while (list->prev != NULL && ad->constrained > pos) {
    list = list->prev;
    ad = list->data;
  }
  return list;
...
static GtkWidget *is_in_applet(int pos, AppletData *ad) {
  g_return_val_if_fail(ad != NULL, NULL);

  if (ad->constrained <= pos && ad->constrained + ad->cells > pos)
    return ad->applet;
  return NULL;
}

static int panel_widget_get_free_spot(PanelWidget *panel, AppletData *ad,
                                      int place) {
  int i, e;
  int start;
  int right = -1, left = -1;
  ...
  if (ad->constrained >= panel->size) return -1;
  ...
  for (e = 0, i = start; i < panel->size; i++) {
    ...
    if (applet == NULL || applet == ad->applet) {
      e++;
      if (e >= ad->min_cells) {
        right = i - e + 1;
        break;
  ...
  start = place + ad->drag_off;
  if (start >= panel->size) start = panel->size - 1;
...
static void panel_widget_nice_move(PanelWidget *panel, AppletData *ad,
                                   int pos) {
  g_return_if_fail(PANEL_IS_WIDGET(panel));
  g_return_if_fail(ad != NULL);

  pos = panel_widget_get_free_spot(panel, ad, pos);
  if (pos < 0 || pos == ad->pos) return;

  ad->pos = ad->constrained = pos;
  adjust_applet_position(panel, ad);

  panel->applet_list = panel_g_list_resort_item(
      panel->applet_list, ad, (GCompareFunc)applet_data_compare);
...
static void panel_widget_applet_move_to_cursor(PanelWidget *panel) {
  int moveby;
  int pos;
  int movement;
  GtkWidget *applet;
  ...
  forb =
      g_object_get_data(G_OBJECT(applet), MATE_PANEL_APPLET_FORBIDDEN_PANELS);

  if (!panel_widget_is_cursor(panel, 10)) {
    GSList *list;

    for (list = panels; list != NULL; list = g_slist_next(list)) {
      PanelWidget *new_panel = PANEL_WIDGET(list->data);

      if (panel != new_panel && panel_widget_is_cursor(new_panel, 10) &&
          panel_screen_from_panel_widget(panel) ==
              panel_screen_from_panel_widget(new_panel) &&
          !g_slist_find(forb, new_panel) && !panel_lockdown_get_locked_down()) {
  ...
  g_object_get(G_OBJECT(settings), "gtk-dnd-drag-threshold", &drag_threshold,
               NULL);
  ...
  if ((moveby < 0 && pos > centered_applet_position &&
       ...
       pos + moveby >=
           centered_applet_position - drag_threshold * SNAP_TOLERANCE_FACTOR) ||
    ...
    if (absolute_moveby < drag_threshold * SNAP_TOLERANCE_FACTOR) return;
  }
...
static int move_timeout_handler(gpointer data) {
  PanelWidget *panel = data;

  g_return_val_if_fail(PANEL_IS_WIDGET(data), FALSE);

  if (been_moved && panel->currently_dragged_applet) {
  ...
  if (panel->currently_dragged_applet && repeat_if_outside) {
    ...
    gdk_window_get_device_position(gtk_widget_get_window(widget), device, &x,
                                   &y, NULL);
...
static void schedule_try_move(PanelWidget *panel, gboolean repeater) {
  if (!panel->currently_dragged_applet) return;
  repeat_if_outside = repeater;
  if (moving_timeout == 0) {
    been_moved = FALSE;
    panel_widget_applet_move_to_cursor(panel);
    moving_timeout = g_timeout_add(50, move_timeout_handler, panel);
  } else
    been_moved = TRUE;
}

static gboolean panel_widget_applet_button_press_event(GtkWidget *widget,
  ...
  if (panel->currently_dragged_applet) {
    g_signal_stop_emission(
        G_OBJECT(widget),
        g_signal_lookup("button-press-event", G_OBJECT_TYPE(widget)), 0);
    return TRUE;
  }
  ...
  if (event->send_event) event_time = GDK_CURRENT_TIME;
...
static gboolean panel_widget_applet_button_release_event(
  ...
  if (panel->currently_dragged_applet) {
    g_signal_stop_emission(
        G_OBJECT(widget),
        g_signal_lookup("button-release-event", G_OBJECT_TYPE(widget)), 0);
    panel_widget_applet_drag_end(panel);
    return TRUE;
  }
...
static gboolean panel_widget_applet_motion_notify_event(GtkWidget *widget,
                                                        GdkEvent *event) {
  GtkWidget *parent;
  PanelWidget *panel;
...
static gboolean panel_widget_applet_key_press_event(GtkWidget *widget,
                                                    GdkEventKey *event) {
  GtkWidget *parent;
  PanelWidget *panel;
  ...
  if (!mate_panel_applet_in_drag) return FALSE;

  return gtk_bindings_activate(G_OBJECT(panel), ((GdkEventKey *)event)->keyval,
                               ((GdkEventKey *)event)->state);
}

static int panel_sub_event_handler(GtkWidget *widget, GdkEvent *event,
                                   gpointer data) {
  ...
  switch (event->type) {
    ...
    case GDK_KEY_PRESS:
      if (mate_panel_applet_in_drag) return gtk_widget_event(data, event);
      break;
    default:
      break;
  }
...
static void bind_applet_events(GtkWidget *widget, gpointer data) {
  g_return_if_fail(GTK_IS_WIDGET(widget));
  ...
  if (gtk_widget_get_has_window(widget))
    g_signal_connect(widget, "event", G_CALLBACK(panel_sub_event_handler),
                     data);

  if (GTK_IS_CONTAINER(widget))
    gtk_container_foreach(GTK_CONTAINER(widget), bind_applet_events, data);
}

static void panel_widget_applet_destroy(GtkWidget *applet, gpointer data) {
  AppletData *ad;
  GtkWidget *parent;
...
static void bind_top_applet_events(GtkWidget *widget) {
  g_return_if_fail(GTK_IS_WIDGET(widget));

  g_signal_connect(widget, "destroy", G_CALLBACK(panel_widget_applet_destroy),
                   NULL);

  g_signal_connect(widget, "button-press-event",
                   G_CALLBACK(panel_widget_applet_button_press_event), NULL);

  g_signal_connect(widget, "button-release-event",
                   G_CALLBACK(panel_widget_applet_button_release_event), NULL);
  g_signal_connect(widget, "motion-notify-event",
                   G_CALLBACK(panel_widget_applet_motion_notify_event), NULL);
  g_signal_connect(widget, "key-press-event",
                   G_CALLBACK(panel_widget_applet_key_press_event), NULL);
  ...
  if (GTK_IS_CONTAINER(widget))
    gtk_container_foreach(GTK_CONTAINER(widget), bind_applet_events, widget);
}

static int panel_widget_find_empty_pos(PanelWidget *panel, int pos) {
  int i;
  int right = -1, left = -1;
  GList *list;
  ...
  if (pos >= panel->size) pos = panel->size - 1;

  if (pos <= 0) pos = 0;

  if (!panel->applet_list) return pos;
...
int panel_widget_add(PanelWidget *panel, GtkWidget *applet, gboolean locked,
                     int pos, gboolean insert_at_pos) {
  AppletData *ad = NULL;
  ...
  if (ad == NULL) {
    ...
    ad->size_hints = NULL;
    g_object_set_data(G_OBJECT(applet), MATE_PANEL_APPLET_DATA, ad);
  ...
  if (panel->orient == GTK_ORIENTATION_HORIZONTAL)
    gtk_fixed_put(GTK_FIXED(panel), applet, pos, 0);
  else
    gtk_fixed_put(GTK_FIXED(panel), applet, 0, pos);

  gtk_widget_queue_resize(GTK_WIDGET(panel));

  g_signal_emit(G_OBJECT(panel), panel_widget_signals[APPLET_ADDED_SIGNAL], 0,
                applet);
...
gboolean panel_widget_reparent(PanelWidget *old_panel, PanelWidget *new_panel,
                               GtkWidget *applet, int pos) {
  AppletData *ad;
  ...
  if (info && info->type == PANEL_OBJECT_APPLET)
    mate_panel_applet_frame_set_panel(MATE_PANEL_APPLET_FRAME(ad->applet),
                                      new_panel);
  ...
  if (focus_widget) {
    ...
    g_signal_emit_by_name(applet, "focus", GTK_DIR_TAB_FORWARD, &return_val);
  }
...
void panel_widget_set_packed(PanelWidget *panel_widget, gboolean packed) {
...
void panel_widget_set_size(PanelWidget *panel_widget, int size) {
  g_return_if_fail(PANEL_IS_WIDGET(panel_widget));

  if (size == panel_widget->sz) return;
...
static void panel_widget_tab_move(PanelWidget *panel, gboolean next) {
  ...
  for (l = panels; l; l = l->next) {
    ...
    if (panel_in_list == panel) {
      ...
      if (!next) previous_panel = panel_in_list;
    }
  }

  g_return_if_fail(l != NULL);

  if (!new_panel && previous_panel) new_panel = previous_panel;

  if (new_panel && (new_panel != panel) && !panel_lockdown_get_locked_down())
...
void panel_widget_focus(PanelWidget *panel_widget) {
  if (panel_toplevel_get_is_attached(panel_widget->toplevel)) return;
...
PanelOrientation panel_widget_get_applet_orientation(PanelWidget *panel) {
  ...
  g_return_val_if_fail(PANEL_IS_TOPLEVEL(panel->toplevel),
...
void panel_widget_set_applet_size_constrained(PanelWidget *panel,
  ...
  if (ad->size_constrained == size_constrained) return;
...
void panel_widget_set_applet_expandable(PanelWidget *panel, GtkWidget *applet,
                                        gboolean major, gboolean minor) {
  AppletData *ad;
  ...
  if (ad->expand_major == major && ad->expand_minor == minor) return;
...
void panel_widget_set_applet_size_hints(PanelWidget *panel, GtkWidget *applet,
                                        int *size_hints, int size_hints_len) {
  AppletData *ad;
...
void panel_widget_set_applet_locked(PanelWidget *panel, GtkWidget *applet,
                                    gboolean locked) {
  AppletData *ad;
...
gboolean panel_widget_get_applet_locked(PanelWidget *panel, GtkWidget *applet) {
  AppletData *ad;
...
void panel_widget_register_open_dialog(PanelWidget *panel, GtkWidget *dialog) {
  ...
  panel->open_dialogs = g_slist_append(panel->open_dialogs, dialog);

  g_signal_connect_object(dialog, "destroy",
                          G_CALLBACK(panel_widget_open_dialog_destroyed), panel,
                          G_CONNECT_SWAPPED);
}
