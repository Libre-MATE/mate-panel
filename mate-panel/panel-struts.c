...
static inline PanelStrut *panel_struts_find_strut(PanelToplevel *toplevel) {
  GSList *l;

  for (l = panel_struts_list; l; l = l->next) {
    PanelStrut *strut = l->data;

    if (strut->toplevel == toplevel) break;
  }

  return l ? l->data : NULL;
}

static void panel_struts_get_monitor_geometry(int monitor, int *x, int *y,
...
static PanelStrut *panel_struts_intersect(GSList *struts,
                                          GdkRectangle *geometry, int skip) {
  GSList *l;
  int i;
  ...
  for (l = struts; l; l = l->next) {
    ...
    if (x2 - x1 > 0 && y2 - y1 > 0 && ++i > skip) break;
  }

  return l ? l->data : NULL;
}

static int panel_struts_allocation_overlapped(PanelStrut *strut,
                                              PanelStrut *overlap,
                                              GdkRectangle *geometry,
                                              gboolean *moved_down, int skip) {
  ...
  overlap_x2 =
      overlap->allocated_geometry.x + overlap->allocated_geometry.width;
  overlap_y2 =
      overlap->allocated_geometry.y + overlap->allocated_geometry.height;
...
static gboolean panel_struts_allocate_struts(PanelToplevel *toplevel,
                                             GdkScreen *screen, int monitor) {
  ...
  for (l = panel_struts_list; l; l = l->next) {
    ...
    if (strut->screen != screen || strut->monitor != monitor) continue;

    panel_struts_get_monitor_geometry(strut->monitor, &monitor_x, &monitor_y,
    ...
    while ((overlap = panel_struts_intersect(allocated, &geometry, skip)))
      skip = panel_struts_allocation_overlapped(strut, overlap, &geometry,
                                                &moved_down, skip);
...
void panel_struts_set_window_hint(PanelToplevel *toplevel) {
  ...
  screen_width =
      WidthOfScreen(gdk_x11_screen_get_xscreen(strut->screen)) / scale;
  screen_height =
      HeightOfScreen(gdk_x11_screen_get_xscreen(strut->screen)) / scale;

  panel_struts_get_monitor_geometry(strut->monitor, &monitor_x, &monitor_y,
                                    &monitor_width, &monitor_height);

  panel_multimonitor_is_at_visible_extreme(strut->monitor, &leftmost,
                                           &rightmost, &topmost, &bottommost);

  switch (strut->orientation) {
    case PANEL_ORIENTATION_TOP:
      if (monitor_y > 0) strut_size += monitor_y;
    ...
    case PANEL_ORIENTATION_BOTTOM:
      ...
      break;
    case PANEL_ORIENTATION_LEFT:
      if (leftmost && monitor_x > 0) strut_size += monitor_x;
  ...
  panel_xutils_set_strut(gtk_widget_get_window(widget), strut->orientation,
                         strut_size, strut->allocated_strut_start,
                         strut->allocated_strut_end, &strut->allocated_geometry,
                         scale);
}

void panel_struts_unset_window_hint(PanelToplevel *toplevel) {
  g_return_if_fail(
      GDK_IS_X11_DISPLAY(gtk_widget_get_display(GTK_WIDGET(toplevel))));

  if (!gtk_widget_get_realized(GTK_WIDGET(toplevel))) return;

  panel_xutils_unset_strut(gtk_widget_get_window(GTK_WIDGET(toplevel)));
}

static inline int orientation_to_order(PanelOrientation orientation) {
  switch (orientation) {
    case PANEL_ORIENTATION_TOP:
      return 1;
    case PANEL_ORIENTATION_BOTTOM:
...
static inline int get_toplevel_depth(PanelToplevel *toplevel) {
  int depth = 0;

  while ((toplevel = panel_toplevel_get_attach_toplevel(toplevel))) depth++;
 ...
 */
static int panel_struts_compare(const PanelStrut *s1, const PanelStrut *s2) {
  int s1_depth;
  int s2_depth;
  ...
  if (s1->monitor != s2->monitor) return s1->monitor - s2->monitor;

  s1_depth = get_toplevel_depth(s1->toplevel);
  s2_depth = get_toplevel_depth(s2->toplevel);
  if (s1_depth != s2_depth) return s2_depth - s1_depth;
  ...
  if (s1->strut_end != s2->strut_end) return s2->strut_end - s1->strut_end;
...
gboolean panel_struts_register_strut(PanelToplevel *toplevel, GdkScreen *screen,
  ...
  g_return_val_if_fail(
      GDK_IS_X11_DISPLAY(gtk_widget_get_display(GTK_WIDGET(toplevel))), FALSE);

  if (!(strut = panel_struts_find_strut(toplevel))) {
    strut = g_new0(PanelStrut, 1);
    new_strut = TRUE;

  } else if (strut->toplevel == toplevel && strut->orientation == orientation &&
             strut->screen == screen && strut->monitor == monitor &&
             strut->strut_size == strut_size &&
             strut->strut_start == strut_start && strut->strut_end == strut_end)
    return FALSE;
  ...
  panel_struts_get_monitor_geometry(monitor, &monitor_x, &monitor_y,
                                    &monitor_width, &monitor_height);
  ...
  if (new_strut) panel_struts_list = g_slist_append(panel_struts_list, strut);

  panel_struts_list =
      g_slist_sort(panel_struts_list, (GCompareFunc)panel_struts_compare);
...
void panel_struts_unregister_strut(PanelToplevel *toplevel) {
  ...
  if (!(strut = panel_struts_find_strut(toplevel))) return;
...
gboolean panel_struts_update_toplevel_geometry(PanelToplevel *toplevel, int *x,
  ...
  if (!(strut = panel_struts_find_strut(toplevel))) return FALSE;
