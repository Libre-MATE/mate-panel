...
static void drawer_click(GtkWidget *widget, Drawer *drawer) {
...
static gboolean key_press_drawer(GtkWidget *widget, GdkEventKey *event,
                                 Drawer *drawer) {
 ...
 */
static gboolean key_press_drawer_widget(GtkWidget *widget, GdkEventKey *event,
                                        Drawer *drawer) {
  PanelWidget *panel_widget;
...
static gboolean drag_motion_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  if (drawer->close_timeout_id) g_source_remove(drawer->close_timeout_id);
...
static gboolean drag_drop_cb(GtkWidget *widget, GdkDragContext *context, int x,
                             int y, guint time_, Drawer *drawer) {
  GdkAtom atom = NULL;

  if (!panel_check_dnd_target_data(widget, context, NULL, &atom)) return FALSE;
...
static void drag_data_received_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  panel_receive_dnd_data(panel_widget, info, -1, selection_data, context,
                         time_);
}
...
static void drawer_button_size_allocated(GtkWidget *widget,
                                         GtkAllocation *alloc, Drawer *drawer) {
...
static void panel_drawer_custom_icon_changed(GSettings *settings, gchar *key,
                                             Drawer *drawer) {
  ...
  char *custom_icon =
      g_settings_get_string(settings, PANEL_OBJECT_CUSTOM_ICON_KEY);
...
static void panel_drawer_tooltip_changed(GSettings *settings, gchar *key,
                                         Drawer *drawer) {
...
static void toplevel_destroyed(GtkWidget *widget, Drawer *drawer) {
...
static void destroy_drawer(GtkWidget *widget, Drawer *drawer) {
  if (drawer->toplevel) {
...
static void drawer_deletion_response(GtkWidget *dialog, int response,
                                     Drawer *drawer) {
  if (response == GTK_RESPONSE_OK) panel_profile_delete_object(drawer->info);
...
static PanelToplevel *create_drawer_toplevel(const char *drawer_id,
  ...
  g_settings_set_string(settings, PANEL_OBJECT_ATTACHED_TOPLEVEL_ID_KEY,
                        toplevel_id);
  g_free(toplevel_id);
...
static void set_tooltip_and_name(Drawer *drawer, const char *tooltip) {
  g_return_if_fail(drawer != NULL);
...
static Drawer *create_drawer_applet(PanelToplevel *toplevel,
  ...
  g_signal_connect(drawer->button, "key-press-event",
                   G_CALLBACK(key_press_drawer), drawer);
  g_signal_connect(drawer->toplevel, "key-press-event",
                   G_CALLBACK(key_press_drawer_widget), drawer);

  gtk_drag_dest_set(drawer->button, 0, NULL, 0, 0);

  g_signal_connect(drawer->button, "drag-data-get",
                   G_CALLBACK(drag_data_get_cb), drawer);
  g_signal_connect(drawer->button, "drag-motion", G_CALLBACK(drag_motion_cb),
                   drawer);
  g_signal_connect(drawer->button, "drag-drop", G_CALLBACK(drag_drop_cb),
                   drawer);
  g_signal_connect(drawer->button, "drag-data-received",
                   G_CALLBACK(drag_data_received_cb), drawer);
  g_signal_connect(drawer->button, "drag-leave", G_CALLBACK(drag_leave_cb),
                   drawer);

  g_signal_connect(drawer->button, "destroy", G_CALLBACK(destroy_drawer),
                   drawer);
  g_signal_connect(drawer->toplevel, "destroy", G_CALLBACK(toplevel_destroyed),
                   drawer);

  gtk_widget_show(drawer->button);

  panel_toplevel_attach_to_widget(drawer->toplevel, parent_toplevel,
                                  GTK_WIDGET(drawer->button));
...
static void panel_drawer_connect_to_gsettings(Drawer *drawer) {
  g_signal_connect(drawer->info->settings,
                   ...
                   G_CALLBACK(panel_drawer_custom_icon_changed), drawer);

  g_signal_connect(drawer->info->settings,
                   "changed::" PANEL_OBJECT_CUSTOM_ICON_KEY,
                   G_CALLBACK(panel_drawer_custom_icon_changed), drawer);

  g_signal_connect(drawer->info->settings, "changed::" PANEL_OBJECT_TOOLTIP_KEY,
                   G_CALLBACK(panel_drawer_tooltip_changed), drawer);
}

static void load_drawer_applet(char *toplevel_id, GSettings *settings,
                               const char *custom_icon,
                               gboolean use_custom_icon, const char *tooltip,
                               PanelToplevel *parent_toplevel, gboolean locked,
                               int pos, gboolean exactpos, const char *id) {
  PanelOrientation orientation;
  ...
  if (toplevel_id) toplevel = panel_profile_get_toplevel_by_id(toplevel_id);

  if (!toplevel) toplevel = create_drawer_toplevel(id, settings);

  if (toplevel) {
    panel_toplevel_hide(toplevel, FALSE, -1);
    drawer = create_drawer_applet(toplevel, parent_toplevel, tooltip,
                                  custom_icon, use_custom_icon, orientation);
  }
  ...
  drawer->info = mate_panel_applet_register(
      drawer->button, drawer, (GDestroyNotify)g_free, panel_widget, locked, pos,
      exactpos, PANEL_OBJECT_DRAWER, id);
  ...
  g_signal_connect_after(drawer->button, "size-allocate",
                         G_CALLBACK(drawer_button_size_allocated), drawer);

  panel_widget_add_forbidden(panel_toplevel_get_panel_widget(drawer->toplevel));
  panel_widget_set_applet_expandable(panel_widget, GTK_WIDGET(drawer->button),
                                     FALSE, TRUE);
  panel_widget_set_applet_size_constrained(panel_widget,
                                           GTK_WIDGET(drawer->button), TRUE);

  mate_panel_applet_add_callback(drawer->info, "add", "list-add",
                                 _("_Add to Drawer..."),
                                 drawer_changes_enabled);

  mate_panel_applet_add_callback(drawer->info, "properties",
                                 "document-properties", _("_Properties"),
                                 drawer_changes_enabled);

  mate_panel_applet_add_callback(drawer->info, "help", "help-browser",
                                 _("_Help"), NULL);

  panel_drawer_connect_to_gsettings(drawer);
}

static void panel_drawer_prepare(const char *drawer_id, GIcon *custom_icon,
                                 gboolean use_custom_icon, const char *tooltip,
  ...
  if (attached_toplevel_id) {
    ...
    g_settings_set_string(settings, PANEL_OBJECT_ATTACHED_TOPLEVEL_ID_KEY,
                          toplevel_id);
    g_settings_set_boolean(toplevel_settings, PANEL_TOPLEVEL_ENABLE_BUTTONS_KEY,
                           TRUE);
    g_settings_set_boolean(toplevel_settings, PANEL_TOPLEVEL_ENABLE_ARROWS_KEY,
                           TRUE);
...
void panel_drawer_create(PanelToplevel *toplevel, int position,
                         GIcon *custom_icon, gboolean use_custom_icon,
                         const char *tooltip) {
  char *id;
...
char *panel_drawer_create_with_id(const char *toplevel_id, int position,
                                  GIcon *custom_icon, gboolean use_custom_icon,
                                  const char *tooltip) {
  char *id;
  char *attached_toplevel_id = NULL;

  id = panel_profile_prepare_object_with_id(PANEL_OBJECT_DRAWER, toplevel_id,
                                            position);

  panel_drawer_prepare(id, custom_icon, use_custom_icon, tooltip,
...
void drawer_load_from_gsettings(PanelWidget *panel_widget, gboolean locked,
  ...
  toplevel_id =
      g_settings_get_string(settings, PANEL_OBJECT_ATTACHED_TOPLEVEL_ID_KEY);
  ...
  load_drawer_applet(toplevel_id, settings, custom_icon, use_custom_icon,
                     tooltip, panel_widget->toplevel, locked, position, TRUE,
                     id);
...
void panel_drawer_set_dnd_enabled(Drawer *drawer, gboolean dnd_enabled) {
  if (dnd_enabled) {
    static GtkTargetEntry dnd_targets[] = {
        {"application/x-mate-panel-applet-internal", 0, 0}};

    gtk_widget_set_has_window(drawer->button, TRUE);
    gtk_drag_source_set(drawer->button, GDK_BUTTON1_MASK, dnd_targets, 1,
                        GDK_ACTION_MOVE);
...
void drawer_query_deletion(Drawer *drawer) {
  if (drawer->toplevel) {
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(drawer_deletion_response),
                     drawer);
