...
static GOptionEntry options[] = {
    {"create-new", 0, 0, G_OPTION_ARG_NONE, &create_new,
     N_("Create new file in the given directory"), NULL},
    {G_OPTION_REMAINING, 0, 0, G_OPTION_ARG_FILENAME_ARRAY, &desktops, NULL,
     N_("[FILE...]")},
...
static void dialog_destroyed(GtkWidget *dialog, gpointer data) {
  ...
  if (dialogs <= 0) gtk_main_quit();
}

static void validate_for_filename(char *file) {
  char *ptr;
...
static char *find_uri_on_save(PanelDItemEditor *dialog, gpointer data) {
  GKeyFile *keyfile;
  char *name;
  char *filename;
  char *uri;
  ...
  if (filename == NULL) filename = g_strdup("foo");
...
static void error_reported(GtkWidget *dialog, const char *primary,
...
int main(int argc, char *argv[]) {
  ...
  if (desktops == NULL || desktops[0] == NULL) {
  ...
  for (i = 0; desktops[i] != NULL; i++) {
    ...
    info = g_file_query_info(file, "standard::type", G_FILE_QUERY_INFO_NONE,
                             NULL, NULL);
    g_object_unref(file);
    ...
    if (info) {
      ...
      if (type == G_FILE_TYPE_DIRECTORY && create_new) {
        dlg = panel_ditem_editor_new(NULL, NULL, NULL, _("Create Launcher"));
        g_object_set_data_full(G_OBJECT(dlg), "dir", g_strdup(path),
                               (GDestroyNotify)g_free);

        panel_ditem_register_save_uri_func(PANEL_DITEM_EDITOR(dlg),
                                           find_uri_on_save, NULL);
        ...
        desktops[i] = g_build_path("/", uri, ".directory", NULL);
        i--;
      } else if (type == G_FILE_TYPE_REGULAR &&
                 g_str_has_suffix(desktops[i], ".directory") && !create_new) {
        dlg = panel_ditem_editor_new_directory(NULL, NULL, uri,
                                               _("Directory Properties"));
      } else if (type == G_FILE_TYPE_REGULAR &&
                 g_str_has_suffix(desktops[i], ".desktop") && !create_new) {
        dlg = panel_ditem_editor_new(NULL, NULL, uri, _("Launcher Properties"));
      } else if (type == G_FILE_TYPE_REGULAR && create_new) {
        g_printerr(
            "mate-desktop-item-edit: %s "
            "already exists\n",
            uri);
      } else {
        g_printerr(
      ...
      /* a non-existant file.  Well we can still edit that
       * sort of.  We will just create it new */
      dlg = panel_ditem_editor_new(NULL, NULL, uri, _("Create Launcher"));

    } else {
      g_printerr(
          "mate-desktop-item-edit: %s does not "
    ...
    if (dlg != NULL) {
      dialogs++;
      g_signal_connect(dlg, "destroy", G_CALLBACK(dialog_destroyed), NULL);
      g_signal_connect(dlg, "error-reported", G_CALLBACK(error_reported), NULL);
      gtk_widget_show(dlg);
    }
