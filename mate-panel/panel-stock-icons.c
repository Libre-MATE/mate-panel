...
GtkIconSize panel_menu_icon_get_size(void) { return panel_menu_icon_size; }

GtkIconSize panel_menu_bar_icon_get_size(void) {
  return panel_menu_bar_icon_size;
}

GtkIconSize panel_add_to_icon_get_size(void) { return panel_add_to_icon_size; }
...
static void panel_init_stock_icons(GtkIconFactory *factory) {
...
static void panel_init_stock_items(GtkIconFactory *factory) {
  GtkStockItem *items;
  gsize n_items;
  gsize i;
...
void panel_init_stock_icons_and_items(void) {
  ...
  if (icon_size <= 0) {
    panel_menu_icon_size =
        gtk_icon_size_register("panel-menu", PANEL_DEFAULT_MENU_ICON_SIZE,
                               PANEL_DEFAULT_MENU_ICON_SIZE);
  } else {
    /* underscores to prevent themes from altering these settings */
    panel_menu_icon_size =
        gtk_icon_size_register("__panel-menu", icon_size, icon_size);
  }

  icon_size = g_settings_get_enum(settings, "icon-size");
  if (icon_size <= 0) {
    panel_menu_bar_icon_size =
        gtk_icon_size_register("panel-foobar", PANEL_DEFAULT_MENU_BAR_ICON_SIZE,
                               PANEL_DEFAULT_MENU_BAR_ICON_SIZE);
  } else {
    panel_menu_bar_icon_size =
        gtk_icon_size_register("__panel-foobar", icon_size, icon_size);
  }

  panel_add_to_icon_size =
      gtk_icon_size_register("panel-add-to", PANEL_ADD_TO_DEFAULT_ICON_SIZE,
