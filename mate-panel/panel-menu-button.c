...
G_DEFINE_TYPE_WITH_PRIVATE(PanelMenuButton, panel_menu_button,
                           BUTTON_TYPE_WIDGET)

static void panel_menu_button_disconnect_from_gsettings(
...
static const char *panel_menu_path_root_to_filename(MenuPathRoot path_root) {
  const char *retval;
  gsize i;
...
static const char *panel_menu_filename_to_scheme(const char *filename) {
  const char *retval;
  gsize i;
  gchar *basename;
  ...
  for (i = 0; i < G_N_ELEMENTS(root_items); i++) {
    if (root_items[i].filename && !strncmp(basename, root_items[i].filename,
...
static MenuPathRoot panel_menu_scheme_to_path_root(const char *scheme) {
  MenuPathRoot retval;
  gsize i;

  retval = LAST_MENU;

  if (!scheme) return retval;

  for (i = 0; i < G_N_ELEMENTS(root_items); i++) {
    if (root_items[i].scheme &&
        !strncmp(scheme, root_items[i].scheme, strlen(root_items[i].scheme))) {
...
static void panel_menu_button_init(PanelMenuButton *button) {
...
static void panel_menu_button_finalize(GObject *object) {
...
static void panel_menu_button_get_property(GObject *object, guint prop_id,
...
static void panel_menu_button_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  PanelMenuButton *button;
...
static void panel_menu_button_associate_panel(PanelMenuButton *button) {
  PanelWidget *panel_widget = NULL;

  if (!button->priv->menu) return;

  if (button->priv->toplevel)
    panel_widget = panel_toplevel_get_panel_widget(button->priv->toplevel);

  mate_panel_applet_menu_set_recurse(GTK_MENU(button->priv->menu), "menu_panel",
                                     panel_widget);
}

static void panel_menu_button_parent_set(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(panel_menu_button_parent_class)->parent_set)
    GTK_WIDGET_CLASS(panel_menu_button_parent_class)
        ->parent_set(widget, previous_parent);
}

static void panel_menu_button_drag_data_get(GtkWidget *widget,
  ...
  gtk_selection_data_set(selection_data,
                         ...
                         (guchar *)drag_data, strlen(drag_data));

  g_free(drag_data);
}

static void panel_menu_button_menu_deactivated(PanelMenuButton *button) {
...
static void panel_menu_button_menu_detacher(GtkWidget *attach_widget,
                                            GtkMenu *menu) {
  (void)menu;
  ...
  g_signal_handlers_disconnect_by_func(
      button->priv->menu, G_CALLBACK(panel_menu_button_menu_deactivated),
      button);

  button->priv->menu = NULL;
}

static GtkWidget *panel_menu_button_create_menu(PanelMenuButton *button) {
  PanelWidget *panel_widget;

  if (button->priv->menu) return button->priv->menu;

  if (!button->priv->toplevel) return NULL;

  panel_widget = panel_toplevel_get_panel_widget(button->priv->toplevel);

  if (button->priv->use_menu_path && button->priv->path_root > FIRST_MENU &&
      button->priv->path_root < LAST_MENU) {
    const char *filename;

    filename = panel_menu_path_root_to_filename(button->priv->path_root);
    button->priv->menu =
        create_applications_menu(filename, button->priv->menu_path, TRUE);
  } else
    button->priv->menu = create_main_menu(panel_widget);

  gtk_menu_attach_to_widget(GTK_MENU(button->priv->menu), GTK_WIDGET(button),
...
static void panel_menu_button_recreate_menu(PanelMenuButton *button) {
  if (button->priv->menu) gtk_widget_destroy(button->priv->menu);
  button->priv->menu = NULL;
}

void panel_menu_button_popup_menu(PanelMenuButton *button, guint n_button,
  ...
  gtk_window_set_attached_to(
      GTK_WINDOW(gtk_widget_get_toplevel(button->priv->menu)),
      GTK_WIDGET(button));
  ...
  gtk_menu_popup_at_widget(GTK_MENU(button->priv->menu), GTK_WIDGET(button),
                           widget_anchor, menu_anchor, NULL);
}

static void panel_menu_button_pressed(GtkButton *gtk_button) {
  PanelMenuButton *button;
...
static void panel_menu_button_clicked(GtkButton *gtk_button) {
  PanelMenuButton *button;
  GdkEvent *event;
  ...
  if ((event = gtk_get_current_event())) {
    panel_menu_button_popup_menu(button, event->button.button,
                                 event->button.time);
...
static void panel_menu_button_class_init(PanelMenuButtonClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_MENU_PATH,
      g_param_spec_string("menu-path", "Menu Path",
                          "The path from which to construct the menu", NULL,
                          G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_CUSTOM_ICON,
      g_param_spec_string("custom-icon", "Custom Icon",
                          "The custom icon for the menu", NULL,
                          G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_TOOLTIP,
      g_param_spec_string("tooltip", "Tooltip",
                          "Tooltip displayed for the menu", NULL,
                          G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_USE_MENU_PATH,
      g_param_spec_boolean("use-menu-path", "Use Menu Path",
                           "Use the path specified by the menu-path property",
                           FALSE, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_USE_CUSTOM_ICON,
      g_param_spec_boolean("use-custom-icon", "Use Custom Icon",
                           "Use the icon specified by the custom-icon property",
                           FALSE, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_HAS_ARROW,
      g_param_spec_boolean(
          "has-arrow", "Has Arrow",
          "Determine if an arrow is drawn over the menu button icon", FALSE,
          G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_DND_ENABLED,
      g_param_spec_boolean(
          "dnd-enabled", "Drag and drop enabled",
          "Whether or not drag and drop is enabled on the widget", FALSE,
          G_PARAM_READWRITE));
}

static void panel_menu_button_gsettings_notify(GSettings *settings, gchar *key,
                                               PanelMenuButton *button) {
  if (!strcmp(key, PANEL_OBJECT_MENU_PATH_KEY)) {
    ...
    panel_menu_button_set_use_custom_icon(
        button, g_settings_get_boolean(settings, key));
...
static void panel_menu_button_connect_to_gsettings(PanelMenuButton *button) {
  gchar *path;
  ...
  g_signal_connect(button->priv->settings, "changed",
                   G_CALLBACK(panel_menu_button_gsettings_notify), button);
  g_free(path);
}
...
static void panel_menu_button_disconnect_from_gsettings(
  ...
  if (button->priv->settings) {
    g_signal_handlers_disconnect_by_func(
        button->priv->settings, G_CALLBACK(panel_menu_button_gsettings_notify),
        button);
...
static void panel_menu_button_load(
    const char *menu_path, gboolean use_menu_path, const char *custom_icon,
    gboolean use_custom_icon, const char *tooltip, PanelWidget *panel,
    gboolean locked, int position, gboolean exactpos, gboolean has_arrow,
    const char *id) {
  PanelMenuButton *button;
  AppletInfo *info;
  ...
  button = g_object_new(PANEL_TYPE_MENU_BUTTON, "menu-path", menu_path,
                        "custom-icon", custom_icon, "tooltip", tooltip,
                        "use-menu-path", use_menu_path, "use-custom-icon",
                        use_custom_icon, "has-arrow", has_arrow, NULL);

  info =
      mate_panel_applet_register(GTK_WIDGET(button), NULL, NULL, panel, locked,
  ...
  mate_panel_applet_add_callback(info, "help", "help-browser", _("_Help"),
                                 NULL);
...
static char *panel_menu_button_get_icon(PanelMenuButton *button) {
  char *retval = NULL;

  if (button->priv->use_custom_icon && button->priv->custom_icon)
    retval = g_strdup(button->priv->custom_icon);

  if (!retval && button->priv->use_menu_path && button->priv->menu_path &&
      panel_menu_button_create_menu(button)) {
    MateMenuTreeDirectory *directory = g_object_get_data(
        G_OBJECT(button->priv->menu), "panel-menu-tree-directory");
    if (!directory) {
      MateMenuTree *tree;

      if ((tree = g_object_get_data(G_OBJECT(button->priv->menu),
                                    "panel-menu-tree"))) {
        directory = matemenu_tree_get_directory_from_path(
            tree, button->priv->menu_path);
        g_object_set_data_full(G_OBJECT(button->priv->menu),
                               "panel-menu-tree-directory", directory,
    ...
    if (directory) {
      ...
      if (gicon != NULL) retval = g_icon_to_string(gicon);
    }
  }

  if (!retval) retval = g_strdup(PANEL_ICON_MAIN_MENU);
...
static const char *split_menu_uri(const char *menu_uri, char **menu_scheme) {
  char *p;

  if (!menu_uri) return NULL;

  p = strchr(menu_uri, ':');

  if (!p || p == menu_uri) return NULL;

  if (menu_scheme) *menu_scheme = g_strndup(menu_uri, p - menu_uri);
...
void panel_menu_button_set_menu_path(PanelMenuButton *button,
  ...
  if (!button->priv->menu_path && (!menu_path || !menu_path[0])) return;
  ...
  if (button->priv->menu) gtk_menu_detach(GTK_MENU(button->priv->menu));
  button->priv->menu = NULL;
...
void panel_menu_button_set_use_menu_path(PanelMenuButton *button,
  ...
  if (button->priv->use_menu_path == use_menu_path) return;
...
void panel_menu_button_load_from_gsettings(PanelWidget *panel, gboolean locked,
                                           int position, gboolean exactpos,
                                           const char *id) {
  GSettings *settings;
  gchar *path;
  char *menu_path;
  ...
  use_menu_path =
      g_settings_get_boolean(settings, PANEL_OBJECT_USE_MENU_PATH_KEY);
  use_custom_icon =
  ...
  panel_menu_button_load(menu_path, use_menu_path, custom_icon, use_custom_icon,
...
gboolean panel_menu_button_create(PanelToplevel *toplevel, int position,
                                  const char *filename, const char *menu_path,
                                  gboolean use_menu_path, const char *tooltip) {
  GSettings *settings;
  gchar *path;
  const char *scheme;
  char *id;
  ...
  g_settings_set_boolean(settings, PANEL_OBJECT_USE_MENU_PATH_KEY,
                         use_menu_path);
...
void panel_menu_button_invoke_menu(PanelMenuButton *button,
  ...
  if (!strcmp(callback_name, "help")) {
    ...
    if (panel_is_program_in_path("menulibre"))
      panel_launch_desktop_file_with_fallback("menulibre.desktop", "menulibre",
                                              screen, NULL);
    else
      panel_launch_desktop_file_with_fallback("mozo.desktop", "mozo", screen,
                                              NULL);
  }
}
...
void panel_menu_button_set_dnd_enabled(PanelMenuButton *button,
  ...
  if (button->priv->dnd_enabled == dnd_enabled) return;
  ...
  if (dnd_enabled) {
    ...
    gtk_drag_source_set(GTK_WIDGET(button), GDK_BUTTON1_MASK, dnd_targets, 1,
...
#define PANEL_IS_MENU_BUTTON_ACCESSIBLE(o) \
  (G_TYPE_CHECK_INSTANCE_TYPE((o), panel_menu_button_accessible_get_type()))

static GType panel_menu_button_accessible_get_type(void);
static gpointer parent_accessible_class = NULL;

static int panel_menu_button_accessible_get_n_children(AtkObject *obj) {
  g_return_val_if_fail(PANEL_IS_MENU_BUTTON_ACCESSIBLE(obj), 0);

  return gtk_accessible_get_widget(GTK_ACCESSIBLE(obj)) ? 1 : 0;
}

static AtkObject *panel_menu_button_accessible_ref_child(AtkObject *obj,
                                                         int index) {
  PanelMenuButton *button;
  GtkWidget *menu;
  ...
  if (!(button =
            PANEL_MENU_BUTTON(gtk_accessible_get_widget(GTK_ACCESSIBLE(obj)))))
    return NULL;

  if (!(menu = panel_menu_button_create_menu(button))) return NULL;
  /*
...
static void panel_menu_button_accessible_class_init(AtkObjectClass *klass) {
...
static GType panel_menu_button_accessible_get_type(void) {
  ...
  if (!type) {
    ...
    accessible_parent_type = panel_a11y_query_accessible_parent_type(
        PANEL_TYPE_MENU_BUTTON, &type_info);

    type = g_type_register_static(accessible_parent_type,
                                  "PanelMenuButtonAccessible", &type_info, 0);
  }

  return type;
}

static AtkObject *panel_menu_button_accessible_new(GObject *obj) {
...
static void panel_menu_button_accessible_factory_class_init(
...
static GType panel_menu_button_accessible_factory_get_type(void) {
  static GType type = 0;
  ...
  if (!type) {
    ...
    type = g_type_register_static(ATK_TYPE_OBJECT_FACTORY,
                                  "PanelMenuButtonAccessibleFactory", &info, 0);
  }
...
static AtkObject *panel_menu_button_get_accessible(GtkWidget *widget) {
  ...
  if (first_time && panel_a11y_get_is_a11y_enabled(widget))
    atk_registry_set_factory_type(
        atk_get_default_registry(), PANEL_TYPE_MENU_BUTTON,
        panel_menu_button_accessible_factory_get_type());

  first_time = FALSE;

  return GTK_WIDGET_CLASS(panel_menu_button_parent_class)
