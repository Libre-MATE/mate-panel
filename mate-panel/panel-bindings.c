...
static void panel_binding_set_from_string(PanelBinding *binding,
                                          const char *str) {
...
static inline GtkBindingSet *get_binding_set(GtkBindingSet *binding_set) {
  if (!binding_set) {
    PanelToplevelClass *toplevel_class;

    toplevel_class = g_type_class_peek(PANEL_TYPE_TOPLEVEL);
    if (!toplevel_class) return NULL;
...
static void panel_binding_clear_entry(PanelBinding *binding,
...
static void panel_binding_set_entry(PanelBinding *binding,
                                    GtkBindingSet *binding_set) {
  binding_set = get_binding_set(binding_set);

  gtk_binding_entry_add_signal(binding_set, binding->keyval, binding->modifiers,
                               binding->signal, 0);
}

static void panel_binding_changed(GSettings *settings, gchar *key,
                                  PanelBinding *binding) {
  if (binding->keyval) panel_binding_clear_entry(binding, NULL);
  ...
  if (!binding->keyval) return;

  panel_binding_set_entry(binding, NULL);
}

static void panel_binding_watch(PanelBinding *binding, const char *key) {
  gchar *signal_name;
  signal_name = g_strdup_printf("changed::%s", key);
  g_signal_connect(marco_keybindings_settings, signal_name,
                   G_CALLBACK(panel_binding_changed), binding);
  g_free(signal_name);
}

static void panel_bindings_mouse_modifier_set_from_string(const char *str) {
...
static void panel_bindings_mouse_modifier_changed(GSettings *settings,
                                                  gchar *key,
                                                  gpointer user_data) {
  gchar *value = g_settings_get_string(settings, key);
...
static void panel_bindings_initialise(void) {
  gsize i;
  char *str;
  ...
  g_signal_connect(marco_settings, "changed::" MARCO_MOUSE_BUTTON_MODIFIER_KEY,
                   G_CALLBACK(panel_bindings_mouse_modifier_changed), NULL);
...
void panel_bindings_set_entries(GtkBindingSet *binding_set) {
  ...
  if (!initialised) panel_bindings_initialise();

  for (i = 0; i < G_N_ELEMENTS(bindings); i++) {
    if (!bindings[i].keyval) continue;
