...
void orientation_change(AppletInfo *info, PanelWidget *panel) {
  ...
  switch (info->type) {
    ...
    case PANEL_OBJECT_DRAWER: {
      ...
      gtk_container_foreach(GTK_CONTAINER(panel_widget), orient_change_foreach,
                            panel_widget);
    } break;
...
static void panel_orient_change(GtkWidget *widget, gpointer data) {
  gtk_container_foreach(GTK_CONTAINER(widget), orient_change_foreach, widget);
}
...
void size_change(AppletInfo *info, PanelWidget *panel) {
  if (info->type == PANEL_OBJECT_APPLET)
    mate_panel_applet_frame_change_size(MATE_PANEL_APPLET_FRAME(info->widget),
                                        panel->sz);
}
...
static void panel_size_change(GtkWidget *widget, gpointer data) {
  gtk_container_foreach(GTK_CONTAINER(widget), size_change_foreach, widget);
}
  ...
  switch (info->type) {
    case PANEL_OBJECT_APPLET:
      mate_panel_applet_frame_change_background(
          ...
          panel->toplevel->background.type);
      break;
    case PANEL_OBJECT_MENU_BAR:
...
static void back_change_foreach(GtkWidget *widget, PanelWidget *panel) {
  AppletInfo *info;
...
static void panel_back_change(GtkWidget *widget, gpointer data) {
  gtk_container_foreach(GTK_CONTAINER(widget), (GtkCallback)back_change_foreach,
                        widget);
...
static void mate_panel_applet_added(GtkWidget *widget, GtkWidget *applet,
                                    gpointer data) {
  AppletInfo *info;
...
static void mate_panel_applet_removed(GtkWidget *widget, GtkWidget *applet,
                                      gpointer data) {
  ...
  if (info->type == PANEL_OBJECT_DRAWER) {
    ...
    if (drawer->toplevel) panel_toplevel_queue_auto_hide(toplevel);
  }
}

static gboolean deactivate_idle(gpointer data) {
  PanelData *pd = data;
...
static void context_menu_deactivate(GtkWidget *w, PanelData *pd) {
...
static void context_menu_show(GtkWidget *w, PanelData *pd) {
  panel_toplevel_push_autohide_disabler(PANEL_TOPLEVEL(pd->panel));
}

static void panel_recreate_context_menu(PanelData *pd) {
  if (pd->menu) g_object_unref(pd->menu);
  pd->menu = NULL;
}

static void panel_destroy(PanelToplevel *toplevel, PanelData *pd) {
  panel_lockdown_notify_remove(G_CALLBACK(panel_recreate_context_menu), pd);

  if (pd->menu) {
    g_signal_handlers_disconnect_by_func(pd->menu, context_menu_deactivate, pd);
  ...
  if (pd->deactivate_idle != 0) g_source_remove(pd->deactivate_idle);
  pd->deactivate_idle = 0;
...
static void mate_panel_applet_move(PanelWidget *panel, GtkWidget *widget,
                                   gpointer data) {
  AppletInfo *info;
...
static GtkWidget *panel_menu_get(PanelWidget *panel, PanelData *pd) {
  if (!pd->menu) {
    pd->menu = g_object_ref_sink(panel_context_menu_create(panel));
    g_signal_connect(pd->menu, "deactivate",
                     G_CALLBACK(context_menu_deactivate), pd);
    g_signal_connect(pd->menu, "show", G_CALLBACK(context_menu_show), pd);
  }
...
static gboolean panel_popup_menu(PanelToplevel *toplevel, guint button,
...
static gboolean panel_popup_menu_signal(PanelToplevel *toplevel) {
  return panel_popup_menu(toplevel, 3, GDK_CURRENT_TIME);
}

static gboolean panel_button_press_event(PanelToplevel *toplevel,
...
static gboolean panel_key_press_event(GtkWidget *widget, GdkEventKey *event) {
#ifdef HAVE_X11
  /*
  ...
  if (GTK_IS_SOCKET(gtk_window_get_focus(GTK_WINDOW(widget))) &&
      ...
      (event->state & gtk_accelerator_get_default_mod_mask()) ==
          GDK_CONTROL_MASK)
    return gtk_bindings_activate(G_OBJECT(widget), event->keyval, event->state);
#endif

  return FALSE;
}

static gboolean can_set_background_image(PanelToplevel *toplevel,
                                         const gchar *uri, gchar **filename) {
  gchar *image;
...
static gboolean set_background_image_from_uri(PanelToplevel *toplevel,
                                              const char *uri) {
  char *image;

  if (!can_set_background_image(toplevel, uri, &image)) return FALSE;
...
static gboolean set_background_color(PanelToplevel *toplevel,
                                     guint16 *dropped) {
  GdkRGBA color;
...
static gboolean drop_url(PanelWidget *panel, int position, const char *url) {
  enum { NETSCAPE_URL_URL, NETSCAPE_URL_NAME };
  char **netscape_url;
  char *name;
  char *comment;
  ...
  if (!panel_profile_id_lists_are_writable()) return FALSE;

  netscape_url = g_strsplit(url, "\n", 2);
  if (!netscape_url || PANEL_GLIB_STR_EMPTY(netscape_url[NETSCAPE_URL_URL])) {
    g_strfreev(netscape_url);
    return FALSE;
  }

  comment = g_strdup_printf(_("Open URL: %s"), netscape_url[NETSCAPE_URL_URL]);
  ...
  panel_launcher_create_from_info(panel->toplevel, position, FALSE,
                                  netscape_url[NETSCAPE_URL_URL], name, comment,
                                  PANEL_ICON_REMOTE);
...
static gboolean drop_menu(PanelWidget *panel, int position,
                          const char *menu_filename, const char *menu_path) {
  if (!panel_profile_id_lists_are_writable()) return FALSE;

  return panel_menu_button_create(panel->toplevel, position, menu_filename,
                                  menu_path, menu_path != NULL, NULL);
}

static gboolean drop_uri(PanelWidget *panel, int position, const char *uri,
  ...
  if (!icon) icon = g_strdup(fallback_icon);
  ...
  panel_launcher_create_from_info(panel->toplevel, position, FALSE, uri, name,
                                  comment, icon);
...
static gboolean drop_caja_desktop_uri(PanelWidget *panel, int pos,
                                      const char *uri) {
  gboolean success;
  const char *id;
  const char *basename;
  ...
  if (strncmp(basename, "trash", strlen("trash")) == 0)
    ...
    panel_launcher_create_with_id(id, pos, "caja-home.desktop");
  else if (strncmp(basename, "computer", strlen("computer")) == 0)
    panel_launcher_create_with_id(id, pos, "caja-computer.desktop");
  else if (strncmp(basename, "network", strlen("network")) == 0)
    panel_launcher_create_with_id(id, pos, "caja-scheme.desktop");
  else
    success = FALSE;
...
enum { RESPONSE_SET_BACKGROUND, RESPONSE_CREATE_LAUNCHER };

static void image_drop_dialog_response(GtkWidget *dialog, int response,
                                       PanelWidget *panel) {
...
static GtkWidget *image_drop_dialog(PanelToplevel *toplevel, const gchar *uri) {
  GtkWidget *dialog;
  GFile *file;
  GFileInfo *info;
  gchar *display_name;
  ...
  dialog = gtk_message_dialog_new(
      GTK_WINDOW(toplevel), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
      GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE,
      _("You dropped an image on the panel, what do you want to do?"));

  secondary_p1 = g_strdup_printf(
      _("You dropped the image \"%s\" on the panel."), display_name);
  if (panel_profile_id_lists_are_writable())
    secondary_p2 =
        _("Do you want to create a launcher for it or "
          "use it as the panel's background?");
  else
    secondary_p2 = _("Do you want to use it as the panel's background?");

  gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), "%s %s",
                                           secondary_p1, secondary_p2);

  g_free(secondary_p1);
  g_free(display_name);

  panel_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), "process-stop",
                          GTK_RESPONSE_CANCEL);
  if (panel_profile_id_lists_are_writable()) {
    panel_dialog_add_button(GTK_DIALOG(dialog), _("Create _Launcher"),
                            "mate-panel-launcher", RESPONSE_CREATE_LAUNCHER);
  }
  panel_dialog_add_button(GTK_DIALOG(dialog), _("Set _Background Image"),
                          "background", RESPONSE_SET_BACKGROUND);
...
static gboolean ask_about_image_drop(PanelWidget *panel, int pos,
                                     const char *uri) {
  GtkWidget *dialog;
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(image_drop_dialog_response),
                   panel);
...
static gboolean drop_urilist(PanelWidget *panel, int pos, char *urilist) {
  char **uris;
  guint n_uris;
  gboolean success;
  int i;
  ...
  for (i = 0; uris[i]; i++) {
    ...
    if (g_ascii_strncasecmp(uri, "http:", strlen("http:")) == 0 ||
      ...
      if (!drop_url(panel, pos, uri)) success = FALSE;
      continue;
    }

    if (g_ascii_strncasecmp(
            uri, "x-caja-desktop:", strlen("x-caja-desktop:")) == 0) {
    ...
    if (info) {
      ...
      can_exec = g_file_info_get_attribute_boolean(
          info, G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE);

      if (n_uris == 1 && mime && g_str_has_prefix(mime, "image")) {
        if (!ask_about_image_drop(panel, pos, uri)) success = FALSE;
      } else if (mime && (!strcmp(mime, "application/x-mate-app-info") ||
        ...
        if (!drop_uri(panel, pos, uri, PANEL_ICON_UNKNOWN)) success = FALSE;
      }
    } else {
      if (!drop_uri(panel, pos, uri, PANEL_ICON_UNKNOWN)) success = FALSE;
    }
...
static gboolean drop_internal_icon(PanelWidget *panel, int pos,
                                   const char *icon_name, int action) {
  ...
  if (action == GDK_ACTION_MOVE) old_launcher = find_launcher(icon_name);
...
static gboolean move_applet(PanelWidget *panel, int pos, int applet_index) {
  GSList *applet_list;
  AppletInfo *info;
  GtkWidget *parent;
  ...
  if (!mate_panel_applet_can_freely_move(info)) return FALSE;

  if (pos < 0) pos = 0;

  parent = gtk_widget_get_parent(info->widget);

  if (info != NULL && info->widget != NULL && parent != NULL &&
    ...
    if (!g_slist_find(forb, panel))
      panel_widget_reparent(PANEL_WIDGET(parent), panel, info->widget, pos);
  }

  return TRUE;
}

static gboolean drop_internal_applet(PanelWidget *panel, int pos,
                                     const char *applet_type, int action) {
  int applet_index = -1;
  gboolean remove_applet = FALSE;
  gboolean success = FALSE;

  if (applet_type == NULL) return FALSE;
  ...
  if (sscanf(applet_type, "MENU:%d", &applet_index) == 1 ||
    ...
    if (!menu_path) {
      ...
      menu_path++;
      success = drop_menu(panel, pos, menu_filename, menu_path);
    ...
    if (panel_profile_id_lists_are_writable()) {
      remove_applet = panel_action_button_load_from_drag(
          panel->toplevel, pos, applet_type, &applet_index);
      success = TRUE;
    } else {
      success = FALSE;
    }
  ...
  if (remove_applet && action == GDK_ACTION_MOVE) {
    AppletInfo *info;
    GSList *applet_list;
    ...
    if (info) panel_profile_delete_object(info);
  }

  return success;
}

static GtkTargetList *get_target_list(void) {
...
gboolean panel_check_dnd_target_data(GtkWidget *widget, GdkDragContext *context,
  ...
  if (!PANEL_IS_TOPLEVEL(widget) && !BUTTON_IS_WIDGET(widget)) return FALSE;

  if (!(gdk_drag_context_get_actions(context) &
...
static void do_highlight(GtkWidget *widget, gboolean highlight) {
  gboolean have_drag;
  ...
  have_drag = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "have-drag"));
  if (highlight) {
    if (!have_drag) {
      g_object_set_data(G_OBJECT(widget), "have-drag", GINT_TO_POINTER(TRUE));
      gtk_drag_highlight(widget);
    }
  } else {
    if (have_drag) {
      g_object_set_data(G_OBJECT(widget), "have-drag", NULL);
...
gboolean panel_check_drop_forbidden(PanelWidget *panel, GdkDragContext *context,
  ...
  if (info == TARGET_APPLET_INTERNAL) {
    ...
    if (BUTTON_IS_WIDGET(source_widget)) {
      ...
      if (g_slist_find(forb, panel)) return FALSE;
    }
  }

  if (info == TARGET_ICON_INTERNAL || info == TARGET_APPLET_INTERNAL) {
...
static gboolean drag_motion_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  if (!panel_check_dnd_target_data(widget, context, &info, NULL)) return FALSE;
...
static gboolean drag_drop_cb(GtkWidget *widget, GdkDragContext *context, gint x,
                             gint y, guint time, Launcher *launcher) {
...
void panel_receive_dnd_data(PanelWidget *panel, guint info, int pos,
  ...
  switch (info) {
    ...
    case TARGET_DIRECTORY:
      success = drop_uri(panel, pos, (char *)data, PANEL_ICON_FOLDER);
      break;
    case TARGET_APPLET:
    ...
    case TARGET_APPLET_INTERNAL:
      success =
          drop_internal_applet(panel, pos, (char *)data,
                               gdk_drag_context_get_selected_action(context));
      break;
    case TARGET_ICON_INTERNAL:
      success =
          drop_internal_icon(panel, pos, (char *)data,
                             gdk_drag_context_get_selected_action(context));
      break;
    default:
      gtk_drag_finish(context, FALSE, FALSE, time_);
...
static void drag_data_recieved_cb(GtkWidget *widget, GdkDragContext *context,
                                  gint x, gint y,
  ...
  panel_receive_dnd_data(panel_widget, info, pos, selection_data, context,
                         time);
}

static void panel_widget_setup(PanelWidget *panel) {
  g_signal_connect(panel, "applet-added", G_CALLBACK(mate_panel_applet_added),
                   NULL);
  g_signal_connect(panel, "applet-removed",
                   G_CALLBACK(mate_panel_applet_removed), NULL);
  g_signal_connect(panel, "applet-move", G_CALLBACK(mate_panel_applet_move),
                   NULL);
  g_signal_connect(panel, "back-change", G_CALLBACK(panel_back_change), NULL);
  g_signal_connect(panel, "size-change", G_CALLBACK(panel_size_change), NULL);
}
...
PanelData *panel_setup(PanelToplevel *toplevel) {
  ...
  panel_lockdown_notify_add(G_CALLBACK(panel_recreate_context_menu), pd);

  panel_widget_setup(panel_widget);

  g_signal_connect(toplevel, "drag-data-received",
                   G_CALLBACK(drag_data_recieved_cb), NULL);
  g_signal_connect(toplevel, "drag-motion", G_CALLBACK(drag_motion_cb), NULL);
  g_signal_connect(toplevel, "drag-leave", G_CALLBACK(drag_leave_cb), NULL);
  g_signal_connect(toplevel, "drag-drop", G_CALLBACK(drag_drop_cb), NULL);
  ...
  g_signal_connect(toplevel, "popup-menu", G_CALLBACK(panel_popup_menu_signal),
                   NULL);
...
static void panel_delete_without_query(PanelToplevel *toplevel) {
  PanelWidget *panel_widget;

  panel_widget = panel_toplevel_get_panel_widget(toplevel);

  if (panel_toplevel_get_is_attached(toplevel) && panel_widget->master_widget) {
    AppletInfo *info;

    info =
        g_object_get_data(G_OBJECT(panel_widget->master_widget), "applet_info");
...
static void panel_deletion_response(GtkWidget *dialog, int response,
                                    PanelToplevel *toplevel) {
...
static void panel_deletion_destroy_dialog(GtkWidget *widget,
                                          PanelToplevel *toplevel) {
...
GtkWidget *panel_deletion_dialog(PanelToplevel *toplevel) {
  ...
  if (panel_toplevel_get_is_attached(toplevel)) {
    ...
    text2 =
        _("When a drawer is deleted, the drawer and its\n"
          "settings are lost.");
  } else {
  ...
  panel_dialog_add_button(GTK_DIALOG(dialog), _("_Delete"), "edit-delete",
                          GTK_RESPONSE_OK);
  ...
  g_signal_connect(dialog, "destroy", G_CALLBACK(panel_deletion_destroy_dialog),
                   toplevel);
...
static void panel_query_deletion(PanelToplevel *toplevel) {
  GtkWidget *dialog;
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(panel_deletion_response),
                   toplevel);

  g_signal_connect_object(toplevel, "destroy", G_CALLBACK(gtk_widget_destroy),
