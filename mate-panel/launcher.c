...
static gboolean launcher_properties_enabled(void);

static GdkScreen *launcher_get_screen(Launcher *launcher) {
...
static void launcher_widget_open_dialog_destroyed(GtkWidget *dialog,
                                                  Launcher *launcher) {
...
static void launcher_widget_destroy_open_dialogs(Launcher *launcher) {
  GSList *l, *list;
  ...
  for (l = list; l; l = l->next) {
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(l->data), G_CALLBACK(launcher_widget_open_dialog_destroyed),
        launcher);
...
static void launcher_register_error_dialog(Launcher *launcher,
                                           GtkWidget *dialog) {
  launcher->error_dialogs = g_slist_append(launcher->error_dialogs, dialog);
  g_signal_connect(dialog, "destroy",
                   G_CALLBACK(launcher_widget_open_dialog_destroyed), launcher);
}

static void launch_url(Launcher *launcher) {
  char *url;
  GdkScreen *screen;
  ...
  if (!url || *url == 0) {
    ...
    error_dialog = panel_error_dialog(NULL, screen, "no_url_dialog", TRUE,
  ...
  if (type && !strcmp(type, "Link"))
    ...
    if (error) {
      ...
      error_dialog = panel_error_dialog(
          NULL, launcher_get_screen(launcher), "cannot_launch_application",
          TRUE, _("Could not launch application"), error->message);
      launcher_register_error_dialog(launcher, error_dialog);
      g_clear_error(&error);
    }
  }
...
static void drag_data_received_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  if (is_using_x11() && panel_global_config_get_enable_animations()) {
    ...
    xstuff_zoom_animate(widget, surface,
  ...
  for (i = 0; uris[i]; i++) file_list = g_list_prepend(file_list, uris[i]);
  ...
  if (error) {
    GtkWidget *error_dialog;
    error_dialog = panel_error_dialog(
        NULL, launcher_get_screen(launcher), "cannot_use_dropped_item", TRUE,
        _("Could not use dropped item"), error->message);
    launcher_register_error_dialog(launcher, error_dialog);
    g_clear_error(&error);
  }
...
static void destroy_launcher(GtkWidget *widget, Launcher *launcher) {
...
static void free_launcher(gpointer data) {
  Launcher *launcher = data;

  if (launcher->key_file) g_key_file_free(launcher->key_file);

  g_free(launcher->location);

  if (launcher->monitor != NULL) g_object_unref(launcher->monitor);

  g_free(launcher);
}

void panel_launcher_delete(Launcher *launcher) {
  if (!launcher->location) return;
...
static gboolean is_this_drop_ok(GtkWidget *widget, GdkDragContext *context) {
  ...
  if (!(gdk_drag_context_get_actions(context) & GDK_ACTION_COPY)) return FALSE;

  if (!text_uri_list)
    text_uri_list = gdk_atom_intern_static_string("text/uri-list");

  for (l = gdk_drag_context_list_targets(context); l; l = l->next) {
    if (GDK_POINTER_TO_ATOM(l->data) == text_uri_list) break;
  }
...
static gboolean drag_motion_cb(GtkWidget *widget, GdkDragContext *context,
                               gint x, gint y, guint time, Launcher *launcher) {
  if (!is_this_drop_ok(widget, context)) return FALSE;
...
enum { TARGET_ICON_INTERNAL, TARGET_URI_LIST };
...
static void drag_data_get_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  if (info == TARGET_URI_LIST) {
    ...
    gtk_selection_data_set(selection_data,
                           ...
                           (unsigned char *)location, strlen(location));
}

static void clicked_cb(Launcher *launcher, GtkWidget *widget) {
#ifdef HAVE_X11
  if (is_using_x11() && panel_global_config_get_enable_animations()) {
    cairo_surface_t *surface;
    surface = button_widget_get_surface(BUTTON_WIDGET(widget));
...
static Launcher *create_launcher(const char *location) {
  GKeyFile *key_file;
  ...
  if (!strchr(location, G_DIR_SEPARATOR)) {
    ...
    if (!g_file_test(path, G_FILE_TEST_EXISTS)) {
       ...
       * this impossible */
      if (path) new_location = g_strdup(path);
    }
    ...
    loaded = panel_key_file_load_from_uri(
        key_file, location,
        G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, &error);
  ...
  if (!new_location) new_location = g_strdup(location);
  ...
  launcher->button =
      button_widget_new(NULL /* icon */, FALSE, PANEL_ORIENTATION_TOP);
  ...
  /*gtk_drag_dest_set (GTK_WIDGET (launcher->button),
                     ...
                     dnd_targets, 2,
                     GDK_ACTION_COPY);*/
  gtk_drag_dest_set(GTK_WIDGET(launcher->button), 0, NULL, 0, 0);
  ...
  g_signal_connect(launcher->button, "drag-motion", G_CALLBACK(drag_motion_cb),
                   launcher);
  g_signal_connect(launcher->button, "drag-drop", G_CALLBACK(drag_drop_cb),
                   launcher);
  g_signal_connect(launcher->button, "drag-leave", G_CALLBACK(drag_leave_cb),
                   launcher);
  g_signal_connect_swapped(launcher->button, "clicked", G_CALLBACK(clicked_cb),
                           launcher);

  launcher->destroy_handler = g_signal_connect(
      launcher->button, "destroy", G_CALLBACK(destroy_launcher), launcher);

  return launcher;
}

static void setup_actions(Launcher *launcher) {
  ...
  for (; *ptr != NULL; ptr++) {
    ...
    mate_panel_applet_add_callback(launcher->info, callback, NULL, action_name,
                                   NULL);
    g_free(callback);
    g_free(action_name);
  }
...
static void setup_button(Launcher *launcher) {
  ...
  mate_panel_applet_add_callback(launcher->info, "launch", "system-run",
                                 _("_Launch"), NULL);

  setup_actions(launcher);

  mate_panel_applet_add_callback(launcher->info, "properties",
                                 "document-properties", _("_Properties"),
                                 launcher_properties_enabled);

  name = panel_key_file_get_locale_string(launcher->key_file, "Name");
  comment = panel_key_file_get_locale_string(launcher->key_file, "Comment");
  ...
  if (icon && icon[0] == '\0') g_clear_pointer(&icon, g_free);

  if (!icon) {
    gchar *exec;
    exec = panel_key_file_get_string(launcher->key_file, "Exec");
    if (exec && exec[0] != '\0') {
      icon = guess_icon_from_exec(
          button_widget_get_icon_theme(BUTTON_WIDGET(launcher->button)), exec);
    }

    g_free(exec);
  }

  if (!icon) icon = g_strdup(PANEL_ICON_LAUNCHER);
...
static char *panel_launcher_find_writable_uri(const char *launcher_location,
                                              const char *source) {
  if (!launcher_location) return panel_make_unique_desktop_uri(NULL, source);
  ...
  if (filename != NULL) {
    ...
    if (g_path_is_absolute(launcher_location))
      return g_filename_to_uri(launcher_location, NULL, NULL);
    else
...
static void launcher_changed(PanelDItemEditor *dialog, Launcher *launcher) {
  /* Setup the button look */
  setup_button(launcher);
}

static void launcher_command_changed(PanelDItemEditor *dialog,
                                     const char *command, Launcher *launcher) {
  ...
  if (revert_key_file) {
    ...
    if (!old_exec || !exec || strcmp(old_exec, exec))
      panel_key_file_remove_key(launcher->key_file, "StartupNotify");
...
static char *launcher_save_uri(PanelDItemEditor *dialog, gpointer data) {
  GKeyFile *key_file;
  char *type;
  char *exec_or_uri;
  Launcher *launcher;
  ...
  if (launcher) {
    new_uri = panel_launcher_find_writable_uri(launcher->location, exec_or_uri);
  } else
  ...
  if (!uri || (new_uri && strcmp(new_uri, uri))) return new_uri;
...
static void launcher_saved(GtkWidget *dialog, Launcher *launcher) {
  const char *uri;
  char *filename;
  ...
  if (uri && launcher->location && strcmp(uri, launcher->location)) {
    g_settings_set_string(launcher->info->settings,
                          PANEL_OBJECT_LAUNCHER_LOCATION_KEY, uri);

    if (launcher->location) g_free(launcher->location);
...
static void launcher_error_reported(GtkWidget *dialog, const char *primary,
                                    const char *secondary, gpointer data) {
  panel_error_dialog(GTK_WINDOW(dialog), NULL, "error_editing_launcher", TRUE,
                     primary, secondary);
}
...
void launcher_properties(Launcher *launcher) {
  ...
  launcher->prop_dialog = panel_ditem_editor_new(
      NULL, launcher->key_file, launcher->location, _("Launcher Properties"));

  panel_widget_register_open_dialog(
      PANEL_WIDGET(gtk_widget_get_parent(launcher->info->widget)),
      launcher->prop_dialog);

  panel_ditem_register_save_uri_func(PANEL_DITEM_EDITOR(launcher->prop_dialog),
                                     launcher_save_uri, launcher);
  ...
  g_signal_connect(launcher->prop_dialog, "saved", G_CALLBACK(launcher_saved),
                   launcher);
  ...
  g_signal_connect(launcher->prop_dialog, "destroy",
                   G_CALLBACK(gtk_widget_destroyed), &launcher->prop_dialog);
...
static void app_desktop_file_changed(GFileMonitor *monitor, GFile *file,
                                     GFile *other_file,
  ...
  if (!strchr(launcher->location, G_DIR_SEPARATOR)) {
    g_key_file_load_from_file(
        launcher->key_file, launcher->location,
        G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);
  } else {
    panel_key_file_load_from_uri(
        launcher->key_file, launcher->location,
        G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);
  }
  setup_button(launcher);
...
static Launcher *load_launcher_applet(const char *location, PanelWidget *panel,
                                      gboolean locked, int pos,
  ...
  launcher->info = mate_panel_applet_register(
      launcher->button, launcher, free_launcher, panel, locked, pos, exactpos,
  ...
  panel_widget_set_applet_expandable(panel, GTK_WIDGET(launcher->button), FALSE,
                                     TRUE);
  panel_widget_set_applet_size_constrained(panel, GTK_WIDGET(launcher->button),
                                           TRUE);

  g_signal_connect(launcher->monitor, "changed",
                   G_CALLBACK(app_desktop_file_changed), launcher);
...
void launcher_load_from_gsettings(PanelWidget *panel_widget, gboolean locked,
  ...
  launcher = load_launcher_applet(launcher_location, panel_widget, locked,
                                  position, TRUE, id);
  ...
  if (launcher) {
    if (!g_settings_is_writable(settings, PANEL_OBJECT_LAUNCHER_LOCATION_KEY)) {
      ...
      menu = mate_panel_applet_get_callback(launcher->info->user_menu,
                                            "properties");
      if (menu != NULL) menu->sensitive = FALSE;
    }
  }
...
static void launcher_new_saved(GtkWidget *dialog, gpointer data) {
  PanelWidget *panel;
  int pos;
  const char *uri;
  char *filename;
...
void ask_about_launcher(const char *file, PanelWidget *panel, int pos,
  ...
  dialog = panel_ditem_editor_new(NULL, NULL, NULL, _("Create Launcher"));
  panel_widget_register_open_dialog(panel, dialog);

  key_file = panel_ditem_editor_get_key_file(PANEL_DITEM_EDITOR(dialog));
  if (file != NULL) panel_key_file_set_string(key_file, "Exec", file);
  ...
  panel_ditem_register_save_uri_func(PANEL_DITEM_EDITOR(dialog),
                                     launcher_save_uri, NULL);

  g_signal_connect(dialog, "saved", G_CALLBACK(launcher_new_saved), NULL);
...
void panel_launcher_create_with_id(const char *toplevel_id, int position,
  ...
  id = panel_profile_prepare_object_with_id(PANEL_OBJECT_LAUNCHER, toplevel_id,
                                            position);
  ...
  if (!no_uri) no_uri = g_strdup(location);

  new_location = panel_launcher_get_filename(no_uri);
  if (new_location == NULL) new_location = g_strdup(no_uri);
...
Launcher *find_launcher(const char *path) {
  ...
  for (l = mate_panel_applet_list_applets(); l; l = l->next) {
    ...
    if (info->type != PANEL_OBJECT_LAUNCHER) continue;

    launcher = info->data;

    if (launcher->key_file == NULL) continue;

    if (launcher->location != NULL && strcmp(launcher->location, path) == 0)
      return launcher;
  }
...
void panel_launcher_set_dnd_enabled(Launcher *launcher, gboolean dnd_enabled) {
  if (dnd_enabled) {
    ...
    gtk_drag_source_set(launcher->button, GDK_BUTTON1_MASK, dnd_targets, 2,
                        GDK_ACTION_COPY | GDK_ACTION_MOVE);
    cairo_surface_t *surface =
        button_widget_get_surface(BUTTON_WIDGET(launcher->button));
    if (surface) {
      GdkPixbuf *pixbuf;
      pixbuf = gdk_pixbuf_get_from_surface(
          surface, 0, 0, cairo_image_surface_get_width(surface),
          cairo_image_surface_get_height(surface));
      gtk_drag_source_set_icon_pixbuf(launcher->button, pixbuf);
