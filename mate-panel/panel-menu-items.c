...
G_DEFINE_TYPE_WITH_PRIVATE(PanelPlaceMenuItem, panel_place_menu_item,
                           GTK_TYPE_IMAGE_MENU_ITEM)
G_DEFINE_TYPE_WITH_PRIVATE(PanelDesktopMenuItem, panel_desktop_menu_item,
                           GTK_TYPE_IMAGE_MENU_ITEM)
...
static void activate_uri(GtkWidget *menuitem, const char *uri) {
  activate_uri_on_screen(uri, menuitem_to_screen(menuitem));
}

static void activate_path(GtkWidget *menuitem, const char *path) {
  char *uri;
...
static void activate_home_uri(GtkWidget *menuitem, gpointer data) {
  activate_path(menuitem, g_get_home_dir());
}

static void activate_desktop_uri(GtkWidget *menuitem, gpointer data) {
  activate_path(menuitem, g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP));
}

static void panel_menu_items_append_from_desktop(GtkWidget *menu, char *path,
                                                 char *force_name,
                                                 gboolean use_icon) {
  GKeyFile *key_file;
  gboolean loaded;
  ...
  if (g_path_is_absolute(path)) {
    loaded = g_key_file_load_from_file(key_file, path, G_KEY_FILE_NONE, NULL);
    full_path = path;
  } else {
    char *lookup_file;
    char *desktop_path;
    ...
    lookup_file =
        g_strconcat("applications", G_DIR_SEPARATOR_S, desktop_path, NULL);
    loaded = g_key_file_load_from_data_dirs(key_file, lookup_file, &path_freeme,
                                            G_KEY_FILE_NONE, NULL);
    full_path = path_freeme;
    g_free(lookup_file);

    if (desktop_path != path) g_free(desktop_path);
  }

  if (!loaded) {
    g_key_file_free(key_file);
    if (path_freeme) g_free(path_freeme);
    return;
  }
  ...
  setup_menuitem_with_icon(item, panel_menu_icon_get_size(), NULL, icon, name);

  panel_util_set_tooltip_text(item, comment);

  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  g_signal_connect_data(
      item, "activate", G_CALLBACK(panel_menu_item_activate_desktop_file),
      g_strdup(full_path), (GClosureNotify)G_CALLBACK(g_free), 0);
...
static void panel_menu_items_append_place_item(
    const char *icon_name, GIcon *gicon, const char *title, const char *tooltip,
    GtkWidget *menu, GCallback callback, const char *uri) {
  GtkWidget *item;
  char *user_data;

  item = panel_image_menu_item_new();
  setup_menuitem_with_icon(item, panel_menu_icon_get_size(), gicon, icon_name,
                           title);
...
static GtkWidget *panel_menu_items_create_action_item_full(
    PanelActionButtonType action_type, const char *label, const char *tooltip) {
  GtkWidget *item;

  if (panel_action_get_is_disabled(action_type)) return NULL;

  item = gtk_image_menu_item_new();
  setup_menuitem_with_icon(item, panel_menu_icon_get_size(), NULL,
                           panel_action_get_icon_name(action_type),
                           label ? label : panel_action_get_text(action_type));

  panel_util_set_tooltip_text(
      item, tooltip ? tooltip : panel_action_get_tooltip(action_type));

  g_signal_connect(item, "activate", panel_action_get_invoke(action_type),
                   NULL);
  g_signal_connect(item, "button-press-event",
                   G_CALLBACK(menu_dummy_button_press_event), NULL);
...
static GtkWidget *panel_menu_items_create_action_item(
    PanelActionButtonType action_type) {
  return panel_menu_items_create_action_item_full(action_type, NULL, NULL);
}

static void panel_place_menu_item_append_gtk_bookmarks(
    GtkWidget *menu, guint max_items_or_submenu) {
  typedef struct {
    char *full_uri;
    char *label;
  } PanelBookmark;
  ...
  filename =
      g_build_filename(g_get_user_config_dir(), "gtk-3.0", "bookmarks", NULL);

  io_channel = g_io_channel_new_file(filename, "r", NULL);
  g_free(filename);

  if (!io_channel) return;
  ...
  for (i = 0; i < MAX_BOOKMARK_ITEMS; i++) {
    ...
    status = g_io_channel_read_line(io_channel, &contents, &length,
                                    &terminator_pos, NULL);

    if (status != G_IO_STATUS_NORMAL) break;

    if (length == 0) break;

    /* Clear the line terminator (\n), if any */
    if (terminator_pos > 0) contents[terminator_pos] = '\0';
  ...
  for (l = lines; l; l = l->next) {
    ...
    if (line[0] && !g_hash_table_lookup(table, line)) {
      ...
      if (g_str_has_prefix(line, "x-caja-search:")) keep = TRUE;

      if (!keep) {
        file = g_file_new_for_uri(line);
        keep = !g_file_is_native(file) || g_file_query_exists(file, NULL);
        g_object_unref(file);
      }
  ...
  if (g_slist_length(add_bookmarks) <= max_items_or_submenu) {
    ...
    item = gtk_image_menu_item_new();
    setup_menuitem_with_icon(item, panel_menu_icon_get_size(), NULL,
                             PANEL_ICON_BOOKMARKS, _("Bookmarks"));
  ...
  for (l = add_bookmarks; l; l = l->next) {
    ...
    if (!label) {
      ...
      if (!label) {
        ...
        if (bookmark->label) g_free(bookmark->label);
        g_free(bookmark);
        continue;
      }
    }
...
static void drive_poll_for_media_cb(GObject *source_object, GAsyncResult *res,
                                    gpointer user_data) {
  GdkScreen *screen;
  GError *error;
  char *primary;
  ...
  if (!g_drive_poll_for_media_finish(G_DRIVE(source_object), res, &error)) {
    if (error->code != G_IO_ERROR_FAILED_HANDLED) {
      ...
      primary = g_strdup_printf(_("Unable to scan %s for media changes"), name);
      g_free(name);
      panel_error_dialog(NULL, screen, "cannot_scan_drive", TRUE, primary,
                         error->message);
      g_free(primary);
    }
    g_error_free(error);
...
static void panel_menu_item_rescan_drive(GtkWidget *menuitem, GDrive *drive) {
  g_drive_poll_for_media(drive, NULL, drive_poll_for_media_cb,
                         menuitem_to_screen(menuitem));
}

static void panel_menu_item_append_drive(GtkWidget *menu, GDrive *drive) {
  GtkWidget *item;
  GIcon *icon;
  char *title;
  char *tooltip;
  ...
  item = panel_image_menu_item_new();
  setup_menuitem_with_icon(item, panel_menu_icon_get_size(), icon, NULL, title);
  g_object_unref(icon);
  ...
  g_signal_connect_data(
      item, "activate", G_CALLBACK(panel_menu_item_rescan_drive),
      g_object_ref(drive), (GClosureNotify)G_CALLBACK(g_object_unref), 0);
...
static void volume_mount_cb(GObject *source_object, GAsyncResult *res,
                            gpointer user_data) {
  ...
  if (!g_volume_mount_finish(G_VOLUME(source_object), res, &error)) {
    ...
    if (error->code != G_IO_ERROR_FAILED_HANDLED) {
      ...
      panel_error_dialog(NULL, mount_data->screen, "cannot_mount_volume", TRUE,
...
static void panel_menu_item_mount_volume(GtkWidget *menuitem, GVolume *volume) {
...
static void panel_menu_item_append_volume(GtkWidget *menu, GVolume *volume) {
  GtkWidget *item;
  GIcon *icon;
  char *title;
  char *tooltip;
  ...
  g_signal_connect_data(
      item, "activate", G_CALLBACK(panel_menu_item_mount_volume),
      g_object_ref(volume), (GClosureNotify)G_CALLBACK(g_object_unref), 0);

  g_signal_connect(item, "button-press-event",
                   G_CALLBACK(menu_dummy_button_press_event), NULL);
}

static void panel_menu_item_append_mount(GtkWidget *menu, GMount *mount) {
  GFile *root;
  GIcon *icon;
  char *display_name;
  char *activation_uri;
  ...
  panel_menu_items_append_place_item(
      NULL, icon, display_name, display_name, /* FIXME tooltip */
      menu, G_CALLBACK(activate_uri), activation_uri);
...
static void panel_place_menu_item_append_local_gio(
    PanelPlaceMenuItem *place_item, GtkWidget *menu) {
  GList *l;
  GList *ll;
  GList *drives;
  GDrive *drive;
  ...
  drives =
      g_volume_monitor_get_connected_drives(place_item->priv->volume_monitor);
  ...
  if (g_slist_length(items) <=
      g_settings_get_uint(place_item->priv->menubar_settings,
                          PANEL_MENU_BAR_MAX_ITEMS_OR_SUBMENU)) {
    add_menu = menu;
  } else {
    GtkWidget *menu_item;

    menu_item = gtk_image_menu_item_new();
    setup_menuitem_with_icon(menu_item, panel_menu_icon_get_size(), NULL,
                             PANEL_ICON_REMOVABLE_MEDIA, _("Removable Media"));
...
/* this is loosely based on update_places() from caja-places-sidebar.c */
static void panel_place_menu_item_append_remote_gio(
    PanelPlaceMenuItem *place_item, GtkWidget *menu) {
  GtkWidget *add_menu;
  GList *mounts, *l;
  GMount *mount;
  ...
  if (g_slist_length(add_mounts) <=
    ...
    item = panel_image_menu_item_new();
    setup_menuitem_with_icon(item, panel_menu_icon_get_size(), NULL,
                             PANEL_ICON_NETWORK_SERVER, _("Network Places"));
...
static GtkWidget *panel_place_menu_item_create_menu(
  ...
  panel_menu_items_append_place_item(
      PANEL_ICON_HOME, NULL, name, _("Open your personal folder"), places_menu,
      G_CALLBACK(activate_home_uri), uri);
  g_free(name);
  g_free(uri);
  ...
  if (!place_item->priv->caja_prefs_settings ||
    ...
    file =
        g_file_new_for_path(g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP));
    uri = g_file_get_uri(file);
    g_object_unref(file);
    ...
    panel_menu_items_append_place_item(
        ...
        _("Open the contents of your desktop in a folder"), places_menu,
        G_CALLBACK(activate_desktop_uri),
        /* FIXME: if the dir changes, we'd need to update the drag data since
           the uri is not the same */
        uri);
    g_free(uri);
  }
  ...
  if (place_item->priv->caja_desktop_settings != NULL)
    gsettings_name =
        g_settings_get_string(place_item->priv->caja_desktop_settings,
  ...
  panel_menu_items_append_place_item(PANEL_ICON_COMPUTER, NULL, gsettings_name,
                                     _("Browse all local and remote disks and "
                                       "folders accessible from this computer"),
                                     places_menu, G_CALLBACK(activate_uri),
                                     "computer://");

  if (gsettings_name) g_free(gsettings_name);

  panel_place_menu_item_append_local_gio(place_item, places_menu);
  add_menu_separator(places_menu);

  panel_menu_items_append_place_item(
      PANEL_ICON_NETWORK, NULL, _("Network"),
      _("Browse bookmarked and local network locations"), places_menu,
      G_CALLBACK(activate_uri), "network://");
  ...
  if (panel_is_program_in_path("caja-connect-server") ||
    ...
    if (item != NULL) gtk_menu_shell_append(GTK_MENU_SHELL(places_menu), item);
  }

  add_menu_separator(places_menu);

  if (panel_is_program_in_path("mate-search-tool"))
    panel_menu_items_append_from_desktop(
        places_menu, "mate-search-tool.desktop", NULL, FALSE);
  else
    panel_menu_items_append_from_desktop(
        places_menu, "gnome-search-tool.desktop", NULL, FALSE);
  ...
  panel_recent_append_documents_menu(
      places_menu, place_item->priv->recent_manager, recent_items_limit);
...
static void panel_place_menu_item_recreate_menu(GtkWidget *widget) {
  PanelPlaceMenuItem *place_item;
  ...
  if (place_item->priv->menu) {
    ...
    mate_panel_applet_menu_set_recurse(GTK_MENU(place_item->priv->menu),
                                       "menu_panel", place_item->priv->panel);
  }
}

static void panel_place_menu_item_key_changed(GSettings *settings, gchar *key,
                                              GtkWidget *place_item) {
  panel_place_menu_item_recreate_menu(place_item);
}

static void panel_place_menu_item_gtk_bookmarks_changed(GFileMonitor *handle,
                                                        GFile *file,
                                                        GFile *other_file,
...
static void panel_place_menu_item_drives_changed(GVolumeMonitor *monitor,
                                                 GDrive *drive,
                                                 GtkWidget *place_menu) {
  panel_place_menu_item_recreate_menu(place_menu);
}

static void panel_place_menu_item_volumes_changed(GVolumeMonitor *monitor,
                                                  GVolume *volume,
                                                  GtkWidget *place_menu) {
  panel_place_menu_item_recreate_menu(place_menu);
}

static void panel_place_menu_item_mounts_changed(GVolumeMonitor *monitor,
                                                 GMount *mount,
                                                 GtkWidget *place_menu) {
  panel_place_menu_item_recreate_menu(place_menu);
}

static void panel_desktop_menu_item_append_menu(GtkWidget *menu,
                                                gpointer data) {
  ...
  if (last != NULL) add_separator = !GTK_IS_SEPARATOR(GTK_WIDGET(last->data));

  g_list_free(children);

  if (add_separator) add_menu_separator(menu);

  panel_menu_items_append_from_desktop(menu, "mate-user-guide.desktop", NULL,
                                       FALSE);
...
static GtkWidget *panel_desktop_menu_item_create_menu(
    PanelDesktopMenuItem *desktop_item) {
  GtkWidget *desktop_menu;

  desktop_menu = create_applications_menu("mate-settings.menu", NULL, FALSE);

  g_object_set_data(G_OBJECT(desktop_menu), "panel-menu-append-callback",
                    panel_desktop_menu_item_append_menu);
  g_object_set_data(G_OBJECT(desktop_menu), "panel-menu-append-callback-data",
                    desktop_item);

  return desktop_menu;
}

static void panel_desktop_menu_item_recreate_menu(
    PanelDesktopMenuItem *desktop_item) {
  if (desktop_item->priv->menu) {
    gtk_widget_destroy(desktop_item->priv->menu);
    desktop_item->priv->menu =
        panel_desktop_menu_item_create_menu(desktop_item);
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(desktop_item),
                              desktop_item->priv->menu);
    mate_panel_applet_menu_set_recurse(GTK_MENU(desktop_item->priv->menu),
                                       "menu_panel", desktop_item->priv->panel);
  }
}

static void panel_place_menu_item_finalize(GObject *object) {
...
static void panel_desktop_menu_item_finalize(GObject *object) {
  PanelDesktopMenuItem *menuitem = (PanelDesktopMenuItem *)object;

  if (menuitem->priv->append_lock_logout)
    panel_lockdown_notify_remove(
        G_CALLBACK(panel_desktop_menu_item_recreate_menu), menuitem);
  G_OBJECT_CLASS(panel_desktop_menu_item_parent_class)->finalize(object);
}

static void panel_place_menu_item_init(PanelPlaceMenuItem *menuitem) {
  GFile *bookmark;
  ...
  error = NULL;
  menuitem->priv->bookmarks_monitor =
      g_file_monitor_file(bookmark, G_FILE_MONITOR_NONE, NULL, &error);
  if (error) {
  ...
  i = 0;
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "drive-changed",
      G_CALLBACK(panel_place_menu_item_drives_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "drive-connected",
      G_CALLBACK(panel_place_menu_item_drives_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "drive-disconnected",
      G_CALLBACK(panel_place_menu_item_drives_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "volume-added",
      G_CALLBACK(panel_place_menu_item_volumes_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "volume-changed",
      G_CALLBACK(panel_place_menu_item_volumes_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "volume-removed",
      G_CALLBACK(panel_place_menu_item_volumes_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "mount-added",
      G_CALLBACK(panel_place_menu_item_mounts_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "mount-changed",
      G_CALLBACK(panel_place_menu_item_mounts_changed), menuitem);
  menuitem->priv->signal_id[i++] = g_signal_connect(
      menuitem->priv->volume_monitor, "mount-removed",
      G_CALLBACK(panel_place_menu_item_mounts_changed), menuitem);

  g_assert(i == N_MENU_ITEM_SIGNALS);
}

static void panel_desktop_menu_item_init(PanelDesktopMenuItem *menuitem) {
  menuitem->priv = panel_desktop_menu_item_get_instance_private(menuitem);
}

static void panel_place_menu_item_class_init(PanelPlaceMenuItemClass *klass) {
  GObjectClass *gobject_class = (GObjectClass *)klass;

  gobject_class->finalize = panel_place_menu_item_finalize;
}

static void panel_desktop_menu_item_class_init(
...
GtkWidget *panel_desktop_menu_item_new(gboolean use_image,
  ...
  if (use_image)
    image =
        gtk_image_new_from_icon_name("computer", panel_menu_icon_get_size());
  else
    image = NULL;
...
void panel_place_menu_item_set_panel(GtkWidget *item, PanelWidget *panel) {
  PanelPlaceMenuItem *place_item;
...
void panel_desktop_menu_item_set_panel(GtkWidget *item, PanelWidget *panel) {
  PanelDesktopMenuItem *desktop_item;
...
void panel_menu_items_append_lock_logout(GtkWidget *menu) {
  ...
  if (panel_lockdown_get_disable_log_out()) return;
  ...
  if (strcmp(translate, "1") == 0) {
    ...
    if (!user_name || !user_name[0]) user_name = g_get_user_name();
  ...
  item = panel_menu_items_create_action_item_full(PANEL_ACTION_LOGOUT, label,
                                                  tooltip);
  g_free(label);
  g_free(tooltip);
  ...
  if (item != NULL) {
    if (!separator_inserted) add_menu_separator(menu);
