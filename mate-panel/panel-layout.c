 ...
 * distributions
 */
static gchar *panel_layout_filename(void) {
  GSettings *settings;
  gchar *layout;
  gchar *filename;
...
static gboolean panel_layout_append_group_helper(
    GKeyFile *keyfile, const char *group, int set_screen_to,
    const char *group_prefix, char *id_list_key, const char *schema,
    const char *path_prefix, const char *default_prefix,
    PanelLayoutKeyDefinition *key_definitions, int key_definitions_len,
  ...
  if (keyfile_keys) {
    ...
    for (i = 0; keyfile_keys[i] != NULL; i++) {
      ...
      for (j = 0; j < key_definitions_len; j++) {
        if (g_strcmp0(keyfile_keys[i], key_definitions[j].name) == 0) {
          found = TRUE;
          break;
        }
      }
      ...
      if (!found) {
        g_warning("Unknown key '%s' for %s", keyfile_keys[i], unique_id);
        return FALSE;
      }
      ...
      switch (key_definitions[j].type) {
        case G_TYPE_STRING: {
          char *value_str =
              g_key_file_get_string(keyfile, group, keyfile_keys[i], NULL);
          if (value_str)
            g_settings_set_string(settings, key_definitions[j].name, value_str);
          g_free(value_str);
          break;
        }
        case G_TYPE_INT: {
          gint value_int =
              g_key_file_get_integer(keyfile, group, keyfile_keys[i], NULL);
          g_settings_set_int(settings, key_definitions[j].name, value_int);
          break;
        }
        case G_TYPE_BOOLEAN: {
          gboolean value_boolean =
              g_key_file_get_boolean(keyfile, group, keyfile_keys[i], NULL);
          g_settings_set_boolean(settings, key_definitions[j].name,
                                 value_boolean);
          break;
        }
        default:
    ...
    if (set_screen_to != -1 && g_strcmp0(schema, PANEL_TOPLEVEL_SCHEMA) == 0)
      g_settings_set_int(settings, PANEL_TOPLEVEL_SCREEN_KEY, set_screen_to);

    GSettings *panel_settings = g_settings_new(PANEL_SCHEMA);
    mate_gsettings_append_strv(panel_settings, id_list_key, unique_id);
    g_object_unref(panel_settings);

    retval = TRUE;
  }

  if (keyfile_keys) g_strfreev(keyfile_keys);

  if (settings) g_object_unref(settings);

  if (unique_id) g_free(unique_id);

  return retval;
}

static void panel_layout_apply_minimal_default(int set_screen_to,
                                               const char *schema,
  ...
  mate_gsettings_append_strv(panel_settings, PANEL_TOPLEVEL_ID_LIST_KEY,
                             unique_id);
...
void panel_layout_apply_default_from_gkeyfile(GdkScreen *screen) {
  ...
  if (layout_file) {
    ...
    if (g_key_file_load_from_file(keyfile, layout_file, G_KEY_FILE_NONE,
                                  &error)) {
      int i;
      ...
      for (i = 0; groups[i] != NULL; i++) {
        if (g_strcmp0(groups[i], "Toplevel") == 0 ||
          ...
          panel_layout_append_group_helper(
              keyfile, groups[i], screen_n, "Toplevel",
              PANEL_TOPLEVEL_ID_LIST_KEY, PANEL_TOPLEVEL_SCHEMA,
              PANEL_TOPLEVEL_PATH, PANEL_TOPLEVEL_DEFAULT_PREFIX,
              panel_layout_toplevel_keys,
              G_N_ELEMENTS(panel_layout_toplevel_keys), "toplevel");

        else if (g_strcmp0(groups[i], "Object") == 0 ||
                 g_str_has_prefix(groups[i], "Object "))

          panel_layout_append_group_helper(
              keyfile, groups[i], -1, "Object", PANEL_OBJECT_ID_LIST_KEY,
              PANEL_OBJECT_SCHEMA, PANEL_OBJECT_PATH,
              PANEL_OBJECT_DEFAULT_PREFIX, panel_layout_object_keys,
              G_N_ELEMENTS(panel_layout_object_keys), "object");

        else

          g_warning("Unknown group in default layout: '%s'", groups[i]);
      }
    ...
    panel_layout_apply_minimal_default(screen_n, PANEL_TOPLEVEL_SCHEMA,
                                       PANEL_TOPLEVEL_PATH);
  }
  ...
  if (layout_file) g_free(layout_file);
}
