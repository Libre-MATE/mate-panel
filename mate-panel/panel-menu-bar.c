...
static gboolean panel_menu_bar_reinit_tooltip(GtkWidget* widget,
...
static gboolean panel_menu_bar_hide_tooltip_and_focus(GtkWidget* widget,
                                                      PanelMenuBar* menubar) {
...
static void panel_menu_bar_setup_tooltip(PanelMenuBar* menubar) {
  ...
  g_signal_connect(menubar->priv->applications_item, "activate",
                   G_CALLBACK(panel_menu_bar_hide_tooltip_and_focus), menubar);
  g_signal_connect(menubar->priv->places_item, "activate",
                   G_CALLBACK(panel_menu_bar_hide_tooltip_and_focus), menubar);
  g_signal_connect(menubar->priv->desktop_item, "activate",
                   G_CALLBACK(panel_menu_bar_hide_tooltip_and_focus), menubar);

  /* Reset tooltip when the menu bar is not used */
  g_signal_connect(menubar, "deactivate",
                   G_CALLBACK(panel_menu_bar_reinit_tooltip), menubar);
}

static void panel_menu_bar_update_visibility(GSettings* settings, gchar* key,
                                             PanelMenuBar* menubar) {
  if (!GTK_IS_WIDGET(menubar)) return;

  gtk_widget_set_visible(
      GTK_WIDGET(menubar->priv->applications_item),
      g_settings_get_boolean(settings, PANEL_MENU_BAR_SHOW_APPLICATIONS_KEY));
  gtk_widget_set_visible(
      GTK_WIDGET(menubar->priv->places_item),
      g_settings_get_boolean(settings, PANEL_MENU_BAR_SHOW_PLACES_KEY));
  gtk_widget_set_visible(
      GTK_WIDGET(menubar->priv->desktop_item),
  ...
  if (g_settings_get_boolean(settings, PANEL_MENU_BAR_SHOW_ICON_KEY)) {
    ...
    gtk_image_menu_item_set_image(
        GTK_IMAGE_MENU_ITEM(menubar->priv->applications_item), image);
    gtk_image_set_pixel_size(GTK_IMAGE(image), icon_height);
    g_free(str);
  } else
    gtk_image_menu_item_set_image(
        GTK_IMAGE_MENU_ITEM(menubar->priv->applications_item), NULL);
}
...
static void panel_menu_bar_init(PanelMenuBar* menubar) {
  ...
  menubar->priv->applications_menu =
      create_applications_menu("mate-applications.menu", NULL, TRUE);
  ...
  g_signal_connect(menubar->priv->settings, "changed",
                   G_CALLBACK(panel_menu_bar_update_visibility), menubar);

  panel_menu_bar_update_text_gravity(menubar);
  g_signal_connect(menubar, "screen-changed",
                   G_CALLBACK(panel_menu_bar_update_text_gravity), NULL);
}

static void panel_menu_bar_get_property(GObject* object, guint prop_id,
...
static void panel_menu_bar_set_property(GObject* object, guint prop_id,
                                        const GValue* value,
                                        GParamSpec* pspec) {
  PanelMenuBar* menubar;
...
static void panel_menu_bar_parent_set(GtkWidget* widget,
  ...
  if (menubar->priv->applications_menu) {
    mate_panel_applet_menu_set_recurse(
        GTK_MENU(menubar->priv->applications_menu), "menu_panel",
        menubar->priv->panel);
  }
...
static void panel_menu_bar_size_allocate(GtkWidget* widget,
  ...
  GTK_WIDGET_CLASS(panel_menu_bar_parent_class)
      ->size_allocate(widget, allocation);

  if (old_allocation.x == allocation->x && old_allocation.y == allocation->y &&
...
static void panel_menu_bar_class_init(PanelMenuBarClass* klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("orientation", "Orientation",
                        "The PanelMenuBar orientation", PANEL_TYPE_ORIENTATION,
                        PANEL_ORIENTATION_TOP, G_PARAM_READWRITE));
}

static gboolean panel_menu_bar_on_draw(GtkWidget* widget, cairo_t* cr,
                                       gpointer data) {
...
static void panel_menu_bar_load(PanelWidget* panel, gboolean locked,
  ...
  menubar->priv->info =
      mate_panel_applet_register(GTK_WIDGET(menubar), NULL, NULL, panel, locked,
  ...
  settings =
      gtk_settings_get_for_screen(gtk_widget_get_screen(GTK_WIDGET(panel)));
  g_object_set(settings, "gtk-shell-shows-app-menu", FALSE,
               "gtk-shell-shows-menubar", FALSE, NULL);

  mate_panel_applet_add_callback(menubar->priv->info, "help", "help-browser",
                                 _("_Help"), NULL);
  ...
  if (!panel_lockdown_get_locked_down() &&
    ...
    mate_panel_applet_add_callback(menubar->priv->info, "edit",
                                   "document-properties", _("_Edit Menus"),
                                   NULL);
  }
  ...
  g_signal_connect_after(menubar, "focus-out-event",
                         G_CALLBACK(gtk_widget_queue_draw), menubar);
  g_signal_connect_after(menubar, "draw", G_CALLBACK(panel_menu_bar_on_draw),
                         menubar);
...
void panel_menu_bar_load_from_gsettings(PanelWidget* panel, gboolean locked,
                                        int position, gboolean exactpos,
                                        const char* id) {
...
void panel_menu_bar_change_background(PanelMenuBar* menubar) {
  panel_background_apply_css(&menubar->priv->panel->toplevel->background,
                             GTK_WIDGET(menubar));
}
...
static void set_item_text_angle_and_alignment(GtkWidget* item,
                                              double text_angle, float xalign,
                                              float yalign) {
  GtkWidget* label;
...
static void panel_menu_bar_update_orientation(PanelMenuBar* menubar) {
  ...
  set_item_text_angle_and_alignment(menubar->priv->applications_item,
                                    text_angle, text_xalign, text_yalign);
  set_item_text_angle_and_alignment(menubar->priv->places_item, text_angle,
                                    text_xalign, text_yalign);
  set_item_text_angle_and_alignment(menubar->priv->desktop_item, text_angle,
                                    text_xalign, text_yalign);
}
