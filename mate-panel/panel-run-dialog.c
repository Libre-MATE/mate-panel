...
static GtkTreeModel *_panel_run_get_recent_programs_list(
  ...
  for (i = 0; items[i] && i < history_max_size; i++) {
    GtkTreeIter iter;
...
static void _panel_run_save_recent_programs_list(PanelRunDialog *dialog,
                                                 GtkComboBox *entry,
                                                 char *lastcommand) {
  ...
  if (history_max_size == 0)
    ...
    /* free the items */
    for (; items_added; items_added--) g_free(items[items_added]);
  }
}

static void panel_run_dialog_destroy(PanelRunDialog *dialog) {
  GList *l;
  ...
  if (dialog->add_items_idle_id) g_source_remove(dialog->add_items_idle_id);
  ...
  if (dialog->dir_hash) g_hash_table_destroy(dialog->dir_hash);
  dialog->dir_hash = NULL;

  for (l = dialog->possible_executables; l; l = l->next) g_free(l->data);
  g_list_free(dialog->possible_executables);
  dialog->possible_executables = NULL;

  for (l = dialog->completion_items; l; l = l->next) g_free(l->data);
...
static const char *panel_run_dialog_get_combo_text(PanelRunDialog *dialog) {
  GtkWidget *entry;
...
static void panel_run_dialog_set_default_icon(PanelRunDialog *dialog,
                                              gboolean set_drag) {
  gtk_image_set_from_icon_name(GTK_IMAGE(dialog->pixmap), PANEL_ICON_RUN,
                               GTK_ICON_SIZE_DIALOG);

  gtk_window_set_icon_name(GTK_WINDOW(dialog->run_dialog), PANEL_ICON_RUN);

  if (set_drag)
    gtk_drag_source_set_icon_name(dialog->run_dialog, PANEL_ICON_LAUNCHER);
}

static void panel_run_dialog_set_icon(PanelRunDialog *dialog, GIcon *icon,
                                      gboolean force) {
  GdkPixbuf *pixbuf = NULL;

  if (!force && g_icon_equal(icon, dialog->icon)) return;
  ...
  if (icon) {
    ...
    GtkIconInfo *icon_info = gtk_icon_theme_lookup_by_gicon(
        icon_theme, icon, size, GTK_ICON_LOOKUP_FORCE_SIZE);
 ...
 */
static void set_environment(gpointer display) {
...
static gboolean panel_run_dialog_launch_command(PanelRunDialog *dialog,
                                                const char *command,
  ...
  if (!command_is_executable(locale_command, &argc, &argv)) return FALSE;
  ...
  result = g_spawn_async(NULL,       /* working directory */
                         ...
                         set_environment, display_name, &pid, &error);

  if (!result) {
    char *primary;

    primary = g_markup_printf_escaped(_("Could not run command '%s'"), command);
    panel_error_dialog(GTK_WINDOW(dialog->run_dialog), NULL,
                       "cannot_spawn_command", TRUE, primary, error->message);
    g_free(primary);
...
static void panel_run_dialog_execute(PanelRunDialog *dialog) {
  GError *error;
  gboolean result;
  char *command;
  char *disk;
  ...
  if (!disk || error) {
    ...
    primary = g_strdup_printf(_("Could not convert '%s' from UTF-8"), command);
    panel_error_dialog(GTK_WINDOW(dialog->run_dialog), NULL,
                       "cannot_convert_command_from_utf8", TRUE, primary,
                       error->message);
    g_free(primary);
  ...
  if (!result) {
    ...
    screen = gtk_window_get_screen(GTK_WINDOW(dialog->run_dialog));
    result = panel_show_uri(screen, uri, gtk_get_current_event_time(), NULL);
...
static void panel_run_dialog_response(PanelRunDialog *dialog, int response,
                                      GtkWidget *run_dialog) {
...
static void panel_run_dialog_append_file_utf8(PanelRunDialog *dialog,
                                              const char *file) {
  const char *text;
  char *quoted;
  GtkWidget *entry;
...
static void panel_run_dialog_append_file(PanelRunDialog *dialog,
                                         const char *file) {
  char *utf8_file;
  ...
  if (utf8_file) panel_run_dialog_append_file_utf8(dialog, utf8_file);

  g_free(utf8_file);
}

static gboolean fuzzy_command_match(const char *cmd1, const char *cmd2,
                                    gboolean *fuzzy) {
  char **tokens;
  char *word1, *word2;
  ...
  *fuzzy = FALSE;
  ...
  if (!strcmp(cmd1, cmd2)) return TRUE;
...
static gboolean panel_run_dialog_make_all_list_visible(GtkTreeModel *model,
                                                       GtkTreePath *path,
                                                       GtkTreeIter *iter,
                                                       gpointer data) {
  gtk_list_store_set(GTK_LIST_STORE(model), iter, COLUMN_VISIBLE, TRUE, -1);

  return FALSE;
}

static gboolean panel_run_dialog_find_command_idle(PanelRunDialog *dialog) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  GtkTreePath *path;
  char *text;
  ...
  do {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_EXEC, &exec, COLUMN_GICON, &icon,
                       COLUMN_NAME, &name, COLUMN_COMMENT, &comment, -1);

    if (!fuzzy && exec && icon && fuzzy_command_match(text, exec, &fuzzy)) {
      ...
      gtk_list_store_set(dialog->program_list_store, &iter, COLUMN_VISIBLE,
                         TRUE, -1);
  ...
  if (gtk_tree_model_get_iter(
          gtk_tree_view_get_model(GTK_TREE_VIEW(dialog->program_list)), &iter,
          path))
    gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(dialog->program_list), path,
                                 NULL, FALSE, 0, 0);
...
static int compare_applications(MateMenuTreeEntry *a, MateMenuTreeEntry *b) {
...
static GSList *get_all_applications_from_alias(MateMenuTreeAlias *alias,
                                               GSList *list) {
  gpointer item;
  ...
  while ((type = matemenu_tree_iter_next(iter)) != MATEMENU_TREE_ITEM_INVALID) {
    ...
    switch (type) {
      ...
      case MATEMENU_TREE_ITEM_DIRECTORY:
        ...
        list =
            get_all_applications_from_dir((MateMenuTreeDirectory *)item, list);
        matemenu_tree_item_unref(item);
        break;
...
static GSList *get_all_applications(void) {
  ...
  tree = matemenu_tree_new("mate-applications.menu",
...
static gboolean panel_run_dialog_add_items_idle(PanelRunDialog *dialog) {
  ...
  /* create list store */
  dialog->program_list_store =
      gtk_list_store_new(NUM_COLUMNS, G_TYPE_ICON, G_TYPE_STRING, G_TYPE_STRING,
  ...
  for (l = all_applications; l; l = l->next) {
    ...
    gtk_list_store_set(
        dialog->program_list_store, &iter, COLUMN_GICON, gicon, COLUMN_NAME,
        g_app_info_get_display_name(G_APP_INFO(ginfo)), COLUMN_COMMENT,
        g_app_info_get_description(G_APP_INFO(ginfo)), COLUMN_EXEC,
        g_app_info_get_commandline(G_APP_INFO(ginfo)), COLUMN_PATH,
        matemenu_tree_entry_get_desktop_file_path(entry), COLUMN_VISIBLE, TRUE,
        -1);
  }
  g_slist_free_full(all_applications, matemenu_tree_item_unref);

  model_filter = gtk_tree_model_filter_new(
      GTK_TREE_MODEL(dialog->program_list_store), NULL);
  gtk_tree_model_filter_set_visible_column(GTK_TREE_MODEL_FILTER(model_filter),
                                           COLUMN_VISIBLE);

  gtk_tree_view_set_model(GTK_TREE_VIEW(dialog->program_list), model_filter);
  ...
  gtk_tree_view_column_set_attributes(column, renderer, "gicon", COLUMN_GICON,
                                      NULL);
...
static char *remove_parameters(const char *exec) {
  GString *str;
  char *retval, *p;
...
static void program_list_selection_changed(GtkTreeSelection *selection,
                                           PanelRunDialog *dialog) {
  ...
  gtk_tree_model_get(child_model, &iter, COLUMN_PATH, &path, -1);
  ...
  /* FIXME: if sure_string () == "", we should display "Will run..." as in
   * entry_changed() */
...
static void panel_run_dialog_setup_program_list(PanelRunDialog *dialog,
                                                GtkBuilder *gui) {
  ...
  if (panel_profile_get_enable_program_list()) {
    ...
    g_signal_connect(selection, "changed",
                     G_CALLBACK(program_list_selection_changed), dialog);

    g_signal_connect(dialog->program_list, "row-activated",
                     G_CALLBACK(program_list_selection_activated), dialog);

    /* start loading the list of applications */
    dialog->add_items_idle_id = g_idle_add_full(
        G_PRIORITY_LOW, (GSourceFunc)panel_run_dialog_add_items_idle, dialog,
        NULL);
  }
}

static void panel_run_dialog_update_content(PanelRunDialog *dialog,
                                            gboolean show_list) {
  if (!panel_profile_get_enable_program_list()) {
    GtkWidget *parent;
    ...
    if (parent)
      gtk_container_remove(GTK_CONTAINER(parent), dialog->list_expander);

    gtk_window_set_resizable(GTK_WINDOW(dialog->run_dialog), FALSE);
    gtk_widget_grab_focus(dialog->combobox);

  } else {
    /* the following two conditions occur, when the user clicks the expander in
     * the dialog if the list is closed and the user wants to see it */
    if (show_list &&
        gtk_expander_get_expanded(GTK_EXPANDER(dialog->list_expander))) {
    ...
    } else if (!show_list && !gtk_expander_get_expanded(
                                 GTK_EXPANDER(dialog->list_expander))) {
    ...
    /* the following two conditions occur, when the user changes the expander
     * setting in GSettings */
    if (show_list &&
        !gtk_expander_get_expanded(GTK_EXPANDER(dialog->list_expander))) {
      /* open the expander, this shows the list */
      gtk_expander_set_expanded(GTK_EXPANDER(dialog->list_expander), TRUE);
...
static void panel_run_dialog_content_notify(GSettings *settings, gchar *key,
                                            PanelRunDialog *dialog) {
  panel_run_dialog_update_content(dialog,
                                  g_settings_get_boolean(settings, key));
}

static void list_expander_toggled(GtkExpander *expander, GParamSpec *pspec,
                                  PanelRunDialog *dialog) {
  panel_profile_set_show_program_list(gtk_expander_get_expanded(expander));
}

static void panel_run_dialog_setup_list_expander(PanelRunDialog *dialog,
                                                 GtkBuilder *gui) {
  ...
  if (panel_profile_get_enable_program_list()) {
    ...
    g_signal_connect(dialog->list_expander, "notify::expanded",
                     G_CALLBACK(list_expander_toggled), dialog);
...
static void file_button_browse_response(GtkWidget *chooser, gint response,
                                        PanelRunDialog *dialog) {
...
static void file_button_clicked(GtkButton *button, PanelRunDialog *dialog) {
  GtkWidget *chooser;

  chooser = panel_file_chooser_dialog_new(
      _("Choose a file to append to the command..."),
      GTK_WINDOW(dialog->run_dialog), GTK_FILE_CHOOSER_ACTION_OPEN,
      "process-stop", GTK_RESPONSE_CANCEL, "gtk-ok", GTK_RESPONSE_OK, NULL);
  ...
  g_signal_connect(chooser, "response", G_CALLBACK(file_button_browse_response),
                   dialog);

  gtk_window_present(GTK_WINDOW(chooser));
}

static void panel_run_dialog_setup_file_button(PanelRunDialog *dialog,
                                               GtkBuilder *gui) {
  dialog->file_button = PANEL_GTK_BUILDER_GET(gui, "file_button");

  g_signal_connect(dialog->file_button, "clicked",
                   G_CALLBACK(file_button_clicked), dialog);
}

static GList *fill_files_from(const char *dirname, const char *dirprefix,
                              char prefix, GList *existing_items) {
  GList *list;
  DIR *dir;
  struct dirent *dent;
  ...
  while ((dent = readdir(dir))) {
    ...
    if (dent->d_name[0] != prefix) continue;
    ...
    if (
        ...
        dent->d_type == DT_DIR ||
        (dent->d_type == DT_LNK && g_file_test(file, G_FILE_TEST_IS_DIR))
#else
...
static GList *fill_possible_executables(void) {
  GList *list;
  const char *path;
  char **pathv;
  int i;
...
static GList *fill_executables(GList *possible_executables,
                               GList *existing_items, char prefix) {
  GList *list;
  GList *l;
  ...
  for (l = possible_executables; l; l = l->next) {
    ...
    if (basename[0] == prefix &&
      ...
      if (g_list_find_custom(existing_items, basename, (GCompareFunc)strcmp)) {
        g_free(basename);
        return NULL;
      }
...
static GtkTreeModel *create_completion_model(GList *list) {
  GtkListStore *store;
  GtkTreeIter iter;
  GList *l;
...
static void completion_add_items(GtkEntryCompletion *completion, GList *list) {
...
static void panel_run_dialog_update_completion(PanelRunDialog *dialog,
                                               const char *text) {
  GList *list;
  GList *executables;
  char prefix;
  ...
  if (text[0] == '/') {
    ...
    if (!strchr(text, '/')) {
      executables = fill_executables(dialog->possible_executables,
                                     dialog->completion_items, text[0]);
      dirprefix = g_strdup("");
    } else {
  ...
  if (!g_hash_table_lookup(dialog->dir_hash, key)) {
    ...
    list =
        fill_files_from(dirname, dirprefix, prefix, dialog->completion_items);
  } else {
    g_free(key);
  }
...
static gboolean entry_event(GtkEditable *entry, GdkEventKey *event,
                            PanelRunDialog *dialog) {
  char *prefix;
  char *nospace_prefix;
  char *temp;
  ...
  */
  ...
  if (!panel_profile_get_enable_autocompletion()) return FALSE;

  /* tab completion */
  if (event->keyval == GDK_KEY_Tab) {
    gtk_editable_get_selection_bounds(entry, &pos, &tmp);

    if (dialog->completion_started && pos != tmp && pos != 1 &&
    ...
    if (dialog->completion_started && pos != tmp && pos != 0 &&
        ((size_t)tmp) == strlen(gtk_entry_get_text(GTK_ENTRY(entry)))) {
      ...
      g_free(temp);
               ...
               ((size_t)tmp) == strlen(gtk_entry_get_text(GTK_ENTRY(entry)))) {
      prefix = g_strconcat(gtk_entry_get_text(GTK_ENTRY(entry)), event->string,
                           NULL);
    } else {
      return FALSE;
    }
    ...
    while (*nospace_prefix != '\0' && g_ascii_isspace(*nospace_prefix))
      nospace_prefix++;
    if (*nospace_prefix == '\0') return FALSE;
...
static void combobox_changed(GtkComboBox *combobox, PanelRunDialog *dialog) {
  char *text = g_strdup(panel_run_dialog_get_combo_text(dialog));
  char *start = text;

  while (*start != '\0' && g_ascii_isspace(*start)) start++;
  ...
  if (!start || !start[0]) {
    ...
    if (panel_profile_get_enable_program_list()) {
      ...
      gtk_tree_model_foreach(GTK_TREE_MODEL(dialog->program_list_store),
                             panel_run_dialog_make_all_list_visible, NULL);

      path = gtk_tree_path_new_first();
      if (gtk_tree_model_get_iter(
              gtk_tree_view_get_model(GTK_TREE_VIEW(dialog->program_list)),
              &iter, path))
        gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(dialog->program_list), path,
                                     NULL, FALSE, 0, 0);
      gtk_tree_path_free(path);
  ...
  gtk_drag_source_set(dialog->run_dialog, GDK_BUTTON1_MASK, NULL, 0,
                      GDK_ACTION_COPY);
  gtk_drag_source_add_uri_targets(dialog->run_dialog);

  if (panel_profile_get_enable_program_list() && !dialog->use_program_list) {
    char *msg = g_strdup_printf(_("Will run command: '%s'"), start);
  ...
  if (panel_profile_get_enable_program_list() && !dialog->use_program_list &&
    ...
    dialog->find_command_idle_id = g_idle_add_full(
        G_PRIORITY_LOW, (GSourceFunc)panel_run_dialog_find_command_idle, dialog,
        NULL);

  g_free(text);
}

static void entry_drag_data_received(GtkEditable *entry,
  ...
  if (gtk_selection_data_get_format(selection_data) != 8 ||
    ...
    g_warning(_("URI list dropped on run dialog had wrong format (%d) or "
                "length (%d)\n"),
...
static void panel_run_dialog_setup_entry(PanelRunDialog *dialog,
                                         GtkBuilder *gui) {
  int width_request;
  GtkWidget *entry;
  ...
  gtk_combo_box_set_entry_text_column(GTK_COMBO_BOX(dialog->combobox), 0);

  /* 1/4 the width of the first monitor should be a good value */
  width_request = panel_multimonitor_width(0) / 4;
  g_object_set(G_OBJECT(dialog->combobox), "width_request", width_request,
               NULL);

  g_signal_connect(entry, "key-press-event", G_CALLBACK(entry_event), dialog);

  dialog->changed_id = g_signal_connect(dialog->combobox, "changed",
                                        G_CALLBACK(combobox_changed), dialog);

  gtk_drag_dest_unset(dialog->combobox);

  gtk_drag_dest_set(dialog->combobox,
                    GTK_DEST_DEFAULT_MOTION | GTK_DEST_DEFAULT_HIGHLIGHT, NULL,
                    0, GDK_ACTION_COPY);
...
static char *panel_run_dialog_create_desktop_file(PanelRunDialog *dialog) {
  GKeyFile *key_file;
  gboolean exec = FALSE;
  char *text;
  char *name;
  ...
  panel_key_file_set_locale_string(
      key_file, "Name", (dialog->item_name) ? dialog->item_name : text);
  ...
  if (dialog->icon) {
    ...
    panel_key_file_set_locale_string(key_file, "Icon", PANEL_ICON_LAUNCHER);
...
static void pixmap_drag_data_get(GtkWidget *run_dialog, GdkDragContext *context,
                                 GtkSelectionData *selection_data, guint info,
                                 guint time, PanelRunDialog *dialog) {
  char *uri;
...
static void panel_run_dialog_style_updated(GtkWidget *widget,
                                           PanelRunDialog *dialog) {
  if (dialog->icon) {
    GIcon *icon;
...
static void panel_run_dialog_screen_changed(GtkWidget *widget,
...
static void panel_run_dialog_setup_pixmap(PanelRunDialog *dialog,
                                          GtkBuilder *gui) {
  dialog->pixmap = PANEL_GTK_BUILDER_GET(gui, "icon_pixmap");

  g_signal_connect(dialog->pixmap, "style-updated",
                   G_CALLBACK(panel_run_dialog_style_updated), dialog);
  g_signal_connect(dialog->pixmap, "screen-changed",
                   G_CALLBACK(panel_run_dialog_screen_changed), dialog);

  g_signal_connect(dialog->run_dialog, "drag-data-get",
                   G_CALLBACK(pixmap_drag_data_get), dialog);
}

/* this runs after entry_event() */
static gboolean key_press_event(GtkWidget *run_dialog, GdkEventKey *event,
                                PanelRunDialog *dialog) {
  ...
  if (panel_profile_get_enable_program_list() &&
      panel_profile_get_show_program_list() && event->keyval == GDK_KEY_F6) {
...
static PanelRunDialog *panel_run_dialog_new(GdkScreen *screen, GtkBuilder *gui,
...
static void panel_run_dialog_disconnect_pixmap(PanelRunDialog *dialog) {
  g_signal_handlers_disconnect_by_func(
      dialog->pixmap, G_CALLBACK(panel_run_dialog_style_updated), dialog);
  g_signal_handlers_disconnect_by_func(
      dialog->pixmap, G_CALLBACK(panel_run_dialog_screen_changed), dialog);
}

static void panel_run_dialog_static_dialog_destroyed(PanelRunDialog *dialog) {
...
void panel_run_dialog_present(GdkScreen *screen, guint32 activate_time) {
  ...
  if (panel_lockdown_get_disable_command_line()) return;
  ...
  gtk_builder_add_from_resource(gui, PANEL_RESOURCE_PATH "panel-run-dialog.ui",
                                NULL);
