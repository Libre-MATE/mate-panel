...
static void panel_background_prepare_css(void) {
...
static gboolean panel_background_prepare(PanelBackground *background) {
  PanelBackgroundType effective_type;
  GtkWidget *widget = NULL;

  if (!background->transformed) return FALSE;
  ...
  switch (effective_type) {
    case PANEL_BACK_NONE:
      if (background->default_pattern) {
        ...
        /* catch invalid images (e.g. -gtk-gradient) before scaling and
         * rendering */
        if (surface != NULL) {
          double width, height;
          cairo_matrix_t m;
          ...
          cairo_matrix_scale(&m, width / background->region.width,
          ...
          g_warning("%s",
                    "unsupported value of 'background-image' in GTK+ theme "
                    "(such as '-gtk-gradient')");
  ...
  gdk_window_get_user_data(GDK_WINDOW(background->window), (gpointer)&widget);
...
static cairo_pattern_t *composite_image_onto_desktop(
  ...
  surface = gdk_window_create_similar_surface(
      background->window, CAIRO_CONTENT_COLOR_ALPHA, width, height);
...
static void free_transformed_resources(PanelBackground *background) {
  free_composited_resources(background);

  background->transformed = FALSE;

  if (background->type != PANEL_BACK_IMAGE) return;
...
static GdkPixbuf *get_scaled_and_rotated_pixbuf(PanelBackground *background) {
  GdkPixbuf *scaled;
  GdkPixbuf *retval;
  ...
  if (!background->loaded_image) return NULL;
  ...
  if (width == orig_width && height == orig_height) {
    ...
    scaled = gdk_pixbuf_scale_simple(background->loaded_image, width, height,
                                     GDK_INTERP_BILINEAR);
  }
  ...
  if (background->rotate_image &&
    ...
    if (!background->has_alpha) {
      ...
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++)
          dest[y + destrowstride * (width - x - 1)] = src[y * srcrowstride + x];
...
static gboolean panel_background_transform(PanelBackground *background) {
  if (background->region.width == -1) return FALSE;

  free_transformed_resources(background);

  if (background->type == PANEL_BACK_IMAGE)
    background->transformed_image = get_scaled_and_rotated_pixbuf(background);
...
static void panel_background_update_has_alpha(PanelBackground *background) {
  gboolean has_alpha = FALSE;

  if (background->type == PANEL_BACK_COLOR)
    has_alpha = (background->color.alpha < 1.);

  else if (background->type == PANEL_BACK_IMAGE && background->loaded_image)
...
static void load_background_file(PanelBackground *background) {
  ...
  if (!g_file_test(background->image, G_FILE_TEST_IS_REGULAR)) return;
  ...
  if (!background->loaded_image) {
    g_assert(error != NULL);
    g_warning(G_STRLOC ": unable to open '%s': %s", background->image,
              error->message);
    g_error_free(error);
  }
...
static void panel_background_set_color_no_update(PanelBackground *background,
                                                 const GdkRGBA *color) {
  g_return_if_fail(color != NULL);

  if (gdk_rgba_equal(color, &background->color)) return;
...
static void panel_background_set_image_no_update(PanelBackground *background,
                                                 const char *image) {
...
void panel_background_set_image(PanelBackground *background,
                                const char *image) {
  if (!background->image && !image) return;

  if (background->image && image && !strcmp(background->image, image)) return;
...
static void panel_background_set_fit_no_update(PanelBackground *background,
                                               gboolean fit_image) {
...
void panel_background_set_fit(PanelBackground *background, gboolean fit_image) {
  ...
  if (background->fit_image == fit_image) return;
...
static void panel_background_set_stretch_no_update(PanelBackground *background,
...
void panel_background_set_stretch(PanelBackground *background,
  ...
  if (background->stretch_image == stretch_image) return;
...
static void panel_background_set_rotate_no_update(PanelBackground *background,
...
void panel_background_set_rotate(PanelBackground *background,
  ...
  if (background->rotate_image == rotate_image) return;
...
void panel_background_set_default_style(PanelBackground *background,
  ...
  if (background->type == PANEL_BACK_NONE) panel_background_prepare(background);
}
...
void panel_background_unrealized(PanelBackground *background) {
  if (background->window) g_object_unref(background->window);
...
void panel_background_change_region(PanelBackground *background,
  ...
  if (background->region.x == x && background->region.y == y &&
  ...
  /* we only need to retransform anything
     ...
     conditions are met */
  if (background->type == PANEL_BACK_IMAGE) {
    if (background->orientation != orientation && background->rotate_image) {
      /* if orientation changes and we are rotating */
      need_to_retransform = TRUE;
    } else if ((background->region.width != width ||
                background->region.height != height) &&
               (background->fit_image || background->stretch_image)) {
  ...
  if (background->region.width != width || background->region.height != height)
...
char *panel_background_make_string(PanelBackground *background, int x, int y) {
  ...
  if (effective_type == PANEL_BACK_IMAGE) {
    ...
    if (!background->composited_pattern) return NULL;

    if (cairo_pattern_get_surface(background->composited_pattern, &surface) !=
        CAIRO_STATUS_SUCCESS)
      return NULL;

    if (cairo_surface_get_type(surface) != CAIRO_SURFACE_TYPE_XLIB) return NULL;

    retval = g_strdup_printf("pixmap:%d,%d,%d",
                             (guint32)cairo_xlib_surface_get_drawable(surface),
                             x, y);
  } else if (effective_type == PANEL_BACK_COLOR) {
    gchar *rgba = gdk_rgba_to_string(&background->color);
    retval = g_strdup_printf("color:%s", rgba);
    g_free(rgba);
  } else
