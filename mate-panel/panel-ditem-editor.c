...
enum { COLUMN_TEXT, COLUMN_TYPE, NUMBER_COLUMNS };
...
static ComboItem type_items[] = {
    {N_("Application"), "Application", PANEL_DITEM_EDITOR_TYPE_APPLICATION},
    {N_("Application in Terminal"), "Application",
     PANEL_DITEM_EDITOR_TYPE_TERMINAL_APPLICATION},
    {N_("Location"), "Link", PANEL_DITEM_EDITOR_TYPE_LINK},
    /* FIXME: hack hack hack: we will remove this item from the combo
     * box if we show it */
    {NULL, "Directory", PANEL_DITEM_EDITOR_TYPE_DIRECTORY}};
...
enum { PROP_0, PROP_KEYFILE, PROP_URI, PROP_TYPEDIRECTORY };

static guint ditem_edit_signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(PanelDItemEditor, panel_ditem_editor,
                           GTK_TYPE_DIALOG)
...
static void setup_icon_chooser(PanelDItemEditor *dialog, const char *icon_name);
...
static PanelDItemEditorType map_type_from_desktop_item(const char *type,
                                                       gboolean terminal) {
  if (type == NULL)
...
static GObject *panel_ditem_editor_constructor(
  ...
  obj = G_OBJECT_CLASS(panel_ditem_editor_parent_class)
  ...
  panel_icon_chooser_set_fallback_icon_name(
      PANEL_ICON_CHOOSER(dialog->priv->icon_chooser), PANEL_ICON_LAUNCHER);
  gtk_box_pack_start(GTK_BOX(dialog->priv->icon_chooser_box),
                     dialog->priv->icon_chooser, FALSE, FALSE, 0);
  ...
  if (dialog->priv->key_file) {
    ...
    if (dialog->priv->type_directory)
      panel_key_file_set_string(dialog->priv->key_file, "Type", "Directory");
  ...
  desktop_type = panel_key_file_get_string(dialog->priv->key_file, "Type");
  ...
  if (dialog->priv->new_file) setup_icon_chooser(dialog, NULL);
...
static void panel_ditem_editor_get_property(GObject *object, guint prop_id,
...
static void panel_ditem_editor_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  PanelDItemEditor *dialog;
  ...
  switch (prop_id) {
    ...
    case PROP_URI:
      ...
      break;
    case PROP_TYPEDIRECTORY:
      panel_ditem_editor_set_type_directory(dialog, g_value_get_boolean(value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
static void panel_ditem_editor_dispose(GObject *object) {
  PanelDItemEditor *dialog;
...
static void panel_ditem_editor_class_init(PanelDItemEditorClass *klass) {
  ...
  ditem_edit_signals[SAVED] =
      g_signal_new("saved", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(PanelDItemEditorClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  ditem_edit_signals[CHANGED] = g_signal_new(
      "changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelDItemEditorClass, changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  ditem_edit_signals[NAME_CHANGED] = g_signal_new(
      "name_changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelDItemEditorClass, name_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  ditem_edit_signals[COMMAND_CHANGED] = g_signal_new(
      "command-changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelDItemEditorClass, command_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  ditem_edit_signals[COMMENT_CHANGED] = g_signal_new(
      "comment_changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelDItemEditorClass, comment_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  ditem_edit_signals[ICON_CHANGED] = g_signal_new(
      "icon_changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelDItemEditorClass, icon_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  ditem_edit_signals[ERROR_REPORTED] = g_signal_new(
      "error-reported", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PanelDItemEditorClass, error_reported), NULL, NULL,
      panel_marshal_VOID__STRING_STRING, G_TYPE_NONE, 2, G_TYPE_STRING,
      G_TYPE_STRING);

  g_object_class_install_property(
      gobject_class, PROP_KEYFILE,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("uri", "URI", "The URI of the .desktop file", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_TYPEDIRECTORY,
      g_param_spec_boolean(
          "type-directory", "Type Directory",
          "Whether the edited file is a .directory file or not", FALSE,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  gtk_widget_class_set_template_from_resource(widget_class, resource);
  gtk_widget_class_bind_template_child_private(widget_class, PanelDItemEditor,
                                               icon_chooser_box);
  gtk_widget_class_bind_template_child_private(widget_class, PanelDItemEditor,
                                               type_label);
  gtk_widget_class_bind_template_child_private(widget_class, PanelDItemEditor,
...
static void setup_combo(GtkWidget *combo_box, ComboItem *items, int nb_items,
                        const char *for_type) {
  GtkListStore *model;
  GtkTreeIter iter;
  ...
  model = gtk_list_store_new(NUMBER_COLUMNS, G_TYPE_STRING, G_TYPE_INT);

  gtk_combo_box_set_model(GTK_COMBO_BOX(combo_box), GTK_TREE_MODEL(model));

  for (i = 0; i < nb_items; i++) {
    if (for_type && strcmp(for_type, items[i].show_for)) continue;

    gtk_list_store_append(model, &iter);
    gtk_list_store_set(model, &iter, COLUMN_TEXT, _(items[i].name), COLUMN_TYPE,
                       items[i].type, -1);
  }

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combo_box), renderer, TRUE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo_box), renderer, "text",
                                 COLUMN_TEXT, NULL);

  gtk_combo_box_set_active(GTK_COMBO_BOX(combo_box), 0);
}

static PanelDItemEditorType panel_ditem_editor_get_item_type(
    PanelDItemEditor *dialog) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  PanelDItemEditorType type;

  if (dialog->priv->type_directory) return PANEL_DITEM_EDITOR_TYPE_DIRECTORY;
...
static void panel_ditem_editor_setup_ui(PanelDItemEditor *dialog) {
  ...
  if (priv->new_file) {
    ...
    if (!priv->combo_setuped) {
      setup_combo(priv->type_combo, type_items, G_N_ELEMENTS(type_items), NULL);
  ...
  if (show_combo) {
    ...
    if (!gtk_tree_model_get_iter_first(model, &iter)) g_assert_not_reached();
    do {
 ...
 */
static void panel_ditem_editor_changed(PanelDItemEditor *dialog) {
  if (!dialog->priv->new_file) {
    if (dialog->priv->save_timeout != 0)
      g_source_remove(dialog->priv->save_timeout);

    dialog->priv->save_timeout = g_timeout_add_seconds(
        SAVE_FREQUENCY, panel_ditem_editor_save_timeout, dialog);

    /* We can revert to the original state */
    if (dialog->priv->revert_key_file != NULL)
      gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog), REVERT_BUTTON,
                                        TRUE);
  }
...
static void panel_ditem_editor_activated(PanelDItemEditor *dialog) {
...
static void panel_ditem_editor_name_changed(PanelDItemEditor *dialog) {
  const char *name;
  ...
  if (!dialog->priv->reverting) {
    ...
    if (name && name[0])
      panel_key_file_set_locale_string(dialog->priv->key_file, "Name", name);
    else
      panel_key_file_remove_all_locale_key(dialog->priv->key_file, "Name");

    panel_key_file_remove_all_locale_key(dialog->priv->key_file,
                                         "X-MATE-FullName");
  }

  g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[NAME_CHANGED], 0, name);
}

static void panel_ditem_editor_command_changed(PanelDItemEditor *dialog) {
  PanelDItemEditorType type;
  ...
  switch (type) {
    ...
    case PANEL_DITEM_EDITOR_TYPE_TERMINAL_APPLICATION:
      ...
      panel_key_file_set_string(dialog->priv->key_file, "Exec", exec_or_uri);
      ...
      if (icon) {
        ...
        current =
            panel_key_file_get_locale_string(dialog->priv->key_file, "Icon");

        if (!current || strcmp(icon, current)) setup_icon_chooser(dialog, icon);
    ...
    case PANEL_DITEM_EDITOR_TYPE_LINK:
      ...
      panel_key_file_set_string(dialog->priv->key_file, "URL", exec_or_uri);
      break;
    default:
      panel_key_file_remove_key(dialog->priv->key_file, "Exec");
      panel_key_file_remove_key(dialog->priv->key_file, "URL");
  ...
  g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[COMMAND_CHANGED], 0,
                exec_or_uri);
}

static void panel_ditem_editor_comment_changed(PanelDItemEditor *dialog) {
  const char *comment;

  comment = gtk_entry_get_text(GTK_ENTRY(dialog->priv->comment_entry));

  if (comment && comment[0])
    panel_key_file_set_locale_string(dialog->priv->key_file, "Comment",
                                     comment);
  else
    panel_key_file_remove_all_locale_key(dialog->priv->key_file, "Comment");

  g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[COMMENT_CHANGED], 0,
                comment);
}

static void panel_ditem_editor_icon_changed(PanelDItemEditor *dialog,
                                            const char *icon) {
  if (icon) {
    panel_key_file_set_string(dialog->priv->key_file, "Icon", icon);
    panel_key_file_set_locale_string(dialog->priv->key_file, "Icon", icon);
  } else
    panel_key_file_remove_all_locale_key(dialog->priv->key_file, "Icon");

  g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[ICON_CHANGED], 0, icon);
}

static void command_browse_chooser_response(GtkFileChooser *chooser,
                                            gint response_id,
                                            PanelDItemEditor *dialog) {
...
static void update_chooser_for_type(PanelDItemEditor *dialog) {
  const char *title;
  gboolean local_only;
  GtkWidget *chooser;

  if (!dialog->priv->command_browse_filechooser) return;
...
static void command_browse_button_clicked(PanelDItemEditor *dialog) {
  GtkWidget *chooser;
  ...
  chooser = panel_file_chooser_dialog_new(
      "", GTK_WINDOW(dialog), GTK_FILE_CHOOSER_ACTION_OPEN, "process-stop",
...
#define CONNECT_CHANGED(widget, callback)                                    \
  g_signal_connect_swapped(widget, "changed", G_CALLBACK(callback), dialog); \
  g_signal_connect_swapped(widget, "changed",                                \
                           G_CALLBACK(panel_ditem_editor_changed), dialog);
...
#undef CONNECT_CHANGED
  ...
  g_signal_connect_swapped(priv->name_entry, "activate",
                           G_CALLBACK(panel_ditem_editor_activated), dialog);
  ...
  g_signal_connect_swapped(priv->command_browse_button, "clicked",
                           G_CALLBACK(command_browse_button_clicked), dialog);
}

static void panel_ditem_editor_block_signals(PanelDItemEditor *dialog) {
  PanelDItemEditorPrivate *priv;

  priv = dialog->priv;

#define BLOCK_CHANGED(widget, callback)                                   \
  g_signal_handlers_block_by_func(G_OBJECT(widget), G_CALLBACK(callback), \
                                  dialog);                                \
  g_signal_handlers_block_by_func(                                        \
      G_OBJECT(widget), G_CALLBACK(panel_ditem_editor_changed), dialog);
...
static void panel_ditem_editor_unblock_signals(PanelDItemEditor *dialog) {
  PanelDItemEditorPrivate *priv;

  priv = dialog->priv;

#define UNBLOCK_CHANGED(widget, callback)                                   \
  g_signal_handlers_unblock_by_func(G_OBJECT(widget), G_CALLBACK(callback), \
                                    dialog);                                \
...
static void panel_ditem_editor_init(PanelDItemEditor *dialog) {
...
static void type_combo_changed(PanelDItemEditor *dialog) {
  ...
  switch (panel_ditem_editor_get_item_type(dialog)) {
    case PANEL_DITEM_EDITOR_TYPE_APPLICATION:
      ...
      if (dialog->priv->combo_setuped) {
        panel_key_file_set_string(dialog->priv->key_file, "Type",
                                  "Application");
        panel_key_file_set_boolean(dialog->priv->key_file, "Terminal", FALSE);
      }
      break;
    case PANEL_DITEM_EDITOR_TYPE_TERMINAL_APPLICATION:
      text = _("Comm_and:");
      if (dialog->priv->combo_setuped) {
        panel_key_file_set_string(dialog->priv->key_file, "Type",
                                  "Application");
        panel_key_file_set_boolean(dialog->priv->key_file, "Terminal", TRUE);
      }
      break;
    case PANEL_DITEM_EDITOR_TYPE_LINK:
      text = _("_Location:");
      if (dialog->priv->combo_setuped) {
        panel_key_file_set_string(dialog->priv->key_file, "Type", "Link");
        panel_key_file_remove_key(dialog->priv->key_file, "Terminal");
      }
      break;
  ...
  gtk_label_set_text_with_mnemonic(GTK_LABEL(dialog->priv->command_label),
                                   text);
...
void panel_ditem_editor_sync_display(PanelDItemEditor *dialog) {
  ...
  if (!buffer) buffer = panel_key_file_get_locale_string(key_file, "Name");
  gtk_entry_set_text(GTK_ENTRY(dialog->priv->name_entry), buffer ? buffer : "");
  g_free(buffer);

  /* Type */
  type = panel_key_file_get_string(key_file, "Type");
  if (!dialog->priv->combo_setuped) {
    setup_combo(dialog->priv->type_combo, type_items, G_N_ELEMENTS(type_items),
                type);
    dialog->priv->combo_setuped = TRUE;
  }

  run_in_terminal = panel_key_file_get_boolean(key_file, "Terminal", FALSE);
...
static gboolean panel_ditem_editor_save(PanelDItemEditor *dialog,
  ...
  g_return_val_if_fail(
      dialog->priv->save_uri != NULL || dialog->priv->uri != NULL, FALSE);

  if (dialog->priv->save_timeout != 0)
    g_source_remove(dialog->priv->save_timeout);
  dialog->priv->save_timeout = 0;

  if (!dialog->priv->dirty) return TRUE;
  ...
  if (const_buf == NULL || const_buf[0] == '\0') {
    if (report_errors) {
      if (!dialog->priv->type_directory)
        g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[ERROR_REPORTED], 0,
  ...
  /* And now, try to save */
  error = NULL;
  panel_key_file_to_file(dialog->priv->key_file, dialog->priv->uri, &error);
  if (error != NULL) {
    if (report_errors)
      g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[ERROR_REPORTED], 0,
                    _("Could not save launcher"), error->message);
    g_error_free(error);
    return FALSE;
  } else {
    g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[SAVED], 0);
  }
  ...
  switch (response_id) {
    case GTK_RESPONSE_HELP:
      if (!panel_show_help(gtk_window_get_screen(GTK_WINDOW(dialog)),
        ...
        g_signal_emit(G_OBJECT(dialog), ditem_edit_signals[ERROR_REPORTED], 0,
                      _("Could not display help document"), error->message);
        g_error_free(error);
      }
      break;
    case REVERT_BUTTON:
      panel_ditem_editor_revert(PANEL_DITEM_EDITOR(dialog));
      gtk_dialog_set_response_sensitive(dialog, REVERT_BUTTON, FALSE);
      break;
  ...
  for (i = 0; i < G_N_ELEMENTS(revert_keys); i++) {
    if (revert_keys[i].type == G_TYPE_STRING) {
      if (revert_keys[i].locale) {
        string = panel_key_file_get_locale_string(revert_key_file,
                                                  revert_keys[i].key);
        if (string == NULL)
          panel_key_file_remove_all_locale_key(key_file, revert_keys[i].key);
        else
          panel_key_file_set_locale_string(key_file, revert_keys[i].key,
                                           string);
      } else {
        string = panel_key_file_get_string(revert_key_file, revert_keys[i].key);
        if (string == NULL)
          panel_key_file_remove_key(key_file, revert_keys[i].key);
        else
          panel_key_file_set_string(key_file, revert_keys[i].key, string);
      }
      g_free(string);
    } else if (revert_keys[i].type == G_TYPE_BOOLEAN) {
      boolean = panel_key_file_get_boolean(revert_key_file, revert_keys[i].key,
                                           revert_keys[i].default_value);
      panel_key_file_set_boolean(key_file, revert_keys[i].key, boolean);
    } else {
      g_assert_not_reached();
    }
  }
...
static void panel_ditem_editor_set_revert(PanelDItemEditor *dialog) {
  gsize i;
  char *string;
  gboolean boolean;
  GKeyFile *key_file;
  ...
  for (i = 0; i < G_N_ELEMENTS(revert_keys); i++) {
    if (revert_keys[i].type == G_TYPE_STRING) {
      if (revert_keys[i].locale) {
        string = panel_key_file_get_locale_string(key_file, revert_keys[i].key);
        if (string != NULL)
          panel_key_file_set_locale_string(revert_key_file, revert_keys[i].key,
                                           string);
      } else {
        string = panel_key_file_get_string(key_file, revert_keys[i].key);
        if (string != NULL)
          panel_key_file_set_string(revert_key_file, revert_keys[i].key,
                                    string);
      }
      g_free(string);
    } else if (revert_keys[i].type == G_TYPE_BOOLEAN) {
      boolean = panel_key_file_get_boolean(key_file, revert_keys[i].key,
                                           revert_keys[i].default_value);
      panel_key_file_set_boolean(revert_key_file, revert_keys[i].key, boolean);
    } else {
      g_assert_not_reached();
    }
  }
...
static void panel_ditem_editor_key_file_loaded(PanelDItemEditor *dialog) {
  ...
  if (dialog->priv->revert_key_file != NULL)
    gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog), REVERT_BUTTON, TRUE);
  else
...
static gboolean panel_ditem_editor_load_uri(PanelDItemEditor *dialog,
  ...
  if (!panel_key_file_load_from_uri(
          key_file, dialog->priv->uri,
...
static GtkWidget *panel_ditem_editor_new_full(GtkWindow *parent,
                                              GKeyFile *key_file,
                                              const char *uri,
                                              const char *title,
                                              gboolean type_directory) {
  ...
  dialog =
      g_object_new(PANEL_TYPE_DITEM_EDITOR, "title", title, "keyfile", key_file,
                   "uri", uri, "type-directory", type_directory, NULL);
...
GtkWidget *panel_ditem_editor_new(GtkWindow *parent, GKeyFile *key_file,
  ...
  return panel_ditem_editor_new_full(parent, key_file, uri, title, FALSE);
}

GtkWidget *panel_ditem_editor_new_directory(GtkWindow *parent,
                                            GKeyFile *key_file, const char *uri,
                                            const char *title) {
  return panel_ditem_editor_new_full(parent, key_file, uri, title, TRUE);
}
...
static void panel_ditem_editor_set_key_file(PanelDItemEditor *dialog,
  ...
  if (dialog->priv->key_file == key_file) return;
...
void panel_ditem_editor_set_uri(PanelDItemEditor *dialog, const char *uri) {
  ...
  if (!dialog->priv->uri && (!uri || !uri[0])) return;

  if (dialog->priv->uri && uri && uri[0] && !strcmp(dialog->priv->uri, uri))
    return;
...
static void panel_ditem_editor_set_type_directory(PanelDItemEditor *dialog,
  ...
  if (dialog->priv->type_directory == type_directory) return;
...
static gboolean panel_ditem_editor_get_type_directory(
    PanelDItemEditor *dialog) {
  g_return_val_if_fail(PANEL_IS_DITEM_EDITOR(dialog), FALSE);
