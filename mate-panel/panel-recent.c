...
static gboolean show_uri(const char *uri, const char *mime_type,
                         GdkScreen *screen, GError **error) {
  return panel_show_uri_force_mime_type(screen, uri, mime_type,
                                        gtk_get_current_event_time(), error);
}

static void recent_documents_activate_cb(GtkRecentChooser *chooser,
                                         gpointer data) {
  ...
  if (show_uri(uri, mime_type, screen, &error) != TRUE) {
    ...
    if (error) {
      ...
      primary = g_strdup_printf(
          _("Could not open recently used document \"%s\""), uri_utf8);
      panel_error_dialog(NULL, screen, "cannot_open_recent_doc", TRUE, primary,
                         error->message);
      g_free(primary);
      g_error_free(error);
    } else {
...
static void panel_recent_manager_changed_cb(GtkRecentManager *manager,
                                            GtkWidget *menu_item) {
  int size;
...
static void clear_dialog_response(GtkWidget *widget, int response,
...
static void recent_documents_clear_cb(GtkMenuItem *menuitem,
                                      GtkRecentManager *manager) {
  gpointer tmp;
  ...
  clear_recent_dialog = gtk_message_dialog_new(
      NULL, 0 /* flags */, GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE,
      _("Clear the Recent Documents list?"));
  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(clear_recent_dialog),
      _("If you clear the Recent Documents list, you clear the following:\n"
        "\342\200\242 All items from the Places \342\206\222 Recent Documents "
        "menu item.\n"
        "\342\200\242 All items from the recent documents list in all "
        "applications."));

  panel_dialog_add_button(GTK_DIALOG(clear_recent_dialog), _("_Cancel"),
                          "process-stop", GTK_RESPONSE_CANCEL);

  gtk_dialog_add_button(GTK_DIALOG(clear_recent_dialog), PANEL_STOCK_CLEAR,
                        GTK_RESPONSE_ACCEPT);
  ...
  g_signal_connect(clear_recent_dialog, "destroy",
                   G_CALLBACK(gtk_widget_destroyed), &clear_recent_dialog);
...
void panel_recent_append_documents_menu(GtkWidget *top_menu,
                                        GtkRecentManager *manager,
  ...
  setup_menuitem_with_icon(menu_item, panel_menu_icon_get_size(), NULL,
                           PANEL_ICON_RECENT, _("Recent Documents"));
  ...
  g_signal_connect(recent_menu, "item-activated",
                   G_CALLBACK(recent_documents_activate_cb), NULL);
  ...
  menu_item = gtk_image_menu_item_new();
  setup_menuitem_with_icon(menu_item, panel_menu_icon_get_size(), NULL,
                           "edit-clear", _("Clear Recent Documents..."));
  panel_util_set_tooltip_text(
      menu_item, _("Clear all items from the recent documents list"));
  gtk_menu_shell_append(GTK_MENU_SHELL(recent_menu), menu_item);

  g_signal_connect(menu_item, "activate", G_CALLBACK(recent_documents_clear_cb),
                   manager);
}
