...
void panel_xutils_set_strut(GdkWindow *gdk_window, PanelOrientation orientation,
  ...
  if (net_wm_strut_partial == None)
    net_wm_strut_partial =
        XInternAtom(xdisplay, "_NET_WM_STRUT_PARTIAL", False);
  ...
  XChangeProperty(xdisplay, window, net_wm_strut, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)&struts, 4);
  XChangeProperty(xdisplay, window, net_wm_strut_partial, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)&struts, 12);
  XChangeProperty(xdisplay, window, gnome_wm_strut_area, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)&area, 4);
...
guint panel_get_real_modifier_mask(guint mask) {
  guint real_mask;
  Display *display;
  ...
  for (i = Mod2MapIndex * max_keycodes_per_modifier;
       i < (Mod5MapIndex + 1) * max_keycodes_per_modifier; i++) {
    int keycode;
    int j;
    ...
    if (keycode < min_keycode || keycode > max_keycode) continue;

    keysyms_for_keycode =
        keysyms_for_keycodes + (keycode - min_keycode) * keysyms_per_keycode;
    ...
    for (j = 0; j < keysyms_per_keycode; j++) {
      switch (keysyms_for_keycode[j]) {
        case XK_Super_L:
        case XK_Super_R:
          if (mask & GDK_SUPER_MASK) real_mask |= map_mask;
          break;
        case XK_Hyper_L:
        case XK_Hyper_R:
          if (mask & GDK_HYPER_MASK) real_mask |= map_mask;
          break;
        case XK_Meta_L:
        case XK_Meta_R:
          if (mask & GDK_META_MASK) real_mask |= map_mask;
          break;
        default:
          break;
      }
