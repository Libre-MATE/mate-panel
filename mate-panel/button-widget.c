...
static void do_colorshift(cairo_surface_t *dest, cairo_surface_t *src,
                          int shift) {
  gint i, j;
  ...
  for (i = 0; i < height; i++) {
    ...
    for (j = 0; j < width; j++) {
      ...
      *(pixdest++) = CLAMP(val, 0, 255);
      val = b + shift;
      *(pixdest++) = CLAMP(val, 0, 255);
      if (has_alpha) *(pixdest++) = *(pixsrc++);
    }
  }
}

static cairo_surface_t *make_hc_surface(cairo_surface_t *surface) {
  cairo_t *cr;
  cairo_surface_t *new;
  ...
  new =
      cairo_surface_create_similar(surface, cairo_surface_get_content(surface),
...
static void button_widget_realize(GtkWidget *widget) {
  ...
  BUTTON_WIDGET(widget)->priv->icon_theme =
      gtk_icon_theme_get_for_screen(gtk_widget_get_screen(widget));
  g_signal_connect_object(BUTTON_WIDGET(widget)->priv->icon_theme, "changed",
                          G_CALLBACK(button_widget_icon_theme_changed), widget,
                          G_CONNECT_SWAPPED);
...
static void button_widget_unset_surfaces(ButtonWidget *button) {
  if (button->priv->surface) cairo_surface_destroy(button->priv->surface);
  button->priv->surface = NULL;

  if (button->priv->surface_hc) cairo_surface_destroy(button->priv->surface_hc);
...
static void button_widget_reload_surface(ButtonWidget *button) {
  ...
  if (button->priv->size <= 1 || button->priv->icon_theme == NULL) return;
  ...
  if (button->priv->filename != NULL && button->priv->filename[0] != '\0') {
    ...
    button->priv->surface =
        panel_load_icon(button->priv->icon_theme, button->priv->filename,
                        button->priv->size * scale,
                        (button->priv->orientation & PANEL_VERTICAL_MASK)
                            ? button->priv->size * scale
                            : -1,
    ...
    if (error) {
      ...
      button->priv->surface = gtk_icon_theme_load_surface(
          icon_theme, "image-missing", GTK_ICON_SIZE_BUTTON, scale, NULL,
          GTK_ICON_LOOKUP_FORCE_SVG | GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
      g_free(error);
    }
  }
...
static void button_widget_icon_theme_changed(ButtonWidget *button) {
  if (button->priv->filename != NULL) button_widget_reload_surface(button);
}

static void button_widget_finalize(GObject *object) {
...
static void button_widget_get_property(GObject *object, guint prop_id,
...
static void button_widget_set_property(GObject *object, guint prop_id,
...
static GtkArrowType calc_arrow(PanelOrientation orientation, int button_width,
                               int button_height, int *x, int *y,
                               gdouble *angle, gdouble *size) {
  GtkArrowType retval = GTK_ARROW_UP;

  if (orientation & PANEL_HORIZONTAL_MASK) {
    if (button_width > 50) button_width = 50;
  } else {
    if (button_height > 50) button_height = 50;
  }

  *size =
      ((orientation & PANEL_HORIZONTAL_MASK) ? button_width : button_height) /
      2;
  *angle = 0;
  ...
  switch (orientation) {
    case PANEL_ORIENTATION_TOP:
      ...
      *y = button_height * .99 -
           (*size) / (3 / 2); /* 3/2 is the approximate ratio of GTK arrows */
      *angle = G_PI;
...
static gboolean button_widget_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  off = (button_widget->priv->activatable &&
         ...
         (state_flags & GTK_STATE_FLAG_ACTIVE))
            ? BUTTON_WIDGET_DISPLACEMENT * height / 48.0
            : 0;
  ...
  if (button_widget->priv->arrow) {
    ...
    calc_arrow(button_widget->priv->orientation, width, height, &x, &y, &angle,
               &size);
...
static void button_widget_get_preferred_width(GtkWidget *widget,
                                              gint *minimal_width,
                                              gint *natural_width) {
  ...
  if (button_widget->priv->orientation & PANEL_HORIZONTAL_MASK) {
    ...
    /* should get this value (50) from gsettings, user defined value in
     * properties of the panel (max_icon_size) OR use 48*/
    if (size > 50) size = 50;
...
static void button_widget_get_preferred_height(GtkWidget *widget,
                                               gint *minimal_height,
                                               gint *natural_height) {
  ButtonWidget *button_widget = BUTTON_WIDGET(widget);
  GtkWidget *parent;
...
static void button_widget_size_allocate(GtkWidget *widget,
  ...
  /* should get this value (50) from gsettings, user defined value in properties
   * of the panel (max_icon_size) OR use 48?*/
  if (button_widget->priv->orientation & PANEL_HORIZONTAL_MASK) {
  ...
  GTK_WIDGET_CLASS(button_widget_parent_class)
...
static void button_widget_activate(GtkButton *button) {
  ButtonWidget *button_widget = BUTTON_WIDGET(button);

  if (!button_widget->priv->activatable) return;
...
static gboolean button_widget_button_press(GtkWidget *widget,
  ...
  if (event->button == 1 && BUTTON_WIDGET(widget)->priv->activatable &&
    ...
    return GTK_WIDGET_CLASS(button_widget_parent_class)
...
static void button_widget_class_init(ButtonWidgetClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("activatable", "Activatable",
                           "Whether the button is activatable", TRUE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      gobject_class, PROP_IGNORE_LEAVE,
      g_param_spec_boolean(
          "ignore-leave", "Ignore leaving to not unhighlight the icon",
          "Whether or not to unhighlight the icon when the cursor leaves it",
          FALSE, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_HAS_ARROW,
      g_param_spec_boolean("has-arrow", "Has Arrow",
                           "Whether or not to draw an arrow indicator", FALSE,
                           G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_DND_HIGHLIGHT,
      g_param_spec_boolean(
          "dnd-highlight", "Drag and drop Highlight",
          "Whether or not to highlight the icon during drag and drop", FALSE,
          G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_ORIENTATION,
      g_param_spec_enum("orientation", "Orientation",
                        "The ButtonWidget orientation", PANEL_TYPE_ORIENTATION,
                        PANEL_ORIENTATION_TOP, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_ICON_NAME,
      g_param_spec_string("icon-name", "Icon Name",
                          "The desired icon for the ButtonWidget", NULL,
                          G_PARAM_READWRITE));
}

GtkWidget *button_widget_new(const char *filename, gboolean arrow,
                             PanelOrientation orientation) {
  GtkWidget *retval;

  retval = g_object_new(BUTTON_TYPE_WIDGET, "has-arrow", arrow, "orientation",
                        orientation, "icon-name", filename, NULL);

  return retval;
}

void button_widget_set_activatable(ButtonWidget *button, gboolean activatable) {
...
void button_widget_set_icon_name(ButtonWidget *button, const char *icon_name) {
  g_return_if_fail(BUTTON_IS_WIDGET(button));
  ...
  if (button->priv->filename) g_free(button->priv->filename);
...
void button_widget_set_orientation(ButtonWidget *button,
  ...
  if (button->priv->orientation == orientation) return;
...
void button_widget_set_has_arrow(ButtonWidget *button, gboolean has_arrow) {
  g_return_if_fail(BUTTON_IS_WIDGET(button));

  has_arrow = has_arrow != FALSE;

  if (button->priv->arrow == has_arrow) return;
...
void button_widget_set_dnd_highlight(ButtonWidget *button,
  ...
  if (button->priv->dnd_highlight == dnd_highlight) return;
...
void button_widget_set_ignore_leave(ButtonWidget *button,
  ...
  if (button->priv->ignore_leave == ignore_leave) return;
...
cairo_surface_t *button_widget_get_surface(ButtonWidget *button) {
  ...
  if (!button->priv->surface) return NULL;
