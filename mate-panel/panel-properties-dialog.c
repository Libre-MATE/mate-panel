...
static void panel_properties_dialog_opacity_changed(
    PanelPropertiesDialog *dialog);

static void panel_properties_dialog_free(PanelPropertiesDialog *dialog) {
  if (dialog->settings) g_object_unref(dialog->settings);
  dialog->settings = NULL;

  if (dialog->background_settings) g_object_unref(dialog->background_settings);
  dialog->background_settings = NULL;

  if (dialog->properties_dialog) gtk_widget_destroy(dialog->properties_dialog);
  dialog->properties_dialog = NULL;

  g_free(dialog);
}

enum { COLUMN_TEXT, COLUMN_ITEM, NUMBER_COLUMNS };
...
static void panel_properties_dialog_orientation_changed(
    PanelPropertiesDialog *dialog, GtkComboBox *combo_box) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  ...
  if (!gtk_combo_box_get_active_iter(combo_box, &iter)) return;

  model = gtk_combo_box_get_model(combo_box);
  gtk_tree_model_get(model, &iter, COLUMN_ITEM, &item, -1);
  if (item == NULL) return;

  panel_profile_set_toplevel_orientation(dialog->toplevel, item->orientation);
}

static void panel_properties_dialog_setup_orientation_combo_sensitivty(
    PanelPropertiesDialog *dialog, GtkToggleButton *n) {
  gboolean expand;
  expand = panel_profile_get_toplevel_expand(dialog->toplevel);

  if (!panel_profile_key_is_writable(dialog->toplevel,
...
static void panel_properties_dialog_setup_orientation_combo(
    PanelPropertiesDialog *dialog, GtkBuilder *gui) {
  PanelOrientation orientation;
  GtkListStore *model;
  GtkTreeIter iter;
  ...
  model = gtk_list_store_new(NUMBER_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER);
  ...
  for (i = 0; i < G_N_ELEMENTS(orientation_items); i++) {
    ...
    gtk_list_store_set(
        model, &iter, COLUMN_TEXT,
        g_dpgettext2(NULL, "Orientation", orientation_items[i].name),
        COLUMN_ITEM, &(orientation_items[i]), -1);
  ...
  g_signal_connect_swapped(
      dialog->orientation_combo, "changed",
      G_CALLBACK(panel_properties_dialog_orientation_changed), dialog);

  panel_properties_dialog_setup_orientation_combo_sensitivty(dialog, NULL);
}

static void panel_properties_dialog_size_changed(PanelPropertiesDialog *dialog,
                                                 GtkSpinButton *spin_button) {
  panel_profile_set_toplevel_size(
      dialog->toplevel, gtk_spin_button_get_value_as_int(spin_button));
}
...
static void panel_properties_dialog_icon_changed(
    PanelIconChooser *chooser, const char *icon,
    PanelPropertiesDialog *dialog) {
  panel_profile_set_attached_custom_icon(dialog->toplevel, icon);
}

static void panel_properties_dialog_setup_icon_chooser(
    PanelPropertiesDialog *dialog, GtkBuilder *gui) {
  char *custom_icon;
  ...
  panel_icon_chooser_set_fallback_icon_name(
      PANEL_ICON_CHOOSER(dialog->icon_chooser), PANEL_ICON_DRAWER);
  gtk_widget_show(dialog->icon_chooser);
  gtk_container_add(GTK_CONTAINER(dialog->icon_align), dialog->icon_chooser);
...
/* Note: this is only for toggle buttons on the general page, if needed for
   togglebuttons elsewhere you must make this respect the writability warning
   thing for the right page */
#define SETUP_TOGGLE_BUTTON(wid, n, p, key)                                    \
  static void panel_properties_dialog_##n(PanelPropertiesDialog *dialog,       \
                                          GtkToggleButton *n) {                \
    panel_profile_set_toplevel_##p(dialog->toplevel,                           \
                                   gtk_toggle_button_get_active(n));           \
  }                                                                            \
  static void panel_properties_dialog_setup_##n(PanelPropertiesDialog *dialog, \
                                                GtkBuilder *gui) {             \
    dialog->n = PANEL_GTK_BUILDER_GET(gui, wid);                               \
    gtk_toggle_button_set_active(                                              \
        GTK_TOGGLE_BUTTON(dialog->n),                                          \
...
static void panel_properties_dialog_color_changed(
    PanelPropertiesDialog *dialog, GtkColorChooser *color_button) {
  GdkRGBA color;
...
static void panel_properties_dialog_setup_color_button(
  ...
  if (!gdk_screen_is_composited(gdk_screen_get_default())) color.alpha = 1.0;

  gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(dialog->color_button), &color);
...
static void panel_properties_dialog_image_changed(
    PanelPropertiesDialog *dialog) {
  char *image;

  image =
      gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog->image_chooser));
...
static void panel_properties_dialog_setup_image_chooser(
    PanelPropertiesDialog *dialog, GtkBuilder *gui) {
  char *image;

  dialog->image_chooser = PANEL_GTK_BUILDER_GET(gui, "image_chooser");
  panel_gtk_file_chooser_add_image_preview(
      GTK_FILE_CHOOSER(dialog->image_chooser));
...
static void panel_properties_dialog_upd_sensitivity(
    PanelPropertiesDialog *dialog, PanelBackgroundType background_type) {
...
static void panel_properties_dialog_background_toggled(
    PanelPropertiesDialog *dialog, GtkWidget *radio) {
...
static void panel_properties_dialog_setup_background_radios(
  ...
  g_signal_connect_swapped(
      dialog->default_radio, "toggled",
      G_CALLBACK(panel_properties_dialog_background_toggled), dialog);
  g_signal_connect_swapped(
      dialog->color_radio, "toggled",
      G_CALLBACK(panel_properties_dialog_background_toggled), dialog);
  g_signal_connect_swapped(
      dialog->image_radio, "toggled",
      G_CALLBACK(panel_properties_dialog_background_toggled), dialog);
...
static void panel_properties_update_arrows_toggle_visible(
    PanelPropertiesDialog *dialog, GtkToggleButton *toggle) {
  if (gtk_toggle_button_get_active(toggle))
    gtk_widget_set_sensitive(
        dialog->arrows_toggle,
        panel_profile_key_is_writable(dialog->toplevel,
                                      PANEL_TOPLEVEL_ENABLE_ARROWS_KEY));
  else
    gtk_widget_set_sensitive(dialog->arrows_toggle, FALSE);
}

static void panel_properties_dialog_response(PanelPropertiesDialog *dialog,
                                             int response,
                                             GtkWidget *properties_dialog) {
  char *help_id;
...
static void panel_properties_dialog_destroy(PanelPropertiesDialog *dialog) {
  ...
  g_object_set_qdata(G_OBJECT(dialog->toplevel), panel_properties_dialog_quark,
                     NULL);
}

static void panel_properties_dialog_update_orientation(
    PanelPropertiesDialog *dialog, PanelOrientation orientation) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  OrientationComboItem *item;
  int max_size;
  ...
  spin_size =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(dialog->size_spin));
  ...
  if (spin_size > max_size)
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(dialog->size_spin), max_size);
...
static void panel_properties_dialog_update_size(PanelPropertiesDialog *dialog,
                                                int size) {
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(dialog->size_spin), size);
}

static void panel_properties_dialog_toplevel_notify(
    GSettings *settings, gchar *key, PanelPropertiesDialog *dialog) {
#define UPDATE_TOGGLE(p, n)                                                \
  ...
  if (!strcmp(key, "orientation"))
    panel_properties_dialog_update_orientation(
        dialog, g_settings_get_enum(settings, key));
  else if (!strcmp(key, "size"))
    panel_properties_dialog_update_size(dialog,
                                        g_settings_get_int(settings, key));
  else
    UPDATE_TOGGLE("expand", expand_toggle)
  else UPDATE_TOGGLE("auto-hide", autohide_toggle) else UPDATE_TOGGLE(
      "enable-buttons", hidebuttons_toggle) else UPDATE_TOGGLE("enable-arrows",
                                                               arrows_toggle)
}

static void panel_properties_dialog_update_background_type(
    PanelPropertiesDialog *dialog, PanelBackgroundType background_type) {
  GtkWidget *active_radio;
...
static void panel_properties_dialog_update_background_color(
    PanelPropertiesDialog *dialog, gchar *str_color) {
  GdkRGBA new_color;
  GdkRGBA old_color;

  if (!gdk_rgba_parse(&new_color, str_color)) return;
...
static void panel_properties_dialog_update_background_image(
    PanelPropertiesDialog *dialog, gchar *text) {
  char *old_text;
  ...
  if (PANEL_GLIB_STR_EMPTY(text) && old_text)
    gtk_file_chooser_unselect_all(GTK_FILE_CHOOSER(dialog->image_chooser));
  else if (!PANEL_GLIB_STR_EMPTY(text) && (!old_text || strcmp(text, old_text)))
    gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(dialog->image_chooser),
                                  text);
...
static void panel_properties_dialog_background_notify(
    GSettings *settings, gchar *key, PanelPropertiesDialog *dialog) {
  if (!strcmp(key, "type")) {
...
static void panel_properties_dialog_remove_orientation_combo(
    PanelPropertiesDialog *dialog) {
...
static void panel_properties_dialog_remove_icon_chooser(
    PanelPropertiesDialog *dialog) {
  GtkContainer *container = GTK_CONTAINER(dialog->general_table);
...
static void panel_properties_dialog_remove_toggles(
    PanelPropertiesDialog *dialog) {
...
static void panel_properties_dialog_update_for_attached(
    PanelPropertiesDialog *dialog, gboolean attached) {
  if (!attached)
...
static PanelPropertiesDialog *panel_properties_dialog_new(
    PanelToplevel *toplevel) {
  PanelPropertiesDialog *dialog;
  ...
  g_object_set_qdata_full(G_OBJECT(toplevel), panel_properties_dialog_quark,
                          dialog, (GDestroyNotify)panel_properties_dialog_free);

  dialog->toplevel = toplevel;

  gui = gtk_builder_new_from_resource(PANEL_RESOURCE_PATH
                                      "panel-properties-dialog.ui");
  gtk_builder_set_translation_domain(gui, GETTEXT_PACKAGE);

  dialog->properties_dialog =
      PANEL_GTK_BUILDER_GET(gui, "panel_properties_dialog");
  ...
  dialog->writability_warn_general =
      PANEL_GTK_BUILDER_GET(gui, "writability_warn_general");
  dialog->writability_warn_background =
      PANEL_GTK_BUILDER_GET(gui, "writability_warn_background");
  ...
  g_signal_connect_swapped(
      dialog->expand_toggle, "toggled",
      G_CALLBACK(panel_properties_dialog_setup_orientation_combo_sensitivty),
      dialog);

  panel_properties_update_arrows_toggle_visible(
      dialog, GTK_TOGGLE_BUTTON(dialog->hidebuttons_toggle));
  g_signal_connect_swapped(
      dialog->hidebuttons_toggle, "toggled",
      G_CALLBACK(panel_properties_update_arrows_toggle_visible), dialog);

  g_object_get(toplevel, "settings-path", &toplevel_settings_path, NULL);
  dialog->settings =
      g_settings_new_with_path(PANEL_TOPLEVEL_SCHEMA, toplevel_settings_path);
  gchar *toplevel_background_path;
  toplevel_background_path =
      g_strdup_printf("%sbackground/", toplevel_settings_path);
  dialog->background_settings = g_settings_new_with_path(
  ...
  g_signal_connect(dialog->settings, "changed",
                   G_CALLBACK(panel_properties_dialog_toplevel_notify), dialog);
  ...
  g_signal_connect(dialog->background_settings, "changed",
                   G_CALLBACK(panel_properties_dialog_background_notify),
                   dialog);

  panel_properties_dialog_update_for_attached(
      dialog, panel_toplevel_get_is_attached(dialog->toplevel));

  panel_toplevel_push_autohide_disabler(dialog->toplevel);
  panel_widget_register_open_dialog(
      panel_toplevel_get_panel_widget(dialog->toplevel),
      dialog->properties_dialog);
  g_object_unref(gui);
  return dialog;
}
...
void panel_properties_dialog_present(PanelToplevel *toplevel) {
  ...
  dialog =
      g_object_get_qdata(G_OBJECT(toplevel), panel_properties_dialog_quark);
  if (dialog) {
    gtk_window_set_screen(GTK_WINDOW(dialog->properties_dialog),
                          gtk_window_get_screen(GTK_WINDOW(toplevel)));
