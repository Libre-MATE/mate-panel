...
static gboolean panel_separator_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (separator->priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
    ...
    gtk_render_line(context, cr, x, padding.top, x, height - padding.bottom);
  } else {
    int y;
    ...
    gtk_render_line(context, cr, padding.left, y, width - padding.right, y);
  }
  cairo_restore(cr);
...
static void panel_separator_get_preferred_width(GtkWidget *widget,
                                                gint *minimal_width,
                                                gint *natural_width) {
  PanelSeparator *separator;
  int size;
...
static void panel_separator_get_preferred_height(GtkWidget *widget,
                                                 gint *minimal_height,
                                                 gint *natural_height) {
  PanelSeparator *separator;
  int size;
...
static void panel_separator_size_allocate(GtkWidget *widget,
...
static void panel_separator_parent_set(GtkWidget *widget,
...
static void panel_separator_class_init(PanelSeparatorClass *klass) {
...
static void panel_separator_init(PanelSeparator *separator) {
...
void panel_separator_set_orientation(PanelSeparator *separator,
  ...
  if (orient == separator->priv->orientation) return;
...
void panel_separator_load_from_gsettings(PanelWidget *panel, gboolean locked,
                                         int position, const char *id) {
  PanelSeparator *separator;

  separator = g_object_new(PANEL_TYPE_SEPARATOR, NULL);

  separator->priv->info = mate_panel_applet_register(
      GTK_WIDGET(separator), NULL, NULL, panel, locked, position, TRUE,
      PANEL_OBJECT_SEPARATOR, id);
  ...
  panel_widget_set_applet_expandable(panel, GTK_WIDGET(separator), FALSE, TRUE);
  panel_widget_set_applet_size_constrained(panel, GTK_WIDGET(separator), TRUE);
}

void panel_separator_create(PanelToplevel *toplevel, int position) {
  char *id;

  id = panel_profile_prepare_object(PANEL_OBJECT_SEPARATOR, toplevel, position);
  panel_profile_add_to_list(PANEL_GSETTINGS_OBJECTS, id);
  g_free(id);
}

void panel_separator_change_background(PanelSeparator *separator) {
  panel_background_apply_css(&separator->priv->panel->toplevel->background,
