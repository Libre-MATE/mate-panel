...
char *panel_util_make_exec_uri_for_desktop(const char *exec) {
  ...
  if (!strchr(exec, ' ')) return g_strdup(exec);
...
int panel_find_applet_index(GtkWidget *widget) {
  ...
  for (i = 0, l = applet_list; l; i++, l = l->next) {
    ...
    if (info->widget == widget) return i;
  }
...
void panel_push_window_busy(GtkWidget *window) {
  int busy =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(window), "Panel:WindowBusy"));
...
void panel_pop_window_busy(GtkWidget *window) {
  ...
  if (busy <= 0) {
    ...
    win = gtk_widget_get_window(window);
    if (win != NULL) gdk_window_set_cursor(win, NULL);

    g_object_set_data(G_OBJECT(window), "Panel:WindowBusy", NULL);
  } else {
    g_object_set_data(G_OBJECT(window), "Panel:WindowBusy",
                      GINT_TO_POINTER(busy));
  }
...
static gboolean panel_ensure_dir(const char *dirname) {
  char *parsed, *p;
  ...
  p = strchr(parsed, '/');
  if (p == parsed) p = strchr(p + 1, '/');

  while (p != NULL) {
    *p = '\0';
    if (g_mkdir(parsed, 0700) != 0 && errno != EEXIST && errno != ENOSYS) {
      g_free(parsed);
      return FALSE;
    }
    *p = '/';
...
gboolean panel_is_uri_writable(const char *uri) {
  ...
  info =
      g_file_query_info(file, "access::*", G_FILE_QUERY_INFO_NONE, NULL, NULL);
  g_object_unref(file);

  if (info) {
    retval = g_file_info_get_attribute_boolean(
...
char *panel_find_icon(GtkIconTheme *icon_theme, const char *icon_name,
                      gint size) {
  GtkIconInfo *info;
  char *retval;
  char *icon_no_extension;

  if (icon_name == NULL || strcmp(icon_name, "") == 0) return NULL;
...
cairo_surface_t *panel_load_icon(GtkIconTheme *icon_theme,
  ...
  if (!file) {
    if (error_msg)
      *error_msg = g_strdup_printf(_("Icon '%s' not found"), icon_name);

    return NULL;
  }

  error = NULL;
  pixbuf = gdk_pixbuf_new_from_file_at_scale(file, desired_width,
                                             desired_height, TRUE, &error);
  if (error) {
...
void panel_lock_screen_action(GdkScreen *screen, const char *action) {
  ...
  if (!mate_gdk_spawn_command_line_on_screen(screen, command, &error)) {
    ...
    panel_error_dialog(NULL, screen, "cannot_exec_screensaver", TRUE, primary,
                       error->message);
    g_free(primary);
    g_error_free(error);
  }
...
static char *panel_launcher_get_personal_path(void) {
  return g_build_filename(g_get_user_config_dir(), "mate", "panel2.d",
                          "default", "launchers", NULL);
}
...
GFile *panel_launcher_get_gfile(const char *location) {
  ...
  if (g_path_is_absolute(location)) return g_file_new_for_path(location);
...
char *panel_make_full_path(const char *dir, const char *filename) {
  char *retval;
  char *freeme = NULL;
  ...
  if (!g_file_test(dir, G_FILE_TEST_EXISTS)) panel_ensure_dir(dir);
...
#define LENGTH_FOR_TMPFILE_EXT 7
  ...
  while (TRUE) {
    ...
    g_snprintf(filename,
               sizeof(filename) - strlen(buf) - LENGTH_FOR_TMPFILE_EXT, "%s",
               name);
...
char *panel_make_unique_desktop_uri(const char *dir, const char *source) {
  char *name, *p;
  char *uri;
  char *path = NULL;
  ...
  if (p) memmove(name, p + 1, strlen(p + 1) + 1);
  p = strrchr(name, '.');
...
GdkPixbuf *panel_util_cairo_rgbdata_to_pixbuf(unsigned char *data, int width,
                                              int height) {
  GdkPixbuf *retval;
...
char *guess_icon_from_exec(GtkIconTheme *icon_theme, const gchar *exec) {
  char *icon_name;
  char *path;
...
static GFile *panel_util_get_gfile_root(GFile *file) {
  GFile *parent;
  GFile *parent_old;
...
static char *panel_util_get_icon_name_from_g_icon(GIcon *gicon) {
  const char *const *names;
  GtkIconTheme *icon_theme;
  int i;

  if (!G_IS_THEMED_ICON(gicon)) return NULL;
...
static char *panel_util_get_file_display_name_if_mount(GFile *file) {
  GList *l;
  char *ret = NULL;
  ...
  for (l = mounts; l != NULL; l = l->next) {
    ...
    if (!ret && g_file_equal(file, compare)) ret = g_mount_get_name(mount);
...
static char *panel_util_get_file_display_for_common_files(GFile *file) {
  GFile *compare;
  ...
  if (g_file_equal(file, compare)) {
    ...
    if (mate_gsettings_schema_exists(CAJA_DESKTOP_SCHEMA)) {
      ...
      caja_home_icon_name = g_settings_get_string(
          caja_desktop_settings, CAJA_DESKTOP_HOME_ICON_NAME_KEY);
...
static char *panel_util_get_file_description(GFile *file) {
  GFileInfo *info;
  char *ret;
  ...
  if (info) {
    ret = g_strdup(g_file_info_get_attribute_string(
...
static char *panel_util_get_file_display_name(GFile *file,
                                              gboolean use_fallback) {
  char *ret = NULL;
  GFileInfo *info =
      g_file_query_info(file, "standard::display-name",
                        G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, NULL);
  if (info) {
...
static char *panel_util_get_file_icon_name_if_mount(GFile *file) {
...
static const char *panel_util_get_icon_for_uri_known_folders(const char *uri) {
  const char *icon;
  char *path;
  int len;
  ...
  else if (g_strcmp0(path, g_get_user_special_dir(G_USER_DIRECTORY_DOWNLOAD)) ==
           0)
  ...
  else if (g_strcmp0(path, g_get_user_special_dir(G_USER_DIRECTORY_PICTURES)) ==
           0)
    icon = PANEL_ICON_FOLDER_PICTURES;
  else if (g_strcmp0(path, g_get_user_special_dir(
                               G_USER_DIRECTORY_PUBLIC_SHARE)) == 0)
    icon = PANEL_ICON_FOLDER_PUBLIC_SHARE;
  else if (g_strcmp0(path,
                     g_get_user_special_dir(G_USER_DIRECTORY_TEMPLATES)) == 0)
    icon = PANEL_ICON_FOLDER_TEMPLATES;
  else if (g_strcmp0(path, g_get_user_special_dir(G_USER_DIRECTORY_VIDEOS)) ==
           0)
...
char *panel_util_get_label_for_uri(const char *text_uri) {
  ...
  if (g_str_has_prefix(text_uri, "file:")) {
    ...
    if (!label) label = panel_util_get_file_description(file);
    if (!label) label = panel_util_get_file_display_name(file, TRUE);
    g_object_unref(file);
  ...
  if (!root_display) /* can happen with URI schemes non supported by gvfs */
  ...
  if (g_file_equal(file, root))
     ...
     */
    label = g_strdup_printf(_("%1$s: %2$s"), root_display, displayname);
    g_free(root_display);
    g_free(displayname);
  }
...
char *panel_util_get_icon_for_uri(const char *text_uri) {
  ...
  if (g_str_has_prefix(text_uri, "burn:")) return g_strdup(PANEL_ICON_BURNER);
  ...
  info = g_file_query_info(file, "standard::icon", G_FILE_QUERY_INFO_NONE, NULL,
                           NULL);
  g_object_unref(file);
...
static gboolean panel_util_query_tooltip_cb(GtkWidget *widget, gint x, gint y,
                                            gboolean keyboard_tip,
                                            GtkTooltip *tooltip,
                                            const char *text) {
  if (!panel_global_config_get_tooltips_enabled()) return FALSE;

  gtk_tooltip_set_text(tooltip, text);
  return TRUE;
}

void panel_util_set_tooltip_text(GtkWidget *widget, const char *text) {
  g_signal_handlers_disconnect_matched(widget, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                       panel_util_query_tooltip_cb, NULL);
  ...
  g_signal_connect_data(widget, "query-tooltip",
                        G_CALLBACK(panel_util_query_tooltip_cb), g_strdup(text),
